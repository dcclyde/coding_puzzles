...
struct TwoFairDice {
    ...
    ll finish(vector<int>& A, vector<int>& B) {
        dbgY(A, B);
        V<ll> helper(13);  // h[k] = num ways to have 2*W + Draws add up to k.
        ...
        dbg(faces_already, score_already);
        dp[faces_already][score_already] = 1;

        FOR(face, faces_already, 6) {
            FOR(score, 0, 13) {
...
int main() {
    ...
    TwoFairDice s;
    V<int> A = {10, 10, 11, 12, 12, 12};
    V<int> B = {10, 10, 10, 12};

    auto result = s.finish(A, B);
    dbgcBold("done", result);
