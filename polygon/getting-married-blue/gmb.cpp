...
void brute() {
    ...
    auto dfs = [&](auto&& self, ll ai, ll cost) -> void {
        dbgP("dfs", ai, used, cost, best);
        if (ai == N) {
            ckmin(best, cost);
            return;
        }
        if (used[ai] != -1) {return self(self, ai+1, cost);}
        for (int bi = ai+1; bi < N; ++bi) {
            if (!legal(A[ai], A[bi])) {continue;}
            if (used[bi] != -1) {continue;}
            used[ai] = bi;
            used[bi] = ai;
            dbg(MP(ai,bi), MP(A[ai], A[bi]), tax(A[ai], A[bi]));
...
void solve() {
    ...
    FOR(i, 0, N) {
        FOR(j, i+1, N) {
            if (!legal(A[i], A[j])) {continue;}
            wm.ae(1+i, 1+j, tax(A[i]) + tax(A[j]) - tax(A[i], A[j]));
        }
    }
