...
void brute() {
    ...
    auto ph = rangeint(0, M);
    do {
        bool ok = true;
        FOR(k, 0, N) {
            if (A[k] > C[ph[k]] || B[k] > D[ph[k]]) {
                ok = false;
                break;
            }
        }
...
void solve() {
    ...
    V<tuple<ll,bool,ll>> events;
    FOR(k, 0, N) {
        events.emplace_back(-A[k], true, B[k]);
    }
    FOR(k, 0, M) {
        events.emplace_back(-C[k], false, D[k]);
    }
    sort(all(events));
    dbg(N, M, pdh(events));

    ll out = 0;
    map<ll,ll> box_heights;
    for(auto& [ma, is_chocolate, b] : events) {
        auto a = -ma;
        dbg(out, MT(a,is_chocolate,b), box_heights);
        if (!is_chocolate) {
            ++box_heights[b];
            continue;
        }
        // use the shortest box that fits.
        auto it = box_heights.lower_bound(b);
        if (it == box_heights.end()) {continue;}
        ++out;
        if (it->second == 1) {
            box_heights.erase(it);
        } else {
            --it->second;
        }
    }
