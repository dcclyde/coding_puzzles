...
void solve() {
    ...
    rv(N, dat);
    dbgY(N, dat);
    vector<pii> xh(N), yh(N);
    FOR(k, 0, N) {
        xh[k] = MP(dat[k].first, k);
        yh[k] = MP(dat[k].second, k);
    }
    sort(all(xh));
    sort(all(yh));
    ...
    V<t3> proposals;
    auto get_prop = [&](V<pii>& source, int a, int b) {
        auto& [av, ai] = source[a];
        auto& [bv, bi] = source[b];
        proposals.emplace_back(bv - av, ai, bi);
        proposals.emplace_back(bv - av, bi, ai);
        return;
    };
    get_prop(xh, 0, N-1);
    get_prop(xh, 1, N-1);
    ...
    get_prop(yh, 0, N-2);
    sort(rall(proposals));
    auto [vmax, amax, bmax] = proposals[0];
    if ( amax > bmax ) {
        swap(amax, bmax);
    }
    int ctr = 2;
    while ( true ) {
        auto [vcurr, acurr, bcurr] = proposals[ctr];
        if ( acurr > bcurr ) {
            swap(acurr, bcurr);
        }
        if ( acurr == amax && bcurr == bmax ) {
            ++ctr;
            continue;
        }
        ps(vcurr);
