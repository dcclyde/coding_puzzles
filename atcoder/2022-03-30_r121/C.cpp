...
void solve() {
    ...
    while ( N > 3 ) {
        // find the (a) last element and move it to the end.
        int max_pos = 0;
        FOR(k, 0, N) {
            if ( dat[k] > dat[max_pos] ) {
                max_pos = k;
            }
        }
        dbg(N, max_pos, dat);
        // if we aren't allowed to move this element forward yet, burn a move.
        // we WANT to name max_pos + 1.
        if ( out.size() % 2 == (max_pos + 1) % 2 ) {
            // choose a position with correct parity that won't mess things up.
            int pi = out.size() % 2 + 1;
            if ( pi == max_pos+1 || pi+1 == max_pos+1 ) {
                pi += 2;
            }
            out.push_back(pi);
            swap(dat[pi], dat[pi-1]);
            dbgc("burned a move", pi, dat);
        }
        FOR(k, max_pos, N-1) {
            out.push_back(k+1);
            swap(dat[k], dat[k+1]);
            dbgcP("push", k, out, dat);
        }
        --N;
    }
    if ( N == 3 ) {
        while ( dat[0] > dat[1] || dat[1] > dat[2] ) {
            if ( out.size() % 2 == 0 ) {
                out.push_back(1);
                swap(dat[0], dat[1]);
            } else {
