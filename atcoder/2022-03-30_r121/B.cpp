...
ll get_cheapest(auto& X, auto& Y) {
    int ix = 0;
    int iy = 0;
    ll out = 1e18;
    while ( ix < X.size() && iy < Y.size() ) {
        ckmin(out, abs(X[ix] - Y[iy]));
        if ( X[ix] < Y[iy] ) {
            ++ix;
        } else {
            ++iy;
...
void solve() {
    ...
    dbg(R, G, B);
    if ( R.size() % 2 == 0 && B.size() % 2 == 0 && G.size() % 2 == 0 ) {
        return ps(0);
    }
    // find which one has even size.
    if ( R.size() % 2 == 0 ) {
        swap(R, G);
    } else if ( B.size() % 2 == 0 ) {
        swap(B, G);
    ...
    sort(all(G));
    sort(all(B));
    ll rb = get_cheapest(R, B);
    ll rg = get_cheapest(R, G);
    ll gb = get_cheapest(G, B);
    dbg(R);
    dbg(G);
    dbg(B);
    dbg(rb, rg, gb);
