...
void solve() {
    ...
    int mult = 1;
    if ( dat[l] <= 0 ) {
        // I'd rather deal with all positives.
        mult = -1;
        tie(lo, hi) = MP(-hi, -lo);
    }
    dbg(d2);
    FOR(k, l, r+1) {
        d2.push_back(mult * dat[k]);
    }
    sort(all(d2));

    ll best = INF;
    for ( int num_singles = d2.size() % 2 ; num_singles <= d2.size() ; num_singles += 2 ) {
        ll local_lo = lo;
        ll local_hi = hi;
        FOR(k, d2.size()-num_singles, d2.size()) {
            ckmin(local_lo, d2[k]);
            ckmax(local_hi, d2[k]);
        }
        FOR(k, 0, d2.size() - num_singles) {
            ll curr = d2[k] + d2[d2.size() - num_singles - 1 - k];
            ckmin(local_lo, curr);
            ckmax(local_hi, curr);
        }
        ckmin(best, local_hi - local_lo);
    }
    ps(best);
