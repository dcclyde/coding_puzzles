...
#define dbgcP(...) ;
    ...
    #undef dbgcHeader
    #define dbgcHeader(...) dbgcbase(OUT_GREEN, OUT_CYAN, OUT_MARK<<__VA_ARGS__)
...
void solve() {
    ...
    /*
        To get the result for step S, I should grab all the A_{N-T} where T and S don't share any bits.
    */
    int MAX_BITS = 30;

#pragma region  // iterating per S, too slow
#if 0
    vector<int> out;
    FOR(S, 0, num_steps) {
        int out_local = 0;
        vector<int> bits_missing;
        FOR(b, 0, MAX_BITS) {
            if ( ((S>>b) & 1) == 0 ){
                bits_missing.push_back( b );
            }
        }
        dbg(S, bits_missing);

        FOR(mask, 0, 1<<(bits_missing.size())) {
            int T = 0;
            FOR(b, 0, bits_missing.size() ) {
                if ( mask & (1<<b) ) {
                    T |= (1<<(bits_missing[b]));
                    // dbgc("here");
                }
            ...
            out_local ^= dat[N-1 - T];
            dbg(mask, T, dat[N-1-T], out_local);
        }
        dbgY(S, out_local);
...
#pragma endregion
    ...
    // faster version: Let's just build the contribution for each bit that could be missing from S?
}
