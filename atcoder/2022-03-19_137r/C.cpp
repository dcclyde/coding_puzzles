...
void learn() {
    ...
    using tup5 = tuple<int,int,int,int,int>;
    tup5 x;
    auto& [a,b,c,d,e] = x;
    set<tup5> losing_positions;
    losing_positions.insert(tup5(0,1,2,3,4));
    dbgc("start");
    for ( int sum = 0 ; sum <= SUM_MAX ; ++sum ) {
        for( a = 0 ; a <= sum ; ++a)
        for( b = a+1 ; a+b <= sum ; ++b)
        for( c = b+1 ; a+b+c <= sum ; ++c)
        for( d = c+1 ; a+b+c+d <= sum ; ++d)
        for( e = d+1 ; a+b+c+d+e <= sum ; ++e) {
            if ( a+b+c+d+e > sum ) {
                continue;
            }
            bool winning = false;
            vector<int> curr = {a,b,c,d,e};
            vector<int> helper;
            FOR(idx, 0, 5) {
                if ( idx != 4 ) {
                    continue;
                }
                FOR(delta, 0, curr[idx]+1) {
                    if ( winning ) {
                        break;
                    }
                    helper = curr;
                    ...
                    bool losing = true;
                    // check for repeats
                    FOR(k, 0, helper.size()-1) {
                        if ( helper[k] == helper[k+1] ) {
                            losing = false;  // invalid move
                            break;
                        }
                    }
                    if ( losing ) {
                        tup5 htup = {helper[0], helper[1], helper[2], helper[3], helper[4]};
                        if ( losing_positions.count(htup) == 0 ) {
                            losing = false;
                        }
                    }
                    if ( losing ) {
            ...
            if ( !winning ) {
                dbgc("losing position", x);
                losing_positions.insert(x);
            }
        }
    }
    el;
    for ( auto& y : losing_positions ) {
        dbg(y);
    }
    vector<tup5> losing_vec;
    losing_vec.insert(losing_vec.end(), all(losing_positions));
    sort(all(losing_vec), [&](auto& x, auto& y) {
        auto& [a,b,c,d,e] = x;
        auto& [q,r,s,t,u] = y;
        tup5 xx(e,d,c,b,a);
        tup5 yy(u,t,s,r,q);
...
void solve() {
    ...
    dbg(N, dat);
    sort(all(dat));
    if ( dat[N-2] + 1 == dat[N-1] && dat[N-1] % 2 != N % 2 ) {
        return ps("Bob");
    } else {
        return ps("Alice");
    }
