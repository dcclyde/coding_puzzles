...
void solve() {
    ...
    for ( auto& x : dat ) {
        ...
        curr_d0 = max(curr_d0, 0);
        curr_d1 = max(curr_d1, 0);
        max_d0 = max(max_d0, curr_d0);
        max_d1 = max(max_d1, curr_d1);
    }
    dbg(max_d0, max_d1, cnt);

    int min_result = cnt[1] - max_d1;
    int max_result = cnt[1] + max_d0;
    int out = max_result - min_result + 1;
