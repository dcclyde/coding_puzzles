...
/*
    1) Compute the score when putting first shot at very first scoring location.

    2) "move forward and update the score as we go".
        Find 0 < d < D s.t. stepping forward by `d` just barely pushes a score up.
            Happens up to M times.
            also record which score would go up.
        Same thing for reverse side too.
*/
...
void solve() {
    ...
    ll score = 0;
    ll pos = 0;
    auto it = transitions.lower_bound(MP(0,0));
    FOR(n, 0, N/2 + 1) {
        // move it so it's barely >= pos.
        while ( it->first < pos ) {
            ++it;
        }
        ...
        ll contrib = mult * S[ it->second ];
        score += contrib;
        dbg(n, *it, mult, contrib, score);
    ...
    map<ll,ll> steps;
    ll lowest_original = -cdiv(N-1, 2) * D;
    ll highest_original = fdiv(N-1, 2) * D;
    FOR( ri , 0 , M ) {
        ll r = R[ri];
        // how far would we need to step forward to just barely gain from this?
        /*
            We'd need start_pos + q === pos (D)
            q === pos - start_pos
        */
        ll step_size = S[ri] - S[ri+1];
        ll step_down_pos = (r + 1) % D;
        // do we actually reach that far?
        if ( highest_original + step_down_pos >= r+1 ) {
            steps[step_down_pos] -= step_size;
        }

        ll step_up_pos = (-r + D*r) % D;
        if ( lowest_original + step_up_pos <= -r ) {
            steps[step_up_pos] += step_size;
        }
    }
    el;
    dbg(steps);
    ...
    ll best_score = score;
    for( auto& [step_pos, step_size] : steps ) {
        if ( step_pos == 0 ) {
            continue;
        }
        score += step_size;
        dbg(step_pos, step_size, score);
