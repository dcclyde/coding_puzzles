...
#if 0
int V = 6;
vector<vector<int>> verts_to_edge(V, vector<int>(V));
vector<pii> edges;

bool check_colors(vector<int>& colors) {
    vector<bool> seen;
    FOR(a, 0, V) FOR(b, a+1, V) FOR(c, b+1, V) {
        seen.assign(3,0);
        int curr = colors[ verts_to_edge[a][b] ];
        if ( seen[curr] ) {
            return
        seen[ curr ] = true;
        if ( seen[curr] >= 2 ) {
...
void rec_helper(vector<int>& colors, vector<int>& needed_per_color, int pos) {
    if ( pos == edges.size() ) {
        bool ok = check_colors( colors );
        if ( ok ) {
            ps(colors);
            exit(1);
        }
    ...
    FOR(color, 0, 3) {
        if ( needed_per_color[color] == 0 ) {
            continue;
        }
        --needed_per_color[color];
        colors[pos] = color;
        rec_helper( colors , needed_per_color , pos+1 );
        ++needed_per_color[color];
        colors[pos] = -1;
    }
    return;
...
void solve() {
    // FOR( a , 0 , V ) {
    //     verts_to_edge.emplace_back();
    //     FOR( b , a+1 , V ) {
    //         verts_to_edge[a][b] = edges.size();
    //         verts_to_edge[b][a] = edges.size();
    //         edges.emplace_back(a, b);
    //     }
    // }
    // int E = edges.size();
    // vector<int> colors( E , -1 );
    // vector<int> needed_per_color( 3 , E/3 );
    // rec_helper(colors, needed_per_color, 0);
    ...
    ll target_total = (N * (N-1)) / 6;
    vector<int> colors(N, -1);
    vector<int> totals(3,0);
    for ( int color = 0 ; color < 3 ; ++color ) {
        for ( int k = N-1 ; k >= 0 ; --k ) {
            if ( colors[k] == -1 && totals[color] + k <= target_total ) {
                colors[k] = color;
                totals[color] += k;
            }
        }
        if ( totals[color] != target_total ) {
            dbgc(":(", N, color, totals);
        }
    }
    cout << "Yes\n";
    string color_names = "RWB";
    // ROF(a, N-1, 0) {
    for ( int a = N-1 ; a >= 0 ; --a ) {
        FOR(b, 0, a) {
            cout << color_names[ colors[a] ];
        }
        cout << '\n';
    }
