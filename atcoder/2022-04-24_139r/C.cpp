...
ll learn(ll rr=-1, ll cc=-1) {
    ...
    FOR(mask, 0, MASK_MAX) {
        ...
        V<pll> points;
        FOR(r, 0, R) FOR(c, 0, C) {
            if (((mask >> (C*r + c)) & 1) == 0) { continue; }
            ll f = r + 3*c;
            ll g = c + 3*r;
            if (fv.find(f) != fv.end() || gv.find(g) != gv.end()) { continue; }
            fv.insert(f);
            gv.insert(g);
...
/**
    If R is big: Hit every f option. (r + 3*c)
    If C is big: Hit every g option. (c + 3*r)      x + 3y

    Out of points with 3x+y = T, what's the smallest possible 3y+x?
    I guess just try to stay close to y=x?
 */


ll solve(ll rr = -1, ll cc = -1) {
    // learn();
    ...
    FOR(gv, 0, GV_MAX+1) {
        ll y = gv / 4;
        ll x = gv - 3*y;
        // move toward the line y=x while possible?
        dbgP(MP(x,y), abs(y-x));
        while (true) {
            ll py = y + 1;  // "proposed y"
            ll px = x - 3;
            if (abs(py - px) >= abs(y - x)) {break;}
            y = py, x = px;
        }
        dbg(MP(x,y), abs(y-x));
        // ok now move into the allowed rectangle.
        // Need x < C and y < R.
        ll steps_needed = 0;
        ckmin(steps_needed, (R-1) - y);
        ckmax(steps_needed, cdiv(x-(C-1), 3));
        y += steps_needed;
        x -= 3*steps_needed;
        dbg(steps_needed, MP(x,y));

        /**
            I should prefer smaller x (and bigger y) if possible.
         */
        if (x > y) {
            ll px = x - 3;
            ll py = y + 1;
            ll pf = 3*px + py;
            if (px < C && py < R && px >= 0 && py >= 0 && fv.find(pf) == fv.end()) {
                x = px, y = py;
            }
        }

        if (x >= C || y >= R || x < 0 || y < 0) {
            dbg(gv, MP(x,y), MP(C,R), fv);
            continue;
            assert(false);
        }
