...
V<mi> wue_helper = {0, 1};
mi ways_using_exactly(int k, int L) {
    /*
        k <= 26.
        To use exactly 4 chars, we should:
        (use 4 chars) - (4 choose 1) * (use 3 chars) + (4 choose 2) * (use 2 chars) - ...
    */
    mi out = 0;
    FOR(s, 1, k+1) {
        int sgn = (k-s) % 2;
        sgn = 1 - 2*sgn;  // 1 or -1
        mi contribution = comb(k, s) * pow(mi(s), L);
        dbgP(k, L, s, sgn, contribution);
        out += contribution * sgn;
    }
    dbgcP("w_u_e", k, L, out); el;
    return out;
}
...
void solve() {
    ...
    mi out = 0;
    FOR(mask, 1, (1<<N)) {
        // find all letters that are in every string indicated by mask.
        V<int> bits;
        FOR(b, 0, N) {
            if ( (mask>>b) & 1 ) {
                bits.push_back(b);
        ...
        // mi local = ways_using_exactly(curr_chars, L);
        dbg(mask, curr_chars, L);
        mi local = pow(mi(curr_chars), L);
        out += local * sgn;
    }
    ps(out);
