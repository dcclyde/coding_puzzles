...
void solve() {
    ...
    int R = N;
    int C = N;
    int1(rstart, cstart, rend, cend);
    vector<string> dat;
    rv(N, dat);
    dbgY(N, dat);

    auto p2f = [&](int r, int c) {return r*N + c;};
    auto f2p = [&](int k) {return MP(k/N, k%N);};
    auto xeqy = [&](int k) {auto& [dx, dy] = stepsDiagonal[k]; return dx == dy;};

    // I guess just start traversing?
    // states are like (flat position, flat direction of last move).
    V<V<bool>> seen(N*N, V<bool>(2, false));
    using t3 = tuple<int,bool,int>;
    deque<t3> todo;
    auto flat_start = p2f(rstart, cstart);
    auto flat_end = p2f(rend, cend);
    todo.emplace_back(flat_start, false, 0);
    todo.emplace_back(flat_start, true, 0);
    seen[flat_start][false] = true;
    seen[flat_start][true] = true;
    while ( !todo.empty() ) {
        auto [curr_pos_flat, curr_axis, curr_cost] = todo.front();
        todo.pop_front();

        if ( curr_pos_flat == flat_end ) {
            return ps(curr_cost);
        }

        auto [r, c] = f2p(curr_pos_flat);
        // auto& [drprev, dcprev] = stepsDiagonal[curr_dir_flat];
        FOR(k, 0, stepsDiagonal.size()) {
            // is this direction a waste of time?
            bool other_axis = xeqy(k);
            if ( other_axis == curr_axis ) {
                continue;
            }
            auto& [dr, dc] = stepsDiagonal[k];
            // try as many steps as we can in this direction.
            FOR(dt, 1, N) {
                int rr = r + dt*dr;
                int cc = c + dt*dc;
                if ( rr < 0 || rr >= N || cc < 0 || cc >= N || dat[rr][cc] == '#' ) {
                    break;
                }
                int other_pos_flat = p2f(rr, cc);

                if ( seen[other_pos_flat][other_axis] ) {
                    break;
                }
                seen[other_pos_flat][other_axis] = true;
                todo.emplace_back(other_pos_flat, other_axis, curr_cost + 1);
            }
        }
    }
    return ps(-1);
...
/*
    // enumerate points.
    V<pii> f2p;  // flat to point
    umap<pii, int> p2f;
    FOR(r, 0, R) FOR(c, 0, C) {
        p2f[MP(r, c)] = f2p.size();
        f2p.emplace_back(r, c);
    }
    dbg(f2p);
    ...
    FOR(fk, 0, N*N) {  // "flat k"
        auto&[r, c] = f2p[fk];
        for(auto& [dr, dc] : stepsDiagonal) {
            FOR(k, 1, N) {
                int rr = r + k*dr;
                int cc = c + k*dc;
                if ( rr < 0 || cc < 0 || rr >= R || cc >= C || dat[rr][cc] == '#' ) {
                    // can't go there.
                    break;
                }
                int ko = p2f[MP(rr,cc)];
    ...
    // now just start traversing I guess.

    int start = p2f[MP(rstart, cstart)];
    int end = p2f[MP(rend, cend)];
    dbg(start, end);
    deque<pii> todo;
    todo.emplace_back(start, 0);
    V<bool> seen(N*N, false);
    seen[start] = true;
    while ( !todo.empty() ) {
        auto [curr, curr_cost] = todo.front();
        todo.pop_front();
        dbg(curr, curr_cost, f2p[curr]);
        if ( curr == end ) {
            return ps(curr_cost);
        }
        for(auto& o : G[curr]) {
            ...
            seen[o] = true;
            todo.emplace_back(o, curr_cost + 1);
        }
    }
    return ps(-1);
