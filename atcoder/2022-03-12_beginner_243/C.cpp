...
void solve() {
    ...
    map<int,int> leftmost_facing_right;
    map<int,int> rightmost_facing_left;

    for ( int k = 0 ; k < N ; ++k ) {
        auto& [x, y] = dat[k];
        if ( dirs[k] == 'R' ) {
            if ( leftmost_facing_right.count( y ) == 0 || leftmost_facing_right[y] > x ) {
                leftmost_facing_right[y] = x;
            }
        } else {
            // dirs[k] is L.
            if ( rightmost_facing_left.count( y ) == 0 || rightmost_facing_left[y] < x ) {
                rightmost_facing_left[y] = x;
            }
        }
    }

    dbg(leftmost_facing_right);
    dbg(rightmost_facing_left);

    for ( auto& [y, x] : leftmost_facing_right ) {
        if ( rightmost_facing_left.count(y) != 0 && x < rightmost_facing_left[y] ) {
            cout << "Yes\n";
            return;
        }
    }
