...
#define OUT_MARK        "\033[0;30;41m"<<std::right<<setw(10)
...
void solve() {
    lls(N);  // declare + read a long long. I could write lls(a, b, c) to load 3 lls.
    vector<pii> dat;  // nothing fancy, pii is just pair<int, int>
    rv(N, dat);  // resize and load from cin. Note it works even though the vector is full of pairs
    strings(directions);
    dbg(N, dat);
    dbg(directions); el;  // el = dump an extra newline to stderr

    map<int, pii> yToPositions;  // y --> (leftmost facing right, rightmost facing left)
    FOR(k, 0, N) {
        auto& [x, y] = dat[k];  // this is just a normal feature these days but it made my C++ life sooo much better.
        if ( yToPositions.count( y ) == 0 ) {
            yToPositions[y] = {INF, -INF};
        }
        auto& [leftmost, rightmost] = yToPositions[y];
        if ( directions[k] == 'L' ) {
            rightmost = max(rightmost, x);
        } else {
            leftmost = min(leftmost, x);
        }
        dbgc("iter done", k, directions[k], dat[k], yToPositions[y]);  // dbgc = "debug with comment"
    }
    dbgc(OUT_MARK<<"output time!", yToPositions);  // brightly marked debug output + example of printing a nested data structure
    for ( auto& [y, pr] : yToPositions ) {
