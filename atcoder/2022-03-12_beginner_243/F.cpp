...
void solve() {
    ...
    vector<vector<mi>> Q( N+1 , vector<mi>( N , 0 ) );
    for ( int m = 1 ; m <= N ; ++m ) {
        for ( int j = 0 ; j < N ; ++j ) {
            // dbg(m, j, N, Q.size(), Q[0].size());
            // dbg(Q[m][j]);
            // dbg(Q[m-1][j]);
            // dbg(P[j]);
            Q[m][j] = Q[m-1][j] + P[j] * (1-Q[m-1][j]);
        }
    }
    for ( int m = 1 ; m <= N ; ++m ) {
        dbg(m, Q[m]);
    }
    vector<vector<mi>> f( N+1 , vector<mi>( K+1 , 0 ) );
    f[0][0] = 1;
    for ( int m = 1 ; m <= M ; ++m ) {
        for ( int k = 1 ; k <= K ; ++k ) {
            for ( int j = 0 ; j < N ; ++j ) {
                /*
                    To get size m with k pulls,
                    I could have size m with k-1 and then pull something I have
                    OR
                    have size m-1 with k-1 and then pull something I want.
                */
               f[m][k] += Q[m-1][j] * P[j] * f[m-1][k];
               f[m][k] += (1-Q[m-1][j]) * P[j] * f[m-1][k-1];
            }
        }
    }
