...
void brute() {
    ...
    FOR(mask, 0, 1<<B) {
        V<ll> dat;
        FOR(b, 0, B) {
            if ((mask >> b) & 1) {dat.push_back(L+b);}
        }
        bool ok = true;
        for(auto& a : dat) for(auto& b : dat) {
            if (a >= b) {continue;}
            auto sa = to_string(a); auto sb = to_string(b);
            if (sa.find(sb) != -1 || sb.find(sa) != -1) {ok = false; break;}
        }
        if (ok) {ckmax(best, dat.size());}
    }
    return ps(best);
}

/**
    If there are >= 3 different lengths allowed,
        just pick the biggest of those piles.
    If 1 length, obvious.
    What if 2 lengths?

    Choose everything from the bigger length.
    Exclude shorter things that directly overlap in last positions.
    Overlap in early positions?
        L = S + 'd'
        S * 10 "=" L

        I should clamp my short range so that
 */

void brute_conjecture() {
    lls(L, R);
    ...
    ROF(k, L, R+1) {
        ...
        bool ok = true;
        for(auto& so : results) {
            if (so.find(sk) != -1) {ok = false; break;}
        }
        if (ok) {results.insert(sk);}
    }
    return ps(results.size());
}

void solve() {
    lls(L, R);
    ll len_L = to_string(L).size();
    ll len_R = to_string(R).size();

    if (len_L == len_R) { dbgcY("out A"); return ps(R - L + 1); }

    ll p10_R_floor = 1;
    FOR(k, 0, len_R - 1) {p10_R_floor *= 10;}
    auto& p10 = p10_R_floor;
    ll p10_L_ceil = 1;
    FOR(k, 0, len_L - 1) {p10_L_ceil *= 10;}

    if (len_L + 2 <= len_R) {
        ll best = 0;
        ckmax(best, R - p10_R_floor + 1);
        ckmax(best, p10_R_floor - p10_R_floor/10);
        dbgcY("out B");
        return ps(best);
    }

    if (to_string(R)[0] >= '2') {
        ll out = R - p10_R_floor + 1;
        dbgcY("out C");
        return ps(out);
    }

    ckmax(L, R/10 + 1);
    ckmax(L, R - p10_R_floor + 1);
    dbgcY("out D");
    return ps(R - L + 1);
