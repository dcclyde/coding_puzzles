...
/**
    ...
    Pull off the smallest?? one
    Apply the operation: [L, R] becomes [2*L, 2*R + X].

    How do I get the answer?
    Smallest max and largest min?

    By "pull off the smallest" I mean "grab the one with smallest MAX".
 */
...
void solve() {
    ...
    // map<ll,ll> mins, maxes;
    map<pll,ll> intervals;  // stores (max, min)!
    ll big_min = -INF_ll;
    ll small_max = INF_ll;
    for(auto& x : dat) {
        // ++mins[x];
    ...
    ll best = INF_ll;
    ll CUTOFF = (INF_ll - X) / 2;
    while (true) {
        // update best.
        auto& [small_max, smctr] = *maxes.begin();
        auto& [big_min, bmctr] = *mins.rbegin();
        ll curr = big_min - small_max;
        ckmax(curr, 0);
        ckmin(best, curr);

        // modify an interval.
        auto [curr_interval, curr_interval_ctr] = *intervals.begin();
        auto [imax, imin] = curr_interval;
        if (curr_interval_ctr == 1) {intervals.erase(curr_interval);} else {--intervals[curr_interval];}
        if (mins[imin] == 1) {mins.erase(imin);} else {--mins[imin];}
        if (maxes[imax] == 1) {maxes.erase(imax);} else {--maxes[imax];}

        // TODO check for overflow and exit if necessary.
        // how do I know if imax*2+X >= BIG?
        if (imax >= CUTOFF) {break;}

        imax = imax * 2 + X;
        imin = imin * 2;
        ++intervals[MP(imax, imin)];
        ++mins[imin];
        ++maxes[imax];
    }
    return ps(best);
