...
void solve() {
    ...
    vector<vector<int>> vpr(R);  // vertices per row
    vector<vector<int>> vpc(C);
    ...
    FOR(r, 0, R) {
        FOR(k, 0, (int)vpr[r].size() - 1) {
            dsu.unite( vpr[r][k] , vpr[r][k+1] );
        }
    }
    FOR(c, 0, C) {
        FOR(k, 0, (int)vpc[c].size() - 1) {
            dsu.unite( vpc[c][k] , vpc[c][k+1] );
        }
    }

    unordered_map<int, pair<unordered_set<int>, unordered_set<int>>> leader_to_seen;
    FOR(v, 0, V) {
        int leader = dsu.get(v);
        auto& [rows_seen, cols_seen] = leader_to_seen[leader];
        auto& [r, c] = positions[v];
        rows_seen.insert( r );
        cols_seen.insert( c );
    }

    int rows_seen_total = 0;
    int cols_seen_total = 0;

    int out = -1;
    for ( auto& [leader, seen] : leader_to_seen ) {
        auto& [rows_seen, cols_seen] = seen;
        rows_seen_total += rows_seen.size();
        cols_seen_total += cols_seen.size();
        ++out;
    }

    int extra = min(R - rows_seen_total, C - cols_seen_total);
    out += extra;
    ps(out);
    return;
}
