...
/*
    If B > 0:
        Lowest negative: Flip then subtract.
        HN: subtract then flip.
        LP: subtract
        HP: flip, subtract, flip

    If B == 0:
        LN: subtract
        HN: -1
        LP: 0
        HP: subtract then flip

    If B < 0:
        LN: subtract
        HN: flip, subtract, flip
        LP: flip, subtract
        HP: subtract, flip

    remember to clamp HN <= -1, LP >= 0.
*/
...
void solve() {
    ...
    ll out_positive = hp - lp + 1;
    ll out_negative = hn - ln + 1;
    ll out = out_positive + out_negative;

    dbg(ln, hn, lp, hp);
    dbg(out_negative, out_positive, out);
