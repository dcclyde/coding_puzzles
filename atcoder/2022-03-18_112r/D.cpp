...
void solve() {
    ...
    // visPerComp[k] = (num rows visible from comp k, num cols visible from comp k)
    vector<pii> visPerComp;
    vector<bool> seen( V , false );
    deque<int> todo;
    unordered_set<int> seenR, seenC;

    FOR(vidx, 0, positions.size()) {
        if ( seen[vidx] ) {
            continue;
        }
        // search this component.
        el; dbgc(OUT_MARK<<"dfs start", vidx, seen);
        todo.push_back( vidx );
        seen[vidx] = true;
        seenR.clear();
        seenC.clear();
        while ( ! todo.empty() ) {
            auto curr = todo.back();
            todo.pop_back();
            auto& [r, c] = positions[curr];
            dbg(curr, positions[curr], todo);
            seenR.insert( r );
            seenC.insert( c );
        ...
        visPerComp.emplace_back( seenR.size() , seenC.size() );
    }
    dbg(visPerComp);

    int out = -1;  // I get 1 component "for free" around the edges.
    int total_seen_r = 0;
    int total_seen_c = 0;
    for ( auto& [seen_r, seen_c] : visPerComp ) {
        ++out;
        total_seen_r += seen_r;
        total_seen_c += seen_c;
    }
    int extra_needed = min(R - total_seen_r, C - total_seen_c);
    dbg(total_seen_r, total_seen_c, extra_needed);
    out += extra_needed;
    ps(out);
