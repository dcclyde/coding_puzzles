...
pair<int, bool> subsolve(vector<pair<int,bool>>& children_results) {
    vector<int> even_good, even_bad, odd;
    dbg(children_results);
    for ( auto& [child_delta, child_parity] : children_results ) {
        if ( child_parity ) {
            odd.push_back( child_delta );
        } else {
            if ( child_delta <= 0 ) {
                even_good.push_back( child_delta );
            } else {
                even_bad.push_back( child_delta );
            }
        }
    }
    sort(all(odd));
    dbg(odd, even_good, even_bad);

    // "score" is delta from pov of the 2nd player at this node.
    int score = -1;  // enemy already got 1 point just for showing up.
    bool flip = (odd.size() % 2 == 0);
    // I immediately visit all the even places that are bad to land
    for ( auto& x : even_good ) {
        score -= x;
    }
    dbgc("after even_good", score);
    // someone gets stuck visiting even_bad at the end.
    int dir;
    if ( flip ) {
        // I get stuck visiting those nodes.
        dir = 1;
    } else {
        dir = -1;
    }
    ...
    dbgc("after even_bad", score);
    // I take the best odd, then enemy takes the next one, etc.
    int odd_toggle = 1;
    for ( auto& x : odd ) {
        score -= odd_toggle * x;
        odd_toggle *= -1;
    }
    pair<int, bool> out = {-score, flip};
    dbg(out);
    return out;
}
...
void solve() {
    ...
    FOR(k, 0, N-1) {
        int1( c );
        children[c].push_back( k+1 );
    }
    dbg(children);

    // (delta from P1's point of view, parity)
    vector<pair<int, bool>> dp( N );

    // int ctr = 10;
    function<void(int)> dfs = [&](int v) {
        dbgc(OUT_MARK<<"START", v);
        // --ctr;
        // if ( ctr <= 0 ) {
        //     exit(1);
        // }
        ...
        el; dbgc("children done", v);

        vector<pair<int,bool>> children_results;
        for ( auto& c : children[v] ) {
            auto& [child_delta, child_parity] = dp[c];
            children_results.emplace_back( child_delta, child_parity );
        }
        dp[v] = subsolve(children_results);

        dbgc("final answer", v, dp[v]);
    };

    dfs( 0 );

    int delta = dp[0].first;
    // out should be N/2 + delta/2.
    int out = (N + delta) / 2;
    ps( out );
...
int main() {
    ...
    //     {{-5, true}, {-3, true}, {-4, true}, {3, true}, {2, false}, {0, false}, {-10, false}};
    // subsolve( A );
    // exit(1);
