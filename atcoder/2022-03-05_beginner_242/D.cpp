...
void solve() {
	...
	ints( num_queries );
	rep( num_queries ) {
		ll S, target_idx; cin >> S >> target_idx;
		--target_idx;
		/*
			first, figure out what character we'll use from original string.
			1 becomes 1 << S.
		*/
		ll source_char_idx;
		ll subidx;
		if ( S >= 60 ) {
			// 1<<S may not even fit in a ll, but anyway it's bigger than target_idx.
			source_char_idx = 0;
			subidx = target_idx;
			dbg(source_char_idx, subidx);
		} else {
			ll per_char = (1LL)<<S;
		...
		ll bits_set = num_bits_set( subidx );
		ll out_char = ( source_char + bits_set + S ) % 3;
		char out = 'A' + out_char;
		cout << out << '\n';
	}
