...
void solve() {
    lls(N, target_max, target_min);
    V<ll> dat;
    rv(N, dat);
    dbgR(N, target_max, target_min, dat);
    ...
    FOR(k, 0, N) {
        ...
        if (dat[k] < target_min || dat[k] > target_max) {
            bad.insert(k);
        } else {
            ok.insert(k);
        }
    }
    if (tmx.size() == 0 || tmn.size() == 0) {
        return ps(0);
    }

    auto check = [&](auto x) {return x >= target_min && x <= target_max;};

    ll start = 0;
    ll out = 0;
    while (start < N) {
        // move past bad stuff.
        while (start < N && !check(dat[start])) {++start;}
        if (start == N) {break;}
        ll end = start;
        while (end < N-1 && check(dat[end+1])) {++end;}

        for(auto maxit = tmx.lower_bound(start) ; maxit != tmx.upper_bound(end) ; ++maxit) {
            int pmax = *maxit;
            auto minit = tmn.lower_bound(pmax);
            int good_right = -1;
            int good_left = -1;
            ...
            auto helper = [&](ll size_left, ll size_right) {
                return (size_left + 1) * (size_right + 1);
            };
            auto subsolve = [&]() -> ll {
                if (good_left < 0 && good_right < 0) {
                    return 0;
                } else if (good_left < 0) {
                    ll size_left = pmax - start;
                    ll size_right = end - good_right;
                    return helper(size_left, size_right);
                } else if (good_right < 0) {
                    ll size_left = good_left - start;
                    ll size_right = end - pmax;
                    return helper(size_left, size_right);
                } else {
                    ll result = 0;
                    result += helper(good_left - start, end - pmax);
                    result += helper(pmax - start, end - good_right);
                    result -= helper(good_left - start, end - good_right);
                    return result;
                }
            };
            out += subsolve();
