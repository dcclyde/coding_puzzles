...
#define local_run (false)
    ...
    #undef dassert
    #define dassert(...) dbg_only(assert(__VA_ARGS__))
...
struct MCMF {
	...
	bool path(int s, int t) { // find lowest cost path to send flow through
		const C inf = 1e12; F0R(i,N) dist[i] = inf;
...
string to_string(MCMF::Edge e) {auto q = MT(e.to, e.flo, e.cap, e.cost); return to_string(q);}



void solve() {
    lls(N);
    V<tuple<ll,ll,ll>> dat;
    rv(N, dat);
    dbgR(N, dat);

    // first, compress.
    umap<ll,ll> ainv, binv;
    for(auto& [a,b,c] : dat) {
        if (ainv.find(a) == ainv.end()) {ainv[a] = ainv.size();} a = ainv[a];
        if (binv.find(b) == binv.end()) {binv[b] = binv.size();} b = binv[b];
    }
    dbg(dat);
    ...
    MCMF mcmf;
    /*
        Vertices 0...MAX_A-1 correspond to A values.
        Then next MAX_B vertices correspond to B values.
        Then next N+1 vertices correspond to
    */
    int OFFSET_A = 1;
    int OFFSET_B = OFFSET_A + MAX_A+1;
    int OFFSET_DEST = OFFSET_B + MAX_B+1;
    int START = OFFSET_DEST + N+1;
    int END = START + 1;
    mcmf.init(END + 1);

    for(auto& [a,b,c] : dat) {
        mcmf.ae(OFFSET_A + a, OFFSET_B + b, 1, MAX_C - c);
    }
    dbgP(pdh(mcmf.adj));
    FOR(a, 0, MAX_A+1) {
        mcmf.ae(START, OFFSET_A + a, 1, 0);
    }
    dbg(pdh(mcmf.adj));
    FOR(b, 0, MAX_B+1) {
        mcmf.ae(OFFSET_B + b, END, 1, 0);
    }
    dbgR(pdh(mcmf.adj));
    FOR(k, 1, N+1) {
        mcmf.ae(END, OFFSET_DEST + k, k, 0);
    }
    dbgB(pdh(mcmf.adj));
    dbg(OFFSET_A, OFFSET_B, OFFSET_DEST)
    dbg(START, END);
    dbg(MAX_C);
    el;
    // dbg_only(sort(all(mcmf.eds)))
    // dbgP(pdh(mcmf.eds));

    V<int> out;
    FOR(k, 1, N+1) {
        auto [flow, cost] = mcmf.calc(START, OFFSET_DEST+k);
        if (flow < k) {break;}
        ll final_cost = k * MAX_C - cost;
        out.push_back(final_cost);
    }
    ps(out.size());
    pv(out);
