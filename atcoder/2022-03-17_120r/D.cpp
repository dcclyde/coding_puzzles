...
void solve() {
    ...
    // color the array.
    vector<pii> B; B.reserve(N);
    FOR(k, 0, N) {
        B.emplace_back( A[k] , k );
    }
    sort(all(B));
    dbg(B);
    ...
    appearances[0].second.push_back( 0 );
    FOR(k, 0, N) {
        ...
        appearances[balance[k+1]].second.push_back( k+1 );
    }
    dbg(balance);
    dbg(appearances);
    el;
    ...
    vector<bool> color1;
    FOR(k, 0, N) {
        if ( color1.size() > 0 && colors[k] == color1.back() ) {
            out[k] = ')';
            color1.pop_back();
        } else {
            out[k] = '(';
            color1.push_back( !colors[k] );
        }
    }
    return ps(out);

#if 0
    function<void(int)> subsolve = [&](int start) {
        el;
        dbg(start, out);
        if ( start == N || out[start] != '.' ) {
            return;
        }
        int my_balance = balance[ start ];
        dbg(my_balance);
        auto& [idx, positions] = appearances[my_balance];
        dbg(idx, positions);
        int next_start = positions[idx + 1];
        ++idx;
        dbg(next_start, idx);
        out[start] = '(';
        out[next_start-1] = ')';
