...
inline namespace Input {
    ...
    void increment() {} // add one from each, for output. USE DEEP COPIES.
    tcTUU> void increment(T t, U&... u) { increment_one(t,+1); increment(u...); }
    void decrement() {} // subtract one from each, for input. USE REFERENCES.
...
/**
    Maybe I should assume I can't set up a straightforward segtree agg
    because the answer shouldn't play nice with updates?



    f(a,b+1) = f(a,b) + max(0, max(dat[a:b]) - dat[b+1])

    Let k = largest a such that a <= b and dat[a] >= dat[b+1].
    For a in [k+1, b], I don't need to update anything.
    For a in [0, k], I "should" add max(dat[a:b]) and subtract dat[b+1].


    Maybe try moving left instead?
    f(a-1, b) = ?
    Say k = first s.t. a <= k and dat[a-1] <= dat[k].
    Then the cost is like dat[a-1] * (k-a+1) - sum(a, k) + f(k+1, b).


    Maybe there's some trick where the right method sounds quadratic
    but actually it works due to a potential-based argument?
    I could track all the "highest so far" (k, dat[k]) counting from the right.
    I guess life would be good if I could also track the costs for those intervals.
    Then the cost within some interval is a separate easier? question.

 ** EVALUATING ONE QUERY
    Assume state has already been updated.
    dat[b] is the king for some interval, then we find increasingly higher kings.
    k0 = b, v0 = dat[b]
    k0 > k1 > k2 > ...
    vj = dat[kj]

    Now I want to eval on [a,b].
    It should be cost(a, kj) + (vj*(b-kj+1) - sum(kj,b)).
    The second part is fine but cost(a,kj) might be scary...
 ...
 ** UPDATING THE STATE
    ...
    OK start *again* based on online hints.
    Answer all queries with a given L.
    Maintain list of "highest seen so far".
    I think this will be similar to what I did above.
    The key difficulty was + will be: When I move L -> L+1, how do I update
        the part near the start of list of maxes?
        I just moved past one of my maxes

 *! OHHHH ok the trick should be that we order queries by DESCENDING L.
 **   Process all queries [L, ?] and then queries [L-1, ?] and so on.
    This makes it easy to update the list of maxes. Just maintain a stack
    with smallest max = top of stack.

    Updating that state is easy.
    Then how do I process a query?
    I guess I need subarray sums in the original data...
...
void solve() {
    ...
    V<V<pll>> queries(N);
    FOR(qid, 0, Q) {
        auto& [l, r] = queries_raw[qid];
        queries[l].emplace_back(r, qid);
    }
    dbgY(pdh(queries));

    // set up for subarray sums.
    V<ll> dat_pref(N+1, 0);
    FOR(k, 0, N) {dat_pref[k+1] = dat_pref[k] + dat[k];}
    auto range_sum = [&](int l, int r) { return dat_pref[r+1] - dat_pref[l]; };

    // stack items are (position, value for this chunk, value from here to the end).
    using t3 = tuple<ll,ll,ll>;
    V<t3> wip;
    V<ll> out(Q, -1);
    ROF(L, 0, N) {
        // Update the stack to include position L.
        // First, get rid of trash in the stack.
        while (wip.size() > 0 && dat[get<0>(wip.back())] <= dat[L]) {
            wip.pop_back();
        }
        // Now insert L.
        if (wip.size() == 0) {
            // only happens on very first iteration.
            wip.emplace_back(L, 0, 0);
        } else {
            auto [op, ov, osuff] = wip.back();
            // ll od = dat[op];
            wip.emplace_back();
            auto& [p, v, suff] = wip.back();
            p = L;
            ll d = dat[p];
            v = d * (op - p) - range_sum(p, op-1);
            suff = osuff + v;
        }
        dbg(L, wip);

        // ok, now process all the queries that end at L.
        for(auto& [R, qid] : queries[L]) {
            el;
            dbgY(L, R, qid);
            int last_chunk = fstTrue(0, wip.size()-1, [&](int x){
                return get<0>(wip[x]) <= R;
            });
            dbg(last_chunk);
            // first get the contribution from fully contained chunks.
            auto& result = out[qid];
            result = get<2>(wip.back());
            if (last_chunk < wip.size()) {
                result -= get<2>(wip[last_chunk]);
            }
            dbgcP("full chunks", result);
            // add contribution from the last partial chunk.
            auto [p, v, suff] = wip[last_chunk];
            dbgP(p, v, suff);
            ll d = dat[p];
            result += d * (R - p + 1) - range_sum(p, R);
            dbgc("final", result);
        }
    }
    pvn(out);
