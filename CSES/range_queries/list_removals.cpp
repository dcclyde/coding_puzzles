...
tcT> struct SegTree { // cmb(ID,b) = b
	...
	int first_satisfying_R(int base, int val, int ind=1, int l=0, int r=-1) {
		...
		return first_satisfying_R(base,val - seg[2*ind],2*ind+1,m+1,r);
	}
...
string to_string(SegTree<T> st) {
    vector<T> out;
    for ( int k = st.n ; k < st.n + st.orig_n ; ++k ) { out.push_back( st.seg[k] ); }
...
void solve() {
    ...
    SegTree<ll> st;
    st.init(N);
    FOR(k, 0, N) {st.seg[st.n+k] = 1;}
    st.build();

    V<ll> out;
    FOR(qid, 0, N) {
        ll1(q);
        int p = st.first_satisfying_R(0, q+1);
        st.upd(p, 0);
        out.push_back(dat[p]);
    }
    pv(out);
