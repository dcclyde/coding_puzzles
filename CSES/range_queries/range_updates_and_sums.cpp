...
tcT, class S> struct LazySeg {
	const T idT{0}; const S idS{0,0};  // ! identity
	...
	void push(int ind, int L, int R) { /// modify values for current node
        auto& [a, b] = lazy[ind];
        if (a == 0) {return;}
        if (a == 1) {seg[ind] += (R-L+1)*b;}
        else {seg[ind] = (R-L+1)*b;}
		// seg[ind] += (R-L+1)*lazy[ind]; // ! lazy * seg
		if (L != R) F0R(i,2) {
            // lazy[2*ind+i] += lazy[ind]; // ! lazy * lazy
            auto& [ca, cb] = lazy[2*ind+i];
            if (ca == 0 or a == 2) {ca=a; cb=b;}
            else {cb += b;}
        }
		lazy[ind] = idS;
...
void solve() {
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype <= 2) {
            lls(x);
            st.upd(l, r, MP(qtype,x));
            st.detailed_printouts();
            dbg(qid, qtype, MP(l,r), st);
        } else {
            ll out = st.query(l,r);
            ps(out);
        }
