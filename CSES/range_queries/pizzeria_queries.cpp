...
void solve() {
    ...
    /**
        ...
        If k < p, price from position k is dat[k] + p-k = st_l[k] + p.
        If p < k, price from position k is dat[k] + k-p = st_r[k] - p.
     */
    SegTree<ll> st_l, st_r;
    st_l.init(N); st_r.init(N);
    FOR(k, 0, N) {
        st_l.seg[st_l.n + k] = dat[k] - k;
        st_r.seg[st_r.n + k] = dat[k] + k;
    }
    st_l.build(); st_r.build();
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            ...
            int1(p);
            ll result_l = st_l.query(0, p) + p;
            ll result_r = st_r.query(p, N-1) - p;
            ll out = min(result_l, result_r);
            ps(out);
        }
    }
    return;
