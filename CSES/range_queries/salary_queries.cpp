...
void solve() {
    ...
    for(auto& [qtype, a, b] : queries) {
        uncompressed.push_back(b);
        if (qtype == '?') {uncompressed.push_back(a);}
    }
    remDup(uncompressed);
    umap<ll,ll> uncinv;
    ll SIZE = uncompressed.size();
    ...
    SegTree<ll> st;
    st.init(SIZE);
    for(auto& x : dat) { ++st.seg[st.n + x]; }
    st.build();

    FOR(qid, 0, Q) {
        auto& [qtype, a, b] = queries[qid];
        if (qtype == '?') {
            ll out = st.query(a, b);
            ps(out);
        } else {
            ll& old_salary = dat[a];
            st.upd(old_salary, st.seg[st.n + old_salary] - 1);
            st.upd(b, st.seg[st.n + b] + 1);
            old_salary = b;
        }
    }
