...
void solve() {
    ...
    // quad[r][c] = num 1s in [0 .. r-1] x [0 .. c-1].
    V<V<ll>> quad(N+1, V<ll>(N+1, 0));
    FOR(r, 0, N) FOR(c, 0, N) {
        quad[r+1][c+1] = (dat[r][c] == '*' ? (1) : (0))
            + quad[r][c+1]
            + quad[r+1][c]
            - quad[r][c];
    }
    ...
    auto query = [&](ll r1, ll c1, ll r2, ll c2) -> ll {
        return quad[r2+1][c2+1] - quad[r1][c2+1] - quad[r2+1][c1] + quad[r1][c1];
    };

    FOR(qid, 0, Q) {
        ll1(r1, c1, r2, c2);
        ll out = query(r1, c1, r2, c2);
        ps(out);
    }
    return;
}
