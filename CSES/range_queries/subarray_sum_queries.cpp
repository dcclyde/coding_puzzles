...
/**
    Can I imagine a solution for the case without queries
    and then use range updates to simulate that DP process quickly?

    Maybe something like dp[k] = max(0, dp[k-1] + dat[k])
    and then max over dp[k]?

    It's also kinda similar to finding smallest and largest prefix sums.
    If k_small < k_big then we just choose pref[k_big] - pref[k_small-1].


    I could also imagine a segtree storing more complex state.
    (best score within this chunk, best score touching L, best score touching R, chunk sum)
    Yeah this idea probably works...
 */

tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{0,0,0,0};  // ! identity
    T cmb(T x, T y) {
        auto& [ax,bx,cx,dx] = x;
        auto& [ay,by,cy,dy] = y;
        T z;
        ...
        // d = sum of whole chunk
        dz = dx + dy;

        // b = best score touching L side.
        bz = max(bx, dx + by);
        cz = max(cy, dy + cx);
        az = max({ax, ay, cx + by});
	...
	ll query(int l, int r) {	// associative op on [l, r]
		T ra = ID, rb = ID;
		...
		// return cmb(ra,rb);
        T all_together = cmb(ra,rb);
        return get<0>(all_together);
	}
...
void solve() {
    ...
    SegTree<tuple<ll,ll,ll,ll>> st;
    st.init(N);
    FOR(k, 0, N) {st.upd(k, dat[k]);}

    FOR(qid, 0, Q) {
        int1(p); lls(v);
        st.upd(p, v);
        ll out = st.query(0, N-1);
        ps(out);
    }
    return;
}
