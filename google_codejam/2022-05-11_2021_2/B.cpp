...
/**
    ...
    I need longest possible sequence with a0 | a1 | a2 | ... | aL
        AND sum(a_j) = N.

    a0 divides N.
    I could divide out that factor.
    Then

    Maybe this is some kinda DP?
    dp[n] = longest sequence ending at n?

 *! At first step I can't pick 2. But at later steps it's fine (like 3 -> 6).

    N = 120
    Say I start with 3. Then I have 117 to go.
    Think of that as 39 to go and 1 point scored so far.
    Now pick 3 again.
    36 --> 12 to go and 2 points scored.
    Now 2.
    5 to go and 3 points scored.
    Now pick 5. Done, 4 points scored.
 */

#ifdef DCCLYDE_LOCAL
const ll MAX = 100;
#else
const ll MAX = 1'000'001;
#endif
...
#if 0
    FOR(d, 2, MAX+1) {
        for(ll m = 1 ; m*d <= MAX ; ++m) {
            // if I'm currently at m*d, I could choose d next.
            ckmax(dp[m*d], dp[m-1] + 1);
        }
    }
#endif
    FOR(m, 1, MAX+1) {
        for(ll d = 2; m*d <= MAX; ++d) {
            ckmax(dp[m*d], dp[m-1] + 1);
        }
    }
    dbgcR("Pre done", dp);
...
void solve() {
    ...
    // * Careful: The first step isn't allowed to be 2.
    auto divs = divisors(N);
    ll out = 0;
    for(auto& d : divs) {
        if (d <= 2) {continue;}
        ckmax(out, dp[N/d - 1] + 1);
    }
    ps(out);
    return;
}
