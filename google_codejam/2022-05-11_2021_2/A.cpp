...
/**
    S j k --> swap positions j and k. Judge prints 1.
    M j k --> get min over [j, k].
    D --> finished, judge outputs 1

    N = 100
    Total cost <= 6e8
    cost of M on length L is 1e8 / L.

    Can I just find min overall, then min over all but that guy, etc?
    N = 100
    so it's 1e8/100 + 1e8/99 + ...
    That's probably fine.
    Yeah, it works per Python.
 */
