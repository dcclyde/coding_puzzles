...
/**
    ...
    Always start with 1.
    What do the deltas look like?
    Can't increase by more than 1.
    Fine to stay the same or decrease by "any" number.
        If dv = +1, new one is smaller than all visible ones.

    Do I always know which indices are visible? Probably yes?

    I think I always learn exactly where the new item fits
        among all VISIBLE past items.

    1 1 2:
        b > a
        c < b
        so b is biggest but a, c could be in either order.

    If I just have a big pile of < comparisons,
        can I find how many permutations will work?

    Per notebook, maybe I want to build a tree representing the ordering info.
    Then run tree dp.
    My score = prod(child scores) * (ways to break my subtree down into child subtrees)
...
void solve() {
    ...
    FOR(k, 0, N-1) {
        if (dat[k+1] >= dat[k] + 2) {return ps(0);}
    }

    // build a tree based on dat.
    V<ll> visible;
    V<ll> parent(N, -2);
    FOR(k, 0, N) {
        ll val = dat[k];
        ll last_removed = -1;
        while (visible.size() >= val) {
            last_removed = visible.back();
            visible.pop_back();
        }
        parent[k] = (visible.empty()) ? (-1) : (visible.back());
        visible.push_back(k);
        if (last_removed != -1) {
            parent[last_removed] = k;
        }
    }
    ll ROOT = visible.front();
    dbg(ROOT, parent);
    ...
    V<ll> subtree_size(N);
    V<mi> dp(N);
    auto dfs = [&](ll curr, ll par, auto&& self) -> void {
        // work on parent
        subtree_size[curr] = 1;
        dp[curr] = 1;
        // todo
        ...
        // todo
        ll size_remaining = subtree_size[curr] - 1;
        for (auto& o : G[curr]) {
            if (o == par) {continue;}
            dp[curr] *= comb(size_remaining, subtree_size[o]);
            size_remaining -= subtree_size[o];
        }
