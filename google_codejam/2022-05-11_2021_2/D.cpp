...
/**
    Grid is up to 10x10.
    Costs 1 to flip a bit and 1 to swap two adjacent bits.
    Just output the cost.

    I guess the trick is to find as many productive swaps as possible.

    How much do I need to worry about "chains"?
    0000001 --> 1000000
        would have been better to just flip the ends.
    10101 --> 01010
        swap, swap, flip

    010
     0
    Can I just find any good swap where

    What's a case where it's bad to swap randomly?
    0101 --> 1010

    Maybe just pick one of the items with smallest # useful swaps available?
 */
...
void solve() {
    ...
    FOR(r, 0, R) {
        strings(raw);
        FOR(c, 0, C) {dat[r].push_back(raw[c] == 'G');}
    }
    V<V<bool>> goal(R);
    FOR(r, 0, R) {
        strings(raw);
        FOR(c, 0, C) {goal[r].push_back(raw[c] == 'G');}
    }
    ll N = R * C;
    dbgR(R, C, pdh(dat), pdh(goal));
    el;

    ll cost = 0;
    while (true) {
        map<pll, V<pll>> swap_options;
        FOR(r, 0, R) FOR(c, 0, C) {
            for(auto& [dr, dc] : stepsOrthogonal) {
                ...
                if (
                    dat[r][c] != goal[r][c]
                    &&
                    dat[rr][cc] != goal[rr][cc]
                    &&
                    dat[r][c] != dat[rr][cc]
                ) {
                    swap_options[MP(r,c)].emplace_back(rr, cc);
                }
            }
        }
        dbgP(pdh(swap_options));
        if (swap_options.empty()) {break;}
        pll best = swap_options.begin()->first;
        for(auto& [base, opts] : swap_options) {
            if (opts.size() < swap_options[best].size()) {best = base;}
        }
        pll other = swap_options[best].front();
        auto& [r, c] = best;
        auto& [ro, co] = other;
        dat[r][c] = dat[r][c] ^ 1;
        dat[ro][co] = dat[ro][co] ^ 1;
        ++cost;
        dbgc("swap", cost, MP(r,c), MP(ro,co), pdh(dat));
    }
    FOR(r, 0, R) FOR(c, 0, C) {
        if (dat[r][c] != goal[r][c]) {++cost;}
    }
    ps(cost);
    return;
}
