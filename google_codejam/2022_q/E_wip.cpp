...
void solve() {
    ...
    if ( N <= K ) {
        ...
        ll degree_sum = 0;
        FOR(k, 0, N) {
            auto [room, deg] = tele(k);
            degrees[k] = deg;
            degree_sum += deg;
        }
        return finish(degree_sum / 2);
    }
    ints(room, deg);
    ...
    while ( K >= 2 ) {
        int moveto = dist(rng);
        tie(room, deg) = tele(moveto);
        d1.push_back(deg);
        tie(room, deg) = walk();
        d2.push_back(deg);
        K -= 2;
    }
    ll steps = d1.size() + d2.size();
    ll d1sum = 0;
    ll d2sum = 0;
    for(auto& x : d1) d1sum += x;
    for(auto& x : d2) d2sum += x;
    ll combined = d1sum + d2sum;
    // ll out = (N*combined) / (2*steps);

    V<ll> estimates;
    FOR(k, 0, d1.size()) {
        estimates.push_back(N*d1[k]);
        estimates.push_back((N-1)*(d2[k]-1) + 2*d1[k]);
    }
    ll esum = 0; for(auto& x : estimates) esum += x;
    ll out = esum / (2LL*estimates.size());
