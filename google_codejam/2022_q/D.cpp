...
void solve() {
    ...
    V<ll> total(N);
    V<ll> fpscore(N);
    function<void(int)> subsolve = [&](int curr) {
        fpscore[curr] = INF;
        for(auto& o : children[curr]) {
            subsolve(o);
            ckmin(fpscore[curr], fpscore[o]);
            total[curr] += total[o];
        }
        if ( fpscore[curr] == INF ) {
            total[curr] = dat[curr];
            fpscore[curr] = dat[curr];
        } else if ( dat[curr] > fpscore[curr] ) {
            total[curr] += dat[curr] - fpscore[curr];
            fpscore[curr] = dat[curr];
        }
    };
