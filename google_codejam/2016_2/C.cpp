...
void solve() {
    ...
    ll BITS = R * C;
    FOR(mask, 0, (1<<BITS)) {
        V<V<char>> attempt(R, V<char>(C, '/'));
        FOR(b, 0, BITS) { if ((mask>>b) & 1) {attempt[b/C][b%C] = '\\';} }
        dbgY(mask, pdh(attempt));

        // now just start tracing paths I guess?
        bool ok = true;
        FOR(k, 0, dat.size()) {
            ll result = trace(attempt, dat[k]);
            dbg(dat[k], result);
            if (dat[k^1] != result) {
                ok = false;
                break;
