...
/**
    Do I just want K/2 highest probabilities and K/2 lowest?
    No way, right?
    If they're

    Maybe I always want some chunk from the bottom and the rest from top?
    N <= 200.
    What's my algorithm?
 *? Is it worth computing everything in long longs?

    How do I get the probability for a given committee?
    Maybe FFT could speed this up?
    But anyway it seems like I could just do DP.
    dp[n][k] = prob that first n members have k yeses.
    dp[n+1][k] =
 */
...
void solve() {
    ...
    FOR(num_low, 0, K+1) {
        V<db> used;
        FOR(k, 0, num_low) {used.push_back(dat[k]);}
        FOR(k, N-K+num_low, N) {used.push_back(dat[k]);}

        V<V<db>> dp(K+1, V<db>(K+1));
        dp[0][0] = 1.;
        dbg(K, used);
        FOR(k, 1, K+1) {
            FOR(y, 0, k+1) {
                // what's probability of y yeses from first k people?
                dbgP(k, y);
                dp[k][y] = (1. - used[k-1]) * dp[k-1][y];
                dp[k][y] += (y > 0) ? (used[k-1] * dp[k-1][y-1]) : 0;
            }
        }
        db curr = dp[K][K/2];
        if (ckmax(out, curr)) {
