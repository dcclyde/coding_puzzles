...
void brute() {
    ...
    // just try all possible start positions.
    V<string> good;
    {
        string options = "PRS";
    string curr;
    auto do_something = [&]() -> bool {
        ...
        // dbgcP("d_s", curr);
        map<char, ll> counts;
        for(auto& c : curr) {++counts[c];}
        if (counts['R'] != R || counts['S'] != S || counts['P'] != P) { return false; }

        string wip = curr;
        FOR(round, 0, N) {
            string after;
            FOR(k, 0, wip.size(), 2) {
                after.push_back(winner(wip[k], wip[k+1]));
                if (after.back() == -1) {return false;}
            }
            wip = after;
        }
        assert(wip.size() == 1);
    ...
    function<void(void)> product_rec_helper = [&]{
        if (curr.size() == (1<<N)) {do_something(); return;}  // ! define max depth
        int n = curr.size();
        FOR(k, 0, 3) {  // ! iterate over options at depth n
                curr.push_back(options[k]);
                product_rec_helper();
                // if (product_rec_helper()) {
                //     return true;
                // }
                curr.pop_back();
        }
...
/**
    R <= 12, so N <= 4e5 ish.
    If I ever have a srict majority before a round, then I lose.
    In each round, I'll use A copies of RP --> P, B of PS --> S, C of RS --> R.

    In alpha order we have P, R, S.
    So for first round I'd like to have as many PRs as possible, then PSs.

    Is there just only one way to split em up?
    Seems that way.
    What coeffs are those?
    A + B = P
    A + C = R
    B + C = S
...
/**
    Given N and the final winner, I think the whole tournament is determined
        up to swapping the two sides in any duel.
 */


const string plays = "RPS";
...
void solve() {
    ...
    for(auto winner : {'R', 'P', 'S'}) {
        string curr;
        curr.push_back(winner);
        FOR(round, 0, N) {
            string next;
            for(auto& c : curr) {
                next.push_back(c);
                next.push_back(beats[c]);
            }
            swap(curr, next);
        }
        ll r = 0;
        ...
        if (R != r || S != s || P != p) {continue;}

        dbgY(winner, curr);
        // now sort stuff.
        FOR(round, 0, N) {
            ll C = (1<<round);  // chunk size
            FOR(k, 0, 1<<(N-round-1)) {
                // do I need to flip these two chunks?
                ll a = 2*k*C;
                ll b = a + C;
                bool need_flip = false;
                FOR(j, 0, C) {
                    if (curr[a+j] == curr[b+j]) {continue;}
                    need_flip = (curr[a+j] > curr[b+j]);
                    break;
                }
                if (need_flip) {
                    FOR(j, 0, C) {swap(curr[a+j], curr[b+j]);}
                }
                dbg(round, k, C, need_flip, curr);
            }
        }
        return ps(curr);
    }
