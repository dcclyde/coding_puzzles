...
/**
    Just time-trialing the Small dataset.
 */
...
void solve() {
    ...
    FOR(k, 0, N) {
        strings(dummy);
        FOR(j, 0, N) {dat[k].push_back(dummy[j] == '1');}
    }
    dbgR(N, pdh(dat));
    el;

    auto contains = [&](auto& v, ll k) {
        for(auto& x : v) {if (k == x) {return true;}}
        return false;
    };

    auto check = [&](auto& prop) -> bool {
        auto ph = rangeint(0, N);
        do {
            // I know their abilities and their arrival order.
            ...
            function<void(void)> product_rec_helper = [&]{
                if (curr.size() == N) {return;}  // ! define max depth
                int n = curr.size();
                bool ok = false;
                dbgcP("prh", curr);
                FOR(k, 0, N) {  // ! iterate over options at depth n
                    if (!prop[ph[n]][k]) {continue;}
                    if (contains(curr, k)) {continue;}
                    dbgc("ok", k);
                    ok = true;
                    curr.push_back(k);
                    product_rec_helper();
    ...
    FOR(mask, 0, (1<<BITS)) {
        auto wip = dat;
        ll cost = 0;
        FOR(b, 0, BITS) { if ((mask>>b) & 1) {wip[b/N][b%N] = 1; ++cost;} }
        dbg(mask, cost, wip);
        // continue;
        if (check(wip)) {
            ckmin(out, cost);
            if (out == 2) {assert(false);}
        }
    }
    ps(out);
    return;
