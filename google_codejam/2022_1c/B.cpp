...
/**
    Given stuff has sum S and sum of squares Q.
    Think about the delta D = S^2 - Q.

    I want to add at least 1, at most K new things.
    If they have sum A and s.o.s. B...
    Combined sum is S+A. Combined sos is Q+B.
    (S+A)^2 = S^2   + A^2 + 2*S*A
            = Q + D + (A+S)^2 - S^2

    S is replaced by S+A
    Q is replaced by Q+B
    S^2 - Q is replaced by S^2 + 2*S*A + A^2 - Q - B
        ignore the S^2 - Q part (it was there before)
        2*S*A + A^2 - B
        2*S*A + (2*x*y for x < y in new stuff)

    (S + x)^2 = S^2 + x^2 + 2*x*S

    (S^2 - Q) increased by 2*x*S

 ** DATA SIZE
    N <= 1000
    dat[j] <= 1000
    If all entries are 1000: S = 1e6, Q = 1e9, D = 1e12 - 1e9
        difference is like 1e12

    C's proposal:
    We add one item that brings S to 1.
    Now S = 1, Q ~ 1e12, D ~ 1e12
 */
