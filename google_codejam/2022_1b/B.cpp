...
void solve() {
    lls(N, P);
    V<V<ll>> raw(N);
    V<V<ll>> dat(N, V<ll>(2, -1));
    FOR(k, 0, N) {
        rv(P, raw[k]);
        dat[k][0] = *min_element(all(raw[k]));
        dat[k][1] = *max_element(all(raw[k]));
    }
    dbgR(N, pdh(dat));
    el;

    /**
        dp[n][0] = cost to finish nth customer and be at the low end.
        dp[n][1] = ... high.

        Hang onto all the memory just because why not. Unnecessary though.

        dp[n] holds the results after moving from states dat[n-1] to dat[n].
     */

    V<V<ll>> dp(N+1, V<ll>(2, INF_ll));
    dp[0][0] = 0;
    dp[0][1] = 0;
    dp[1][0] = dat[0][1] + (dat[0][1] - dat[0][0]);
    dp[1][1] = dat[0][1];

    FOR(n, 1, N) {
        FOR(a, 0, 2) FOR(b, 0, 2) {
            ll c1 = abs(dat[n-1][a] - dat[n][b]);
            ll c2 = abs(dat[n][0] - dat[n][1]);
            ll curr_cost = c1 + c2;
            ckmin(dp[n+1][b], curr_cost + dp[n][a]);
            dbgP(n,a,b, " ", MP(c1, c2), curr_cost, dp[n][a], dp[n+1][b]);
        }
        dbg(n, dp[n], dat[n]);
        el;
    }
    dbgY(pdh(dp));
    ll out = min(dp[N][0], dp[N][1]);
    ps(out);
    return;
}
