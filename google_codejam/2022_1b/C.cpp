...
void learn() {
    ...
    // find the valid masks (up to rotation).
    set<int> valid_masks_set;
    auto& vms = valid_masks_set;
    FOR(mask, 0, N) {
        bool ok = true;
        FOR(shift, 0, BITS) {
            int m2 = ((mask >> shift) | (mask << (BITS-shift))) & ((1<<BITS) - 1);
            if (vms.find(m2) != vms.end()) {
                ok = false;
                break;
            }
        }
        if (ok) {vms.insert(mask);}
    }
    V<int> valid_masks;
    valid_masks.insert(valid_masks.end(), all(vms));
    FOR(k, 0, valid_masks.size()) {
        dbg(k, valid_masks[k], bitset<BITS>(valid_masks[k]));
    }
    el;
    ll S = valid_masks.size();
    V<int> vminv(N);
    FOR(vmi, 0, S) {
        auto& mask = valid_masks[vmi];
        FOR(shift, 0, BITS) {
            int m2 = ((mask >> shift) | (mask << (BITS-shift))) & ((1<<BITS) - 1);
            vminv[m2] = vmi;
        }
    ...
    V<pair<int,V<bitset<BITS>>>> dp(S, MP(INF_i, V<bitset<BITS>>()));
    dp[0].first = 0;
    ...
    while(!stop) {
        stop = true;
        FOR(ai, 0, S) {
            int a = valid_masks[ai];
            auto& [cost, options] = dp[ai];
            if (cost != INF_i) {continue;}
            stop = false;
            for(auto& mask : valid_masks) {
                ...
                FOR(shift, 0, BITS) {
                    int m2 = ((mask >> shift) | (mask << (BITS-shift))) & ((1<<BITS) - 1);
                    int b = a ^ m2;
                    ckmax(worst_cost, 1 + dp[vminv[b]].first);
                }
                if (worst_cost < cost) {options.clear();}
                if (ckmin(cost, worst_cost)) {options.emplace_back(mask);}
            }
        }
    }
    el; el; el;
    FOR(k, 0, valid_masks.size()) {
        dbg(k, valid_masks[k], pct(valid_masks[k]), bitset<BITS>(valid_masks[k]), dp[k]);
    }
    el; el;
    V<pair<int,V<bitset<BITS>>>> per_num_bits(BITS + 1);
    V<V<pair<int,V<bitset<BITS>>>>> per_both(BITS+1);
    for(auto& q : per_both) {q.resize(BITS/2 + 1);}
    for(auto& mask : valid_masks) {
        int pc = pct(mask);
        int pc2 = 0;
        FOR(b, 0, BITS, 2) { pc2 += (mask>>b) & 1; }
        auto& [ctr, instances] = per_num_bits[pc];
        ++ctr;
        instances.emplace_back(mask);

        auto& [ctr2, instances2] = per_both[pc][pc2];
        ++ctr2;
        instances2.emplace_back(mask);
    }
    dbgP(pdh(per_num_bits));
    el; el;
    FOR(a, 0, BITS+1) FOR(b, 0, BITS/2+1) {
        if (per_both[a][b].first == 0) {continue;}
        dbg(a, b, per_both[a][b]);
    }
    assert(false);
}

/**
    I think this is just an augmented graph.
    There are only 36 base states, and then we augment with 2^(num options with same popcount) for which states seem possible
 */
