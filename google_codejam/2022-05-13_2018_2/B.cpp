...
/**
    R, B <= 500.
    What's my dp?
    f(r, b, s) = best if I start with r, b but I'm paying s extra Rs per person?
    f(r,b,0) = answer
    f(r,b,s)?
        Need biggest k such that f(r-ks, b) >= k.

    f(r,b,0)?
        Condition on t = number of guys with only Bs.
        t + f(r, b-(t*(t+1)/2), 1)

    then maybe I don't need any s > 1?
...
void precompute(ll R, ll B) {
    f.assign(R+1, V<ll>(B+1));
    g.assign(R+1, V<ll>(B+1));

    FOR(r, 0, R+1) FOR(b, 0, B+1) {
        if (r == 0 && b == 0) {f[r][b] = 1; g[r][b] = 0; continue;}
        FOR(k, 0, r+1) {
            if (f[r-k][b] >= k) {g[r][b] = k;}
            else {break;}
        }
        FOR(t, 0, R+1) {
            ll tri = t * (t+1) / 2;
            if (tri > b) {break;}
            ckmax(f[r][b], t+1 + g[r][b - tri]);
        }
        // dbg(MP(r,b), f[r][b], g[r][b]);
        // timebomb(10);
    }
    FOR(r,0,R+1) FOR(b,0,B+1) {
        if (f[r][b] != f[b][r]) {
            dbgR(MP(r,b),f[r][b],f[b][r]);
            assert(false);
        };
        // if (g[r][b] != g[b][r]) {
        //     dbgR(MP(r,b),g[r][b],g[b][r]);
        //     assert(false);
        // };
    }
