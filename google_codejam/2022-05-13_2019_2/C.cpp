...
/**
    Each diff is like p*A + q*B and I need them all to be strictly > 0.
    Some of them may be trivially satisfied or trivially IMPOSSIBLE.
    The other 2 (open) quadrants give an upper or lower bound.

    Choose the most extreme upper/lower bounds. Might be IMPOSSIBLE but otherwise
        I now have an interval and I need to choose a fraction in there.
        I want smallest possible denominator I guess? Tiebreak by small num?

    "The fast way to do this is to develop the continued fraction representation
        of the endpoints until they differ."
 */

#pragma region  // continued fractions
//
// Continued Fraction
//
// Description:
...
void solve() {
    ...
    FOR(k, 0, N-1) {
        auto& [a,b] = dat[k];
        auto& [c,d] = dat[k+1];
        diffs.emplace_back(c-a, d-b);
    }

    auto reduce = [](ll& a, ll& b) -> void {
        // assume a > 0, b < 0.
        ll g = gcd(a,b);
        a /= g; b /= g;
        return;
    ...
    for(auto& [a, b] : diffs) {
        ...
        if (a < 0) {
            /**
                ...
                -a * ud < b * un
             */
            if (-a * ud < b * un) { un = -a; ud = b; }
        } else {
            if (a * ld > -b * ln) { ln = a; ld = -b; }
        }
    }
    dbgY(upper, lower);
    // if upper <= lower, it's impossible.
    if (un * ld <= ln * ud) {return IMPOSSIBLE;}

    // otherwise, I need some continued fraction crap?
    // any chance there's another way to get this done? CFs seem so mathy...
    auto ucf = Rational(un, ud);
    auto lcf = Rational(ln, ld);
    dbg(ucf.head->value());
    // dbg(ucf.head->next());
