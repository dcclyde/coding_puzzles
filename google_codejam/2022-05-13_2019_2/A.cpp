...
/**
    (1,1) (2,1) (1,2)
    diffs are (1,0), (0,1), (1,-1).

    Maybe I just want # distinct "diff slopes" with x > 0, y < 0?
 */
...
void solve() {
    ...
    uset<pll> results;
    for(auto& [a,b] : dat) for(auto& [r,s] : dat) {
        ll dx = r - a;
        ll dy = s - b;
        if (dx <= 0 || dy >= 0) {continue;}

        // make some canonical form I guess
        auto frac = reduce(dx, dy);
        results.insert(frac);
    }
    ll out = results.size() + 1;
    dbgY(out, results);
    ps(out);
    return;
}
