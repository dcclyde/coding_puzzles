...
// __int128_t modinvLL(__int128_t a, __int128_t b){
//     return 1<a ? b - modinvLL(b%a,a)*b/a : 1;
// }
...
void solve() {
    ...
    V<ll> dat = {A, B, C};
    sort(all(dat));
    ll MOD = 360LL * 12LL * 10'000'000'000;
    do {
        ll H = dat[0];
        ll M = dat[1];
        ll S = dat[2];
        ...
        dbg(H, M, S);
        dbg(v11, v719);
        if ((v11 * 719) % MOD != (v719 * 11) % MOD) {
            el;
            continue;
        }
        __int128_t m11 = modinvLL((__int128_t)11, (__int128_t)MOD);
        ll v = (m11 * v11) % MOD;
        dbg(v);
        ...
        ll mout = v / (MOD/(12*60));
        v %=(MOD/(12*60));
        ll sout = v / (MOD/(12*60*60));
        v %= (MOD/(12*60*60));
        ll nout = v;
        ps(hout, mout, sout, nout);
        return;
