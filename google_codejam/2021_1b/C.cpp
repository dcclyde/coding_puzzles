...
void solve() {
    ...
    FOR(step, 0, N*B) {
        lls(curr);
        if (curr == 9 || (curr >= 7 && (db(step) / (N*B)) > .9)) {
            int big = 0;
            while (dat[big].size() == B) {
                ++big;
            }
            FOR(k, big+1, N) {
                if (dat[k].size() < B && dat[k].size() > dat[big].size()) {
                    big = k;
                }
            }
            dat[big].push_back(curr);
            cout << big+1 << '\n' << flush;
        } else {
            int small = 0;
            while (dat[small].size() == B) {
                ++small;
            }
            FOR(k, small+1, N) {
                if (dat[k].size() < B && dat[k].size() < dat[small].size()) {
                    small = k;
                }
            }
            dat[small].push_back(curr);
            cout << small+1 << '\n' << flush;
        }
    }
    return;
