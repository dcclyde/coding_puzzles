...
/**
    S = skips_needed = N^2-1 - K = N^2 - 1 - K.
    N = 5, N^2 = 25, K = 4. Without skips I reach 5. Need 20 skips.
...
void brute() {
    ...
    auto rotate = [](ll& dr, ll& dc) {
        if (dc == 1) {dr = 1; dc = 0; return;}
        if (dr == 1) {dr = 0; dc = -1; return;}
        if (dc == -1) {dr = -1; dc = 0; return;}
        if (dr == -1) {dr = 0; dc = 1; return;}
        dbg(dr, dc);
        assert(false);
    ...
    while (todo <= N*N) {
        board[r][c] = todo;
        if (r + dr == R || c + dc == C || r + dr < 0 || c + dc < 0 || board[r+dr][c+dc] != 0) {
            rotate(dr, dc);
        }
        r += dr; c += dc;
        ++todo;
...
bool learn_helper(ll N, ll K) {
    ll S = N*N - K - 1;
    dbgR(S);
    if (S % 2 == 1) {IMPOSSIBLE; return false;}

    V<ll> skip_sizes;
    ll total_skipped = 0;
    ll next_skip = 6;
    while (total_skipped < S) {
        skip_sizes.push_back(next_skip);
        total_skipped += next_skip;
        next_skip += 8;
    }
    dbgY(S, total_skipped, skip_sizes);

    // now total_skipped is a little above S.
    // I have leeway: I could subtract up to 6 from each of those skips.
    ll ns = skip_sizes.size();
    ll min_possible = total_skipped - 6 * ns;
    if (min_possible > S) {IMPOSSIBLE; return false;}

    // subtract from smaller skips first.
    ll idx = 0;
    while (total_skipped > S) {
        ll ds = minll(total_skipped - S, 6);
        skip_sizes[idx] -= ds;
        total_skipped -= ds;
        ++idx;
    }
    dbg(S, total_skipped, skip_sizes);

    // now I just need to find where these skips will actually happen.
    // Think in terms of distance from the end.
    V<pll> out;
    FOR(k, 0, skip_sizes.size()) {
        if (skip_sizes[k] == 0) {continue;}
        ll skip_offset = skip_sizes[k] % 8;
        ll skip_sidenum = (6 - skip_offset) / 2;

        ll square_sidelength = 2*k + 3;
        ll square_size = square_sidelength * square_sidelength;
        ll top_left = N*N - square_size + 1;
        ll shortcut_start = top_left + (square_sidelength / 2);
        shortcut_start += skip_sidenum * (square_sidelength - 1);

        ll shortcut_end = shortcut_start + skip_sizes[k] + 1;
        out.emplace_back(shortcut_start, shortcut_end);
    }
    ...
    auto check = [&]() {
        ...
        assert(skips.size() == out.size());
        ll pos = 1;
        ll turns = 0;
        while (pos < N*N) {
            if (skips.find(pos) != skips.end()) {
                pos = skips[pos];
            } else {
                ++pos;
            }
...
void solve() {
    ...
    if (K < N-1) {return IMPOSSIBLE;}

    // do some complete circuits to start?
    // MAX_SKIPS = N^2-1 - N+1 = N*N - N.
    auto max_skips = [&](ll n) {
        return n*n - n;
    };
    ll inner = N;
    while (max_skips(inner - 1) >= S) { --inner; }
    ll start = (inner * inner) - 1;


    // if we're here, it should be possible.
    ll Q = (K - (N-1)) / 2;
    dbg(Q);
    ll MAX_Q = (N*N - 1 - (N-1)) / 2;
    /**
        4 + 8 + ... >= P
     */
    ll P = (N-1)/2;
    ...
    auto check_impossible = [&]() {
        if (S % 2 == 1) {return true;}
        if (K < N-1) {return true;}
        assert(false);
    };

    if (S % 2 == 1) {dbg_only(check_impossible()); return IMPOSSIBLE;}
    ...
    ll ns = skip_sizes.size();
    ll min_possible = total_skipped - 6 * ns;
    if (min_possible > S) {dbg_only(check_impossible());return IMPOSSIBLE;}
