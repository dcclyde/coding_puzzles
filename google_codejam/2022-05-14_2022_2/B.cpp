...
/**
    ...
    Good version: Round distance to NEAREST integer, ties round DOWN.

    Bad version: Need to find x s.t. round(sqrt(R^2 - x^2)) = y.

    Basically we're looking for (x,y) s.t. round(sqrt(R^2 - x^2)) skips y.
...
void solve() {
    ...
    FOR(x, 0, R+1) {
        FOR(r, x, R+1) {
            // Want first y s.t. r^2 - x^2 <= (y + .5)^2
            /**
                4(r^2 - x^2) <= (2y + 1)^2
             */
            ll targ = 4 * (r*r - x*x);
            ll y = fstTrue(0, R, [&](ll m) {return (2*m+1)*(2*m+1) >= targ;});
            dbgcR("label", r, MP(x,y));
            out.emplace(x,y);
            out.emplace(-x,y);
            out.emplace(x,-y);
            out.emplace(-x,-y);
