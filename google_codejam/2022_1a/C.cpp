...
int S;
V<V<int>> f2s;  // "flat to state"
V<V<int>> f2ctrs;
map<V<int>, int> s2f;
map<V<int>, V<int>> s2opts;  // map from list of ctrs to all flat states.
V<V<int>> transitions;
void precompute() {
    int W = 3;
    // Make a flat listing of all possible stack states.
    {
        V<int> curr;
        auto do_something = [&]() {
            // ! do something here
            // I want all permutations of this combination of items.
            V<int> items;
            FOR(k, 0, W) {
                rep(curr[k]) {items.push_back(k);}
            }
            do {
                s2f[items] = f2s.size();
                s2opts[curr].push_back(f2s.size());
                if (items.size() <= 1) {
                    dbgY(items, curr, s2opts[curr]);
                }
                f2s.push_back(items);
                f2ctrs.push_back(curr);
            } while (next_permutation(all(items)));
        };

        function<void(void)> product_rec_helper = [&]{
            if (curr.size() == W) {return do_something();}
            FOR(k, 0, 4) {
                curr.push_back(k);
                product_rec_helper();
                curr.pop_back();
    ...
    S = f2s.size();
    dbgc("built states", TIME(), S);
    // dbg(pdh(f2s));
    // dbgP(pdh(s2f));

    // compute costs of transitioning from flat state s to t.
    transitions.assign(S, V<int>(S, -1));
    FOR(s, 0, S) FOR(t, s, S) {
        auto& curr = f2s[s];
        auto& other = f2s[t];
        int k = 0;
        while (k < curr.size() && k < other.size() && curr[k] == other[k]) {
            ++k;
        }
        int cost = curr.size() + other.size() - 2*k;
        transitions[s][t] = cost;
        transitions[t][s] = cost;
    }
    dbgcP("built transitions", TIME());
}

void brute() {
    lls(N, W);
    V<V<int>> dat(N+2, V<int>(W, 0));
    FOR(k, 0, N) {
        rv(W, dat[k+1]);
    }
    for(auto& q : dat) {
...
#pragma endregion
    ...
    FOR(n, 0, N) {
        dbgR(n, dat[n], s2opts[dat[n]]);
    }

    const ll INF = 1e18;
    V<V<ll>> dp(N+1, V<ll>(S+1, INF));
    dp[0][0] = 0;
    FOR(n, 0, N-1) {
        dbgR(n, s2opts[dat[n+1]]);
        FOR(s, 0, S) {
            if (dp[n][s] == INF) {continue;}
            dbgW(s, f2s[s], s2opts[dat[n+1]]);
            // try all possible next states.
            for(auto& t : s2opts[dat[n+1]]) {
                dbg(t, f2s[t]);
                ckmin(dp[n+1][t], dp[n][s] + transitions[s][t]);
                dbgcP("update", n+1, t, dp[n+1][t], f2s[t]);
            }
        }
    }
    // dbg(pdh(dp));
