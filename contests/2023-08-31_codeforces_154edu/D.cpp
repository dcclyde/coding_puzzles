...
void solve() {
    ...
    // mup[n] = num cases with A[k] >= A[k+1] in [0:n].
    V<ll> messed_up_prefix(N+1);
    FOR(k, 1, N) {
        messed_up_prefix[k+1] = messed_up_prefix[k] + (A[k-1] >= A[k]);
    }

    V<ll> reverse_messed_up_prefix(N+1);
    FOR(k, 1, N) {
        reverse_messed_up_prefix[k+1] = reverse_messed_up_prefix[k] + (A[k-1] <= A[k]);
    }

    dbg(messed_up_prefix);
    dbgP(reverse_messed_up_prefix);
    el;

    // if I don't negate anything, I just pay mup[N] I think.
    ll out = messed_up_prefix[N];
    FOR(last_negated, 0, N) {
        // we're negating `last_negated+1` items.
        ll cost_prefix = reverse_messed_up_prefix[last_negated + 1];
        /*
            [1, 5, 8, 2, 4, 3]
            N = 6
            last_negated = 3
            I should just include the [4, 3] part.
            mup[6]

            last_negated = 0
            I should include [5, 8, 2, 4, 3].
            mup[6] tells me the total messups.
            mup[2] tells me how many I should ignore.
        */
        ll cost_suffix = 0;
        if (last_negated + 2 <= N) {
            cost_suffix = messed_up_prefix[N] - messed_up_prefix[last_negated + 2];
        }
        ll cost_negate = 1;
        ll cost = cost_prefix + cost_suffix + cost_negate;
        ckmin(out, cost);
        dbgY(last_negated, cost_prefix, cost_suffix, cost, out);
    }
