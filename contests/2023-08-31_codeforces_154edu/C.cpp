...
void solve() {
    ...
    V<char> tracking = {'1'};
    auto process_char = [&](char c) -> bool {
        if (c == '-') {
            bool is_sorted = (tracking.back() == '1');
            tracking.pop_back();
            if (is_sorted) {
                if (tracking.back() == '0') {return false;}
                tracking.back() = '1';
            }
        } else if (c == '+') {
            if (tracking.back() == '0') { tracking.push_back('0'); }
            else if (tracking.size() == 1) {tracking.push_back('1');}
            else {tracking.push_back('?');}
        } else if (c == '0') {
            if (tracking.back() == '1') {return false;}
            tracking.back() = '0';
        } else if (c == '1') {
            if (tracking.back() == '0') {return false;}
            tracking.back() = '1';
    ...
    FOR(k, 0, N) {
        auto c = S[k];
        if (!process_char(c)) {return NO;}
    }
    while (tracking.size() > 1) {
        if (!process_char('-')) {return NO;}
    }
