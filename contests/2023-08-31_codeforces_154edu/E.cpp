...
tcT, class S> struct LazySeg {
    ...
    void assign(int idx, S value) {
        auto curr = query(idx, idx);
        upd(idx, idx, value - curr);
    }
...
void solve_segtreeattempt() {
    ...
    // auto firstgood = ndvec<mi>(N+1, B+1, mi(0));

    // // base case
    // firstgood[B][B] = fact[B];

    // // dp
    // FOR(n, B+1, N+1) {
    //     FOR(k, 1, B) {  // don't include k=B. That should always be left as 0.
    //         // fill in a good item.
    //         firstgood[n][k] += firstgood[n-1][k-1] * (B - (k-1));

    //         // fill in a bad item.
    //         // to have k good things left, we should start with at least k good things.
    //         FOR(j, k, B+1) {
    //             firstgood[n][k] += firstgood[n-1][j];
    //         }
    //     }
    //     dbgY(n, firstgood[n]); el;
    // }
    // dbgB(pdh(firstgood)); el;

    // V<mi> firstgood_anyk(N+1);
    // FOR(n, 0, N+1) {
    //     FOR(k, 1, B+1) {
    //         firstgood_anyk[n] += firstgood[n][k];
    //     }
    // }
    // dbgR(firstgood_anyk); el;

    // build firstgood using a segtree instead.
    V<mi> firstgood_anyk(N+1);
    {
        LazySeg<mi, mi> st;
        st.init(B+1);
        ...
        FOR(n, B+1, N+1) {
            FOR(k, 1, B) {  // don't include k=B. That should always be left as 0.
                // fill in a good item.
                // firstgood[n][k] += firstgood[n-1][k-1] * (B - (k-1));
                st.upd(k, k, st.query(k-1, k-1) * (B - (k-1)));
                dbgP(n, k, st.query(k, k));
                ...
                //     firstgood[n][k] += firstgood[n-1][j];
                // }
                st.upd(k, k, st.query(k+1, B));
            }
            st.assign(B, 0);
            firstgood_anyk[n] = st.query(1, B);
            dbgY(n, firstgood_anyk[n], st); el;
        }
    }
    dbgR(firstgood_anyk); el;
    ...
    V<mi> score(N+1);
    FOR(n, 1, N+1) {
        // score using first n positions, i.e. up through n-1.
        score[n] = firstgood_anyk[n];
        el; dbgY(n, firstgood_anyk[n]);

        // cases: assume the most recent point was scored at position prev_good.
        // That leaves positions pg+1 through n-1 for us.
        FOR(prev_good, 0, n-1) {
            auto curr = score[prev_good+1] * firstgood_anyk[(n-1) - (prev_good+1) + 1];
            score[n] += curr;
            dbgP(n, prev_good, curr, score[n]);
        }
        dbg(score[n]);
    }
...
void solve() {
    ...
    // base case
    firstgood[B][B] = fact[B];
    firstgood_anyk[B] = fact[B];

    // dp
    FOR(n, B+1, N+1) {
        mi suffix_sum = firstgood_anyk[n-1];
        FOR(k, 1, B) {  // don't include k=B. That should always be left as 0.
            // fill in a good item.
            firstgood[n][k] += firstgood[n-1][k-1] * (B - (k-1));
            ...
            //     firstgood[n][k] += firstgood[n-1][j];
            // }
            suffix_sum -= firstgood[n-1][k-1];
            firstgood[n][k] += suffix_sum;
