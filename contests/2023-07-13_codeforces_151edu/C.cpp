...
void solve() {
    ...
    auto next_occurrence = ndvec<ll>(SZ, 10, -1LL);
    V<ll> helper(10, SZ);
    ROF(k, 0, SZ) {
        ll d = S[k] - '0';
        helper[d] = k;
        next_occurrence[k] = helper;
    }
    ...
    FOR(n, 1, N+1) {
        el; dbgY(n);
        ll curr = dp[n-1] + 1;
        if (curr >= SZ) {return YES;}
        auto& fut = dp[n];
        fut = curr;
        FOR(d, lo[n-1]-'0', hi[n-1]-'0'+1) {
            dbg(curr, d);
            ll next = next_occurrence[curr][d];
            ckmax(fut, next);
            dbgP(next, fut);
            if (fut == SZ) {return YES;}
        }
    }
