...
V<ll> subsolve(ll goal, ll max, ll banned) {
    if (max == 1) {NO; return V<ll>();}
    if (banned != 1) {
        YES;
        V<ll> out(goal, 1);
        ps(out.size());
...
void check() {
    FOR(goal, 1, 101) {
        FOR(max, 1, goal+1) {
            FOR(banned, 1, max+1) {
                auto out = subsolve(goal, max, banned);
                if (out.size() == 0) {continue;}
                el; dbg(MT(goal, max, banned), out);
                // out should consist of nums from 1 to max, with no banned, summing to goal.
                ll sum = sumv(out);
                assert(sum == goal);
                for(auto& x : out) {
                    assert(1 <= x && x <= max);
