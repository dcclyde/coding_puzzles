...
void brute() {
    ...
    // Find all possible products of integers in [1, A].
    set<ll> products;
    FOR(a, 1, A+1) FOR(b, 1, A+1) {products.insert(a*b);}

    // Find the smallest product that's >= B.
    auto it = lower_bound(all(products), B);
    if (it == products.end()) {ps(-1);}
    else {ps(*it);}
    return;
}
...
void solve() {
    ...
    if (A*A < B) {return ps(-1);}


    ll candidate = B;
    while (true) {
        auto divs = divisors(candidate);
        // dbgR(divs);

        // find the largest divisor that's <= A.
        auto it = lower_bound(all(divs), A);
        ll d = -1;
        if (*it == A) {d = A;}
        ...
        if (d != -1) {
            dbg(candidate, d, candidate/d);
            if (candidate / d <= A) {return ps(candidate);}
        }
