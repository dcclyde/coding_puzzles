...
pair<db,db> try_strat(db strat, ll NUM_TRIALS=10'000'000) {
    db total_score = 0;
    db zeros = 0;
    rep(NUM_TRIALS) {
        db running = 0;
        while (running < strat) {running += rand_dbl(rng);}
        if (running > 1) {++zeros; continue;}
        total_score += running + rand_dbl(rng);
    }
    return MP(total_score / NUM_TRIALS, zeros / NUM_TRIALS);
}
...
void solve() {
    ...
    while (hi - lo > EP) {
        dbg(lo, hi);
        db m1 = (2*lo + hi) / 3;
        auto [s1, z1] = try_strat(m1);
        db m2 = (lo + 2*hi) / 3;
        auto [s2, z2] = try_strat(m2);
    ...
    el;
    db c = (lo + hi) / 2;
    const db c_exact = .5 * (sqrt(13) - 3);
    dbgR(c, c_exact, abs(c-c_exact));

    el; el; el;

    auto [sc, zc] = try_strat(c_exact);
    db d_exact = 1 + .5 * (sqrt(13) - 5) * exp(c_exact);
    dbgR(zc, d_exact, abs(zc-d_exact));
