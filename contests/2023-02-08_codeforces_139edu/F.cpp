...
struct NetworkSimplex {
	...
	void init(int _n) { n = _n; B.rsz(n+1); pei.assign(n+1,-1);
		depth.rsz(n+1); P.rsz(n+1); tree.rsz(n+1); solved=false;}
	int ae(V_id a, V_id b, Flow l, Flow u, Cost c) {
        if (!solved) {dbgcP("ae", a, b, l, u, c)};
	...
	i128 solve() {
		...
		F0R(i,n) {
            dbg(i, E[m+2*i].flow, E[m+2*i].cap);
...
/**
    If I try to just pre-use the parity stuff, then I basically end up with a MCMF-ish problem
        except that some vertices have pre-existing inflow or outflow.
 */


void solve() {
    lls(N, E);
    V<tuple<ll,ll,ll,ll>> dat;
    rv(E, dat);
    dbgR(MP(N,E), dat);
    el;
    ...
    NetworkSimplex solver; solver.init(N);

    V<ll> extra(N);
    V<ll> edge_ids;
    ll capsum = 0;
    FOR(k, 0, E) {
        auto& [a, b, capacity, cost] = dat[k];
        --a; --b;
        // I need parity of flow to match the capacity for each edge.
        if (capacity % 2 == 1) {
            --capacity;
            out[k] += 1;
            --extra[a];
        ...
        cost *= 2; capacity /= 2;
        ll eid = solver.ae(a, b, 0, capacity, cost);
    ...
    dbgY(out, extra);
    FOR(k, 1, N-1) {
        if (extra[k] % 2 == 1) {return Impossible;}

        if (extra[k] < 0) {solver.ae(k, N-1, -extra[k]/2, -extra[k]/2, 0);}
        else if (extra[k] > 0) {solver.ae(0, k, extra[k]/2, extra[k]/2, 0);}
        capsum += abs(extra[k]/2);
    }
    // add an extra edge from end to start?
    solver.ae(N-1, 0, 0, capsum, 0);

    umap<ll,ll> eid_inv;
    assert(edge_ids.size() == E);
    FOR(k, 0, E) {
        dbg(k, edge_ids[k]);
        eid_inv[edge_ids[k]] = k;
    }

    auto sol = solver.solve();
    if (sol == solver.INF) {return Impossible;}
    dbgY(sol);

    FOR(k, 0, solver.E.size()) {
        auto& [src, dst, flow, cap, cost] = solver.E[k];
        dbgP(k, src, dst, cap, cost, flow);
    }

    // add assertions.
    V<ll> inout(N);
    FOR(e, 0, E) {
        auto& [src, dst, flow, cap, cost] = solver.E[2*e];
        out[e] += 2 * flow;
        inout[src] -= flow; inout[dst] += flow;
    }
    dbg(inout);
    FOR(k, 1, N-1) {
        if (-2*inout[k] != extra[k]) {return Impossible;}  // TODO why is this needed?
    }
