...
/**
    Basically: are there any 2-char substrings that repeat?
    It does NOT count if we just include "aaa".
 */
...
void solve() {
    ...
    uset<string> seen;
    FOR(k, 0, N-1) {
        // have we seen the current substring?
        string curr = dat.substr(k, 2);
        if (seen.find(curr) != seen.end()) {return YES;}

        if (k > 0) {
            // add an older substring to the list of options.
            string older = dat.substr(k-1, 2);
            seen.insert(older);
        }
    }
