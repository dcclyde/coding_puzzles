...
/**
    Two conditions.
    1) I need the black cells to be connected.
    2) I need the
 */
...
/**
    Wait no.
    I think I need:
        Optional 2-high blocks at the ends
        a nice "snake" in the middle
 */
...
void solve() {
    ...
    dbgY(MP(cs, rs), MP(ce, re));

    if (cs == C) {return YES;}

    // can I walk from (cs, rs) to (ce, re)?
    ll rr = rs; ll cc = cs;
    while (cc < ce) {
        // if I can't move right, that's bad.
        if (dat[rr][cc+1] == 'W') {return NO;}

        // ok, move right.
        ++cc;

        // if I need the other cell in this column, just grab it now.
        if (dat[rr^1][cc] == 'B') {rr ^= 1;}
    }
