...
void solve() {
    ...
    FOR(have, 0, 4) {
        FOR(need, 0, 4) {
            ll take = min(X[have][need], X[need][have]);
            X[have][need] -= take;
            X[need][have] -= take;
            save += take;
        }
    ...
    FOR(a, 0, 4) FOR(b, 0, 4) FOR(c, 0, 4) {
        if (a == b || b == c || c == a) {continue;}
        ll take = min(X[a][b], min(X[b][c], X[c][a]));
        X[a][b] -= take;
        X[b][c] -= take;
        X[c][a] -= take;
        save += take;
    ...
    FOR(a, 0, 4) FOR(b, 0, 4) FOR(c, 0, 4) FOR(d, 0, 4) {
        ...
        ll take = min(X[a][b], min(X[b][c], min(X[c][d], X[d][a])));
        X[a][b] -= take;
        X[b][c] -= take;
        X[c][d] -= take;
        X[d][a] -= take;
