...
V<T> pqueue_to_iterable(priority_queue<T,B,C> PQ) { // PASS BY VALUE!
    V<T> working;
...
template <typename T, typename B, typename C>
string tsdbg(priority_queue<T,B,C> PQ) {
...
void solve() {
    lls(N, E, C);
    V<ll> payout_per_performance;
    auto& ppp = payout_per_performance;
    rv(N, ppp);
    dbgR(N, E, C, ppp);
    V<V<pll>> G(N);
    FOR(k, 0, E) {
    ...
    ll GOAL = N-1;
    V<map<ll,pll>> dp(N);
    // dp[SOURCE][ppp[SOURCE]] = MP(0, -C);
    using T = pair<pll,pll>;  // (vertex, best ppp), (curr shows, -curr money)
    pqg<T> todo;
    todo.emplace(MP(0, -C), MP(SOURCE, ppp[SOURCE]));
    while (!todo.empty()) {
        auto [shows_money, vertex_ppp] = todo.top();
        todo.pop();
        el;
        dbgP(pdh(dp));
        dbgY(todo);
        dbgR(shows_money, vertex_ppp);
        // is shows_money better than our current result for vertex_ppp?
        /**
            If we insert dp[v][p] = shows_money then we need to fix to get nice ordering.
            dp[v] should have p < q --> dat[p] < dat[q].
            If bigger things become obsolete, throw em out.
            If this one is obsolete, just move on with our lives.
         */
        auto& [v, p] = vertex_ppp;

        auto it = dp[v].lower_bound(p);
        if (it != dp[v].end() && it->second <= shows_money) {continue;}
        ...
        while (true) {
            ...
            --it;
            if (it->second < shows_money) {break;}
            dp[v].erase(it);
        }

        // insert children.
        auto& [shows, money] = shows_money;
        for(auto& [o, flight_cost] : G[v]) {
            ll money_here = money;
            money_here += flight_cost;
            ll performances_needed = cdiv(maxll(money_here, 0), p);
            ll shows_here = shows + performances_needed;
            money_here -= performances_needed * p;
            ll op = max(p, ppp[o]);
            // would this currently be a useful item in dp?
            it = dp[o].lower_bound(op);
            if (it != dp[o].end() && it->second <= MP(shows_here, money_here)) {continue;}

            todo.emplace(
                MP(shows_here, money_here),
                MP(o, op)
            );
            dbg(o, dp[o], MP(MP(shows_here, money_here),
                MP(o, op)));
        }
        dbg(pdh(dp[v]));
        // timebomb(10);
    ...
    if (dp[GOAL].empty()) {return ps(-1);}
    auto [shows, money] = dp[GOAL].begin()->second;
    ll out = shows;
    return ps(out);
}
