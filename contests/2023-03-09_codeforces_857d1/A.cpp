...
void learn() {
    ...
    ll C = 5;
    R = 4; C = 4;
    auto F = ndvec<ll>(R, C, 0LL);
    ll b = 0;
    FOR(c, 0, C) {F[0][c] = (1LL<<b); b = (b+1) % BIT_END;}
    FOR(r, 1, R) {F[r][0] = (1LL<<b); b = (b+1) % BIT_END;}

    auto special = ndvec<ll>(2, 4, 0LL);

    set<ll> seen;
    FOR(r, 1, R) {
        FOR(c, 1, C) {
            if (r == 1 && c <= 4) {
                F[r][c] = (1LL<<b); b = (b+1) % BIT_END;
                special[r%2][c%4] = F[r][c] ^ F[r-1][c] ^ F[r-1][c-1] ^ F[r][c-1];
            } else {
                F[r][c] = F[r-1][c] ^ F[r-1][c-1] ^ F[r][c-1] ^ special[r%2][c%4];
            }
        }
    }
    FOR(r, 0, R) FOR(c, 0, C) {seen.insert(F[r][c]);}
    dbgY(MP(R,C), seen.size());
    dbg(pth(F));
    assert(false);
}
...
void solve() {
    ...
    auto F = ndvec<ll>(R, C, 0LL);
    FOR(r, 0, R) FOR(c, 0, C) {F[r][c] = r ^ (c << 20);}
    ps(R*C);
    FOR(r, 0, R) {pv(F[r]);}
    return;
}
