...
void solve() {
    ...
    ll out = INF_ll;
    ROF(k, 0, N) {
        auto [A_curr, idx] = A_options[k];
        ll B_blocked = B[idx];

        // remove this from maybe
        B_maybe.erase(B_maybe.find(B_blocked));

        // find the best answer according to current state
        ll big = -INF_ll;
        if (B_required.size() > 0) {
            big = *B_required.rbegin();
        }
        ll curr_cost = abs(big - A_curr);
        if (big > A_curr) {

        } else {
            // try just smaller and/or just bigger.
            auto it = B_maybe.upper_bound(A_curr);
            if (it != B_maybe.end()) {ckmin(curr_cost, abs(A_curr - *it));}
            if (it != B_maybe.begin()) {--it; ckmin(curr_cost, abs(A_curr - *it));}
        }
        ckmin(out, curr_cost);

        B_required.insert(B_blocked);
    }
