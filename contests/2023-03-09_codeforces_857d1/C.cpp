...
tcT, class S> struct LazySeg {
	...
	void push(int ind, int L, int R) { /// modify values for current node
		...
		// lazy[ind] = idS;
        ckmax(seg[ind], lazy[ind]);
        if (L != R) F0R(i,2) ckmax(lazy[2*ind+i], lazy[ind]);
        lazy[ind] = idS;
...
void solve() {
    ...
    FOR(n, 0, N) {
        ...
        FOR(k, 0, sz) {
            lls(x);
            if (dat[n].size() == 0 || x > dat[n].back()) {dat[n].push_back(x);}
        }
        // rv(sz, dat[n]);
    }
    dbgR(N, pdh(dat));
    ...
    // compress dat.
    V<ll> uncompressed = {-1};
    for(auto& q : dat) for(auto& x : q) {uncompressed.push_back(x);}
    remDup(uncompressed);
    umap<ll,ll> uncinv;
    FOR(k, 0, uncompressed.size()) {
        uncinv[uncompressed[k]] = k;
    }
    for(auto& q : dat) for(auto& x : q) {x = uncinv[x];}
    dbg(pdh(dat)); el;
    // ll NN = uncompressed.size();


    // // Make each row of dat increasing.
    // FOR(n, 0, N) {
    //     V<ll> temp = {dat[n][0]};
    //     for(auto& x : dat[n]) { if (x > temp.back()) {temp.push_back(x);} }
    //     swap(dat[n], temp);
    // }
    // dbg(pdh(dat));
    // el;

    // sort(all(dat), [](auto& A, auto& B) {return A.back() < B.back();});
    // dbgB(pdh(dat));
    // el;

    V<ll> order_helper;
    FOR(k, 0, N) {order_helper.push_back(k);}
    sort(all(order_helper), [&](auto a, auto b) {return dat[a].back() < dat[b].back();});

    // ll MAX_VALUE = dat.back().back();
    ll MAX_VALUE = 0; for(auto& V : dat) {ckmax(MAX_VALUE, V.back());}

    // LazySeg<ll,ll> st;
    // st.init(MAX_VALUE + 1);
    // st.upd(0, MAX_VALUE, 0);
    SegTree<ll> st;
    st.init(MAX_VALUE + 1);
    FOR(k, 0, MAX_VALUE+1) {st.seg[st.n + k] = 0;}
    st.build();

    FOR(n, 0, N) {
        auto& curr = dat[order_helper[n]];
        dbgB(n, curr);
        ll last = curr.back();
        ll last_score = st.seg[st.n + last];
        FOR(k, 0, curr.size()) {
            ll used_here = k+1;
            ll last_used = curr[curr.size()-1-k];
            // st.upd(
            //     last,
            //     MAX_VALUE,
            //     used_here + st.query(last_used-1, last_used-1)
            // );
            ll proposed = used_here + st.query(0, last_used-1);
            ckmax(last_score, proposed);
        }
        st.upd(last, last_score);
        dbgY(st);
        el;
    }

    ll out = st.query(MAX_VALUE, MAX_VALUE);
    return ps(out);
}
