...
void solve() {
    ...
    auto binsearch_helper = [&](ll X) -> bool {
        el; dbgcBold("binsearch helper", X);

        V<ll> todo, available, source;
        bool ok;
        V<ll> candidate_roots;
        auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
            if (!ok) {return;}

            // work on parent
            // if I'm a leaf, start a new snake.
            if (parent != -1 && G[curr].size() == 1) {
                source[curr] = curr;
                todo[curr] = X-1;
                available[curr] = 0;
            }
                ...
                if (todo[o] > 0) {
                    if (todo[curr] >= 0) {
                        ...
                        if (candidate_roots.size() == 0) {
                            candidate_roots.push_back(source[o]);
                            candidate_roots.push_back(source[curr]);
                        }
                        return;
                    }
                    todo[curr] = todo[o] - 1;
                    source[curr] = source[o];
                }
                ckmax(available[curr], available[o] + 1);
            }
            // update parent after all children
            if (todo[curr] >= 0) {
                available[curr] = 0;
            }
            dbg(curr, todo[curr], available[curr]);

            // dbgP(curr, parent, subtree_size[curr]);
        };

        auto try_root = [&](ll ROOT) -> bool {
            todo.assign(N, -1);
            available.assign(N, 0);
            source.assign(N, -1);
            ok = true;
            dfs(ROOT, -1, dfs);
            bool output = ok && todo[ROOT] <= 0;
            dbgY(ROOT, output, ok, todo, available, source, candidate_roots);
            el;
            return output;
        };
        if (try_root(0)) {return true;}

        if (ok) {return try_root(source[0]);}

        FOR(k, 0, 2) {if (try_root(candidate_roots[k])) {return true;}}
        return false;
    };

    ll out = lstTrue(2, N, binsearch_helper);
    return ps(out);
}
