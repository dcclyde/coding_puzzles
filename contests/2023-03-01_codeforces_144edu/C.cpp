...
V<pair<ll,mi>> pre;
ll PRECOMPUTE_SIZE = 1'000'001;
void precompute() {
    if (pre.size() > 0) {return;}
    pre.resize(PRECOMPUTE_SIZE);
    pre[1] = MP(1, 1);
    FOR(n, 1, PRECOMPUTE_SIZE-1) {
        auto& [F, G] = pre[n];
        ll kn = 2*n;
        while (kn < PRECOMPUTE_SIZE) {
            if (ckmax(pre[kn].f, 1+F)) {pre[kn].s = 0;}
            pre[kn].s += G;
            kn += n;
        }
    }
    ...
    pair<ll,mi> best = {0, 0};
    FOR(n, 1, PRECOMPUTE_SIZE) {
        if (pre[n].f > best.f) {
            best = pre[n];
        } else if (pre[n].f == best.f) {
            best.s += pre[n].s;
        }
        pre[n] = best;
    }
...
void solve() {
    if (pre.size() == 0) {precompute();}
    lls(L, R);
    dbgR(L, R);
    el;

    pair<ll,mi> out = pre[R / L];
    ll s = L+1;
    while (s <= R) {
        if (pre[R/s].f != out.f) {break;}
        ll next_s = fstTrue(L, R, [&](ll x) -> bool {return R/x < R/s;});
        out.s += pre[R/s].s * (next_s - s);
