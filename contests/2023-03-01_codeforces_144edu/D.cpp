...
tcT, class S> struct LazySeg {
	const T idT{-INF_ll}; const S idS{};  // ! identity
...
void solve() {
    ...
    if (X >= 0) {
        // check small subarrays by hand.
        ll out = 0;
        FOR(start, 0, N) {
            ll cur = 0;
            FOR(len, 1, K+1) {
                if (start+len-1 < dat.size()) {cur += dat[start+len-1];}
                else {break;}
                ckmax(out, cur + len*X);
            }
        }

        // * Now handle long ones.

        for(auto& q : dat) {q -= X;}

        // prefix_sum[n] = sum of 0, 1, 2, ..., n-1.
        V<ll> prefix_sum(N+1);
        FOR(k, 0, N) {prefix_sum[k+1] = prefix_sum[k] + dat[k];}

        // sm[n] = largest value of prefix_sum[k] with k > n
        //       = biggest prefix sum with final element >= position n.
        V<ll> suffix_max(N, -INF_ll);
        ROF(k, 0, N) {
            if (k < N-1) {suffix_max[k] = suffix_max[k+1];}
            ckmax(suffix_max[k], prefix_sum[k+1]);
        }
        dbgB(prefix_sum);
        dbgP(suffix_max);

        FOR(start, 0, N) {
            ll first_legal_end = start + K;  // length K+1
            if (first_legal_end >= suffix_max.size()) {break;}
            ll best_sum = suffix_max[first_legal_end] - prefix_sum[start];
            ll prop = best_sum + 2*K*X;
            ckmax(out, prop);
            dbg(start, first_legal_end, best_sum, prop, out);
        }
        ...
        // * Check "small complement" subarrays by hand.
        ll S = sumv(dat);
        FOR(L, 0, K) {
            FOR(R, N-K, N) {
                ll len = R - L + 1;
                if (len < 0) {continue;}
                ll leftover = N - len;
                ll num_bonus = min(len, N-K);
                ll bonus = -X * num_bonus + X * (len - num_bonus);

                ll range_sum = prefix_sum[R+1] - prefix_sum[L];
                ll score = range_sum + bonus;
                ckmax(out, score);
            }
        }

        // now check stuff with len <= N-K.

        el;
        LazySeg<ll,ll> st; st.init(N+1);
        FOR(k, 0, N+1) {
            prefix_sum[k] -= (k+1) * X;
            st.seg[st.n + k] = prefix_sum[k];
        }
        st.build();
        dbgY(st);

        FOR(start, 0, N) {
            ll last_legal_end = min(start + N-K - 1, N-1);
            // what's the biggest sum starting from start and ending <= last_legal_end?
            ll big_sum = st.query(start, last_legal_end + 1);
            ll curr = big_sum - prefix_sum[start];
            ckmax(out, curr);
            dbg(start, last_legal_end, big_sum, curr, out);
        }
        return ps(out);
    }
}
