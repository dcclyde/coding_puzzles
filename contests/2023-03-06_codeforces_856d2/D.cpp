...
void solve() {
    ...
    // need num distinct primes >= N.
    if (nA < N) {return ps(0);}

    el;
    auto F = ndvec<mi>(nA+1, N+1, mi(0));
    FOR(n, 0, nA+1) {F[n][0] = 1;}
    FOR(n, 1, nA+1) {
        FOR(k, 1, N+1) {
            if (k > n) {continue;}
            F[n][k] = F[n-1][k] + A[n-1] * F[n-1][k-1];
            dbgB(MP(n,k), F[n][k]);
        }
    }
    el;

    mi total_permutations = 1;
    ll todo = 2*N;
    for(auto& [v, cnt] : help) {
        total_permutations *= binom(todo, cnt);
        todo -= cnt;
    }

    mi base = total_permutations * ifac[2*N] * fac[N];

    mi out = base * F[nA][N];
    dbgR(total_permutations, base, F[nA][N], out);
