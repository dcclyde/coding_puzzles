...
void solve() {
    ...
    V<ll> cards(2);
    FOR(suffix_len, 0, N) {
        // Player 1 should get the first card.
        // The pattern should look like 1, 0, 0, 1, 1, 0, 0, 1, ...
        ll should_get_next = (suffix_len % 4 == 0 || suffix_len % 4 == 3);

        // Assume the wrong person gets this card, so that we know the winner now.
        ll winner = !should_get_next;
        mi curr = comb(N-cards[0]-cards[1]-1, N/2 - cards[winner] - 1);
        win[winner] += curr;

        // Now, assume the right person gets this card.
        ++cards[should_get_next];
    }

    return ps(win[0], win[1], 1);
}
