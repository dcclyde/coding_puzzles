    ...
    FOR(r, 0, R-1) {
        ROF(o, 0, 2) {
            FOR(c, 0, 2) {
                ...
                if (o == 0) {
                    // Just step down.
                    ckmin(dp[r+1][c][x[r+1][c^1]], dp[r][c][o]);
                    dbgP(MT(r+1, c, x[r+1][c^1]), dp[r][c][o], dp[r+1][c][x[r+1][c^1]]);
                // } else if (o == 2) {
                } else {
                    // o == 1
                    if (x[r+1][c] == 1) {
                        // Try stepping sideways (delete the 1 below)
                        // After that we'll always step down.
                        ckmin(dp[r+1][c^1][0], dp[r][c][o] + 1);

                        // Try stepping down (delete the 1 to the side)
                        ckmin(dp[r+1][c][x[r+1][c^1]], dp[r][c][o] + 1);
                    } else {
                        // Just step sideways.
