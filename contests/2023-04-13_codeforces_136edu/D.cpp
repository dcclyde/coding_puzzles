...
template<int SZ, bool VALS_IN_EDGES> struct HLD {
    ...
    int jmp(int x, int d) {
        // assert(depth[x] >= d);
        ckmin(d, depth[x]);  // TODO test behaviour when I jump too far.
        int target_depth = depth[x] - d;
        dbgcB("jmp", MP(x,d), target_depth, pos[x]);
        int y = x;
        dbgR(rpos);
        while (true) {
            int o = root[y];
            dbg(MT(y, depth[y], pos[y]), MT(o, depth[o], pos[o]));
            if (depth[o] <= target_depth) {
                int out = rpos[pos[o] + target_depth - depth[o]];
                dbgcP("jmp done", MP(x,d), out);
                return out;
            }
            y = par[o];
        }
...
void solve() {
    ...
    V<pll> depth_order(N);
    FOR(k, 0, N) {depth_order[k] = {-hld.depth[k], k};}
    sort(all(depth_order));
    dbg(depth_order); el;


    auto binsearch_helper = [&](ll H) -> bool {
        dbgcBold("binsearch", H);

        // Special case for H=1.
        // We need one operation for every node not connected to the root.
        if (H == 1) { return N-1 - hld.adj[ROOT].size() <= K; }

        hld.tree.init(N);
        ll curr = 0;
        FOR(op, 0, K+1) {
            // Find the deepest node that has hld value = 0.
            while (curr < N) {
                auto [depth, v] = depth_order[curr];
                depth = -depth;
                if (depth <= H) {return true;}
                if (hld.queryPath(v,v) == 0) {break;}
                ++curr;
            }
            assert(curr < N);
            ...
            ll o = hld.jmp(v, H-1);
            hld.modifySubtree(o, 1);
            dbg(op, v, o);
        }
        dbgY(H);
    };

    ll out = fstTrue(1, N, binsearch_helper);
    return ps(out);
}
