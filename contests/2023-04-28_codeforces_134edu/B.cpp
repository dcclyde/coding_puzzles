...
void solve() {
    ...
    // Can I travel along the left and top?
    bool top_ok = (cL > D+1 && rL < R-D);
    bool bottom_ok = (cL < C-D && rL > D+1);
    if (top_ok || bottom_ok) {return ps(R+C-2);}
    else {return ps(-1);}

    #if 0
    auto lasered = [&](ll r, ll c) { return abs(r-rL) + abs(c-cL) <= D; };
    ...
    while (!todo.empty()) {
        ...
        ll d = dist[cur];
        dbg(cur, d);
        if (lasered(cur.f, cur.s)) {continue;}
        if (cur == MP(R,C)) {
            return ps(dist[cur]);
        }
        ...
        for(auto& [dr, dc] : stepsOrthogonal) {
            ll r = cur.first+dr;
            ll c = cur.second+dc;
            if (dist.find(MP(r,c)) != dist.end()) continue;
            dist[{r,c}] = d+1;
            todo.push_back({r,c});
        }
    }
