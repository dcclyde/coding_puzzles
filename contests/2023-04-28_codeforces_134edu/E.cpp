...
void solve() {
    ...
    auto dp = kmp(S);
    dbgY(dp);
    dp.resize(dp.size() + 15);

    // If I'm at position k and I want to match letter 'a'+j, where will I end up?
    auto backup = ndvec<ll>(N+1, 26, -1LL);
    FOR(k, 1, N+1) {
        FOR(j, 0, 26) {
            char c = 'a' + j;
            auto& cur = backup[k][j];
            for(cur = dp[k-1]; cur != -1 && S[cur] != c;) cur = dp[cur];
            ++cur;
        }
    }
    dbgB(pdh(backup));
    ...
    FOR(qid, 0, Q) {
        string T = queries[qid];
        V<ll> out;
        // Imagine how kmp() would have gone if T had been appended on the end of S.
        auto s = [&](ll j) -> char {
            if (j < N) return S[j];
            return T[j-N];
        };
        FOR(k, N+1, N+1+T.size()) {
            string cur = T.substr(0, k-N);
            if (memo.find(cur) != memo.end() && false) {
                dp[k] = memo[cur];
            } else {
                ll alt;
                for(alt = dp[k-1]; alt != -1 && s(alt) != s(k-1);) {
                    dbgP(k, s(k-1), alt);
                    if (alt <= N && false) {
                        ll b1 = backup[alt][s(k-1) - 'a'];
                        // ll b2 = backup[alt-1][s(k-1) - 'a'];
                        dbgcY(":O", alt, b1);
                        alt = backup[alt][s(k-1) - 'a'];
                    } else {
                        alt = dp[alt];
                    }
                }
                ...
                for(dp[k] = dp[k-1]; dp[k] != -1 && s(dp[k]) != s(k-1);) {
                    dbgP(k, s(k-1), dp[k]);
                    if (dp[k] <= N && false) {
                        ll b1 = backup[dp[k]][s(k-1) - 'a'];
                        ll b2 = backup[dp[k]-1][s(k-1) - 'a'];
                        dbgcY(":O", dp[k], b1, b2);
                        dp[k] = backup[dp[k]][s(k-1) - 'a'];
                    } else {
                        dp[k] = dp[dp[k]];
                    }
                }
