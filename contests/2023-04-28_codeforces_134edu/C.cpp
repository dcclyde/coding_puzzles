...
void solve() {
    ...
    V<ll> out_low(N), out_high(N);
    ll next_bad = -1;
    FOR(k, 0, N) {
        ll low_idx = fstTrue(0, N-1, [&](ll x) -> bool {
            return B[x] >= A[k];
        });
        out_low[k] = B[low_idx] - A[k];

        if (next_bad < k) {
            next_bad = k;
            while (next_bad < N-1 && A[next_bad+1] <= B[next_bad]) {++next_bad;}
        }
        out_high[k] = B[next_bad] - A[k];
    }
    pv(out_low);
    pv(out_high);
    return;
