...
void solve() {
    ...
    auto bit = [&](int x, int b) -> int { return (x >> b) & 1; };
    V<int> chunks = {0, N};
    int out = 0;
    ROF(b, 0, 30) {
        // Set the current bit if possible.
        bool can_set = true;
        V<int> new_chunks;
        FOR(k, 0, chunks.size()-1) {
            int cs = chunks[k], ce = chunks[k+1];
            auto ita = stable_partition(A.begin()+cs, A.begin()+ce, [&](int x) { return bit(x, b) == 0; });
            int zeros_A = ita - (A.begin()+cs);
            auto itb = stable_partition(B.begin()+cs, B.begin()+ce, [&](int x) { return bit(x, b) == 1; });
            int ones_B = itb - (B.begin()+cs);
            if (zeros_A == ones_B) {
                new_chunks.pb(cs);
                if (zeros_A > 0 && zeros_A < ce-cs) {new_chunks.pb(cs + zeros_A);}
            } else {
                can_set = false;
                break;
            }
