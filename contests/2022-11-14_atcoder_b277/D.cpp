...
void solve_() {
    ...
    dat.push_back(0);
    FOR(k, 0, N) {dat.push_back(dat[k]);}

    ll curr = 0;
    ll best = 0;
    FOR(k, 0, 2*N) {
        // is it part of the previous group or is it a new group?
        ll d = 0;
        if (k > 0) {d = dat[k] - dat[k-1];}
        if (d == 0 || d == 1 || d == 1-MOD) {curr += dat[k];}
        else {ckmax(best, curr); curr = dat[k];}
    }
    ckmax(best, curr);
    dbg(best, curr, sum_all);
...
void solve() {
    ...
    ll sum_all = sumv(dat);
    // sum_all = 0; for(auto& x : dat) {sum_all += x;}
    ...
    FOR(k, 0, N) {
        // is it part of the previous group or is it a new group?
        if (k > 0 && dat[k] - dat[k-1] <= 1) {curr += dat[k];}
        else {ckmax(best, curr); curr = dat[k];}
    }
    ckmax(best, curr);

    dbgc("stage 1", best, curr);
    // "wrap around" from the top.
    if (best < sum_all && dat.back() == MOD-1 && dat[0] == 0) {
        dbgcP("s2 start");
        FOR(k, 1, N) {
            if (dat[k] - dat[k-1] > 1) {break;}
            curr += dat[k];
        }
        ckmax(best, curr);
    }
