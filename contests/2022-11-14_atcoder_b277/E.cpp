...
void solve() {
    lls(N, E, num_switches);

    Dijkstra<ll,false> ds;
    ds.init(2*N);

    FOR(k, 0, E) {
        ll1(a, b);
        lls(state);
        ds.ae(a+N*(1-state), b+N*(1-state), 1);
    }

    FOR(k, 0, num_switches) {
        ll1(a);
        ds.ae(a, a+N, 0);
    }
    ds.ae(N-1, 2*N-1, 0);  // I don't care which version of the end I reach.

    ds.gen(0);

    if (ds.dist[N-1] > 2*N) {return ps(-1);}
    return ps(ds.dist[N-1]);
}
