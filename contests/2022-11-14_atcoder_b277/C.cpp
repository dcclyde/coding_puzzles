...
void solve() {
    ...
    /**
        After conversion:
            dat[k] now holds only values 0 .. NN-1
            uncompressed[dat[k]] = original dat[k].
     */
    // uncompressed[k] = kth value that showed up in original data.
    V<ll> uncompressed;
    FOR(k, 0, N) {
        uncompressed.push_back(dat[k].f);
        uncompressed.push_back(dat[k].s);
    }
    remDup(uncompressed);
    umap<ll,ll> uncinv;
    ...
    FOR(k, 0, N) {
        dat[k].f = uncinv[dat[k].f];
        dat[k].s = uncinv[dat[k].s];
    }
    ...
    DSU dsu;
    dsu.init(NN);
    FOR(k, 0, N) {dsu.unite(dat[k].f, dat[k].s);}

    dbg(dsu.e);

    if (uncompressed[0] != 1) {return ps(1);}
    ROF(k, 0, NN) {
        if (dsu.sameSet(k, 0)) {return ps(uncompressed[k]);}
    }
