...
void brute() {
        ...
        FOR(len, 1, n+1) {
            FOR(L, 0, n) {
                ...
                if (len == 1) {
                    ...
                    ckmax(dp[L][R], dp[L+1][R]);
                    ckmax(dp[L][R], dp[L][R-1]);
                    if (S[L] == S[R]) { ckmax(dp[L][R], 2 + dp[L+1][R-1]); }
                }
            }
        }
        return dp[0][n-1];
    };
    auto subsolve_brute = [&]() -> ll {
        ll BITS = S.size();
        ll MASK_END = (1LL << BITS);
        ll subout = 0;
        FOR(mask, 0, MASK_END) {
            string T;
            FOR(k, 0, BITS) {
            ...
            FOR(k, 0, n) {
                ll o = n-1-k;
                if (k > o) {break;}
                if (T[k] != T[o]) {pal = false; break;}
            }
            if (pal) {ckmax(subout, n);}
        }
        return subout;
    };
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        S.push_back(dat[curr]);
        // ll score = subsolve();
        ll score = subsolve_brute();
        ckmax(out, score);
...
void solve() {
    ...
    // (depth, u, v, unext, vnext)
    using t5 = tuple<ll,ll,ll,ll,ll>;
    V<t5> paths;

    ll ROOT = -1;
    ll first_step = -1;
    auto dfs = [&](ll curr, ll parent, ll depth, auto&& self) -> void {
        // work on parent
        if (parent == ROOT) {first_step = curr;}
        if (curr > ROOT) {
            paths.emplace_back(depth, ROOT, curr, first_step, parent);
        }

        for (auto& o : G[curr]) {
            if (o == parent) {continue;}
            self(o, curr, depth+1, self);
    ...
    ll out = 0;
    auto dp = ndvec<ll>(N, N, 1LL);
    FOR(u, 0, N) {dp[u][u] = 1;}
    for(auto& [deep, v, w, vnext, wnext] : paths) {
        if (deep == 1) {
            dp[v][w] = 1 + (dat[v] == dat[w]);
        } else {
            ckmax(dp[v][w], dp[vnext][w]);
            ckmax(dp[v][w], dp[v][wnext]);
            if (dat[v] == dat[w]) {ckmax(dp[v][w], 2 + dp[vnext][wnext]);}
        }
        dp[w][v] = dp[v][w];
        ckmax(out, dp[v][w]);
        dbgY(MT(deep, v, w, vnext, wnext), dp[v][w]); el;
    }
