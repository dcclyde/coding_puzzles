...
template<class T, class S, int SZ> struct PersistentSegTree {
	...
	void pull(int c) {
        // ll x = d[c].val; ll y = cmb(d[d[c].l].get(), d[d[c].r].get());
        // dbgcGeneral(((x==y)?(OUT_GREEN):(OUT_RED)), "pull", c, x, y);
        d[c].val =
    ...
    int lstTrue_from_base(int L, int time, auto&& check) {
        ...
        while (l != r) {
            ...
            dbgcB("stage 3", l,r, "", fut, path, this);
...
template<class T, class S, int SZ>
string tsdbg(PersistentSegTree<T, S, SZ>* st) {
    int c = st->loc.bk;
	assert(st->orig_n != -1);
	V<T> out;
	FOR(k, 0, st->orig_n) {out.pb(st->query(c, k, k));}
...
void solve() {
    ...
    FOR(L, 0, N) {
        ...
        // out += len;
        out += len_backward;
        dbgB(L, R, len, out); el; el; el; el; el; el; el; el; el; el;
    }
