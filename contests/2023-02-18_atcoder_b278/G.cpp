...
void solve() {
    ...
    V<ll> grundy(N+1);
    FOR(n, L, N+1) {
        // what's the grundy number if we start with size n?
        uset<ll> wip;
        FOR(k, 0, n-L) {
            ll a = k;
            ll b = n-L-k;
            if (a > b) {break;}
            wip.insert(grundy[a] ^ grundy[b]);
        }
        auto& mex = grundy[n];
        mex = 0;
        while (wip.find(mex) != wip.end()) {++mex;}
        // dbgY(n, grundy[n]);
    }
    dbgY(pdh(grundy));

    ll GRUNDY_END = 1 + *max_element(all(grundy));
    // There are only N^2 possible turns. Check what effect each of them have.
    FOR(start, 0, N-1) {
        FOR(len, 1, N-start+1) {
            ll g = grundy[]
        }
    ...
    ll G = grundy[N];
    set<pll> state = {{0, N}};
    map<ll,ll> lengths = {{N, 1}};
    auto do_turn = [&](ll start, ll len) -> void {
        auto it = state.upper_bound(MP(start, INF_ll));
        --it;  // needs to be valid
        auto [chunk_start, chunk_len] = *it;

        // add new segments.
        if (start > chunk_start) {
            state.emplace(chunk_start, start - chunk_start);
            ++lengths[start - chunk_start];
            G ^= grundy[start - chunk_start];
        }
        if (start + len < chunk_start + chunk_len) {
            state.emplace(start + len, chunk_start+chunk_len - (start+len));
            ++lengths[chunk_start+chunk_len - (start+len)];
            G ^= grundy[chunk_start+chunk_len - (start+len)];
        }

        // remove the segment we broke.
        state.erase(it);
        if (!--lengths[len]) {lengths.erase(len);}
        G ^= grundy[len];
    };
    ...
    if (grundy[N]) {
        ...
        cout << "Second" << endl;
        pll enemy_turn = listen();
        do_turn(enemy_turn.f, enemy_turn.s);
    }

    while (true) {
        // choose my turn.
        assert(G != 0);  // I'd better be in a winning position.

        /**
            I need to edit some chunk such that G becomes 0.
            Maybe I could have precomputed: For each G, what's the smallest chunk size + corresponding edit position to xor grundy number by G?
         */
    }
