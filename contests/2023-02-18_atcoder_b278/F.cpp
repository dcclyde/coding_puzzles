...
const ll BITS = 6;  // for debug printouts only
...
void solve() {
    ...
    V<V<ll>> letter_start(26), letter_end(26);
    auto& ls = letter_start; auto& le = letter_end;
    FOR(k, 0, N) {
        ls[dat[k][0] - 'a'].push_back(k);
        le[dat[k].back() - 'a'].push_back(k);
    }

    auto dp = ndvec<bool>(1<<N, 26, false);
    FOR(mask, 1, 1<<N) {  // 0 = already chosen, 1 = still available.
        FOR(prev_letter, 0, 26) {
            dbgR(mask, prev_letter);
            // Pull DP.
            // What's the answer if I'm in state mask and recent letter is prev_letter?
            // recent string must have ended with prev_letter.
            // next string should start with prev_letter.
            for(auto& o : ls[prev_letter]) {
                if (((mask >> o) & 1) != 1) {continue;}
                ll newmask = mask & ~(1<<o);
                ll nltr = dat[o].back() - 'a';
                dp[mask][prev_letter] = dp[mask][prev_letter] || !dp[newmask][nltr];
                dbg(o, MP(bitset<BITS>(newmask),char(nltr+'a')), dp[newmask][nltr], dp[mask][prev_letter]);
            }
            dbgY(MP(bitset<BITS>(mask), char(prev_letter+'a')), MP(mask, prev_letter), dp[mask][prev_letter]); el;
        }
    }

    bool result = false;
    FOR(prev_letter, 0, 26) {
        result = result || dp[(1<<N)-1][prev_letter];
    }

    if (result) {return ps("First");}
    else {return ps("Second");}
}
