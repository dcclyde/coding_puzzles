...
void solve() {
    ...
    dbgR(R, C, N, RLEN, CLEN, pdh(dat));
    el;

    auto out = ndvec<ll>(R - RLEN + 1, C - CLEN + 1, -1LL);
    FOR(rstart, 0, R - RLEN + 1) {
        dbgR(rstart);
        umap<ll,ll> seen;
        FOR(r, 0, R) FOR(c, 0, C) {
            dbgP(MP(r,c));
            if (c < CLEN && (r >= rstart && r < rstart + RLEN)) {continue;}
            ++seen[dat[r][c]];
            dbg(MP(r,c), seen[dat[r][c]]);
        }
        out[rstart][0] = seen.size();
        dbgY(MP(rstart,0), seen.size(), seen); el;
        FOR(cstart, 1, C - CLEN + 1) {
            // get rid of bad stuff.
            el; el; dbg(MP(rstart, cstart), seen);
            FOR(r, rstart, rstart + RLEN) {
                ll rem = dat[r][cstart + CLEN - 1];
                ll add = dat[r][cstart-1];
                ++seen[add];
                if (!--seen[rem]) {seen.erase(rem);}
                // dbgB(MP(r, cstart + CLEN - 1), dat[r][cstart+CLEN-1], "", MP(r, cstart-1), dat[r][cstart-1]);
            }
            out[rstart][cstart] = seen.size();
            dbgY(MP(rstart,cstart), seen.size(), seen); el;
        }
    }
