...
ll modinv(ll a, ll MOD) {
    return (1<a) ? (MOD - ll(modinv(MOD%a,a))*MOD/a) : (1);
}
...
void brute() {
    ...
    FOR(k, 0, WHEELS/2) {
        ll curr = INF_ll;
        FOR(j, 0, MOD) {
            if ((dat[k] + j*STEP) % MOD == dat[WHEELS-1-k]) {
                ckmin(curr, j);
                break;
            }
        }
        FOR(j, 1, MOD) {
            if (dat[k] == (dat[WHEELS-1-k] + j*STEP) % MOD) {
                ckmin(curr, j);
                break;
            }
        }
...
void solve() {
    ...
    // auto [g, coeffs] = gcdExtended(STEP, MOD);
    // ll sc = (coeffs.f % MOD + MOD) % MOD;
    ll g = gcd(MOD, STEP);
    ll step_inv = modinv(STEP/g, MOD/g);
    step_inv = (step_inv % (MOD/g) + MOD/g) % (MOD/g);
    dbgY(g, step_inv);
    ...
    FOR(k, 0, WHEELS/2) {
        ...
        // What's the smallest k such that k*STEP = delta (mod MOD)?
        if (delta % g != 0) {
            ps("IMPOSSIBLE");
            return;
        }
        delta /= g;
        ll curr = (delta * step_inv) % (MOD/g);
        ckmin(curr, MOD/g-curr);
        out += curr;
        dbg(MP(a,b), delta, curr, out);
    }
