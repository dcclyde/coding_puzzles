...
void solve() {
    ...
    V<pll> helper;
    FOR(k, 0, N) {helper.emplace_back(A[k], k);}
    sort(all(helper));
    dbgP(helper); el;

    auto subsolve = [&]() -> V<ll> {
        // dp holds the answer for the first k+1 elements, 0 .. k.
        dbgcR("subsolve", helper);
        V<ll> dp(N);
        dp[0] = 1;
        FOR(k, 1, N) {
            dp[k] = dp[k - 1];

            // find the largest j such that helper[j].first + D <= helper[k].first.
            ll j = lstTrue(0, k-1, [&](ll x) -> bool {return abs(helper[x].first - helper[k].first) >= D;});
            if (j >= 0) {ckmax(dp[k], dp[j] + 1);}
        }
        return dp;
    };
    auto pref = subsolve();
    ...
    V<ll> out(N);
    FOR(k, 0, N) {
        auto [val, idx] = helper[k];
        dbgY(k, MP(val, idx));
        ll score = 1;  // include this element.

        // Find the last j such that helper[j].f + D <= val.
        ll j = lstTrue(0, k-1, [&](ll x) -> bool {return helper[x].first + D <= val;});
        if (j >= 0) {
            score += pref[j];
            dbg(j, pref[j], score);
        }

        // Find the first R such that val + D <= helper[R].f.
        ll R = fstTrue(k+1, N-1, [&](ll x) -> bool {return val + D <= helper[x].first;});
        if (R < N) {
            score += suff[N-1-R];
            dbgP(R, suff[N-1-R], score);
        }
        out[idx] = score;
    }
