...
void solve() {
    ...
    FOR(k, 0, LINES) {
        lls(line_size);
        rv1(line_size, line[k]);
    }
    dbgR(N, LINES, pdh(line));
    el;
    ...
    dbg(pdh(bcc.vertSets));
    dbgP(pdh(bcc.edgeSets));

    umap<ll,ll> cnt;
    for(auto& q : bcc.vertSets) {
        for(auto& x : q) {
            if (x >= N) {++cnt[x];}
        }
    }
    ...
    // FOR(k, 0, LINES) {
    //     FOR(j, 0, sz(line[k])) {
    //         twocc.ae(N+k, line[k][j]);
    //     }
    // }
    // dbgB(pdh(twocc.DSU.e));
    // twocc.gen();
    // dbgP(twocc.extra);
    // dbg(pdh(twocc.DSU.e));
