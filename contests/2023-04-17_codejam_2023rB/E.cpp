...
void solve() {
    ...
    queue<ll> todo;
    FOR(k, 0, N) {if (ei[k].empty()) todo.push(k);}
    dbgcY("initial todo", todo);
    ll ans = 0;
    V<bool> visited(N);
    while (!todo.empty()) {
        ll k = todo.front(); todo.pop();
        if (visited[k]) {continue;}
        visited[k] = true;
        dbgR(k);

        // Add cars here if needed.
        if (need[k] > 0) {
            ans += need[k];
            need[k] = 0;
            // dbgR(cars, ans);
        ...
        // Send cars to the child.
        need[dest[k]] -= cars[k];
        ei[dest[k]].erase(k);
        if ((!visited[dest[k]]) && (need[dest[k]] <= 0 || ei[dest[k]].empty())) {
            todo.push(dest[k]);
        }
    }

    auto solve_cycle = [&](auto& cycle) {
        el; dbgc("sc", cycle);
        ll out = 0;
        // Give everyone enough so that they'll win if the previous person wins.
        FOR(k, 0, cycle.size()) {
            ll curr = cycle[k];
            ll prev = (k == 0) ? (cycle.back()) : (cycle[k-1]);
            ll add_here = max(0LL, need[curr] - cars[prev]);
            out += add_here;
            need[curr] -= add_here;
            dbgcP("sc 1", k, MP(curr, prev), add_here, out);
        }

        // Fill up the cheapest guy.
        ll cheapest = INF_ll;
        FOR(k, 0, cycle.size()) {
            ll curr = cycle[k];
            ckmin(cheapest, need[curr]);
        }
        out += cheapest;
        dbgcR("sc done", out);
        return out;
    ...
    // Remaining locations make up some number of cycles.
    FOR(k, 0, N) {
        if (visited[k]) {continue;}
        V<ll> cycle = {k, dest[k]};
        while (cycle.back() != k) {
            cycle.pb(dest[cycle.back()]);
        }
        cycle.pop_back();
        ans += solve_cycle(cycle);
        for(auto& x : cycle) {visited[x] = true;}
    }
