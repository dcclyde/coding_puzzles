...
void solve() {
    ...
    // ps[n] = sum over 0, 1, ..., n-1
    V<ll> prefix_sums(N+1);
    FOR(k, 1, N+1) {
        prefix_sums[k] = prefix_sums[k-1] + dat[k-1];
    }
    // ss[n] = sum over n, n+1, ...
    V<ll> suffix_sums(N+1);
    ROF(k, 0, N) {
        suffix_sums[k] = suffix_sums[k+1] + dat[k];
    }

    auto in_b = [&](ll x) -> bool {return B.f <= x && x <= B.s;};
    ...
    FOR(a, A.f, A.s+1) {
        if (in_b(a)) {
            ...
            ll curr = TOTAL;
            if (in_b(a+1)) {
                // A gets all piles from 0 to a.
                ckmin(curr, prefix_sums[a+1]);
            }
            if (in_b(a-1)) {
                // A gets all piles from a to N-1.
                ckmin(curr, suffix_sums[a]);
            }
            ckmax(out, curr);
        } else {
            // B should choose the closest pile to a.
            ll b = B.f; ll dir = 1;
            if (b < a) {b = B.s; dir = -1;}

            // Then they'll run toward each other.
            ll dist = abs(a-b);
            ll a_travel_dist = dist / 2;
            ll a_final = a + a_travel_dist * dir;

            if (dir == 1) {
                // A gets all piles from 0 to a_final.
                ckmax(out, prefix_sums[a_final+1]);
            } else {
                // A gets all piles from a_final to N-1.
                ckmax(out, suffix_sums[a_final]);
            }
        }
    }
