...
ll s2score(auto& s) {
    ll out = 0;
    for(auto& c : s | views::reverse) {out *= 3; out += c;}
    return out;
}
...
void solve() {
    ...
    V<string> dat;
    rv(N, dat);
    for(auto& s : dat) {sort(all(s));}
    dbgR(N, dat);
    el;

    map<string, V<ll>> string_to_indices;
    auto& s2i = string_to_indices;
    FOR(k, 0, N) {s2i[dat[k]].push_back(k);}

    V<tuple<string, string, string>> swaps_phase_1 = {
        {"iii", "nnn", "in"},
        {"iii", "nnw", "in"},
        {"iii", "nww", "iw"},
        {"iii", "www", "iw"},
    ...
    V<tuple<string, string, string>> swaps_phase_2 = {
        {"inn", "iww", "nw"},
        {"iin", "nww", "iw"},
        {"iiw", "nnw", "in"}
    };

    V<tuple<ll, char, ll, char>> out;

    auto swap_one = [](string s, char x, char y) {
        FOR(k, 0, 3) {if (s[k] == x) {s[k] = y; break;}}
        sort(all(s)); return s;
    };

    auto helper = [&](auto& swaps) {
        V<pair<string, string>> swaps_results;
        for(auto& [s1, s2, chars] : swaps) {
            // what do I get if I swap chars[0] for chars[1] in s1?
            swaps_results.emplace_back(
                swap_one(s1, chars[0], chars[1]),
                swap_one(s2, chars[1], chars[0])
            );
        }

        auto try_swap = [&](ll k) {
            auto& [s1, s2, chars] = swaps[k];
            if (s2i[s1].size() * s2i[s2].size() == 0) {return false;}
            out.emplace_back(s2i[s1].back(), chars[0], s2i[s2].back(), chars[1]);
            s2i[swaps_results[k].f].push_back(s2i[s1].back()); s2i[s1].pop_back();
            s2i[swaps_results[k].s].push_back(s2i[s2].back()); s2i[s2].pop_back();
            return true;
        };
        ...
        while (!done) {
            ...
            FOR(k, 0, swaps.size()) {
                if (try_swap(k)) {done = false; break;}
            }
        }
    };

    helper(swaps_phase_1);
    helper(swaps_phase_2);

    // now maybe I'm left with stuff like abb bcc caa? or other way around?
    ll n1 = s2i["inn"].size();
    ll n2 = s2i["iin"].size();
    if (n1 > 0) {
        auto& A = s2i["inn"];
        ...
        FOR(k, 0, n1) {
            /**
                ...
                --> win, nnw, iiw -->
             */
            out.emplace_back(A[k], 'n', B[k], 'w');  // win, nnw, iiw
            out.emplace_back(B[k], 'n', C[k], 'i');
        }
    } else {
        auto& A = s2i["iin"];
        auto& B = s2i["nnw"];
        auto& C = s2i["iww"];
        FOR(k, 0, n2) {
            out.emplace_back(A[k], 'i', B[k], 'w');  // win, inn, iww
            out.emplace_back(B[k], 'n', C[k], 'w');
        }
    }

    // now write the output.
    ps(out.size());
    for(auto& [p1, c1, p2, c2] : out) {
        ps(p1+1, c1, p2+1, c2);
    }
    return;
}
