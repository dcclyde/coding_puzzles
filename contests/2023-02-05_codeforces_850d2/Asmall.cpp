...
void solve() {
    ...
    auto dat = ndvec<ll>(2, 2, 0LL);
    ll pos = 0;
    ll step = 1;
    while (true) {
        dbg(step, pos, dat);
        ll next_pos = pos + step;
        ll curr_end = minll(next_pos, N);  // use cards in [pos, curr_end).
        ll sz = curr_end - pos;

        // player goes 0, 1, 1, 0, 0, 1, 1, 0, 0, ...
        ll player = 0;
        if (step % 4 == 3 || step%4 == 2) {player = 1;}
        dat[player][0] += sz / 2;
        dat[player][1] += sz / 2;
        if (sz % 2 == 1) {dat[player][pos%2] += 1;}
    ...
    dbgY(pdh(dat));
    // return ps(dat[0][0], dat[0][1], dat[1][0], dat[1][1]);
    return ps(dat[0][0] + dat[0][1], dat[1][0] + dat[1][1]);
}
