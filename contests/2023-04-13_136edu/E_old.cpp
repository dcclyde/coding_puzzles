...
void solve() {
    ...
    ll total_ones = 0;
    V<array<ll, 2>> x(R);
    FOR(r, 0, R) FOR(c, 0, C) {x[r][c] = dat[c][r] - '0'; total_ones += x[r][c];}
    dbgY(pdh(x)); el;

    V<ll> y(R);
    FOR(r, 0, R) {y[r] = x[r][0] + 2*x[r][1];}

    /*
        dp[r][c][o] = cost to reach (r, c) with o in slot (r, c^1).
        Update rules:
            If o == 0, we just step to (r+1, c, ?).
            If o == 1:
                If x[r+1][c] == 1, we have to pay. Try both options.
                If x[r+1][c] == 0, just step to (r, c^1, 0)?

        Actually, I'm changing what o means.
        o = 2: opposite guy is 1.
        o = 1: opposite guy is 0 and I previously changed the diagonal guy to 0.
        o = 0: opposite guy is 0 and I didn't mess with diagonal guy.
    */
    auto dp = ndvec<ll>(R, 2, 3, INF_ll);
    dp[0][0][2*x[0][1]] = 0;

    FOR(r, 0, R-1) {
        ROF(o, 0, 3) {
            FOR(c, 0, 2) {
                dbgY(MP(r,c),o, "", dp[r][c][o]);
                if (o == 0) {
                    // Just step down.
                    ckmin(dp[r+1][c][2*x[r+1][c^1]], dp[r][c][o]);
                    dbgP(MT(r+1, c, 2*x[r+1][c^1]), dp[r][c][o], dp[r+1][c][2*x[r+1][c^1]]);
                } else if (o == 2) {
                    if (x[r+1][c] == 1) {
                        // Try stepping sideways (delete the 1 below)
                        ckmin(dp[r][c^1][1], dp[r][c][o] + 1);

                        // Try stepping down (delete the 1 to the side)
                        ckmin(dp[r+1][c][2*x[r+1][c^1]], dp[r][c][o] + 1);
                    } else {
                        // Just step sideways.
                        ckmin(dp[r][c^1][0], dp[r][c][o]);
                    }
                } else {
                    // o == 1.
                    ckmin(dp[r+1][c][0], dp[r][c][o]);
                }
            }
        }
    }
    ...
    ll best = INF_ll;
    FOR(c, 0, 2) FOR(o, 0, 3) {
        ckmin(best, dp[R-1][c][o]);
        dbgB(MP(R-1,c),o, "", dp[R-1][c][o], best);
    }

    ll out = total_ones - best;
    return ps(out);
}
