...
using indexed_set = tree<K, null_type, cmp, rb_tree_tag, tree_order_statistics_node_update>;
template<class K, class V, class cmp = less<K>>
using indexed_map = tree<K, V, cmp, rb_tree_tag, tree_order_statistics_node_update>;
...
V<ll> tot;
V<ll> totients_till_1;

// indexed_map<ll,ll> totients_needed;
map<ll,ll> totients_needed;
...
tcT, class S> struct LazySeg {
    // ops needed, final tied value
	const T idT{0, -1, -1};

    // num totients to apply.
    const S idS{};  // ! identity
	...
	T cmb(T a, T b) {  // ! seg * seg
        ...
        // how many totient ops before a.s and b.s match?
        T out = {asz+bsz, af + bf, -1};
        auto& [osz, of, os] = out;
        while (as != bs) {
            if (as < bs) {bs = tot[bs]; of += bsz;}
            else {as = tot[as]; of += asz;}
            // ++of;
        }
        os = as;
        dbgcW("cmb end", out); el;
        return out;
	}
	void init(int _n) {
    ...
    void lazy_seg(int ind, int L, int R) {
        if (lazy[ind] == idS) return;
        auto& [indsz, indf, inds] = seg[ind];
        dbgcR("lazy_seg", ind, MP(L,R), MT(indsz, indf, inds), lazy[ind], totients_needed);
        FOR(k, 0, lazy[ind]) {
            inds = tot[inds];
        }
        if (inds > 1) {return;}
        ...
        indf = 0;
        V<ll> to_delete;
        for(auto it = totients_needed.lower_bound(L); it != totients_needed.upper_bound(R); ++it) {
            ll curr = min(lazy[ind], it->s);
            if ((it->s -= curr) == 0) {to_delete.push_back(it->f);};
            // indf -= curr;
            indf += it->s;
            dbgB(*it, curr, indf);
        }
        for(auto& x : to_delete) {totients_needed.erase(x);}
        dbgR(indsz, indf, inds, totients_needed);

        // ll steps = totients_needed.order_of_key(R+1) - totients_needed.order_of_key(L);
        // indf -= (L-R+1) - steps;
    };
...
void brute() {
    ...
    ll B = *max_element(all(dat));
    // V<ll> tot(B+1, 0);
    tot.assign(B+1, 0);
    // V<ll> totients_till_1(B+1, 0);
    totients_till_1.assign(B+1, 0);
    FOR(k, 1, B+1) {
        tot[k] = totient(k);
        if (k > 1) totients_till_1[k] = totients_till_1[tot[k]] + 1;
    }
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            ...
            ll out = 0;
            while (*wip.begin() != *wip.rbegin()) {
                ll curr = *wip.rbegin();
                wip.erase(wip.find(curr));
                wip.insert(tot[curr]);
                ++out;
            }
            ps(out);
...
void solve() {
    ...
    FOR(k, 0, N) {
        st.seg[st.n + k] = MT(1, 0, dat[k]);
        ll tt1 = totients_till_1[dat[k]];
        if (tt1 > 0) {totients_needed[k] = tt1;}
    }
    st.build();
    dbg(totients_needed);
    dbgR(st); el; el; el;
    st.detailed_printouts(); el; el;

    FOR(qid, 0, Q) {
        lls(qtype); ll1(L, R);
        el; dbgBold(qtype, MP(L,R));
        // dbgY(st);
        dbgB(totients_needed);
        if (qtype == 1) {
            st.upd(L, R, 1);
            // auto ststring = tsdbg(st);
            // dbg(ststring);
            st.detailed_printouts(); el; el;
        } else {
            ll out = get<1>(st.query(L, R));
            ps(out);
        }
    }
