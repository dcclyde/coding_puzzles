...
void solve() {
    ...
    FOR(r, 0, R) {rv(N, dat[r]);}
    dbgR(N, pdh(dat));
    el;

    auto reflect = [&](ll r, ll c) -> pll {
        // return dat[R-1-r][C-1-c];
        return MP(R-1-r, C-1-c);
    };

    ll num_different = 0;
    FOR(r, 0, R) {
        FOR(c, 0, C) {
            auto [r2, c2] = reflect(r, c);
            if (dat[r][c] != dat[r2][c2]) {
                ++num_different;
            }
        }
    }
    assert(num_different % 2 == 0);
    num_different /= 2;

    bool has_middle = (R % 2 == 1 && C % 2 == 1);

    if (num_different <= K) {
        if (has_middle || (num_different % 2 == K % 2)) {
            return YES;
        }
    }
    return NO;
