...
struct LCA {
	...
	void init(int _N) {  N = _N;
        // In this problem, I secretly know depth <= log2(N).
        int big_depth = 1; while ((1<<big_depth) <= N) ++big_depth;
		int d = 1; while ((1<<d) <= big_depth) ++d;
    ...
    int vdist(int ancestor, int descendant) {
        return depth[descendant] - depth[ancestor];
    }
    ll total_size() {
        ll out = 0;
        for(auto& q : par) {out += q.size() * sizeof(int);}
        for(auto& q : adj) {out += q.size() * sizeof(int);}
        out += depth.size() * sizeof(int);
        return out * 1. / (1<<20);
    }
};
...
LCA lca;
V<int> tot;
V<int> totients_till_1;

// indexed_map<ll,ll> totients_needed;
map<int,int> totients_needed;


tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{0,-1,-1};  // ! identity
    T cmb(T a, T b) {
        // return a+b;
        auto& [asz, asteps, av] = a;
        auto& [bsz, bsteps, bv] = b;
        if (asz == 0) {return b;}
        if (bsz == 0) {return a;}

        T out = {asz+bsz, asteps+bsteps, -1};
        auto& [osz, osteps, ov] = out;
        // how many steps needed to merge av and bv?

        ov = lca.lca(av, bv);
        osteps += lca.vdist(ov, av) * asz;
        osteps += lca.vdist(ov, bv) * bsz;
        return out;
    }  // ! seg * seg
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            ...
            for(auto it = totients_needed.lower_bound(L); it != totients_needed.upper_bound(R); ++it) {
                auto& [idx, totients] = *it;
                totients -= 1;
                if (dat[idx] != 1) {
                    dat[idx] = tot[dat[idx]];
                    st.upd(idx, MT(1, 0, dat[idx]));
                }
                if (totients == 0) { to_delete.push_back(idx); }
            }
            for (auto idx : to_delete) {
                totients_needed.erase(idx);
            }
            dbg(st);
        } else {
