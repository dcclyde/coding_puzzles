...
void solve() {
    ...
    V<ll> subtree_weight(nV), subtree_size(nV);
    V<ll> par(nV);
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        ...
        for (auto& o : G[curr]) {
            ...
            subtree_weight[curr] += subtree_weight[o];
        }
    ...
    V<set<pll>> children(N);
    FOR(k, 1, N) {
        ll p = par[k];
        children[p].emplace(-subtree_size[k], k);
    }

    FOR(qid, 0, nQ) {
        lls(qtype); ll1(v);
        if (qtype == 1) {
            ll out = subtree_weight[v];
            ps(out);
        } else {
            if (children[v].empty()) {continue;}
            if (par[v] == -1) {continue;}
            ll p = par[v];
            ll bc = children[v].begin()->second;
            ll ss = subtree_size[v];
            ll sw = subtree_weight[v];

            ll pss = subtree_size[p];
            ll psw = subtree_weight[p];

            ll bcss = subtree_size[bc];
            ll bcsw = subtree_weight[bc];

            dbgY(v, MP(subtree_size[v],subtree_weight[v]));
            dbg(bc, MP(subtree_size[bc], subtree_weight[bc]));
            dbg(p, MP(subtree_size[p], subtree_weight[p]));
            dbgR(children[v]);

            children[p].erase({-ss, v});
            children[v].erase({-bcss, bc});
            ...
            children[p].emplace(-new_bcss, bc);
            children[bc].emplace(-new_ss, v);
            par[v] = bc;
            par[bc] = p;
            subtree_size[v] = new_ss;
            subtree_weight[v] = new_sw;
            subtree_size[bc] = new_bcss;
            subtree_weight[bc] = new_bcsw;

            dbgP(v, MP(subtree_size[v],subtree_weight[v]));
            dbgP(bc, MP(subtree_size[bc], subtree_weight[bc]));
            dbgP(p, MP(subtree_size[p], subtree_weight[p]));
            el;
        }
    }
