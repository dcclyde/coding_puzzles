...
void solve() {
    ...
    if (r1 >= R || c1 >= C) {
        if (r1 >= R) {
            // answer is definitely in column c1.
            ll get_row = ask(0, c1);
            return finish(get_row, c1);
        } else {
            ll get_col = ask(r1, 0);
            return finish(r1, get_col);
        }
    }

    ll a2 = ask(R-1, C-1);
    ll r2 = R-1 - a2; ll c2 = C-1 - a2;
    if (r2 < 0 || c2 < 0) {
        if (r2 < 0) {
            // answer is definitely in column c2.
            ll get_row = ask(0, c2);
            return finish(get_row, c2);
        } else {
            ll get_col = ask(r2, 0);
            return finish(r2, get_col);
        }
    }

    if (c1 == c2) {
        // answer is in column c1.
        ll get_row = ask(0, c1);
        return finish(get_row, c1);
    }
    if (r1 == r2) {
    ...
    // If we're here, answer should be (r1, c2) or (r2, c1).
    ll a3 = ask(r1, c2);
    if (a3 == 0) {
        return finish(r1, c2);
    } else {
