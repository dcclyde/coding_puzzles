...
void solve() {
    ...
    vecs(t3, NUM_SHOTS, shots);
    el;

    V<uset<ll>> row_shots(R+1);
    V<uset<ll>> col_shots(C+1);
    for(auto& [t, dir, pos] : shots) {
        if (dir == 1) {
            row_shots[pos].insert(t);
        } else {
            col_shots[pos].insert(t);
        }
    }

    auto check = [&](ll s, ll r, ll c) -> bool {
        ll t = r + c + s;
        if (s > NUM_SHOTS) return false;
        if (r < 0 || r > R || c < 0 || c > C) return false;
        if (row_shots[r].find(t) != row_shots[r].end()) return false;
        if (col_shots[c].find(t) != col_shots[c].end()) return false;
        return true;
    };

    auto seen = ndvec<bool>(R+1, C+1, NUM_SHOTS+1, false);
    seen[0][0][0] = 1;
    FOR(r, 0, R+1) {
        FOR(c, 0, C+1) {
            FOR(s, 0, NUM_SHOTS+1) {
                // dbg(MT(r,c,s), r+c+s, seen[r][c][s]);
                if (!seen[r][c][s]) continue;
                if (check(s+1, r, c)) { seen[r][c][s+1] = true; }
                if (check(s, r+1, c)) { seen[r+1][c][s] = true; }
                if (check(s, r, c+1)) { seen[r][c+1][s] = true; }
            }
        }
    }
