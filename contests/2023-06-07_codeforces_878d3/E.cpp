...
void solve() {
    ...
    ll diff = 0;
    FOR(k, 0, N) {diff += (A[k] != B[k]);}

    queue<pll> blocked;
    FOR(qid, 0, Q) {
        // unblock stuff if needed.
        while (!blocked.empty() && blocked.front().first == qid) {
            auto [dummy, p] = blocked.front(); blocked.pop();
            diff += (A[p] != B[p]);
        }

        lls(qtype);
        el; dbgY(qid, diff, qtype, blocked);
        if (qtype == 1) {
            ll1(p);
            // remove diff associated with p
            diff -= (A[p] != B[p]);
            // mark this to be unblocked later
            blocked.push({qid+T, p});
        } else if (qtype == 2) {
            ll1(s1, p1, s2, p2);
            // remove diff associated with p1 and/or p2
            set<ll> positions;
            positions.insert(p1); positions.insert(p2);
            for(auto& p : positions) {
                diff -= (A[p] != B[p]);
            }
            // swap
            char* x = (s1 == 0) ? (&A[p1]) : (&B[p1]);
            char* y = (s2 == 0) ? (&A[p2]) : (&B[p2]);
            if (x != y) { swap(*x, *y); }
            // add diff associated with p1 and/or p2
            for(auto& p : positions) {
                diff += (A[p] != B[p]);
            }
        } else {
