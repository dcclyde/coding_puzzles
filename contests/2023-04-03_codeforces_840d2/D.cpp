...
void brute() {
    lls(N);
    ll1(xpos, ypos, x, y);
    dbgcR("brute", N, MP(xpos, x), MP(ypos, y));
    el;
    ...
    if (x > y) {
        ...
        xpos = N-1 - xpos;
        ypos = N-1 - ypos;
        dbgR("swapped", N, MP(xpos, x), MP(ypos, y));
    }

    V<ll> ph;
    FOR(k, 0, N) {ph.push_back(k);}
    ll out = 0;
    do {
        if (ph[xpos] != x || ph[ypos] != y) {continue;}
        // test if ph is "ascending then descending".
        if (ph[0] > ph[1]) {continue;}
        ll stage = 0;
        FOR(k, 0, N-1) {
            if (stage == 0 && ph[k] > ph[k+1]) {stage = 1;}
            if (stage == 1 && ph[k] < ph[k+1]) {stage = 2; break;}
        }
        if (stage == 1) {dbg(ph); ++out;}
...
void solve() {
    ...
    // dp[recent][above_y] = ways to spend n time ending at recent with etc.
    auto dp = ndvec<mi>(N, N, mi(0));
    auto zeros = dp;
    mi out = 0;

    if (xpos == 0) {dp[x][0] = 1;}
    else {FOR(start, 0, x+1) { dp[start][0] = 1; }}

    FOR(n, 1, N) {
        el; dbgY("pre", n, pdh(dp));

        auto dpnext = zeros;
        FOR(recent, 0, N) {
            FOR(above_y, 0, maxll(recent - y, 0) + 1) {
                FOR(future, recent+1, N) {
                    // don't use y too early.
                    if (n < ypos && future == y) {continue;}
                    ll ay = (future > y);
                    dpnext[future][above_y + ay] += dp[recent][above_y];
                }
            }
        }
        swap(dp, dpnext);
        ...
        // Build in the constraints.
        if (n == xpos) {
            dbgcB("xpos constraint");
            // we'd better be at value x right now.
            FOR(recent, 0, N) {
                FOR(above_y, 0, N) {
                    if (recent != x) {
                        dp[recent][above_y] = 0;
        ...
        // Add current contribution to out.
        ll num_placed = n + 1;
        if (num_placed > xpos) {
            FOR(above_y, 0, N) {
                if (num_placed > ypos) {
                    if (num_placed == N) {continue;}
                    out += dp[N-1][above_y];
                    dbg(n, above_y, dp[N-1][above_y], out);
                } else {
                    ll total_above_y = N-1 - y;
                    ll todo_above_y = total_above_y - above_y;
                    ll arrive_at_y = num_placed + todo_above_y;
                    if (arrive_at_y == ypos) {
                        out += dp[N-1][above_y];
                        dbgP(n, num_placed, above_y, arrive_at_y, dp[N-1][above_y], out);
                    }
                }
            }
        }
