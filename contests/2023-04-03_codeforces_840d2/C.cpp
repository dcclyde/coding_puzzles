...
void solve() {
    ...
    if (N >= 4) {
        ll out = N * (*max_element(all(dat)));
        return ps(out);
    } else if (N == 2) {
        ll out = sumv(dat);
        ll option = 2 * abs(dat[0] - dat[1]);
        ckmax(out, option);
        return ps(out);
    } else {
        V<ll> options;
        options.push_back(sumv(dat));
        options.push_back(3 * abs(dat[0] - dat[1]));
        options.push_back(3 * abs(dat[1] - dat[2]));
        options.push_back(3 * dat[0]);
        options.push_back(3 * dat[2]);
        ll out = *max_element(all(options));
        return ps(out);
    }
