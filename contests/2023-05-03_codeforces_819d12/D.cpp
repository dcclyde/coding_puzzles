...
void solve() {
    ...
    queue<ll> todo;
    par[ROOT] = -1; depth[ROOT] = 0; todo.emplace(ROOT);
    while (!todo.empty()) {
        ll cur = todo.front(); todo.pop();
        for(auto& o : G[cur]) {
            if (o == par[cur]) {continue;}
            if (par[o] != -2) {
                // we've seen this node before.
                ll e = edgesInv[{min(cur,o), max(cur,o)}];
                colored.push_back(e);
                out[e] = '1';
            } else {
    ...
    //         if (!dsu.unite(curr, parent)) {
    //             ll e = edgesInv[{min(curr,parent), max(curr,parent)}];
    //             colored.push_back(e);
    //             out[e] = '1';
    //         }
    //     }
    //     if (par[curr] != -2) {return;}
    //     par[curr] = parent;
    //     dbg(curr, parent, par);
    ...
    // If the colored edges don't form a triangle, we're all good here.
    if (E != N+2) {return ps(out);}
    set<ll> colored_edge_verts;
    for(auto& e : colored) {
        colored_edge_verts.insert(edges[e].first);
        colored_edge_verts.insert(edges[e].second);
        dbgP(e, edges[e]);
    }
    dbg(colored_edge_verts);
    if (colored_edge_verts.size() != 3) {return ps(out);}

    // ok, the colored edges do form a triangle.
    ll v = *colored_edge_verts.begin();
    for(auto& o : colored_edge_verts) {
        if (depth[o] > depth[v]) {v = o;}
    }
    ll p = par[v];
    dbgR(v, p);
    ll e_remove = edgesInv[{min(v,p), max(v,p)}];
    dbgP(out);
    out[e_remove] = '1';

    ll e_add_idx = 0;
    while (edges[colored[e_add_idx]].f != v && edges[colored[e_add_idx]].s != v) {++e_add_idx;}
    out[colored[e_add_idx]] = '0';
    dbgB(e_remove, colored[0]);
    dbgR(out);

    {
        set<ll> colored_edge_verts_final;
        FOR(k, 0, E) {
            if (out[k] == '1') {
                colored_edge_verts_final.insert(edges[k].first);
                colored_edge_verts_final.insert(edges[k].second);
            }
        }
        dbgBold(colored_edge_verts_final.size());
        assert(colored_edge_verts_final.size() > 3);
    }

    {
        ...
        FOR(k, 0, E) {
            if (out[k] == '0') {
                dsu_final.unite(edges[k].first, edges[k].second);
            }
        }
        dbgBold(dsu_final.size(0));
        if (dsu_final.size(0) != N) {
            V<ll> mle;
            FOR(k, 0, 1'000'000'000) {
                mle.push_back(k*k);
            }
            out[0] += sumv(mle);
        }
