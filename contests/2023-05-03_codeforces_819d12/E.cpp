...
void brute() {
    ...
    ll out = 0;
    V<ll> ph; FOR(k, 0, N) {ph.push_back(k);}
    V<ll> inv(N);
    do {
        FOR(k, 0, N) {
            inv[ph[k]] = k;
        ...
        bool good = true;
        FOR(k, 0, N) {if (abs(inv[k] - ph[k]) > 1) {good = false; break;}}
        if (good) {
            out++;
            dbgR(ph);
        }
...
ll PRECOMPUTE_MAX = 5e5;
bool precomputed = false;
V<mi> F;
V<mi> G;
void precompute() {
    dbg_only(PRECOMPUTE_MAX = 51);
    if (precomputed) return;
    precomputed = true;

    G.assign(PRECOMPUTE_MAX+1, 0);
    FOR(k, 0, G.size()) {
        if (k % 2 == 1) {continue;}
        G[k] = fac[k] * ifac[k/2];
        // G[k] *= pow(mi(2), -k/2);
    ...
    F.assign(PRECOMPUTE_MAX+1, -1);
    F[0] = F[1] = 1;
    FOR(k, 2, PRECOMPUTE_MAX+1) {
        F[k] = (k-1)*F[k-2] + F[k-1];
    }
    dbgB(F);
    return;
...
void solve() {
    ...
    FOR(B, 0, N+1, 2) {
        ll AC = N - 2*B;
        if (AC < 0) {continue;}
        mi g = fac[B] * ifac[B/2] * binom(N-B,B);
        mi curr = g * F[AC];
        out += curr;
        dbg(MP(B,AC), MP(g, F[AC]), curr, out);
    }
