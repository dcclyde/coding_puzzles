...
void solve() {
    ...
    ll p0 = n0 * (n0-1) / 2;
    ll p1 = n1 * (n1-1) / 2;
    ll playable = total_pairs - p0 - p1;
    ll GOAL = playable / 2;


    // auto dp = ndvec<ll>(N+1, )

    // greedy for now...
    ll score = 0;
    FOR(a, 0, N) {
        FOR(b, a+1, N) {
            if (S[a] == '0' && S[b] == '1') {++score;}
        }
    }
    ...
    dbgR(GOAL, score_backup);
    if (out == 1) {
        // make sure it's actually doable in 1 step.
        FOR(a, 0, N) {
            if (a + score_backup-GOAL >= N) {break;}
            if (S[a] == '0' && S[a + score_backup-GOAL] == '1') {
                return ps(1);
            }
        }
        return ps(2);
