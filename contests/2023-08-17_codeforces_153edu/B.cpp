...
void solve() {
    ...
    // convert piles of weak coins into strong coins first.
    if (n1 - (TARG % V) >= V) {
        ll num_piles = (n1 - (TARG % V)) / V;
        n1 -= num_piles * V;
        nv += num_piles;
    }

    ll num_strong = TARG / V;
    ll num_weak = TARG - V*num_strong;

    ll out_strong = maxll(0, num_strong - nv);
    ll out_weak = maxll(0, num_weak - n1);

    ll out = out_strong + out_weak;
    ps(out);
    return;
}
