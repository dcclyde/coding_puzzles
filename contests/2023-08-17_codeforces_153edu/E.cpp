...
void brute() {
    ...
    assert(Q == 1);
    auto& [start, goal] = queries[0];
    if (start == goal) {return ps(0);}

    V<ll> seen(nS-1, false);
    seen[start] = 1;
    ...
    while (true) {
        timebomb(10);
        dbgY(out, todo, future);
        while (todo.size() > 0) {
            ll cur = todo.front(); todo.pop();
            if (cur == goal) {
                ps(out);
                return;
            }
            ...
            if (cur < nS-2 && !seen[cur+1]) {
                seen[cur+1] = true;
                future.push(cur+1);
            }
            FOR(o, 0, nS-1) {
                if (S[cur] == S[o] && S[cur+1] == S[o+1] && !seen[o]) {
                    seen[o] = true;
                    future.push(o);
                }
            }
...
void solve() {
    ...
    auto flat = [&](char a, char b) -> ll {
        ll x = a - 'a';
        ll y = b - 'a';
        return x * 26 + y;
    };
    auto unflat = [&](ll f) -> string {
        ll x = f / 26;
        ll y = f % 26;
        string out;
        out.push_back('a' + x);
    ...
    ll nodes_basic = nS - 1;
    ll nodes_augment = 26*26;
    ll nodes_total = nodes_basic + nodes_augment;

    auto nodestr = [&](ll k) -> string {
        string sa = to_string(k);
        if (k >= nodes_basic) {
            sa = unflat(k - nodes_basic);
        }
        return sa;
    };

    V<V<pll>> G(nodes_total);
    auto ae = [&](ll a, ll b, ll w) -> void {
    ...
    FOR(k, 0, nodes_basic) {
        ...
        ll id = flat(S[k], S[k+1]);
        ae(k, nodes_basic + id, 0);
    }
    // dbgY(pdh(G));
    dbg_only(
        {
            FOR(a, 0, nodes_total) {
                for(auto& [b, w] : G[a]) {
                    dbgP(nodestr(a), nodestr(b), w);
                }
            }
        }
    );

    // Now build costs from id to k for all (id, k).
    auto costs = ndvec<ll>(26*26, nodes_total, INF_ll);
    FOR(id, 0, 26*26) {
        if (G[nodes_basic + id].size() == 0) {continue;}
        deque<ll> todo;
        todo.push_back(nodes_basic + id);
        costs[id][nodes_basic + id] = 0;
        while (sz(todo)) {
            dbg_only(
                V<pair<string, ll>> todo_strings;
                auto helper = todo;
                while (sz(helper)) {
                    auto h = helper.front();
                    todo_strings.emplace_back(nodestr(h), costs[id][h]);
                    helper.pop_front();
                }
                dbgB(todo_strings);
            );
            ll cur = todo.front(); todo.pop_front();
            ll cur_cost = costs[id][cur];
            if (cur == nodes_basic + flat('f', 'b')) {
                dbgR("found fb", nodestr(nodes_basic + id), nodestr(cur), cur_cost, G[cur], costs[cur][0]);
            }
            if (cur == 0) {
                dbgW("found 0", nodestr(nodes_basic + id), nodestr(cur), cur_cost, G[cur]);
            }
            for(auto& [o, w] : G[cur]) {
                if (cur == nodes_basic + flat('f', 'b')) {
                    dbg(o, w, costs[id][o]);
                ...
                if (o == 0) {
                    dbgY("reaching 0", nodestr(nodes_basic + id), nodestr(cur), cur_cost, w);
                }
                // if (costs[id][o] != INF_ll) {continue;}
                // costs[id][o] = cur_cost + w;
                if (!ckmin(costs[id][o], cur_cost + w)) {continue;}
                if (w == 0) {todo.push_front(o);}
    ...
    dbg_only(
        {
            auto be = flat('b', 'e');
            FOR(n, 0, nodes_total) {
                if (costs[be][n] >= INF_ll) {continue;}
                dbgB(nodestr(n), costs[be][n]);
            }
        }
    )

    for(auto& [start, goal] : queries) {
        ll out = abs(goal - start);
        FOR(id, 0, 26*26) {
            ll cost = costs[id][start] + costs[id][goal] + 1;
            ckmin(out, cost);
            if (G[nodes_basic + id].size() > 0) {
                dbg(unflat(id), costs[id][start], costs[id][goal], cost, out);
            }
        }
        ps(out);
    }
