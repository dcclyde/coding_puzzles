...
void solve() {
    ...
    ll out = 0;
    FOR(k, 0, N) {
        // have I seen any losing position with value < A[k]?
        ll num_winning_positions = stw.query(0, A[k]-1);
        ll num_losing_positions = stl.query(0, A[k]-1);
        dbgY(k, A[k], num_winning_positions, num_losing_positions);

        // if I can't move then I win.
        // otherwise, if I'm forced to move to a winning position then I lose.
        if (num_winning_positions > 0 && num_losing_positions == 0) {
            dbgc("lose", k);
            stl.upd(A[k], A[k], 1);
            ++out;
        } else {
