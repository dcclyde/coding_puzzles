...
void brute() {
    ...
    // Find the biggest right endpoint over all the query intervals.
    ll B = 0;
    ...
    ll out = 0;
    ll MASK_MAX = powll(3, Q-1);
    FOR(mask, 0, MASK_MAX) {
        V<ll> in(B, 0);
        FOR(k, queries[0].f, queries[0].s+1) {in[k] = true;}
...
string tsdbg(Lazy a) { return tsdbg(MP(a.type, a.v)); }

void Lazy_Lazy(const Lazy& a, Lazy& b) {
    if (a.type == 1) {b.type = 1; b.v = a.v;}
    else {b.v *= a.v;}
}

mi Lazy_seg(const Lazy& a, mi& s, ll L, ll R) {
    if (a.type == 1) {return a.v * (R-L+1);}
    else {return a.v * s;}
}

tcT, class S> struct LazySeg {
	const T idT{0}; const S idS{0, 1};  // ! identity
	...
	void push(int ind, int L, int R) { /// modify values for current node
        ...
        // seg[ind] = lazy[ind].c * seg[ind] + lazy[ind].m * (R-L+1);
        seg[ind] = Lazy_seg(lazy[ind], seg[ind], L, R);

		if (L != R) F0R(i,2)
            // lazy[2*ind+i] += lazy[ind]; // ! lazy * lazy
            // lazy[2*ind+i] = {
            //     lazy[2*ind+i].c * lazy[ind].c,
            //     lazy[ind].c * lazy[2*ind+i].m + lazy[ind].m
            // };
            Lazy_Lazy(lazy[ind], lazy[2*ind+i]);
...
void solve() {
    ...
    st.init(B);
    dbg(st);
    st.upd(queries[0].f, queries[0].s, {1, 1});
    dbgR(st);

    mi pow3 = 1;
    FOR(qid, 1, Q) {
        auto& [l, r] = queries[qid];
        el; dbgY(qid, MP(l,r), pow3);
