...
void solve() {
    ...
    auto gc = [&](ll i) -> ll { return covers[i] - '0'; };

    // dp[n][c] = score for 0..n-1
    auto dp = ndvec<ll>(N+1, 2, 0LL);

    FOR(n, 1, N+1) {
        // do I move a cover away from position n-1?
        if (n-1 > 0 && gc(n-1)) {
            dbg(n);
            ckmax(dp[n][0], dp[n-1][0] + dat[n-2]);
        }
        ckmax(dp[n][gc(n-1)], max(dp[n-1][0], dp[n-1][1]) + gc(n-1) * dat[n-1]);
        dbgY(n, dp[n]); el;
    }

    ll out = *max_element(all(dp[N]));
    return ps(out);
}
