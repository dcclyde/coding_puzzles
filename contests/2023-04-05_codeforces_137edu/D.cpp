...
void solve() {
    ...
    // trim leading 0s from dat, but keep at least one 0.
    ll leading_0s_end = 0;
    while (leading_0s_end < N && dat[leading_0s_end] == '0') {++leading_0s_end;}
    if (leading_0s_end == N) {return ps(0);}
    dat = dat.substr(leading_0s_end);
    N = dat.size();

    // Find the first 0 in dat.
    ll first_0 = 0;
    while (first_0 < N && dat[first_0] == '1') {++first_0;}
    if (first_0 == N) {return ps(dat);}
    dbg(first_0);

    uset<ll> starts;
    FOR(k, 0, first_0) {starts.insert(k);}
    FOR(master, first_0 + 1, N) {
        if (starts.size() == 1) {break;}
        if (dat[master] == '1') {continue;}
        // are there any starts that have a 1 here?
        uset<ll> new_starts;
        for (ll start : starts) {
            if (dat[start + master - first_0] == '1') {
                new_starts.insert(start);
            }
        }
        if (!new_starts.empty()) {starts = new_starts;}
    }

    ll start = *starts.begin();
    dbgY(start);
    ROF(k, first_0, N) {
        dbg(k, k-first_0+start, dat[k], dat[k-first_0+start]);
        if (dat[k - first_0 + start] == '1') {dat[k] = '1';}
    }
    return ps(dat);
}
