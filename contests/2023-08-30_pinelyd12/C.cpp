...
void solve() {
    ...
    // find missing item from A.
    auto B = A;
    sort(all(B));
    ll missing = 0;
    while (missing < N && B[missing] == missing) {
        ++missing;
    }
    dbgY(missing);
    A.push_back(missing);
    ...
    auto compose_permutation = [&](V<ll>& P1, V<ll>& P2) {
        V<ll> P3;
        FOR(j, 0, N+1) {
            P3.push_back(P1[P2[j]]);
        }
        return P3;
    };

    // now raise P to the power k.
    while (K > 0) {
        if (K % 2 == 1) {
            curr = compose_permutation(curr, P);
        }
        P = compose_permutation(P, P);
        K /= 2;
    }

    dbgB(curr);

    // auto ans = compose_permutation(curr, A);
    // ans.pop_back();

    V<ll> ans;
    FOR(k, 0, N) {
        ans.push_back(A[curr[k]]);
    }
