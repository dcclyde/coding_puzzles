...
void solve() {
    lls(N, E, HOURS);
    vecs(ll, N, A);  // hour when quest n should be completed.
    ...
    while (!todo.empty()) {
        ll n = todo.front(); todo.pop();
        toposort.emplace_back(n);
        for (ll o : G[n]) {
            --indeg[o];
            if (indeg[o] == 0) {
                todo.push(o);
    ...
    // dp[n] = (offset in days, start time in hours)
    V<pll> dp(N, MP(-1, -1));
    FOR(n, 0, N) {
        dp[n] = MP(0, A[n]);
    }
    ...
    FOR(j, 0, N) {
        ...
        // dp[n] should be fully updated already.
        // dp[n] tells us when we needed to start so that quest n finishes
        // at time (0 days, A[n] hours).

        for(auto& o : G[n]) {
            if (A[o] >= A[n]) {
                // the time to start for finishing A[n] will also work for A[o].
                pll candidate = dp[n];
                // is this earlier than the current stored A[o]?
                // if (candidate.f < dp[o].f || (candidate.f == dp[o].f && candidate.s < dp[o].s)) {
                //     dp[o] = candidate;
                // }
                ckmin(dp[o], candidate);
            } else {
                pll candidate = dp[n];
                candidate.f -= 1;
                ckmin(dp[o], candidate);
    ...
    // sort the leaves by listed hour.
    sort(all(leaves), [&](ll a, ll b) {return A[a] < A[b];});
    dbgR(leaves);

    ll out = INF_ll;
    pll start_time = MP(0, HOURS-1);
    FOR(k, 0, N) {
        ckmin(start_time, dp[k]);
    }
    // start_time now holds the real start time if I begin at hour 0.

    el;
    ROF(ki, 0, leaves.size()) {
        ll k = leaves[ki];
        // update out assuming this leaf gets finished last.
        ll cost = -start_time.f * HOURS + A[k] - start_time.s;
        dbg(ki, k, A[k], dp[k], start_time, "", cost, out);
        ckmin(out, cost);

        // update start_time assuming this leaf actually comes *after* the finish hour.
        dp[k].f -= 1;
        ckmin(start_time, dp[k]);
    }
