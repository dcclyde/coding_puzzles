...
ll solve() {
    ...
    FOR(r, 0, R) {
        if (score[r] != -INF_ll) {continue;}
        if (G[r].empty()) {++leftover_rows; continue;}
        ...
        while (!q.empty()) {
            ...
            for (ll v : G[u]) {
                ...
                if (score[v] == -INF_ll) {
                    score[v] = (score[u] + upd + HOURS) % HOURS;
                    q.push(v);
                } else {
                    if (score[v] != (score[u] + upd + HOURS) % HOURS) {
                        ps(0);
                        return 0;
                    }
                }
    ...
    FOR(c, 0, C) {if (G[R+c].empty()) {++leftover_cols;}}

    ll e = B-1 + leftover_rows + leftover_cols;
    mi out = pow(mi(HOURS), e);
    dbg(B, leftover_rows, leftover_cols, e);
    ps(out);
    return (int)out;
}
...
ll solve_other() {
    ...
    return binpow(h, cmp - 1 + emp_cnt) * ok;
}
