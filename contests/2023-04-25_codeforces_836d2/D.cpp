...
void solve() {
    ...
    ll a_plus_b = N*N + 2*N + 1 - (N*N - N)/2;
    // I need to choose a, b in [1, N-1] such that a+b = a_plus_b mod N.
    ll targ = a_plus_b % N;
    if (targ <= 1) {targ += N;}
    ll a = min(targ-1, N-1);
    ll b = targ - a;

    assert(a >= 1); assert(a <= N-1);
    assert(b >= 1); assert(b <= N-1);
    assert((a+b) % N == a_plus_b % N);

    // ! Watch out: 0 is not allowed. Will this solution ever try to use 0?
    V<ll> out;
    FOR(k, 0, N) {out.push_back(k);}
    FOR(k, 0, a) {out[N-1-k]++;}
    FOR(k, 0, b) {out[N-1-k]++;}
    dbg(out);

    ll rhs = out.back() - out.front();
    rhs *= rhs;
