...
void solve() {
    ...
    dbgR(MP(R,C), HOURS, pdh(dat));
    el;

    auto test_consistent_constraints = [&]() -> bool {
        // Build a graph where each row and column is a node, and each known value is an edge.
        ll N = R+C;
        V<V<ll>> G(N);
        FOR(r, 0, R) {
            FOR(c, 0, C) {
        ...
        FOR(r, 0, R) {
            ...
            while (!q.empty()) {
                ...
                for (ll v : G[u]) {
                    ...
                    // }
                    ll upd;
                    if (u < R) {upd = dat[u][v-R];}
                    else {upd = -dat[v][u-R];}
                    if (score[v] == -1) {
                        score[v] = score[u] + upd;
                        q.push(v);
                    } else {
                        if (score[v] != score[u] + upd) {return false;}
                    }
                }
            }
        }
    ...
    auto solve_assuming_nonzero = [&]() -> mi {
        DSU dsu;
        dsu.init(R+C);
        FOR(r, 0, R) {
            FOR(c, 0, C) {
        ...
        ll leftover_rows = 0; ll leftover_cols = 0;
        map<ll, pll> leader_to_dims;
        FOR(r, 0, R) {
            ll leader = dsu.get(r);
            if (dsu.size(leader) == 1) {++leftover_rows;}
            else {++leader_to_dims[leader].f;}
        }
        FOR(c, 0, C) {
            ll leader = dsu.get(R+c);
            if (dsu.size(leader) == 1) {++leftover_cols;}
            else {++leader_to_dims[leader].s;}
        }
        ll B = leader_to_dims.size();
        ll e = B-1 + leftover_rows + leftover_cols;
        mi out = pow(mi(HOURS), e);
        return out;
    };

    bool has_solutions = test_consistent_constraints();
    if (!has_solutions) {return ps(0);}
    mi out = solve_assuming_nonzero();
    return ps(out);
}
