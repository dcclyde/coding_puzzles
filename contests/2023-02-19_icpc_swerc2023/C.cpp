...
void solve() {
    ...
    /**
        Idea:
        For a given block size X, I could place all the blocks with size >= X
            to cover X-1, 2X-1, 3X-1, ...
        What's the biggest c such that c*X - 1 < S?
        S / X
     */
    bool B_wins = false;
    ll X;
    FOR(k, 0, N) {
        X = dat[k];  // size of block  // 4
        ll blocks_placed = k+1;  // k=2, so blocks_placed = 3
        ll blockers_needed = S / X;  // 2
        if (blocks_placed > blockers_needed) {B_wins = true; break;}
    }
    ...
    if (B_wins) {
        ...
        FOR(turn, 0, N) {
            ...
            if (len >= X) {
                // start = 5, X = 4
                // I should use 4
                ll favorite_point = (start + X-1) / X * X;
                cout << favorite_point << endl;
            } else {
                cout << start << endl;
            }
        }
        ...
        FOR(turn, 0, N) {
            ll len = dat[turn];
            el; dbgY(turn, len, seen);
            auto it = seen.begin();
            auto it2 = seen.begin(); ++it2;
            while (it2 != seen.end()) {
                dbgP(*it, *it2);
                if (*it2 - *it > len) {
                    // this chunk works.
                    ps(len, 1 + *it); cout << flush; break;
                    dbgB(len, 1+*it);
                }
                ++it; ++it2;
            }
