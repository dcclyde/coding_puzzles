...
void solve() {
    ...
    Dijkstra<ll, false> solver;
    solver.init(2*N);
    for(auto& [a, b] : edges) {
        if (dat[a] == dat[b]) {
            solver.ae(2*a, 2*b, 1); solver.ae(2*a+1, 2*b+1, 1);
        } else {
            solver.ae(2*a, 2*b+1, 1); solver.ae(2*a+1, 2*b, 1);
        }
    }
    ...
    FOR(source, 0, N-1) {
        solver.gen(2*source);
        FOR(targ, source+1, N) {
            ll even_cost = solver.dist[2*targ];
            ll odd_cost = solver.dist[2*targ + 1];

            ll X = min(even_cost, odd_cost);
            ll Y = max(even_cost, odd_cost);
            ll cost_here = INF_ll;
            if (X + K <= Y) {
                cost_here = X + K;
            } else {
                // take Y-X steps to make them equal.
                // then I add (leftover // 2) to my score.
                ll leftover = K - (Y-X);
                cost_here = Y + leftover / 2;
            }

            // ll C = (K + odd_cost - even_cost) / 2;
            // ll cost_here = min(even_cost + C, odd_cost + K - C);
            ckmax(out, cost_here);
            dbg(MP(source, targ), MP(even_cost, odd_cost), K, "", cost_here);
        }
    }
