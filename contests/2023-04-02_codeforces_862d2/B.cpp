...
void solve() {
    ...
    V<V<ll>> positions(26);
    for ( int i = 0 ; i < N ; ++i ) {
        positions[dat[i] - 'a'].push_back(i);
    }

    ll pos = 0;
    FOR(i, 0, 26) {
        el; dbgY(i, positions[i]);
        if (positions[i].size() == 0) {continue;}
        if (positions[i].back() == pos + positions[i].size() - 1) {
            pos += positions[i].size();
            return ps(dat);
            continue;
        }
        // swap positions[i].back() into the first slot that doesn't match character i.
        ll a = positions[i].back();
        string out;
        out.push_back('a'+i);
        FOR(k, 0, N) {if (k != a) {out.push_back(dat[k]);}}
        return ps(out);
    }

    // if we're here, then the string was already sorted I think.
    return ps(dat);
}
