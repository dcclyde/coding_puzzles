...
void solve() {
    ...
    V<t3> parabolas;
    rv(nP, parabolas);
    dbgR(nP, pdh(parabolas));

    el;

    set<pll> lines_set;
    FOR(k, 0, nL) {lines_set.emplace(lines[k], k);}

    FOR(qid, 0, nP) {
        auto [a, b, c] = parabolas[qid];
        el; dbgY(qid, MT(a, b, c));

        if (c <= 0) {NO; continue;}
        db lo = -sqrt(db(c) / a);
        db hi = sqrt(db(c) / a);
        db lo_val = a*lo*lo + b*lo + c;
        db hi_val = a*hi*hi + b*hi + c;
        db lo_slope = lo_val / lo;
        db hi_slope = hi_val / hi;

        if (lo_slope > hi_slope) {swap(lo_slope, hi_slope);}
        lo_slope += EPS; hi_slope -= EPS;

        ll lo_ll = ceil(lo_slope);
        ll hi_ll = floor(hi_slope);
        dbg(MP(lo,hi), MP(lo_val,hi_val), MP(lo_slope,hi_slope), MP(lo_ll,hi_ll));

        // I need a line with slope in (lo_slope, hi_slope).
        auto it = lines_set.lower_bound({lo_ll, -1});
        if (it == lines_set.end() || it->first > hi_ll) {NO; continue;}
        YES;
        ps(it->first);
    }
