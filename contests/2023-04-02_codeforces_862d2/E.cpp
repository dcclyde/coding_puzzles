...
tcT, class S> struct LazySeg {
	...
	void push(int ind, int L, int R) { /// modify values for current node
        ...
        seg[ind] = max(seg[ind], lazy[ind]); // ! lazy * seg
        if (L != R) F0R(i,2) lazy[2*ind+i] = max(lazy[2*ind+i], lazy[ind]); // ! lazy * lazy
...
void solve() {
    ...
    FOR(k, 0, E) {
        ...
        G[a].emplace_back(b);
        G[b].emplace_back(a);
        edge_to_idx[MP(a,b)] = k;
        edge_to_idx[MP(b,a)] = k;
        hld.ae(a, b);
    }
    dbgY(pdh(G));
    hld.init(N, ROOT);
    ...
    /*
        Steps:
        1) Find the biggest value M that occurs 3+ times. Set the whole segtree to that value.
        2) If an other value is smaller than M or occurs only once, skip it.
        3) Otherwise, everything EXCEPT the path between those nodes gets maxed against v.
    */

    umap<ll, V<ll>> val_to_positions;
    ll biggest_common = 0;
    FOR(k, 0, N) {
        val_to_positions[dat[k]].push_back(k);
        if (val_to_positions[dat[k]].size() >= 3) {
            ckmax(biggest_common, dat[k]);
        }
    }
    dbg(biggest_common);
    hld.modifySubtree(0, biggest_common);  // should hit everything, right?
    dbg(hld.tree);
    // hld.tree.upd(0, hld.tree.orig_n-1, biggest_common);  // this is the same as above, right?

    auto get_path_chunks = [&](ll a, ll b) {
        V<pll> chunks;
        hld.processPath(a, b, [&](ll l, ll r) {
            chunks.emplace_back(l, r);
        });
        sort(all(chunks));
        return chunks;
    };

    for(auto& [val, positions] : val_to_positions) {
        if (val <= biggest_common || positions.size() == 1) { continue; }
        el; dbgY(val, positions);

        ll a = positions[0]; ll b = positions[1];
        // I should max val against everything EXCEPT the path between a and b.
        // This is kinda tricky because it's not really what HLD is designed for...
        // V<pll> chunks;
        // hld.processPath(a, b, [&](ll l, ll r) {
        //     chunks.emplace_back(l, r);
        // });

        // chunks.emplace_back(-1, -1);
        // chunks.emplace_back(N, N);
        // sort(all(chunks));
        auto chunks = get_path_chunks(a, b);
        dbgP(chunks);
        if (chunks[0].f > 0) { hld.tree.upd(0, chunks[0].f-1, val); }
        // ll stn = hld.tree.orig_n;
        ll stn = hld.tree.n;
        if (chunks.back().s < stn-1) { hld.tree.upd(chunks.back().s+1, stn-1, val); }
        FOR(k, 1, chunks.size()) {
            hld.tree.upd(chunks[k-1].s+1, chunks[k].f-1, val);
        }
        dbgB(hld.tree);
    }
    // dbgY(hld.par[0]);

    dbg(hld.tree.query(1, 0));
    dbg(hld.tree.query(5, 5));

    el;
    V<ll> out(E);
    FOR(k, 1, N) {
        ll parent = hld.par[k];
        ll edge_idx = edge_to_idx[MP(k, parent)];
        ll edge_val = hld.queryPath(k, parent);
        out[edge_idx] = edge_val;
        auto chunks = get_path_chunks(k, parent);
        dbg(MP(k,parent), edge_idx, edge_val, chunks);
    }

    dbgR(hld.tree.query(50, 80));
