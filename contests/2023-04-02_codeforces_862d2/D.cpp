...
void solve() {
    ...
    // For each vertex in G, I need to find the longest path from it to a leaf.
    ll ROOT = 0;
    V<pll> height(N);
    V<pll> height2(N);
    V<ll> depth(N);
    {
        auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
            ...
            for (auto& o : G[curr]) {
                ...
                // todo
                height_options.emplace_back(height[o].first + 1, o);

            }
            // update parent after all children
            // todo
            sort(all(height_options));
            if (height_options.size() == 0) {
                height[curr] = {0, -1}; height2[curr] = {0, -1};
            } else if (height_options.size() == 1) {
                height[curr] = height_options[0];
            } else {
                height[curr] = height_options.back();
                height2[curr] = height_options[height_options.size()-2];
            }
    ...
    V<pll> out2(N);
    {
        auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
            ...
            V<pll> out_options;
            if (parent != -1) {
                if (out[parent].second != curr) {
                    out_options.emplace_back(out[parent].first + 1, parent);
                }
                if (out2[parent].second != curr) {
                    out_options.emplace_back(out2[parent].first + 1, parent);
                }
            }
            out_options.push_back(height[curr]);
            out_options.push_back(height2[curr]);

            sort(all(out_options));
            out[curr] = out_options.back();
            out2[curr] = out_options[out_options.size()-2];
    ...
    V<ll> help(N);
    FOR(k, 0, N) {help[k] = out[k].f;}
    sort(all(help));

    V<ll> result(N);
    FOR(n, 1, N+1) {
        // How many entries with help[k] < n?
        result[n-1] = lower_bound(all(help), n) - help.begin();
