...
void solve() {
    ...
    umap<ll,ll> combined_val2cnt;
    for(auto& x : dat) {++combined_val2cnt[x];}
    ...
    V<ll> B(N, 0);
    V<uset<ll>> B_useless(N);
    V<map<ll,ll>> B_val2cnt(N);

    V<ll> subtree_size(N);
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        subtree_size[curr] = 1;
        A_sets[curr].insert(dat[curr]);
        ll A_big_child = -1;
        ll B_big_child = -1;

        ++B_val2cnt[curr][dat[curr]];
        // todo
        ...
        for (auto& o : G[curr]) {
            ...
            if (A_big_child == -1 || A_sets[o].size() > A_sets[A_big_child].size()) {
                A_big_child = o;
            }
            ckmax(A[curr], A[o]);

            if (B_big_child == -1 || subtree_size[o] > subtree_size[B_big_child]) {
                B_big_child = o;
            }
        ...
        // todo
        if (A_big_child != -1) {
            A_sets[curr].swap(A_sets[A_big_child]);
        }

        if (B_big_child != -1) {
            B_val2cnt[curr].swap(B_val2cnt[B_big_child]);
            B_useless[curr].swap(B_useless[B_big_child]);
        }

        for(auto& o : G[curr]) {
            if (o == parent) {continue;}
            for(auto& x : A_sets[o]) {
                if (A_sets[curr].find(x) != A_sets[curr].end()) {
                    ckmax(A[curr], x);
                } else {
                    A_sets[curr].insert(x);
                }
            }

            for(auto& x : B_useless[o]) { B_useless[curr].insert(x); }

            for(auto& [x, cnt] : B_val2cnt[o]) {
                if (B_useless[curr].find(x) != B_useless[curr].end()) {continue;}
                B_val2cnt[curr][x] += cnt;
                if (B_val2cnt[curr][x] >= combined_val2cnt[x] - 1) {
                    B_useless[curr].insert(x);
                    B_val2cnt[curr].erase(x);
                }
            }
        }

        // Delete all elements in A_sets[curr] that are <= A[curr]
        while (!A_sets[curr].empty() && *A_sets[curr].begin() <= A[curr]) {
            A_sets[curr].erase(*A_sets[curr].begin());
        }
    ...
    // ! Remember to process the output in A and B to create the final results.
