...
void solve() {
    ...
    auto binsearch_helper = [&](V<ll>& A, ll guess) -> ll {
        // how many pairs of elements in A have sum <= guess?
        ll out = 0;
        ll R = A.size() - 1;
        FOR(L, 0, A.size()-1) {
            while (R >= 0 && A[L] + A[R] > guess) {R--;}
            if (R <= L) {break;}
            out += R-L;
            // dbgB(L, R, out);
        }
    ...
    auto buildF = [&](ll guess) -> V<ll> {
        V<ll> out;
        ll R = dat.size() - 1;
        FOR(L, 0, dat.size()-1) {
            while (R >= 0 && dat[L] + dat[R] > guess) {R--;}
            if (R <= L) {break;}
            FOR(r, L+1, R+1) {out.pb(dat[L] + dat[r]);}
        }
        sort(all(out));
    ...
    while (N > 1) {
        assert(dat.size() == N);
        el; dbgY(N, out, dat);
        // get rid of the smallest value and account for it in out.
        out += pow(mi(2), N-1) * dat[0];
        ROF(k, 0, N) {dat[k] -= dat[0];}

        // Now actually compute F.
        ll target = fstTrue(0, 1e18, [&](ll x) {
            // return binsearch_helper(dat, x) >= N-1;
            auto bs = binsearch_helper(dat, x);
            // dbgR(x, bs);
            return bs >= N-1;
        });
        dbg(out, target, dat);
