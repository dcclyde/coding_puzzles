...
void solve() {
    ...
    SegTree<pll> st;
    st.init(N);
    FOR(k, 0, N) {st.seg[st.n+k] = MP(dat[k], k);}
    st.build();
    dbgR(st); el;

    // Find last occurrence for each value.
    V<V<ll>> occurrences(P);
    FOR(k, 0, N) {occurrences[dat[k]].push_back(k);}

    V<pll> lastocc(P);  // (last occurrence, value)
    FOR(k, 0, N) {lastocc[dat[k]].f = k;}
    FOR(k, 0, P) {lastocc[k].s = k;}
    sort(all(lastocc));
    ...
    V<ll> out;
    while (out.size() < P) {
        // Find the min value and position from start_point through lastocc position.
        auto [val, pos] = st.query(start_point, lastocc[lastocc_ptr].f);

        out.push_back(val);
        start_point = pos+1;
        used[val] = true;
        while (lastocc_ptr < P && used[lastocc[lastocc_ptr].s]) {++lastocc_ptr;}
        for(auto& x : occurrences[val]) {
            st.upd(x, MP(INF_ll, x));
        }
    }
