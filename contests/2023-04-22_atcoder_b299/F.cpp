...
void solve() {
    ...
    mi out = 0;
    auto dp = ndvec<mi>(N+1, N+1, mi(0));
    FOR(c, 0, N) {
        FOR(d, c+1, N) {
            if (dat[c] != dat[d]) {continue;}
            dp[c][d] += 1;
            out += dp[c][d];
            el; dbgY(MP(c,d), dp[c][d], out);

            // For each character, find the first instance in [c+1, d-1] and in [d+1, N-1].
            V<ll> first_position(26, -1);
            FOR(a, c+1, d) {
                if (first_position[dat[a]-'a'] == -1) {
                    first_position[dat[a]-'a'] = a;
                }
            }
            dbg(pdh(first_position));
            FOR(b, d+1, N) {
                auto& a = first_position[dat[b]-'a'];
                if (a != -1) {
                    dp[a][b] += dp[c][d];
                    dbgR(MP(a,b), dp[a][b]);
                    a = -1;
