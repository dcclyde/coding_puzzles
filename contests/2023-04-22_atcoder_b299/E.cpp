...
void solve() {
    ...
    V<ll> color(N, 1);  // 0 = white, 1 = black

    {
        // run BFS to change color to 0 for all nodes within distance d of point p.
        queue<pll> todo;
        for(auto& [p, d] : pts) {
            if (d > 0) {todo.emplace(p, d-1);}
        }

        while (!todo.empty()) {
            auto [curr, togo] = todo.front(); todo.pop();
            if (color[curr] == 0) {continue;}
            color[curr] = 0;
            for (auto& o : G[curr]) {
                if (togo > 0) {todo.emplace(o, togo-1);}
            }
        }
    }
    dbgY(color); el;

    V<ll> dist_from_black(N, -1);
    {
        // run BFS to find distance from black nodes.
        queue<pll> todo;
        FOR(k, 0, N) {
            if (color[k] == 1) {todo.emplace(k, 0);}
        }

        while (!todo.empty()) {
            auto [curr, dist] = todo.front(); todo.pop();
            dbgP(curr, dist);
            if (dist_from_black[curr] != -1) {continue;}
            dist_from_black[curr] = dist;
            for (auto& o : G[curr]) {
                todo.emplace(o, dist+1);
            }
        }
    }
    dbgB(dist_from_black);
    for(auto& [p, d] : pts) {
        if (dist_from_black[p] != d) {return No;}
    }
    Yes;
    string out(N, '0');
    FOR(i, 0, N) {out[i] += color[i];}
    return ps(out);
}
