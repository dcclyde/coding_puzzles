...
void solve() {
    ...
    V<pll> queries;
    rv(Q, queries);
    for(auto& [a, b] : queries) {--a;}
    dbgR(N, dat);
    dbgR(Q, queries);
    el;

    ll MAX_VALUE = N + Q + 1;
    V<ll> appearances_per_value(MAX_VALUE);
    auto& apv = appearances_per_value;
    V<ll> start_per_value(MAX_VALUE, -10);
    auto& spv = start_per_value;

    umap<ll,ll> occ;
    for(auto& x : dat) {++occ[x]; spv[x] = -1;}

    FOR(qid, 0, Q) {
        auto& [j, v] = queries[qid];
        // turn dat[j] into v.
        if (--occ[dat[j]] == 0) {
            apv[dat[j]] += qid - spv[dat[j]];
            spv[dat[j]] = -10;
        }
        dat[j] = v;
        if (++occ[dat[j]] == 1) {
