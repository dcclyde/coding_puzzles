...
void learn() {
    dbg_only(
        ll MAX = 30;
        auto N = ndvec<mi>(MAX+1, MAX+1, mi(0));
        auto F = ndvec<mi>(MAX+1, MAX+1, mi(0));
        N[0][0] = 1; F[0][0] = 1;

        FOR(x, 0, MAX+1) FOR(y, 0, MAX+1) {
            if (x+y == 0) {continue;}
            FOR(r, 0, x+1) FOR(s, 0, y+1) {
                if (r+s == 0) {continue;}
                N[x][y] += N[x-r][y-s];
            }
        }
        ...
        FOR(x, 0, MAX+1) FOR(y, 0, MAX+1) {
            ...
            FOR(r, 0, x+1) FOR(s, 0, y+1) {
                if (r+s == 0) {continue;}
                F[x][y] += F[x-r][y-s] + N[x-r][y-s];
            }
            if (x >= y) {dbgY(MP(x,y), N[x][y], F[x][y]);}
        }
    );
    return;
}
...
void precompute() {
    const ll MAX = 10'000'000;
    genFac(MAX+1);
    pow2.resize(MAX+1);
    FOR(n, 1, MAX+1) {pow2[n] = pow2[n-1] * 2;}
    // fact.reserve(MAX+1); ifact.reserve(MAX+1); pow2.reserve(MAX+1);
    // FOR(n, 1, MAX+1) {
    //     fact[n] = fact[n-1] * n;
...
void solve() {
    ...
    // Answer has generating function [f(n,m)] * (1-x)^2 (1-y)^2.
    // f * ((1 - 2x + x^2 - 2y + 4xy - 2x^2y + y^2 - 2xy^2 + x^2y^2))
    mi out = f(N, M) - 2*f(N-1, M) + f(N-2, M)
        - 2*f(N, M-1) + 4*f(N-1, M-1) - 2*f(N-2, M-1)
        + f(N, M-2) - 2*f(N-1, M-2) + f(N-2, M-2);
