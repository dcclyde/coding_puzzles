...
void solve() {
    lls(X, Y, Z);  // cost of letter, cost of shift+letter, cost of capslock
    strings(S);
    el;
    ...
    /*
        Cheapest way to have first n chars typed while capslock is (on, off)?
        Start with n-1.
        Start with correct caps:
            just press the button
            press caps, button+shift, caps

        start with wrong caps:
            caps, button
            button+shift, caps



        End with correct caps:
            Start with correct caps.
                a
                caps shift caps
            start with wrong caps
        ...
        What are all the things I plausibly could do to type next char?
        (caps) button or button+shift

    */

    auto dp = ndvec<ll>(N+1, 2, INF_ll);
    dp[0][0] = 0;
    dp[0][1] = Z;
    FOR(n, 0, N) {
        FOR(caps, 0, 2) {
            bool caps_good = (caps == (S[n] == 'A'));

            // don't press caps.
            ckmin(
                dp[n+1][caps],
                dp[n][caps] + (caps_good ? X : Y)
            );

            // do press caps.
            ckmin(
                dp[n+1][!caps],
                dp[n][caps] + (caps_good ? Z+Y : Z+X)
            );
        }
    }
