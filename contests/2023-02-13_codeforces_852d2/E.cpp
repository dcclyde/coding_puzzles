...
void brute() {
    ...
    FOR(qid, 0, Q) {
        ...
        V<bool> used(q);
        ll out = 0;
        // just try all assignments I guess...
        FOR(strat, 0, powll(q, N)) {
            ll wip = strat;
            FOR(k, 0, q) {used[k] = 0;}
            FOR(k, 0, N) {B[k] = wip % q; used[B[k]] = 1; wip /= q;}
            ll num_used = 0; FOR(k, 0, q) {num_used += used[k];}
            if (num_used < q) {continue;}
            // now, how many people are happy?
            ll num_happy = 0;
            FOR(k, 0, N) {
                ll matches = 0;
                for(auto& x : B) {matches += (x == B[k]);}
                num_happy += (matches >= A[k]);
            }
            ckmax(out, num_happy);
        }
        ps(out);
...
void solve() {
    ...
    auto get = [&](ll x) -> ll {return st.seg[st.n + x];};
    // // dp[k] = max of st[j]+N-j for k <= j.
    // V<ll> dp(N+1);
    // FOR(k, 0, N+1) {dp[k] = get(k) + N - k;}
    // ROF(k, 0, N) {ckmax(dp[k], dp[k+1]);}
    // dbgB(pdh(dp));

    // how many groups can I create while making x people happy?
    V<ll> make_x_happy(N+1);
    auto& mh = make_x_happy;
    mh[0] = N;
    FOR(k, 1, N+1) {
        // ll s = st.query(k,k);
        ll s = get(k);
        if (s > 0) {mh[k] = s + N-k;}
        else if (A[k-1] > N) {mh[k] = 0;}
        else {mh[k] = N + 1 - A[k-1];}
    }
    dbgP(mh);
    ...
    FOR(qid, 0, Q) {
        ll q = queries[qid];
        ll out = lstTrue(0, N, [&](ll x) -> bool {
            // can I break into q groups and make x people happy?
            return mh[x] >= q;
        });
        ps(out);
    }
