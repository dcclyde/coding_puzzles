...
tcT> struct SegTreeMax { // cmb(ID,b) = b
	const T ID{-INF_ll};  // ! identity
    T cmb(T a, T b) { return max(a,b); }  // ! seg * seg
	int n; V<T> seg; int orig_n;
	void init(int _n) { // upd, query also work if n = _n
		for (n = 1; n < _n; ) n *= 2;
		seg.assign(2*n,ID); orig_n = _n;}
...
void solve() {
    ...
    umap<ll,ll> compressed;
    auto sortedA = A;
    sort(all(sortedA));
    FOR(k, 0, N) {compressed[sortedA[k]] = k;}

    umap<ll,ll> Ainv;
    FOR(k, 0, N) {Ainv[A[k]] = k;}

    // organize the queries.
    V<V<pll>> queries_organized(N);
    FOR(k, 0, Q) {
        auto& [l, r] = queries[k];
        queries_organized[r].emplace_back(l, k);
    }
    FOR(k, 0, N) {sort(all(queries_organized[k]));}

    V<ll> out(Q);
    SegTreeMin<ll> st; st.init(N);
    auto stget = [&](ll k) -> ll {return st.seg[st.n + k];};

    // if I've seen A[k] = v, then I should set lookup[compressed[v]] = k.
    SegTreeMax<ll> lookup; lookup.init(N);
    FOR(k, 0, N) {lookup.seg[lookup.n + k] = -1;} lookup.build();
    auto lget = [&](ll k) -> ll {return lookup.seg[lookup.n + k];};
    lookup.upd(compressed[A[0]], 0);
    ...
    FOR(n, 1, N) {
        ...
        ll ca = compressed[a];
        el; dbgY(n, MP(a, ca));
        lookup.upd(compressed[A[n]], n);
        {
            ll hipos = N-1;
            ll hi = sortedA[hipos];
            while (true) {
                // find the latest item with value in [a+1, hi].
                ll new_pos = lookup.query(ca+1, hipos);
                dbgc("hi", lookup, ca, hipos, new_pos);
                // update that position's answer if needed.
                // ! First check if we found a valid position at all...
                if (new_pos < 0) {break;}
                ll old = stget(new_pos);
                if (ckmin(old, abs(A[n]-A[new_pos]))) {st.upd(new_pos, old);}
                // in next round, we only care about stuff closer to A[n]
                //      than to A[new_pos].
                hi = A[n] + (A[new_pos] - A[n] - 1) / 2;
                hipos = lstTrue(0, N-1, [&](ll x) {return sortedA[x] <= hi;});
            }
        }
        {
            ll lopos = 0;
            ...
            while (true) {
                // find the latest item with value in [a+1, hi].
                ll new_pos = lookup.query(lopos, ca-1);
                dbgcP("lo", lookup, ca, lopos, new_pos);
                // update that position's answer if needed.
                // ! First check if we found a valid position at all...
                if (new_pos < 0) {break;}
                ll old = stget(new_pos);
                ...
                // in next round, we only care about stuff closer to A[n]
                //      than to A[new_pos].
                lo = A[new_pos] + (A[n] - A[new_pos]) / 2 + 1;
                lopos = fstTrue(0, N-1, [&](ll x) {return sortedA[x] >= lo;});
            }
        }
        dbgB(st);

        // now answer some queries.
        for(auto& [l, qid] : queries_organized[n]) {
            out[qid] = st.query(l, n);
            dbgW(qid, MP(l,n), out[qid]);
        }
    }
