...
void solve() {
    ...
    ll L = 0; ll R = N-1;
    ll small = 0; ll big = N-1;
    while (L <= R) {
        bool done = true;
        if (dat[L] == small) {++small; ++L; done = false;}
        else if (dat[L] == big) {--big; ++L; done = false;}
        if (dat[R] == small) {++small; --R; done = false;}
        else if (dat[R] == big) {--big; --R; done = false;}
        if (done) {break;}
    }
    if (L > R) {return ps(-1);}
    else {return ps1(L, R);}
