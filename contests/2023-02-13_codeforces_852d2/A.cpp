...
/**
    Pay A, get 1
    Pay K*A, get K+1. So, cost per 1 is K*A/(K+1).
 */
...
void solve() {
    ...
    // buy everything on the second day.
    {
        ll cost = B * N;
        options.push_back(cost);
    }
    // buy groups on first day and then leftovers later.
    {
        ll groups = N / (K+1);
        ll leftover = N % (K+1);
        ll cost = groups * K*A;
    ...
    ll out = INF_ll;
    for(auto& x : options) {ckmin(out, x);}
    return ps(out);
}
