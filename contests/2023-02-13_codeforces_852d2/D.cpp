...
void solve() {
    ...
    V<ll> Ainv(N), Binv(N);
    FOR(k, 0, N) {Ainv[A[k]] = k; Binv[B[k]] = k;}
    dbgB(Ainv);
    dbgP(Binv);
    el;
    ...
    {
        ll c = Ainv[0]; ll d = Binv[0];
        if (c > d) {swap(c,d);}
        // how many ways staying below c?
        ll ways_below_c = c * (c+1) / 2;
        ll ways_between = (d-c-1) * (d-c) / 2;
        ll ways_above_d = (N-1-d) * (N-d) / 2;
        ll cur = ways_below_c + ways_between + ways_above_d;
        out += cur;
        nl = c; nr = d;
        dbgc("k = 0", MP(c,d), MT(ways_below_c, ways_between, ways_above_d), out);
    }
    FOR(k, 1, N) {
        // how many ways if we skip both of the new items?
        ll c = Ainv[k]; ll d = Binv[k];
        if (c > d) {swap(c,d);}
        ll cur = -1;
        if ((nl <= c && c <= nr) || (nl <= d && d <= nr)) {cur = 0;}
        else {
            ll lo = 0; ll hi = N-1;
            if (c < nl) {ckmax(lo, c+1);} else {ckmin(hi, c-1);}
            if (d < nl) {ckmax(lo, d+1);} else {ckmin(hi, d-1);}
            cur = (nl - lo + 1) * (hi - nr + 1);
            dbgP(MP(lo,hi));
        }
        out += cur;
        el; dbgY(k, MP(nl,nr), MP(c,d), cur, out);
        nl = min({nl, c, d});
        nr = max({nr, c, d});
    }
    out += 1;  // handle case N?
