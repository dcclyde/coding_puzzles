...
/**
    NxN board, N <= 1000
    ask 20 questions: how many rooks are in a certain rectangle?
    20 is juuuust barely bigger than 2^(num squares).
 */
...
void solve() {
    ...
    ll r = fstTrue(0, N-1, [&](ll m) {return ask(0, m, 0, N-1) < m+1;});
    ll c = fstTrue(0, N-1, [&](ll m) {return ask(0, N-1, 0, m) < m+1;});
