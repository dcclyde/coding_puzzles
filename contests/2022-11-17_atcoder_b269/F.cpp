...
void solve() {
    ...
    auto seqsum = [&](mi start, ll step, ll terms) -> mi {
        mi out = start * terms;
        out += mi(terms) * (terms-1) / 2 * step;
        return out;
    };
    ...
    FOR(qid, 0, Q) {
        ...
        FOR(rp, 0, 2) FOR(cp, 0, 2) {
            if ((rp + cp) % 2 == 1) {continue;}
            ll aa = a; ll bb = b; ll cc = c; ll dd = d;
            if (aa % 2 != rp) {++aa;} if (bb % 2 != rp) {--bb;}
            if (cc % 2 != cp) {++cc;} if (dd % 2 != cp) {--dd;}
            dbgP(aa, bb, cc, dd);
            if (aa > bb || cc > dd) {continue;}

            ll r1start = aa*C + cc + 1;
            ll r1step = 2;
            ll r1terms = (dd-cc+1 + 1) / 2;
            mi first_row = seqsum(r1start, r1step, r1terms);
            dbgB(r1start, r1step, r1terms, first_row);

            ll bigstep = 2*C * r1terms;
            ll bigterms = (bb-aa+1 + 1) / 2;
            mi local = seqsum(first_row, bigstep, bigterms);
            dbg(first_row, bigstep, bigterms, local);

            out += local;
            dbg(rp, cp, local, out); el;
        }
        ps(out); el;
    }
