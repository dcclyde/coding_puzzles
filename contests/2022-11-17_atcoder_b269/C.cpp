...
void solve() {
    ...
    V<ll> bit_positions;
    FOR(b, 0, 60) {
        if (N & (1LL<<b)) {bit_positions.push_back(b);}
    }
    ll MASK_SIZE = bit_positions.size();
    dbg(MASK_SIZE, bit_positions);

    V<ll> out;
    FOR(mask, 0, (1<<MASK_SIZE)) {
        ll curr = 0;
        FOR(b, 0, MASK_SIZE) {
            if (mask & (1<<b)) {curr |= (1LL <<bit_positions[b]);}
        }
        out.push_back(curr);
    }
