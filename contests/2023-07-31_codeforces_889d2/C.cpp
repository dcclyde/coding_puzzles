...
void solve() {
    ...
    V<pll> B;
    FOR(k, 0, N) {B.eb(A[k], k);}
    sort(all(B));
    dbgY(B);

    ll lo = min_element(all(B))->first;
    ll hi = max_element(all(B))->first;
    ll neg = 0; ll pos = 0;
    for(auto& x : A) {
        if (x < 0) {++neg;}
        if (x > 0) {++pos;}
    ...
    bool flipped = false;
    auto flip = [&]() -> void {
        dbgcR("flip", MP(lo, hi), MP(neg, pos), pdh(B));
        for(auto& [x, idx] : B) {
            x = -x;
            // idx = N-1 - idx;
        }
        ...
        dbgcR("flip END", MP(lo, hi), MP(neg, pos), pdh(B));
        assert(lo == B[0].f);
        assert(hi == B.back().first);
        return;
    };

    V<pll> out;
    // X[a] += X[b]
    auto op = [&](ll a, ll b) -> void {
        dbgcP("op", a, b);
        B[a].f += B[b].f;
        out.emplace_back(B[a].s, B[b].s);
        lo = min_element(all(B))->first;
        hi = max_element(all(B))->first;
    };
    auto finish_nonneg = [&]() -> void {
        dbgcB("finish_nonneg", pdh(out), pdh(B));
        sort(all(B), [](pll x, pll y) -> bool {
            return (x.s < y.s);
        });
        if (!flipped) {
            FOR(k, 0, N-1) {
    ...
    if (hi <= 0) {
        ...
        // WOLOG make the heaviest thing negative.
        if (-lo < hi) {flip();}
        if (pos <= 12) {
            // spend `pos` steps killing positives.
            FOR(k, 0, N) {
                if (B[k].f > 0) {
                    op(k, 0);
                }
            ...
            // pos >= 13, so neg <= 7.
            // build a big positive and then spend `neg` steps killing negatives.
            FOR(k, 0, 5) {
                op(N-1, N-1);
            }
            FOR(k, 0, N) {
