...
void solve() {
    ...
    set<ll> seen;
    for(auto& x : dat) {
        if (seen.find(x) != seen.end()) {base += C;}
        seen.insert(x);
    }

    // always possible to remove everything and insert 1.
    ll best_cost = C * seen.size() + D;
    ll ctr = 1;
    for(auto& x : seen) {
        // try building the permutation 1..x.
        ll cost_here =
            C * (seen.size() - ctr)
            + D * (x - ctr);
        dbg(x, ctr, cost_here);
        ckmin(best_cost, cost_here);
        ++ctr;
    }

    ll out = base + best_cost;
    return ps(out);
}
