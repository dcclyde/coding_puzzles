...
void solve() {
    ...
    FOR(start, 0, N) {
        ...
        while (!todo.empty()) {
            el;
            // dbgR(dsu.size(4));
            pll curr = todo.top(); todo.pop();
            dbgY(curr, todo);
            if (-curr.f > dsu.size(start)) {break;}
            dbg(G[curr.s], dsu.size(curr.s));

            // if (dsu.size(curr.s) > 1) {continue;}
            // if (curr.s != start && !dsu.unite(start, curr.s)) {continue;}
            if (visited.find(curr.s) != visited.end()) {continue;}
            visited.insert(curr.s);

            dsu.unite(start, curr.s);

            // now insert all adjacent stuff that we haven't seen before.
            for(auto& o : G[curr.s]) {
                if (!dsu.sameSet(start, o)) {todo.emplace(-dat[o], o);}
            }
            dbgP(dsu.size(start));
        }
        if (dsu.size(start) == N) {return YES;}
        dbg_only(
            V<ll> connected;
            FOR(n, 0, N) {if (dsu.sameSet(start, n)) {connected.push_back(n);}}
            dbgR(connected);
        );
    }
