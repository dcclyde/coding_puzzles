...
void brute() {
    ...
    auto solve_abh = [](ll A, ll B, ll H) -> ll {
        ll out = 0;
        ll height = 0;
        while (true) {
            ++out;
            height += A;
            if (height >= H) {return out;}
            height -= B;
        }
    };

    const ll MAX_HEIGHT = 1000;
    V<ll> out;
    V<bool> possible(MAX_HEIGHT+1, true);
    possible[0] = 0;
    FOR(qid, 0, Q) {
        lls(qtype);
        if (qtype == 1) {
            lls(A, B, N);
            dbgR(qid, qtype, A, B, N);
            // is this plausible?
            bool ok = false;
            FOR(H, 1, MAX_HEIGHT+1) {
                if (!possible[H]) {continue;}
                if (solve_abh(A, B, H) == N) {ok = true; break;}
            }
            if (!ok) {out.push_back(0); continue;}
            out.push_back(1);
            FOR(H, 1, MAX_HEIGHT+1) {
                if (!possible[H]) {continue;}
                if (solve_abh(A, B, H) != N) {possible[H] = 0;}
            }
        } else {
            lls(A, B);
            dbgR(qid, qtype, A, B);
            ...
            FOR(H, 1, MAX_HEIGHT+1) {
                if (!possible[H]) {continue;}
                ll N = solve_abh(A, B, H);
                if (result != -2 && result != N) {result = -1; break;}
                result = N;
            }
...
void solve() {
    ...
    auto solve_abn = [](ll A, ll B, ll N) -> pll {
        if (N == 1) {return MP(1, A);}
        ll D = A-B;
        ll lo = (N-2)*D + A + 1;
        ll hi = (N-1)*D + A;
        return MP(lo, hi);
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            lls(A, B, N);
            auto curr = solve_abn(A, B, N);
            if (possible.s == -1) {possible = curr; out.push_back(1);}
            else if (possible.f > curr.s || possible.s < curr.f) {out.push_back(0);}
            else {ckmax(possible.f, curr.f); ckmin(possible.s, curr.s); out.push_back(1);}
            dbgB(qid, MT(A,B,N), possible, out.back(), out); el;
        } else {
            lls(A, B);
            if (possible.s == -1) {out.push_back(-1);}
            else {
                ll lo = solve_abh(A, B, possible.f);
                ll hi = solve_abh(A, B, possible.s);

                // ll lo = fstTrue(1, 10, [&](ll x) {
                //     return (x-1) * (A-B) + A >= possible.f;
                // });
                // ll hi = fstTrue(1, 10, [&](ll x) {
                //     return (x-1) * (A-B) + A >= possible.s;
                // });
                if (lo == hi) {out.push_back(lo);}
                else {out.push_back(-1);}
                dbgY(qid, MT(A,B), lo, hi, out.back(), out); el;
            }
        }
    }
    return pv(out);
