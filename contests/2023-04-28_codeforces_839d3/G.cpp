...
void brute() {
    ...
    while (score < TARG) {
        dbg(out, score);
        if (score <= START && out > 0) {return ps(-1);}
        ll curr = out % N;
        if (score >= A[curr]) {++score;}
        else {--score;}
        ++out;
    }
    return ps(out);
...
void solve() {
    ...
    ll score = START;
    ll sofar = 0;
    ll pos = 0;  // pos points at the first guy I can't beat.
    while (pos < N && score >= A[pos]) { ++score; ++pos; ++sofar; }

    if (score >= TARG) {return ps(TARG - START);}
    if (pos == N) {return ps(TARG - START);}
    if (pos <= N/2) {return ps(-1);}

    ll win_per_cycle = pos;
    ll lose_per_cycle = N - pos;
    ll delta_per_cycle = win_per_cycle - lose_per_cycle;

    ll ans = INF_ll;
    while (true) {
        el; dbgY(pos, score, sofar);
        dbg(MT(win_per_cycle, lose_per_cycle, delta_per_cycle));
        // how many deltas does it take for me to reach TARG?
        {
            ll delta = TARG - score;
            ll cycles = cdiv(delta, delta_per_cycle);
            ckmin(ans, sofar + cycles*N - (score + cycles*delta_per_cycle - TARG));
        }
        if (pos == N || A[pos] >= TARG) {break;}

        // how many times do I need to add delta_per_cycle before I get to A[pos]?
        {
            ll delta = A[pos] - score;
            ll cycles = cdiv(delta, delta_per_cycle);
            sofar += cycles * N;
            score += cycles * delta_per_cycle;
            while (pos < N && score >= A[pos]) { ++score; ++pos; ++sofar; }
            if (score >= TARG) {return ps(sofar - (score - TARG));}
            win_per_cycle = pos;
            lose_per_cycle = N - pos;
            delta_per_cycle = win_per_cycle - lose_per_cycle;
        }
    }
    return ps(ans);
}
