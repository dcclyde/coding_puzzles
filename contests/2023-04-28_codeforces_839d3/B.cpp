...
void solve() {
    ...
    // cin >> dat[0][0] >> dat[0][1] >> dat[1][0] >> dat[1][1];
    // dbg(pdh(dat));
    vecs(ll, 4, A);
    el;

    ll small_idx = min_element(all(A)) - A.begin();
    ll big_idx = max_element(all(A)) - A.begin();
    dbg(small_idx, big_idx);
    return YESNO(small_idx + big_idx == 3);
}
