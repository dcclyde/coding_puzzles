...
void solve() {
    lls(R, C, N);
    ++N;
    V<pair<ll, V<V<ll>>>> dat(N);
    // auto dat = ndvec<ll>(N, R, C, 0LL);
    FOR(n, 0, N) {
        dat[n].f = n;
        dat[n].s = ndvec<ll>(R, C, 0LL);
        FOR(r, 0, R) {
            strings(curr);
            FOR(c, 0, C) {
                dat[n].s[r][c] = curr[c] - '0';
            }
        }
        dbgR(dat[n].f, pdh(dat[n].s));
    }
    el; el; el;

    // Order the pictures.
    auto compare_pics = [&](const auto& A, const auto& B) -> bool {
        FOR(r, 0, R) {
            FOR(c, 0, C) {
                if (A.s[r][c] != B.s[r][c]) {
                    if (A.s[r][c-1] == A.s[r][c]) {
                        // A came later.
                        return false;
                    } else {
                        return true;
    ...
    sort(all(dat), compare_pics);
    FOR(n, 0, N) {dbgY(dat[n].f, pdh(dat[n].s));}
    el; el; el;

    V<V<ll>> ops;
    FOR(n, 0, N-1) {
        // Go from time n to n+1.
        auto& A = dat[n].s;
        auto& B = dat[n+1].s;
        FOR(r, 0, R) {
            FOR(c, 0, C) {
        ...
        ops.push_back({2, dat[n+1].f + 1});
    }

    ps(dat[0].f + 1);
    ps(ops.size());
    for(auto& op : ops) { pv(op); }
