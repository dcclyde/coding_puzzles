...
// const ll LAZY_EMPTY = -1e18;
tcT, class S> struct LazySeg {
	const T idT{}; const S idS{'x', 0};  // ! identity
	...
	void push(int ind, int L, int R) { /// modify values for current node
        ...
        } else if (lazy[ind].type == '=') {
            // set the range to a constant.
            seg[ind] = (R-L+1)*lazy[ind].v; // ! lazy * seg
    		if (L != R) F0R(i,2) lazy[2*ind+i] = lazy[ind]; // ! lazy * lazy
        } else if (lazy[ind].type == '+') {
            // Within this range, set st[k] = k + v.
            ll extra = (R*(R+1) - (L-1)*L) / 2;
            seg[ind] = (R-L+1)*lazy[ind].v + extra; // ! lazy * seg
...
void brute() {
    ...
    FOR(n, 0, N) {
        ll cost = 0;
        {
            ll prev_health = dat[n];
            FOR(k, n+1, N) {
                ll health_here = min(dat[k], prev_health - 1);
                ckmax(health_here, 0);
                cost += dat[k] - health_here;
                prev_health = health_here;
                dbg(k, cost);
            }
        }
        {
...
void solve() {
    ...
    auto subsolve = [&]() -> V<ll> {
        dbgR(dat);
        LazySeg<ll,lazy_dat> st; st.init(N);
        FOR(k, 0, N) {st.seg[st.n + k] = dat[k] + k;}
        st.build();
        dbg(st);

        // // I guess try using a whole second segtree that doesn't know about 0?
        // LazySeg<ll,lazy_dat> st_nofloor; st_nofloor.init(N);
        // FOR(k, 0, N) {st_nofloor.seg[st_nofloor.n + k] = dat[k] + k;}
        // st_nofloor.build();
        V<pll> first_leq_cur_helper = {{N, -1e18}};
        auto& flh = first_leq_cur_helper;
        flh.emplace_back(N-1, dat[N-1] + N-1);

        V<ll> suffix_total(N+1);
        ROF(k, 0, N) {suffix_total[k] = suffix_total[k+1] + dat[k] + k;}
        dbgY(suffix_total);

        V<ll> cost(N);
        // the last guy has no cost to his right.
        ROF(n, 0, N-1) {
            el; dbgY(n);
            // find the biggest r such that st[n+1, ..., r] will need to be modified.
            // ll cur = st.query(n, n);
            ll cur = dat[n] + n;
            // ll r = lstTrue(n+1, N-1, [&](ll x) -> bool {
            //     dbgW(cur, x, st_nofloor.query(x,x));
            //     return st_nofloor.query(x, x) > cur;
            // });
            dbg(flh);
            while (flh.size() > 0) {
                auto [idx, val] = flh.back();
                if (val < cur) {break;}
                flh.pop_back();
            }
            ll r = flh.back().f - 1;
            flh.emplace_back(n, cur);
            dbgP(flh);

            // the first chunk could be set to constant. Second chunk could be set to k.
            dbg(cur, r, st);
            if (r > n) {
                ll cutoff = min(cur, r);
                st.upd(n+1, cutoff, {'=', cur});
                if (cutoff < r) {
                    st.upd(cutoff+1, r, {'+', 0});
                }
                // st_nofloor.upd(n+1, r, {'=', cur});
            }
            dbgP(st);
            ll total_here = st.query(n+1, N-1);
            cost[n] = suffix_total[n+1] - total_here;
            dbgY(total_here, cost);
        }
        el;
        dbgR(dat);
        dbgBold(cost);
    ...
    auto cost_right = subsolve();
    reverse(all(dat));
    auto cost_left = subsolve();
    reverse(all(dat));
    reverse(all(cost_left));

    ll out = 1e18;
    FOR(k, 0, N) {
        ll cost_here = dat[k] + cost_right[k] + cost_left[k];
        ckmin(out, cost_here);
        dbgR(k, MT(dat[k], cost_left[k], cost_right[k]), cost_here, out);
    }
