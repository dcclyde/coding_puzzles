...
void solve() {
    lls(N);
    // A = amount of tea
    // B = amount this person can drink
    V<ll> A, B;
    rv(N, A); rv(N, B);
    dbgR(N, A, B);
    el;
    ...
    V<ll> out(N);
    LazySeg<ll,ll> st; st.init(N);

    // running[n] = sum of B over [0, n-1].
    V<ll> running = {0};
    FOR(k, 0, N) { running.push_back(running.back() + B[k]); }
    dbg(running);

    el;
    FOR(k, 0, N) {
        // when will the current tea run out?
        // I need the LARGEST j such that running[j] - running[k] >= A[k].

        // * No, I need the FIRST j such that running[j] - running[k] >= A[k].
        ll tea_runs_out = fstTrue(k, N, [&](ll x) -> ll {
            bool result = (running[x] - running[k] >= A[k]);
            dbgcW("tro", x, running[x], running[k], A[k], result);
            return result;
        });
        --tea_runs_out;
        dbgP(k, A[k], tea_runs_out, running[k], running[tea_runs_out]);

        dbgBold("what if the tea never runs out?)");

        if (tea_runs_out < N) {
            ll amount_last_guy = (A[k] - (running[tea_runs_out] - running[k]));
            ll amount_missing = B[tea_runs_out] - amount_last_guy;
            out[tea_runs_out] -= amount_missing;
            dbgB(amount_last_guy, amount_missing, MP(tea_runs_out, k));
        }
        st.upd(k, tea_runs_out, 1);
        dbgY(st, out);
        el;
    }

    FOR(k, 0, N) {
        ll num_chugs = st.query(k,k);
        out[k] += num_chugs * B[k];
    }
