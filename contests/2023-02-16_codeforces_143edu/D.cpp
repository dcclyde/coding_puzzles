...
void solve() {
    ...
    // ll out = sumv(dat);
    // FOR(n, 0, N/3) {
    //     out -= *min_element(dat.begin() + 3*n, dat.begin() + 3*n+3);
    // }
    mi out = 1;
    FOR(n, 0, N/3) {
        ll small = *min_element(dat.begin() + 3*n, dat.begin() + 3*n+3);
        ll ns = 0;
        FOR(k, 3*n, 3*n+3) {ns += (dat[k] == small);}

        // ns tells how many options I have for which will be "odd one out".
        // I still need to think about which color will be majority each time.
