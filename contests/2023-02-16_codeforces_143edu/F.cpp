...
void solve() {
    ...
    lls(num_chips);
    V<ll> chip_starts;
    rv1(num_chips, chip_starts);
    dbgR(num_chips, chip_starts);
    el;
    ...
    ll ROOT = 0;
    V<ll> depth(N, 0);
    V<pll> vertices_by_depth; auto& vbd = vertices_by_depth;

    {
        auto dfs = [&](ll cur, ll par, auto&& self) -> void {
            // work on parent
            // subtree_size[curr] = 1;
            parent[cur] = par;
            if (cur != ROOT) {depth[cur] = depth[par] + 1;}
            vbd.emplace_back(depth[cur], cur);
            // todo
            ...
            for (auto& o : G[cur]) {
                ...
                // subtree_size[cur] += subtree_size[o];
                // todo
    ...
    V<ll> steps_needed(N, -1); auto& sn = steps_needed;
    V<bool> visited(N);
    V<ll> possible_steps_down(N, -1); auto& psd = possible_steps_down;

    auto binsearch_helper = [&](ll K) -> bool {
        el; el; dbgcBold("binsearch", K);
        sn.assign(N, -1);
        visited.assign(N, false);
        psd.assign(N, -1);

        FOR(j, 0, num_chips) {
            auto x = chip_starts[j];
            steps_needed[x] = (K + num_chips - j - 1) / num_chips;
            visited[x] = true;
        }
        dbg(steps_needed);
        for(auto& [deep, cur] : vbd) {
            /**
                Update psd: Need max of psd over unvisited children.
             */
            psd[cur] = 0;
            for(auto& o : G[cur]) {
                if (o == parent[cur]) {continue;}
                if (visited[o]) {continue;}
                ckmax(psd[cur], psd[o] + 1);
            }
            dbg(cur, visited[cur], psd[cur], sn[cur]);

            if (sn[cur] > psd[cur]) {
                if (cur == ROOT || visited[parent[cur]]) {return false;}
                sn[parent[cur]] = sn[cur] - 1;
    ...
    ll out = lstTrue(0, N, binsearch_helper);
    return ps(out);
}
