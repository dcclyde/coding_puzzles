...
void solve() {
    ...
    // ! Is there one vertex that just touches everybody?
    FOR(j, 0, N) {
        if (G[j].size() == N-1) {
            V<ll> out(N, j);
            out[j] = *G[j].begin();
    ...
    auto subsolve = [&]() {
        el; el; dbgcBold("subsolve", root_helper);
        dbg(pdh(G));
        // F[mask][j] = true if there's a hamiltonian walk in "mask" from 0 to j.
        auto F = ndvec<bool>(MASK_END, N, false);
        F[1][0] = true;
        FOR(mask, 3, MASK_END, 2) {  // only consider masks including 0.
            uset<ll> nearby;
            ll good_ending = -1;
            FOR(j, 0, N) {
                if ((mask & (1<<j)) == 0) {continue;}  // only consider j in the mask.
                nearby.insert(j);
                ll smaller_mask = mask ^ (1<<j);
                for(auto& k : G[j]) {
                    nearby.insert(k);
                    if (F[smaller_mask][k]) {F[mask][j] = true;}
                }
                dbgY(mask, bitset<6>(mask), j, F[mask][j]);

                if (F[mask][j] && G[j].find(0) != G[j].end()) {good_ending = j;}
            }
            if (good_ending != -1 && nearby.size() == N) {
                dbgcBold("found solution", mask, bitset<6>(mask), good_ending);
                // TODO found a solution. Now just return it.
                out.assign(N, -1);
                out[good_ending] = 0;
                ll curr = good_ending;
                ll curr_mask = mask;
                while (curr != 0) {
                    // what goes just before curr?
                    ll next_mask = curr_mask ^ (1<<curr);
                    for(auto& k : G[curr]) {
                        if (F[next_mask][k]) {out[k] = curr; curr = k; break;}
                    }
                    curr_mask = next_mask;
                }
                FOR(j, 0, N) {
    ...
    // Try all possible roots that border the loneliest vertex.
    ll lonely = 0;
    FOR(k, 1, N) {
        if (G[k].size() < G[lonely].size()) {lonely = k;}
    }
    ...
    for(auto ROOT : roots) {
        root_helper = ROOT;
        swap_vertices(0, ROOT);
        subsolve();
        if (out.size()) {
            // swap in out.
            swap(out[0], out[ROOT]);
            FOR(k, 0, N) {
                if (out[k] == 0) {out[k] = ROOT;}
                else if (out[k] == ROOT) {out[k] = 0;}
            }
            Yes;
            return pv1(out);
        }
