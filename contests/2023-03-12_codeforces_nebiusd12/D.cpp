...
pll brute_row(string& dat) {
    ll N = dat.size();
    pll out = {INF_ll, -INF_ll};

    string ph = string(N/2, 's') + string(N/4, 'd');
    sort(all(ph));
    do {
        ll curr = 0;
        ll idx = 0;
...
pll solve_row(string& dat) {
    ll N = dat.size();
    pll out;
    ll ones = 0;
    for(auto& x : dat) {ones += (x == '1');}
    {
        // Make out really SMALL.
        // put in as many double 1s as possible.

        ll double_1s = 0;
        ll idx = 0;
        while (idx < N) {
            if (idx < N-1 && dat[idx] == '1' && dat[idx+1] == '1') {
                idx += 2;
                double_1s += 1;
            } else {
                idx += 1;
        ...
        dbgY(double_1s);
        ckmin(double_1s, N / 4);
        out.f = ones - double_1s;
        dbgY(double_1s, out.f);
    }
    {
        // Make out really BIG.
        // I want lots of apts with 1 light each.
        // I want a small number of double 1s.
        ...
        dbgB(double_not1s);
        ckmin(double_not1s, N / 4);
        ll double_1s = N/4 - double_not1s;
        ckmin(double_1s, N/4);
        out.s = ones - double_1s;
        dbgB(double_not1s, double_1s, out.s);
    }
    return out;
}
