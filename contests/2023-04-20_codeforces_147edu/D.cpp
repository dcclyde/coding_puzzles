...
void brute() {
    lls(N, TARGET);
    V<pll> dat(N);
    FOR(k, 0, N) {cin >> dat[k].f;}
    FOR(k, 0, N) {cin >> dat[k].s;}
    dbgR(N, TARGET, dat);
    el;
    ...
    FOR(mask, 1, MASK_END) {
        ...
        FOR(b, 0, BITS) {
            ...
            ++segments_used;
            ckmax(right_endpoint, dat[b].s);
            score += dat[b].s - dat[b].f + 1;
            if (score >= TARGET) {
                right_endpoint -= (score - TARGET);
                score = TARGET;

                ll cost = right_endpoint + 2*segments_used;
                ckmin(out, cost);
                break;
            }
        }
...
/*
    Strategy:
    Process blocks. Track the sum. Move until we have enough.
    Add another block: Remove small blocks until we have barely enough again.
*/
...
void solve() {
    ...
    FOR(k, 0, N) {
        auto [l, r] = dat[k];
        ll len = r - l + 1;
        el; dbgY(k, MP(l,r), total, blocks);

        total += len;
        while (total >= TARGET) {
            ll len_needed = TARGET - (total - len);
            ckmax(len_needed, 0);
            ll cost = 2*blocks.size() + 2 + r - len + len_needed;
            ckmin(out, cost);

            // Can I delete a block and keep going?
            if((!blocks.empty()) && total - blocks.top() >= TARGET) {
                total -= blocks.top();
                blocks.pop();
            } else { break; }
        }
