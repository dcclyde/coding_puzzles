...
#pragma region  // wrong recurrence
void brute_wrong() {
    lls(D, N, K);
    ++K;
    dbgR(D, N, K);
    ...
    auto dp = ndvec<mi>(D+1, N+1, mi(0));
    FOR(d, 0, D+1) {dp[d][0] = 1;}

    FOR(d, K, D+1) {
        FOR(n, 1, N+1) {
            dp[d][n] = dp[d-1][n] + 2*dp[d-K][n-1];
            dbg(d, n, dp[d][n]);
        }
    }
    return ps(dp[D][N]);
...
#pragma endregion  // wrong recurrence
...
void solve() {
    ...
    FOR(k, 0, N+1) {
        ...
        ll B = k;
        ll C = N-k;
        ll ypow_sofar = B*L + C*(2*L-1);
        ll A = D - ypow_sofar;
        if (A < 0) continue;

        mi term_B = pow(mi(2), B) * binom(A+B+C, B);
        mi term_C = pow(mi(-1), C) * binom(A+C, C);
        mi curr = term_B * term_C;
        out += curr;
    }
