...
void solve() {
    ...
    FOR(step, 0, K) {
        ...
        if (best == 0) {break;}
        ll targ = best_pos - best;
        dbg(best, best_pos, targ);

        string newdat;
        FOR(k, 0, targ) {newdat.push_back(dat[k]);}
        newdat.push_back(dat[best_pos]);
        FOR(k, targ, best_pos) {newdat.push_back(dat[k]);}
        FOR(k, best_pos+1, N) {newdat.push_back(dat[k]);}
        dat = newdat;
        dbg(dat);
    }
