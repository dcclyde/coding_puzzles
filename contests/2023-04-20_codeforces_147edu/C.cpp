...
void solve() {
    ...
    ll out = N;
    FOR(c, 'a', 'z'+1) {
        // Find length of the longest chunk of consecutive letters that aren't c.
        ll best_score = 0;
        ...
        while (L < N) {
            ...
            ckmax(best_score, score);
            L = R+1;
        }
        ckmin(out, best_score);
        dbgY(char(c), best_score, out); el;
    }
    return ps(out);
}
