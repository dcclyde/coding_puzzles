...
namespace MO {
    ...
    */
    ...
    ll out[MAX_Q];  // store output
    V<t3> queries; // store left, right, index of ans

    int l = 0, r = -1;
    ll ans = 0;
    // umap<ll,ll> xor2cnt;
    // V<ll> xor2cnt(MAX_N*2+1);
    V<ll> xor2cnt(2'000'001);

    bool cmp(const t3& a, const t3& b) { // sort queries
    ...
    void del(ll v) {
        xor2cnt[v] -= 1;
        // if (xor2cnt[v] == 0) xor2cnt.erase(v);
    }
    ll get(ll v) {
        // if (xor2cnt.find(v) == xor2cnt.end()) {return 0;}
        return xor2cnt[v];
    }

    // x = index, p = +1 (add) or -1 (remove), side = 0 (left) or 1 (right)
    void modify(int x, int p, int side) {
        dbgR(x, p, side, MP(l,r));
        // xor2cnt holds counts of pref[k] for k in [l,r].
        ll v = pref[x];
        if (p == 1) {ins(v);}

        // Now update the answer too.
        ll delta = 0;
        if (side == 1) {
            // We're moving the right endpoint.
            // I need number of k in l..x such that pref[k] = TARGET ^ pref[x+1].
            delta = get(TARGET ^ pref[x+1]);
            dbg(TARGET ^ pref[x+1], delta);
        } else {
            // We're moving the left endpoint.
            // I need number of k in x..r such that pref[k+1] = TARGET ^ pref[x].
            ins(pref[r+1]); del(pref[x]);
            delta = get(TARGET ^ pref[x]);
            del(pref[r+1]); ins(pref[x]);
            dbgP(TARGET ^ pref[x], delta);
        }

        if (p == -1) {del(v);}

        ans += p*delta;
        // dbgcB("modify", x, v, p, delta, ans, xor2cnt); el;
    }

    int solve_one(int L, int R) { // modify just interval
        while (l > L) modify(--l, 1, 0);
        while (r < R) modify(++r, 1, 1);
        while (l < L) modify(l++, -1, 0);
        while (r > R) modify(r--, -1, 1);
        dbgcP("answer", L, R, ans); el;
        return ans;
    }
...
void solve() {
    ...
    FOR(qid, 0, Q) {
        ll1(L, R);
        MO::queries[qid] = {L, R, qid};
    }

    pref.resize(N+1);
    FOR(k, 1, N+1) {pref[k] = pref[k-1] ^ dat[k-1];}
