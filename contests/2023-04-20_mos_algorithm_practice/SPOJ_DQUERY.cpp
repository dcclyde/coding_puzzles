...
#pragma region  // segtree solution
/**
...
void solve_segtree() {
    ...
    ++B;
    V<pair<V<ll>, ll>> v2pos(B);
    FOR(k, 0, N) {
        v2pos[dat[k]].f.push_back(k);
    }

    umap<ll,V<pll>> queries_map;
    FOR(qid, 0, Q) {
        auto [L, R] = queries[qid];
        queries_map[R].push_back({L, qid});
    }
    ...
    FOR(R, 0, N) {
        ll v = dat[R];
        // Remove the previous time v was counted.
        auto& [vpos, vcount] = v2pos[v];
        if ( vcount > 0 ) {
            ll prevpos = vpos[vcount-1];
            st.upd(prevpos, 0);
        }
        st.upd(R, 1);
        ++vcount;
        for(auto& [L, qid] : queries_map[R]) {
            ll ans = st.query(L, R);
            out[qid] = ans;
        }
    }
