...
void solve() {
    ...
    V<pll> queries;
    rv(Q, queries);
    umap<ll, V<pll>> queries_map;
    FOR(qid, 0, Q) {
        auto& [v, k] = queries[qid];
        --v;
        queries_map[v].push_back({k, qid});
    }
    V<ll> out(Q, -1);

    V<indexed_set<pll>> v2cnt2color(N);  // important for extracting answers
    V<umap<ll,ll>> v2color2cnt(N);  // helper for updating v2cnt2color

    // V<ll> subtree_size(N);
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        v2color2cnt[curr][A[curr]] = 1;
        v2cnt2color[curr].insert({1, A[curr]});
        ...
        for (auto& o : G[curr]) {
            ...
            if (v2color2cnt[o].size() > v2color2cnt[curr].size()) {
                swap(v2color2cnt[o], v2color2cnt[curr]);
                swap(v2cnt2color[o], v2cnt2color[curr]);
            }
        }
        // update parent after all children
        for(auto& o : G[curr]) {
            if (o == parent) {continue;}
            for (auto& [color, cnt] : v2color2cnt[o]) {
                auto& curr_cnt = v2color2cnt[curr][color];
                v2cnt2color[curr].erase({curr_cnt, color});
                curr_cnt += cnt;
                v2cnt2color[curr].insert({curr_cnt, color});
            }
        }
        dbgY(curr, v2color2cnt[curr], v2cnt2color[curr]); el;

        // Answer queries in curr.
        for (auto& [k, qid] : queries_map[curr]) {
            // How many colors have cnt >= k?
            out[qid] = v2cnt2color[curr].size() - v2cnt2color[curr].order_of_key({k, -1});
        }
