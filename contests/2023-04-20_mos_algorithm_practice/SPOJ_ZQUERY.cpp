...
struct Mo {
    ...
    void init(ll N_, ll Q_) {
        ...
        l = 0; r = -1;
        ans = 0;
        ll NUM_BLOCKS = N / BLOCK + 1;
        scores_per_block.resize(NUM_BLOCKS);
        scores.resize(BLOCK * NUM_BLOCKS);
        adjocc.assign(N, {-1, -1});
    }
    ...
    void upd_score(ll v, ll inc) {
        scores[v] += inc;
        ll block = v / BLOCK;
        scores_per_block[block] += inc;
    }
    ll big_score() {
        ROF(block, 0, scores_per_block.size()) {
            if (scores_per_block[block] > 0) {
                ll start = block * BLOCK;
                ROF(v, start, start + BLOCK - 1) {
                    if (scores[v] > 0) {
                        return v;
                    }
                }
    ...
    void print(ll idx, ll dir) {
        dbgcY("modify", MP(l,r), idx, A[idx], dir);
        dbg(long_span);
        dbgP(scores_per_block, scores);
        el;
    }
    ...
    void solve_one(ll L, ll R) {
        while (l > L) {
            --l;
            // modify(l, 1);
            ll future = adjocc[l].s;
            if (future != -1 && future <= r) {
                long_span[l].s = long_span[future].s + (future - l);
                upd_score(long_span[l].s, 1);
                upd_score(long_span[future].s, -1);
                long_span[future].s = -1;
            } else {
                long_span[l].s = 0;
                upd_score(0, 1);
            }
        ...
        while (r < R) {
            ...
            ll past = adjocc[r].f;
            if (past != -1 && past >= l) {
                long_span[r].f = long_span[past].f + (r - past);
                upd_score(long_span[r].f, 1);
                upd_score(long_span[past].f, -1);
                long_span[past].f = -1;
            } else {
                long_span[r].f = 0;
                upd_score(0, 1);
        ...
        while (l < L) {
            ...
            ll future = adjocc[l].s;
            if (future != -1 && future <= r) {
                long_span[future].s = long_span[l].s - (future - l);
                upd_score(long_span[future].s, 1);
                upd_score(long_span[l].s, -1);
                long_span[l].s = -1;
            } else {
        ...
        while (r > R) {
            ...
            ll past = adjocc[r].f;
            if (past != -1 && past >= l) {
                long_span[past].f = long_span[r].f - (r - past);
                upd_score(long_span[past].f, 1);
                upd_score(long_span[r].f, -1);
                long_span[r].f = -1;
            } else {
    ...
    void solve() {
        ...
        for (auto& [L, R, qid] : queries) {
            el; dbgBold(qid, MP(L,R));
            solve_one(L, R);
            out[qid] = ans;
            dbgR(qid, MP(L,R), ans); el;
        }
    }
};
...
void solve() {
    ...
    V<ll> dat;
    rv(N, dat);
    V<ll> pref(N+1);  // pref[k] = sum from 0 to k-1.
    FOR(k, 0, N) {pref[k+1] = pref[k] + dat[k];}
