...
struct Mo {
    ...
    void modify(ll idx, ll dir) {
        // ! Update state when we add/remove idx.
        ll v = A[idx];
        cnt_and_good[v].f += dir;
        if (dir == 1 && cnt_and_good[v].f == 1) {
            ++num_distinct;
            ++num_good;
            cnt_and_good[v].s = true;
        } else if (dir == -1 && cnt_and_good[v].f == 0) {
            --num_distinct;
            --num_good;
            cnt_and_good[v].s = false;
        } else {
            // update "good".
            ll u = upper_bound(all(v2occ[v]), r) - v2occ[v].begin();
            --u;
            if (u < 0) {dbgP(u);}
            else {dbgP(u, aseq_len[v2occ[v][u]], cnt_and_good[v]);}

            if (u < 0 || aseq_len[v2occ[v][u]] < cnt_and_good[v].f) {
                if (cnt_and_good[v].s) { --num_good; }
                cnt_and_good[v].s = false;
            } else {
                if (!cnt_and_good[v].s) { ++num_good; }
                cnt_and_good[v].s = true;
        ...
        ans = num_distinct + (num_good == 0);
        dbgcY("modify", MP(l,r), idx, dir);
        dbgB(num_distinct, num_good, ans);
        dbg(cnt_and_good);
        el;
    }
    ...
    void solve() {
        ll VMAX = *max_element(all(A)); ++VMAX;
        v2occ.assign(VMAX, V<ll>());
        FOR(k, 0, N) {v2occ[A[k]].pb(k);}
        cnt_and_good.resize(VMAX);

        aseq_len.assign(N, 1);
        FOR(k, 0, N) {
            ll ki = lower_bound(all(v2occ[A[k]]), k) - v2occ[A[k]].begin();
            if (ki <= 1) {aseq_len[k] = ki+1; continue;}
            if (v2occ[A[k]][ki] - v2occ[A[k]][ki-1] == v2occ[A[k]][ki-1] - v2occ[A[k]][ki-2]) {
                aseq_len[k] = aseq_len[v2occ[A[k]][ki-1]] + 1;
            } else {
                aseq_len[k] = 2;
            }
        }
