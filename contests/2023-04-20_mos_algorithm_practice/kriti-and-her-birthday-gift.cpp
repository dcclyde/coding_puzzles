...
H hash_string(const string& S) {
    H out {};
    for(auto& c : S) { out = out * base + makeH(c); }
    return out;
}
...
struct Mo {
    ...
    void solve() {
        ll SMAX = -1;
        for(auto& [L, R, S, qid] : queries) {
            ckmax(SMAX, S);
        }
        ++SMAX;
        cnt.assign(SMAX, 0);

        auto cmp = [&](const t4& a, const t4& b) -> bool {
            const auto& [la, ra, sa, qa] = a;
            const auto& [lb, rb, sb, qb] = b;
...
void solve() {
    ...
    lls(Q);
    V<tuple<ll,ll,string>> queries;
    rv(Q, queries);
    for(auto& [L, R, S] : queries) { --L; --R; }

    // Compress the strings into nice ints.
    V<string> uncompressed;
    FOR(k, 0, N) { uncompressed.push_back(dat[k]); }
    FOR(qid, 0, Q) { uncompressed.push_back(get<2>(queries[qid]));}
    remDup(uncompressed);
    umap<string, ll> uncinv;
    ...
    Mo mo;
    mo.init(N, Q);
    FOR(k, 0, N) { mo.A[k] = uncinv[dat[k]]; }
    FOR(qid, 0, Q) {
        auto& [L, R, S] = queries[qid];
        mo.queries[qid] = MT(L, R, uncinv[S], qid);
    }
