...
void solve() {
    ...
    FOR(dr, 1, R+1) {
        FOR(dc, 1, C+1) {
            // How many ways to choose a rectangle of size dr x dc?
            ll num_rects = (R-dr+1)*(C-dc+1);

            // How many ways to achieve each of these?
            mi ways = G(dr, dc);

            mi prob = ways * num_rects / F(R, C);
            out += prob * (dr * dc);
            dbg(MP(dr,dc), num_rects, ways, prob, out); el;
        }
    }
