...
V<pll> compress(auto& X) {
    V<pll> ret;
    ll last = -INF_ll;
    ll cnt = 0;
    for (auto& x : X) {
        if (x != last) {
            if (cnt > 0) {ret.push_back({last, cnt});}
            last = x;
            cnt = 1;
        } else {
            cnt++;
...
void learn() {
    ...
    // ll L = 6; ll R = 8;  // (0, 6) (1, 6) (2, 2)
    // ll L = 5; ll R = 8;  // (0, 5) (1, 5) (2, 3)
    // ll L = 3; ll R = 4;
    ...
    FOR(n, 0, MAX+1) {
        ...
        while (seen.find(G[n]) != seen.end()) {G[n]++;}
    }
    dbg(pdh(G));
    dbgY(compress(G));
    return;
