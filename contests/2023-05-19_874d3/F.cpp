...
void solve() {
    ...
    for(auto& x : A) {++cnts[x];}
    V<pll> counted;
    for(auto& [v, c] : cnts) {counted.eb(v, c);}
    dbgB(counted);
    if (counted.size() < NUM_USED) {return ps(0);}

    mi out = 0;
    mi running = 1;  // running product of last D items.
    FOR(k, 0, NUM_USED-1) { running *= counted[k].second; }
    FOR(k, NUM_USED-1, counted.size()) {
        running *= counted[k].second;
        el; dbgY(k, running);
        if (counted[k-NUM_USED+1].f == counted[k].f - (NUM_USED-1)) {
            out += running;
            dbgR(out);
        }
        running /= counted[k-NUM_USED+1].second;
    }
