...
void solve() {
    ...
    dbgB(A);
    /*
        We get to reverse any prefix and any suffix. They can't overlap though.
        Choose the prefix to put N at the start.
        Choose the suffix to put the end as early as it should go.
    */
    ll N_position = max_element(A.begin(), A.end()-1) - A.begin();
    if (N_position == 0) {N_position = -1;}
    reverse(A.begin(), A.begin() + 1 + N_position);
    dbgY(A);

    ll last = A.back();
    FOR(suffix_start, N_position + 2, N) {
        if (A[suffix_start] < last) {
            dbgR(suffix_start);
            reverse(A.begin() + suffix_start, A.end());
            break;
        }
    }
