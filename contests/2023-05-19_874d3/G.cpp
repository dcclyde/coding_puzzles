...
void solve() {
    ...
    if (N % 3 != 0) {dbgcP("fail"); dbgcP("fail"); return ps(-1);}

    V<ll> depth(N, -1);
    V<ll> par(N, -1);
    V<uset<ll>> children(N);
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        if (parent == -1) {depth[curr] = 0;}
        else {depth[curr] = depth[parent] + 1;}
        par[curr] = parent;
    ...
    /*
        I want to repeatedly pull out deepest item.
        Then I'll delete / possibly adjust depths of some items.
    */

    V<bool> used(N, false);
    V<pll> depth_order_helper;
    FOR(k, 0, N) {depth_order_helper.emplace_back(-depth[k], k);}
    sort(all(depth_order_helper));
    dbgB(depth_order_helper); el;

    uset<ll> out;
    for(auto& [dummy, v] : depth_order_helper) {
        if (used[v]) {continue;}
        ll p = par[v];
        if (used[p]) {dbgcP("fail"); return ps(-1);}
        // dbg(v, p, children[p]);
        if (children[p].size() > 2) {dbgcP("fail"); return ps(-1);}
        if (children[p].size() == 2) {
            for(auto& o : children[p]) {
                if (o == v) {continue;}
                used[o] = true;
            }
            used[v] = true;
            used[p] = true;
            if (p != ROOT) {
                out.insert(edge_to_id[{p, par[p]}]);
                dbgcR("del edge", MP(p, par[p]));
                children[par[p]].erase(p);
            }
        } else {
            // parent has no other children.
            ll pp = par[p];
            if (used[pp]) {dbgcP("fail"); return ps(-1);}
            used[v] = true;
            used[p] = true;
            used[pp] = true;
            if (pp != ROOT) {
                out.insert(edge_to_id[{pp, par[pp]}]);
                dbgcR("del edge", MP(pp, par[pp]));
                children[par[pp]].erase(pp);
            }
            V<ll> to_delete;
            for(auto& o : children[pp]) {
                if (o == p) {continue;}
                out.insert(edge_to_id[{pp, o}]);
                dbgcR("del edge", MP(pp, o));
                to_delete.emplace_back(o);
            }
            for(auto& o : to_delete) { children[pp].erase(o); }
        }
    }
