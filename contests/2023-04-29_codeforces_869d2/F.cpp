...
void solve() {
    ...
    ll total = N-2;
    ll right = (N-3) / 2;
    ll topleft = cdiv(total - right, 2);
    if (topleft * 2 + right == total) {++topleft;}
    ckmin(topleft, N-2-K+2);
    ll botleft = total - topleft - right;
    dbgY(total, right, topleft, botleft);

    if (topleft > botleft) {
        out += "URDRUL";
        rep(botleft) {out += "DRUL";}
        return ps(out);
    }

    out += "UR";
    rep(botleft+1) {out += "DRUR";}
    out += "L";
    rep(2*botleft) {out += "DRUL";}
    return ps(out);
}
