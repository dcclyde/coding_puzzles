...
def solve(testID):
    N, Q = lm()
    dat = lm()
    queries = [lm() for _ in range(Q)]
    dbg(N, Q, dat, queries)

    running_subsequence = [0, 1, 2]
    for i in range(2, N):
        if dat[i] <= dat[i - 1] and dat[i - 1] <= dat[i - 2]:
            running_subsequence.append(running_subsequence[-1])
        else:
            running_subsequence.append(running_subsequence[-1] + 1)

    dbgG(running_subsequence)
    next_increase = [-1 for _ in range(N)]
    for i in range(N - 2, -1, -1):
        if dat[i] < dat[i + 1]:
            next_increase[i] = i + 1
        else:
            next_increase[i] = next_increase[i + 1]
    ...
    for l, r in queries:
        l -= 1
        r -= 1
        next_increase_from_l = next_increase[l]
        if next_increase_from_l == -1 or next_increase_from_l > r:
            print(min(2, r - l + 1))
            continue
        end_amount = running_subsequence[r + 1] - running_subsequence[next_increase_from_l]
        print(min(2, next_increase_from_l - l) + end_amount)
