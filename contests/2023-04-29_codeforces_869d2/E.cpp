...
void solve() {
    ...
    // Find the top two coefficients given values.
    auto subsolve = [&](auto& P) -> pmi {
        pmi out;
        auto& [xd, xdm1] = out;
        FOR(k, 0, D+1) {
            mi denom = fac[k] * fac[D-k];
            if ((D-k) % 2 == 1) {denom = -denom;}
            mi curr = P[k] / denom;
            xd += curr;
            xdm1 += curr * (k - binom(D+1, 2));
        }
        return out;
    };

    auto [ad, adm1] = subsolve(A);
    auto [bd, bdm1] = subsolve(B);

    mi out = (bdm1 - adm1) / (D * ad);
    return ps(out);
}
