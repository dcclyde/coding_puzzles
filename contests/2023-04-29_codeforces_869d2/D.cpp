...
void solve() {
    ...
    V<ll> parent(N);
    FOR(ROOT, 0, N) {
        if (G[ROOT].size() < 4) {continue;}
        // Look for a cycle starting at ROOT.
        for(auto& s : G[ROOT]) {
            // Run a dfs from here, but not allowed to step from s to ROOT.
            // If we find a cycle, then we're done.
            parent.assign(N, -1);
            parent[s] = ROOT;
            auto dfs = [&](ll curr, auto&& self) -> void {
                // work on parent

                for (auto& o : G[curr]) {
                    if (curr == s && o == ROOT) {continue;}
                    if (parent[o] != -1) {continue;}
                    parent[o] = curr;
                    self(o, self);
            ...
            // Found a cycle.
            V<ll> cycle = {ROOT};
            while (cycle.back() != s) {cycle.push_back(parent[cycle.back()]);}
            dbgY(cycle);

            ll ai = 0;
            while (G[ROOT][ai] == cycle[1] || G[ROOT][ai] == cycle.back()) {++ai;}
            ll bi = ai+1;
            while (G[ROOT][bi] == cycle[1] || G[ROOT][bi] == cycle.back()) {++bi;}
            ll a = G[ROOT][ai];
            ll b = G[ROOT][bi];

            // Are a and/or b contained in the cycle?
            FOR(k, 0, cycle.size()) {
                if (cycle[k] == a || cycle[k] == b) {
                    if (cycle[k] == a) {swap(a, b);}
                    cycle.resize(k+1);
                    // Find a new b.
                    bi = 0;
                    while (G[ROOT][bi] == cycle[1] || G[ROOT][bi] == cycle.back() || G[ROOT][bi] == a) {++bi;}
                    b = G[ROOT][bi];
                    break;
                }
            }

            // OK, we have a good cycle. Just output.
            YES;
            ps(cycle.size() + 2);
            ps1(ROOT, a);
            ps1(ROOT, b);
            FOR(k, 0, cycle.size()) {
                ps1(cycle[k], cycle[(k+1)%cycle.size()]);
            }
            return;
        }
    }
