...
V<pdd> ask(db a, db b, db c) {
    ps("?", a, b, c); cout << flush;
    vecs(pdd, N, out);
    return out;
}
void finish(V<pdd>& ans) {
    cout << "! ";
    pv(ans);
    cout << flush;
    return;
...
void solve() {
    ...
    /*
        Project only x = 0.
        In other words, give me the y coordinates.
    */
    auto y_coords_pairs = ask(1, 0, 0);
    V<db> y_coords;
    for(auto& [x, y] : y_coords_pairs) {
        assert(abs(x) < 1e-3);
    ...
    /*
        Project onto x = y/1000.
            y = 1000x.

            x_out = (x + 250y) / 62501.
            y_out = (250x + 62500y) / 62501.
    */
    auto dat_pairs = ask(25., -0.1, 0);
    V<db> dat_y;
    for(auto& [x, y] : dat_pairs) {
        dat_y.push_back(y);
    }
    sort(all(dat_y));
