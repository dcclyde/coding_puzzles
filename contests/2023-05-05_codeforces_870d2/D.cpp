...
void solve() {
    ...
    SegTree<ll> apk, amk;
    apk.init(N); amk.init(N);
    FOR(k, 0, N) {
        apk.seg[apk.n+k] = A[k] + k;
        amk.seg[amk.n+k] = A[k] - k;
    }
    apk.build(); amk.build();

    ll out = 0;
    FOR(middle, 1, N-1) {
        ll lo = apk.query(0, middle-1);
        ll hi = amk.query(middle+1, N-1);
        /*
            Say I have 10 2 5 2 8 with mid = 2.
            apk[0] = 10
            amk[4] = 4
        */
        ll score = lo + hi + A[middle];
        ckmax(out, score);
    }
