...
void solve() {
    ...
    ll out = max_depth * W;
    if (C >= W) {return ps(out);}

    ll distant_vertex = max_element(all(depth)) - depth.begin();
    auto depth_from_0 = depth;
    dfs(distant_vertex, -1, dfs);
    FOR(k, 0, N) {
        ll score_here = W*depth[k] - C*depth_from_0[k];
        ckmax(out, score_here);
    }
    return ps(out);
}
