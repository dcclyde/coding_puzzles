...
void solve() {
    ...
    if (N % 2 == 1) {return ps(-1);}

    // watch out: what if the most common char occurs in more than half the positions?
    map<char, ll> freq;
    for(auto& c : dat) {freq[c]++;}
    for(auto& [c, f] : freq) {
        if (f > N/2) {return ps(-1);}
    }

    map<char,ll> bad_pairs;
    FOR(k, 0, N/2) {
        if (dat[k] == dat[N-k-1]) {
            ++bad_pairs[dat[k]];
        }
    ...
    dbg(bad_pairs); el;
    priority_queue<ll> bp_counts;
    for(auto& [c, f] : bad_pairs) {bp_counts.push(f);}

    ll out = 0;
    while (!bp_counts.empty()) {
        if (bp_counts.size() == 1) {out += bp_counts.top(); break;}
        auto a = bp_counts.top(); bp_counts.pop();
        auto b = bp_counts.top(); bp_counts.pop();
        --a; --b;
        if (a > 0) {bp_counts.push(a);}
        if (b > 0) {bp_counts.push(b);}
        ++out;
    }
