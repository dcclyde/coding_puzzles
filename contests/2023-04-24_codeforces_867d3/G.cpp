...
void solve_old() {
    ...
    map<ll,ll> cnt;
    for(auto& x : dat) {++cnt[x];}

    ll out = 0;
    // handle the self-pairs separately.
    for(auto& [x, c] : cnt) { out += c*(c-1)*(c-2); }
    dbg(out);

    for(auto& [x, c] : cnt) {
        ll mrt = 2;
        ll z = x * mrt * mrt;
        // Find a "square multiple" of x that's also in our set.
        while (true) {
            if (z > B) {break;}

            ll fut = cnt.lower_bound(z)->f;
            if (fut == z) {
                if (cnt.count(x*mrt)) {
                    ll score_here = cnt[x] * cnt[x*mrt] * cnt[x*mrt*mrt];
                    out += score_here;
                }
            }

            // Move up to a "square multiple" of x.
            ll mrt_new = fstTrue(mrt+1, R_MAX+1, [&](ll q) {
                return x * q * q >= z;
            });
            mrt = mrt_new;
            z = x * mrt * mrt;
...
void solve() {
    ...
    for(auto& x : dat) {++cnt[x];}
    V<pll> cntvec;
    for(auto& [x, c] : cnt) {cntvec.emplace_back(x,c);}
    sort(all(cntvec));
    ...
    ll R_CUTOFF = rootll(B, 3);
    FOR(R, 2, R_CUTOFF+1) {
        ll Rsq = R*R;
        for(auto& [z, c] : cntvec) {
            if (z % Rsq != 0) {continue;}
            if (cnt.find(z/Rsq) != cnt.end() && cnt.find(z/R) != cnt.end()) {
                ll score_here = cnt[z] * cnt[z/R] * cnt[z/Rsq];
                out += score_here;
            }
        }
    }

    // now handle cases with R big.
    /*
        R > R_CUTOFF
        X * R^2 <= B
        X <= B / R^2 < B / R_CUTOFF^2

        X * R_CUTOFF^2 < B
    */
    ll LAST_X = (B-1) / (R_CUTOFF * R_CUTOFF);
    for(auto& [y, cy] : cntvec) {
        for(auto& [x, cx] : cntvec) {
            if (x > LAST_X) {break;}
            if (y % x != 0) {continue;}
            if (y / x <= R_CUTOFF) {continue;}
            ll z = y*y/x;
            if (cnt.find(z) == cnt.end()) {continue;}
            ll score_here = cx * cy * cnt[z];
            out += score_here;
        }
    }
...
void solve() {
    ...
    for(auto& [z, cz] : cntvec) {
        // iterate over perfect squares that divide z.
        auto facs = factor(z);
        V<pair<ul,int>> facs_sq;
        for(auto& [p, e] : facs) {
            if (e == 1) {continue;}
            else {facs_sq.emplace_back(p, e/2);}
        }
        V<ul> divisors = {1};
        for(auto& [p, e] : facs_sq) {
            int sz = divisors.size(); ul ppow = p;
            rep(e) {
                FOR(k, 0, sz) { divisors.push_back(divisors[k] * ppow); }
                ppow *= p;
            }
        }
        FOR(k, 1, divisors.size()) {
            ll R = divisors[k];
            if (cnt.find(z/R) != cnt.end() && cnt.find(z/R/R) != cnt.end()) {
                dbg_only(ll x = z/R/R; ll y = z/R; dbg(x, y, z);)
                ll score_here = cz * cnt[z/R] * cnt[z/R/R];
                out += score_here;
            }
        }
    }
