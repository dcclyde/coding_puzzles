...
void solve() {
    ...
    lls(start_A, start_B);
    lls(MAX_A, MAX_B);
    dbgR(N, MP(start_A, start_B), MP(MAX_A, MAX_B));
    el;

    ll needed_A = N - start_A;
    ll needed_B = N - start_B;
    dbg(needed_A, needed_B);

    auto dp = ndvec<mi>(needed_A+1, needed_B+1, mi(0));
    // Base case: if a is 0, then we win.
    FOR(b, 0, needed_B+1) {dp[0][b] = 1;}

    mi base = mi(1) / MAX_A / MAX_B;
    dbgR(base); el;

    FOR(a, 1, needed_A+1) {
        FOR(b, 1, needed_B+1) {
            FOR(ra, 1, MAX_A+1) {  // roll a
                FOR(rb, 1, MAX_B+1) {  // roll b
                    ll new_a = maxll(a-ra, 0);
                    ll new_b = maxll(b-rb, 0);
                    dp[a][b] += dp[new_a][new_b];
                }
            }
            dp[a][b] *= base;
            dbgY(MP(a,b), dp[a][b]);
    ...
    return ps(dp[needed_A][needed_B]);
}
