...
void brute() {
    ...
    ll rmin = INF_ll; ll rmax = -INF_ll;
    ll cmin = INF_ll; ll cmax = -INF_ll;
    for(auto& [r, c, v] : dat) {
        ckmin(rmin, r);
        ckmax(rmax, r);
        ckmin(cmin, c);
...
void solve() {
    ...
    if (N == 1) { return ps(get<2>(dat[0])); }

    umap<pll,ll> marked;
    umap<ll,ll> row_sums, col_sums;
    for(auto& [r, c, v] : dat) {
        row_sums[r] += v;
        col_sums[c] += v;
        marked[MP(r,c)] = v;
    ...
    V<pll> row_sums_ordered, col_sums_ordered;
    for(auto& [r, s] : row_sums) {row_sums_ordered.emplace_back(-s, r);}
    for(auto& [c, s] : col_sums) {col_sums_ordered.emplace_back(-s, c);}
    sort(all(row_sums_ordered));
    sort(all(col_sums_ordered));
    dbg(row_sums_ordered);
    dbg(col_sums_ordered);

    // Now start iterating through large (row sum, col sum) combinations.
    queue<pll> todo;
    todo.emplace(0, 0);
    ll ctr = 0;
    uset<pll> seen;
    ...
    while (ctr <= N+1 && !todo.empty()) {
        ...
        auto [sr, r] = row_sums_ordered[ri];
        auto [sc, c] = col_sums_ordered[ci];

        ll score = -sr - sc;
        if (marked.find(MP(r,c)) != marked.end()) {
            score -= marked[MP(r,c)];
        }
        ckmax(out, score);

        // insert children into todo.
        if (ri+1 < row_sums_ordered.size()) { todo.emplace(ri+1, ci); }
        if (ci+1 < col_sums_ordered.size()) { todo.emplace(ri, ci+1); }
    }
