...
void solve() {
    ...
    ll out = 0;
    V<V<ll>> good_divisors(N+1);
    FOR(k, 1, N+1) {
        if (dat[k-1] == '1') {continue;}
        good_divisors[k].push_back(k);
        sort(all(good_divisors[k]));
        dbgR(k, good_divisors[k]);

        out += good_divisors[k].front();
        for(auto& d : good_divisors[k]) {
            if (k+d > N) {break;}
            good_divisors[k+d].push_back(d);
        }
    }
