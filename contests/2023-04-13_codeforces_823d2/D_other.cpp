...
void solve() {
    ...
    ll right_power = dat[k-1];
    ll left_power = dat[k-1];
    ...
    while(true){
        dbg(left, right)
        dbg(left_power, right_power)
        old_left  = left;
        old_right = right;

        // Try going right
        ll cur_power = right_power + right_weight;
        while(right + 1 < N  && cur_power + dat[right+1] >=0){
            cur_power += dat[right+1];
            right_weight += dat[right+1];
            left_power = max(left_power, dat[k-1] + right_weight);
            right++;
        }

        // Try going left
        cur_power = left_power + left_weight;
        while(left >0 && cur_power + dat[left-1] >=0){
            cur_power += dat[left-1];
            left_weight += dat[left-1];
            right_power = max(right_power, dat[k-1] + left_weight);
            left--;
        }
        if (old_left == left && old_right == right) break;
    }
