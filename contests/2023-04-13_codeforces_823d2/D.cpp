...
void brute() {
    ...
    V<ll> todo[2];
    FOR(k, K+1, N) {todo[0].push_back(dat[k]);}
    ROF(k, 0, K) {todo[1].push_back(dat[k]);}

    ll s0 = todo[0].size();
    ll s1 = todo[1].size();

    string helper = string(s0, 0) + string(s1, 1);
    sort(all(helper));
    ...
    auto check = [&]() -> bool {
        ...
        V<ll> pos = {0, 0};
        for (auto& c : helper) {
            running += todo[c][pos[c]];
            if (running < 0) {return false;}
            ++pos[c];
            if (pos[c] == todo[c].size()) {return true;}
        }
        return true;
    };
    ...
    do {
        if (check()) {return YES;}
    } while (next_permutation(all(helper)));
    return NO;
}
...
void solve() {
    ...
    auto helper = [&](auto& wip, ll& extra, ll start, ll end, ll step) {
        ll running = 0;
        ll min_so_far = 0;
        ll max_so_far = 0;
        ll sum_deltas = 0;
        ll last_used = start - step;
        wip.emplace_back(0, 0);
        for (ll k = start; k != end; k += step) {
            running += dat[k];
            ckmin(min_so_far, running);
            if (ckmax(max_so_far, running)) {
                wip.emplace_back(-min_so_far, running - sum_deltas);
                sum_deltas += wip.back().s;
                last_used = k;
            }
            dbgP(k, dat[k], running, sum_deltas, "", min_so_far, max_so_far, wip);
        }
        // What's the lowest running sum we hit travelling from last_used+1 till the end?
        extra = 0;
        ll running2 = 0;
        for (ll k = last_used+step; k != end; k += step) {
            running2 += dat[k];
            ckmin(extra, running2);
        }
        extra = -extra;
    };
    helper(options[0], leftover[0], K+1, N, 1);
    dbgY(leftover[0], options[0]);
    el;
    helper(options[1], leftover[1], K-1, -1, -1);
    dbgB(leftover[1], options[1]);

    ll score = dat[K];
    ll L = 0; ll R = 0;
    while (true) {
        dbg(MP(L,R), score);
        // Can I safely move left?
        if (L == options[0].size()) {
            if (score - leftover[0] >= 0) {
                return YES;
            }
        } else if (options[0][L].f <= score) {
            score += options[0][L].s;
            ++L;
            continue;
        }

        // Can I safely move right?
        if (R == options[1].size()) {
            if (score - leftover[1] >= 0) {
                return YES;
            }
        } else if (options[1][R].f <= score) {
            score += options[1][R].s;
            ++R;
            continue;
        }
