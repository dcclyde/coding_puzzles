...
V<mi> fact_helper = {1};
mi fact(ll n) {
    while (fact_helper.size() <= n) {
        fact_helper.push_back(fact_helper.back() * fact_helper.size());
    }
...
void solve() {
    ...
    // dp[n] = num blue-dominant solutions of size n.
    V<mi> dp(N+1);
    dp[1] = 1;

    FOR(n, 2, N+1) {
        FOR(k, 1, n) {
            // red-dominant solution of size k
            mi red_k = dp[k];

            // any solution of size n-k
            mi any_nmk = dp[n - k];
            if (n-k > 1) {any_nmk *= 2;}

            // ways to choose which k elements we'll pick
            mi choose = bin(n-1, k-1);

            mi curr = red_k * any_nmk * choose;
            dp[n] += curr;
            dbg(n, k, MT(red_k, any_nmk, choose), curr);
        }
        dbgY(n, dp[n]); el;
    }
