...
// string tsdbg(uint64_t& x) {return to_string(x);}
...
V<ul> divisors_from_factors(auto&& cnt) {
	V<ul> out = {1};
	for(auto& [p, e] : cnt) {
...
V<ul> divisors(ul n) {
	map<ul,int> cnt; factor_rec(n,cnt);
    return divisors_from_factors(cnt);
}
ll totient(ul n) {
    map<ul,int> cnt; factor_rec(n,cnt);
    ll out = n;
...
void solve() {
    ...
    // factor M.
    map<ul,int> facs_map;
    factor_rec(M1, facs_map); factor_rec(M2, facs_map);
    dbg(facs_map);
    V<pair<ul,int>> facs;
    for(auto& [p, e] : facs_map) {facs.emplace_back(p, e);}

    // auto divs = divisors_from_factors(facs);

    // V<ll> dp(divs.size());
    umap<ll,ll> dp;
    dp[1] = 1;

    uset<ll> distinct_prime_factors;
    auto& dpf = distinct_prime_factors;
    auto rec = [&](auto&& self, ll f, ll curr) {
        if (f == facs.size()) {
            // ! work here
            // if (curr == 9) {dbgBold(curr);}
            if (curr <= N) {
                dp[curr] = curr;
                dbgW(curr, dp[curr]);
            } else {
                for(auto& p : dpf) {
                    dp[curr] = maxll(dp[curr], dp[curr/p]);
                    // ckmax(dp[curr], dp[curr / p]);
                }
                dbgB(curr, dp[curr]);
            }
            return;
        ...
        // try using it at different levels.
        auto& [p, e] = facs[f];
        dpf.insert(p);
        FOR(k, 1, e+1) {
            curr *= p;
