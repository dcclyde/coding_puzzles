...
ll nums2ll(auto& nums) {
    ll out = 0;
    for(auto& x : nums) {out *= 10; out += x;}
    return out;
}
...
void solve() {
    lls(N, L);
    V<V<ll>> dat(N);
    for(auto& dk : dat) {rv(L, dk); for(auto& x : dk) {--x;}}
    dbgR(N, pdh(dat));
    el;

    V<uset<ll>> hashes(L+1);
    for(auto& dk : dat) {
        ll curr = 0;
        V<ll> dkinv(L);
        FOR(k, 0, L) {dkinv[dk[k]] = k;}
        dbg(dk, dkinv);
        FOR(k, 0, L) {
            curr *= 10; curr += dkinv[k];
            hashes[k+1].insert(curr);
        }
    }
    dbgP(pdh(hashes));

    // now start looking things up I guess...
    V<ll> out(N);
    FOR(n, 0, N) {
        auto& dk = dat[n];
        ll curr = 0;
        FOR(k, 0, L) {
            curr *= 10; curr += dk[k];
            if (hashes[k+1].find(curr) == hashes[k+1].end()) {break;}
            ++out[n];
        }
        dbgY(n, dk, out[n]); el;
    }
