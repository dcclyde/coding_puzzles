...
void solve() {
    ...
    ll leftover = maxll(A, B) - m + O;

    // current score is AB. Then we lose 1 point per joke until we get to -1.
    // can't tell more than leftover jokes in any case.

    ll phase_2_jokes = min(leftover, AB + 1);
    out += phase_2_jokes;
