...
void solve() {
    ...
    FOR(cl, 0, N-1) {
        FOR(cr, cl+1, N) {
        // ll cr = cl + 1;
            el; dbgY(MP(cl,cr), MP(dat[cl],dat[cr]));
            // how many ways to make these dots move toward each other?
            ll l = lstTrue(0, cl-1, [&](ll x) {
                return dat[cr] - dat[cl] < dat[cl] - dat[x];
            });
            /**
                Need to omit (l, cl).
                OK to include any combination of [0, l].
            */
            mi lpart = pow(mi(2), l+1);

            ll r = fstTrue(cr+1, N-1, [&](ll x) {
                return dat[x] - dat[cr] >= dat[cr] - dat[cl];
            });
            /**
                Need to omit (cr, r).
                OK to include any combination of [r, N-1].
            */
            mi rpart = pow(mi(2), N-r);

            mi local = lpart * rpart;
            out += local;
            dbg(MP(cl,cr), local, out);
        }
    }
