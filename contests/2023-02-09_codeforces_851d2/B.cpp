...
void solve() {
    ...
    while(N > 0) {
        ll d = N % 10; N /= 10;
        digs[0].push_back(d/2); digs[1].push_back(d/2);
        // out[0] *= 10; out[1] *= 10;
        // out[0] += d/2; out[1] += d/2;
        if (d % 2 == 1) {
            ++digs[delta].back();
    ...
    dbgY(pdh(digs));
    // dbgR(out);
    // reverse(all(digs[0])); reverse(all(digs[1]));

    auto build = [&](auto& digits) -> ll {
        ll x = 0;
        ROF(k, 0, digits.size()) { x *= 10; x += digits[k]; }
        return x;
    };
    return ps(build(digs[0]), build(digs[1]));
}
