...
void solve() {
    ...
    mi out = 0;
    FOR(center, 0, N) {
        el; dbgY(center, dat[center]);
        // how many ways to make this dot move right?
        mi local = 0;
        ROF(l, 0, center) {
            // what if dot l is the closest available left dot?
            // I'll need a pretty close r... how close?
            ll rmax = lstTrue(center, N-1, [&](ll x) {
                return dat[x]-dat[center] < dat[center]-dat[l];
            });
            if (rmax == center) {break;}
            /**
                center needs to exist
                l needs to exist
                (l, center) should not exist
                need at least one element from (center, rmax]
                [0, l) can do whatever.
                (rmax, N-1] can do whatever.
             */
            mi a = pow(mi(2), rmax - center) - 1;
            mi b = pow(mi(2), l);
            mi c = pow(mi(2), N-1-rmax);
            mi here = a * b * c;
            local += here;
            dbg(l, MT(a, b, c), here);
        }
        // if there's nothing to the left then we can pick anything we want to the right.
        // can't pick nothing at all though! The subset needs at least 2 dots.
        mi none_left = pow(mi(2), N-1-center) - 1;
        dbgB(none_left);
        local += none_left;
        out += local;
        dbgY(center, local, out);
    }
