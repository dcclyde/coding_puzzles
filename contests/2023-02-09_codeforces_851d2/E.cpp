...
void brute() {
    ...
    // auto dp = ndvec<ll>(N+1, N+1, 0LL);
    V<ll> dp(N+1);
    FOR(k, 0, N) {
        if (k > 0) {dp[k] = dp[k-1];}
        ll interval_sum = 0;
        FOR(len, 0, k+1) {
            if (interval_sum >= 0) {ckmax(dp[k], dp[k-len] + len);}
            interval_sum += dat[k-len];
        }
        if (interval_sum >= 0) {dp[k] = k+1;}
        dbgY(k, dp[k]);
    }
    ps(dp[N-1]);
}
...
void solve() {
    ...
    V<ll> pref(N+1);
    FOR(k, 1, N+1) {
        pref[k] = dat[k-1] + pref[k-1];
        // dbgB(k, dat[k-1], pref[k-1], pref[k]);
    }
    // dbgP(pref);
    remDup(pref);
    ll psz = pref.size();
    ...
    umap<ll,ll> pinv;
    FOR(k, 0, pref.size()) {pinv[pref[k]] = k;}

    SegTree<ll> st; st.init(psz);
    {
        FOR(k, 0, psz) {st.upd(k, -N);}
        st.upd(pinv[0], 1);
        // st.upd(0, )
        // ll pi0 = pinv[0];
        // FOR(k, 0, pi0) {st.upd(k, 0);}
        // FOR(k, pi0, psz) {st.upd(k, 1);}
    }
    ll running = 0;
    ll prev_score = 0;
    ll prev_running = 0;
    FOR(k, 0, N) {
        ...
        running += dat[k];
        /**
            What section should I be querying?
            I guess I need running - old_running >= 0
            old_running <= running
            actually this shows [j+1, k] is ok, not [j, k].
         */
        ll pos = pinv[running];
        // ll posprev = pinv[prev_running];
        ll q = st.query(0, pos);  // this is the best "f" in my notes.
        ll val = q + k;
        ll option = prev_score;
        if (dat[k] >= 0) {++option;}
        ckmax(val, option);  // this is "dp" in my notes.
        dbg(MP(k, dat[k]), pos, q, val);

        // now update something.
        // f[k] = dp[k] - k
        ll update_maybe = st.query(pos, pos);
        ll f_here = val - k;
        if (f_here > update_maybe) {
            st.upd(pos, f_here);
        }
        prev_score = val;
        prev_running = running;
        dbgY(k, dat[k], running, "", q, val, update_maybe, f_here, prev_score); el;
    }

    ll out = prev_score;
    ll o2 = st.query(pinv[running], pinv[running]) + N;
    dbgY(out, o2);
