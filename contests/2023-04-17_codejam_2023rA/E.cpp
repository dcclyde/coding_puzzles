...
void solve() {
    ...
    // Find all the run lengths in dat, allowing wrapping around the end.

    ll out = 0;
    ll start = 0;
    ll stop = N-1;

    if (dat.front() == dat.back()) {
        // Find the first run of the same character.
        while (start < N-1 && dat[start+1] == dat[start]) { ++start; }
        if (start == N-1) {return ps(cdiv(N, 2));}

        while (stop > 0 && dat[stop-1] == dat[stop]) { --stop; }

        ll run = start + 1 + (N-1 - stop + 1);
        out += run / 2;
        ++start; --stop;
    }
    ...
    while (start < stop) {
        ll curr = start;
        while (curr < stop && dat[curr+1] == dat[start]) { ++curr; }
        ll run = curr - start + 1;
        out += run / 2;
        start = curr + 1;
    }
