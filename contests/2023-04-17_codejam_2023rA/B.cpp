...
void solve() {
    ...
    ll out = 0;
    ll prev = -R;
    set<ll> lights; for(auto& x : dat) {lights.insert(x);}

    while (prev < STOP-R) {
        // Find the largest item in lights that's <= prev + 2R.
        auto it = lights.upper_bound(prev + 2*R);
        if (it == lights.begin()) { return IMPOSSIBLE; }
        else {
            --it;
            ll pos = *it;
            if (pos == prev) { return IMPOSSIBLE; }
            prev = pos;
            ++out;
        }
    }
