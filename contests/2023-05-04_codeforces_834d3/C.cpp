...
void solve() {
    ...
    if (start == targ) {return ps(0);}

    auto check = [&](ll a, ll b) -> bool {return abs(a-b) >= min_delta;};

    set<ll> locs = {start, targ, L, R};
    queue<ll> todo;
    map<ll,ll> dist;
    todo.push(start); dist[start] = 0;
    while (!todo.empty()) {
        ll cur = todo.front(); todo.pop();
        if (cur == targ) {return ps(dist[cur]);}
        for (ll loc : locs) {
            if (check(cur, loc) && !dist.count(loc)) {
                todo.push(loc);
                dist[loc] = dist[cur] + 1;
            }
        }
