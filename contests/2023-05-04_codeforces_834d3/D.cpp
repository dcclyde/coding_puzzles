...
void solve() {
    ...
    ll m = 1;
    while (facs[2] < facs[5] && 2*m <= power) { m *= 2; ++facs[2]; }
    while (facs[2] > facs[5] && 5*m <= power) { m *= 5; ++facs[5]; }
    while (10*m <= power) { m *= 10; ++facs[2]; ++facs[5]; }

    // that's all the 0s we're getting. We can still just scale up though.
    ll extra = power / m;
    ll out = start * m * extra;
    return ps(out);
}
