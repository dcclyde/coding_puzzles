...
tcT, class S> struct LazySeg {
	const T idT{-INF_ll}; const S idS{-INF_ll + 1};  // ! identity
...
void solve() {
    ...
    st.init(N);
    FOR(k, 0, N/2) {
        st.seg[st.n + 2*k+1] = B[k];
        out[2*k+1] = B[k];
    }
    st.build();

    V<bool> inB(N+1);
    for(auto& x : B) {if (inB[x]) {return ps(-1);} inB[x] = true;}
    dbgP(inB);

    ROF(v, 1, N+1) {
        if (inB[v]) {continue;}
        int pos = st.fstTrue_from_base(N-1, [&](auto cmb, ll L) -> bool {
            return cmb <= v;
        });
        if (pos == 0) {return ps(-1);}
        pos -= 2;
        out[pos] = v;
        dbgY(v, pos, st);
        st.upd(pos+1, pos+1, st.idT);
        dbg(st);
        el;
    }
