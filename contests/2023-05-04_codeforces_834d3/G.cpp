...
void solve() {
    ...
    V<bool> inB(N+1);
    for(auto& x : B) {inB[x] = true;}
    dbgP(inB);
    ll delta = 0;
    FOR(k, 1, N+1) {
        if (!inB[k]) { ++delta; }
        else {
            if (delta <= 0) { return ps(-1); }
            st.seg[st.n + k] = delta;
            --delta;
        }
    }
    ...
    ll outinv_idx = 0;
    FOR(k, 0, N/2) {
        // Find the largest position where delta = 1.
        // ll pos = lstTrue(0, N, [&](ll x) -> bool {
        //     return st.query(x, N) > 1;
        // });
        ll val_to_place = prev_val + 1;
        while (inB[val_to_place]) { ++val_to_place; }

        ll pos = st.fstTrue_from_base(N, [&](ll v, ll r) -> bool {
            return v > 1;
        });
        --pos;
        ll idx;
        if (pos == -1) {
            // There are no binding constraints right now.
            // Instead, just find the first index that still needs a value.
            while (outinv[outinv_idx] != -1) { ++outinv_idx; }
            idx = outinv_idx;
        } else {
            // need to pair the current value with the last binding constraint.
            // pos = the *value*.
            idx = outinv[pos] - 1;
        }
        dbgR(st);
        dbgY(k, val_to_place, pos, idx, outinv_idx);
        outinv[outinv_idx] = val_to_place;
        st.upd(val_to_place, N, -1);
        prev_val = val_to_place;
    }

    dbgB(outinv);
    V<ll> out(N);
    FOR(k, 1, N+1) { out[outinv[k]] = k; }
