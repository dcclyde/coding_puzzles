...
void solve() {
    ...
    set<ll> seen;
    seen.insert(all(A));
    while (seen.count(last)) {
        if (last == start) {return ps(0);}
        last = (last + (P-1)) % P;
    }

    if (last >= start) {return ps(last - start);}
    ...
    ROF(k, 0, N) {
        ...
        seen.insert(A[k]);
        if (A[k] != 0) {break;}
        if (k == 0) {seen.insert(1);}
    }
    dbg(A);
    dbgB(seen);
    while (seen.count(last)) {
        if (last == start) {return ps(rollover);}
        last = (last + (P-1)) % P;
    }

    ll steps = (last - start + P) % P;
    ckmax(steps, rollover);
    return ps(steps);
}
