/*
    Someone was asking for debugging help with this solution for E.
    See https://codeforces.com/blog/entry/109254?#comment-974152
*/
...
int solve_inner(vector<int> &a, int n, int i, long long h, int green, int blue) {
    // dbgP(i,h,green,blue);
	if(i==n) return 0;
    // dbg(i,h,green,blue);
    ...
    dbgB(i,h,green,blue, "", g, b);
	return max(g, b);
}
int solve(vector<int> &a, int n, int i, long long h, int green, int blue) {
    int so = solve_inner(a, n, i, h, green, blue);
    dbgcY("solve", i, h, green, blue, "", so); el;
    return so;
}
