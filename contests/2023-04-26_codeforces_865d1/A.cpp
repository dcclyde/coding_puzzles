...
void solve() {
    ...
    if (N % 2 == 1) {return YES;}
    if (N == 2) {return YESNO(a[0] <= a[1]);}

    // Use the constraints to compute the extremest possible values of c.
    // c[k] = (a, b) means c_k is a*c_0 + b.
    V<pll> c(N-1);
    c[0] = {1, 0};

    FOR(k, 0, N-2) {
        // c(k-1) - c(k+1) <= a(k+1) - a(k)
        // c(k+1) >= a(k) - a(k+1) + c(k-1)
        ll da = a[k] - a[k+1];
        if (k > 0) {c[k+1].f += c[k-1].f; c[k+1].s += c[k-1].s;}
        c[k+1].s += da;
        dbg(k+1, c[k+1]);
    }

    // We have no control over the last constraint.
    // Just test whether it works out.

    // c(N-3) <= a(N-1) - a(N-2)
    dbgY(pdh(c));
    if (c[N-3].f == 0 && c[N-3].s > a[N-1] - a[N-2]) {return NO;}
    else {return YES;}
}
