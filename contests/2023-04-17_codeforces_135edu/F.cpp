...
void solve_wrong() {
    ...
    FOR(k, 0, N) {pq.emplace(-dat[k], dat[k]);}

    ll out = 0;
    ll recent = 0;
    while (!pq.empty()) {
        auto [mscore, step] = pq.top(); pq.pop();
        ll score = -mscore;
        if (score > recent) {
            recent = score;
            out += score;
        } else {
            // Find smallest k such that score + k*step > recent.
            ll k = cdiv(recent-score+1, step);
            pq.emplace(mscore - k*step, step);
        }
    }

    return ps(out);
}
#pragma endregion  // solve_wrong;
