...
void solve() {
    ...
    ll last_nonneg = lstTrue(0, N-1, [&](int k) { return A[k] >= 0; });
    ll num_nonneg = last_nonneg + 1;
    dbg(sum_B, num_nonneg);

    // ps[k] = sum from 0 to k-1.
    V<ll> prefix_sum(N+1, 0);
    FOR(k, 0, N) {
    ...
    FOR(qid, 0, Q) {
        ...
        auto [g, coeffs] = gcdExtended(X, Y);
        dbgW(g, coeffs);
        if (N % g != 0) {ps(-1); continue;}
        auto [c, d] = coeffs;
        ll r = c * N/g; ll s = d * N/g;
        dbg(MP(c, d), MP(r, s));
        // r*X + s*Y = N.
        pll delta = MP(Y/g, -X/g);

        // Find first t such that r + t*delta.first >= 0.
        ll t = -r / delta.first;
        r += t*delta.f; s += t*delta.s;
        while (r >= 0) {r -= delta.f; s -= delta.s;}
        while (r < 0) {r += delta.f; s += delta.s;}
        dbgc("first r>=0", MP(r,s));
        if (s < 0) {ps(-1); continue;}

        ll min_A = r*X;
        ll step_A = delta.f * X;

        // Find last t such that r + t*delta.first <= N/X.
        t = (N/X-r) / delta.first;
        dbgR(N, X, r, N/X-r, delta.f, t);
        r += t*delta.f; s += t*delta.s;
        ll max_A = r*X;
        dbgP(min_A, max_A, step_A);

        ll out = -2;
        if (min_A >= num_nonneg) {out = prefix_sum[min_A];}
        else if (max_A <= num_nonneg) {out = prefix_sum[max_A];}
        else {
            // Find smallest v such that min_A + v*step_A >= num_nonneg.
            ll v = cdiv(num_nonneg - min_A, step_A);
            dbg(v);
            // dbgP(v, prefix_sum[min_A + v*step_A], prefix_sum[num_nonneg])
            out = prefix_sum[min_A + v*step_A];
            ckmax(out, prefix_sum[min_A + (v-1)*step_A]);
        }
        out += sum_B;
        ps(out);
    }
