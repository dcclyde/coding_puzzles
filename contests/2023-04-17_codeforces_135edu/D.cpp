...
void solve() {
    ...
    // -1 = B wins, 0 = draw, 1 = A wins
    auto dp = ndvec<int>(N+1, N+1, -1);

    // Base case: Empty string is a draw.
    FOR(s, 0, N+1) {dp[s][s] = 0;}
    ...
    FOR(len, 2, N+1, 2) {
        FOR(s, 0, N) {
            ...
            auto subans = [](char a, char b, ll sub) -> ll {
                dbgcW("subans", a, b, sub);
                if (sub != 0) {return sub;}
                dbgcW("asdf");
                if (a < b) {return 1;}
                dbgcW("asdf");
            ...
            ll result_00 = subans(A[s], A[s+1], dp[s+2][e]);
            ll result_01 = subans(A[s], A[e-1], dp[s+1][e-1]);
            ll result_10 = subans(A[e-1], A[s], dp[s+1][e-1]);
            ll result_11 = subans(A[e-1], A[e-2], dp[s][e-2]);
            dbgP(MT(result_00, result_01, result_10, result_11));

            ll result_0 = min(result_00, result_01);
            ll result_1 = min(result_10, result_11);
            dbg(MP(result_0, result_1));

            ll result = max(result_0, result_1);
            dp[s][e] = result;
            dbgY(MP(s,e), A.substr(s, len), result); el;
        }
    }

    if (dp[0][N] == 1) {ps("Alice");}
    else if (dp[0][N] == 0) {ps("Draw");}
    else {ps("Bob");}
    return;
}
