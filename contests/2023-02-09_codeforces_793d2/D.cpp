...
void solve() {
    ...
    ll num_odd = 0;
    for(auto& c : dat) {if (c == '1') {++num_odd;}}
    if (num_odd % 2 == 1) {return NO;}
    if (num_odd == 0) {return NO;}
    ...
    while (num_odd > 0) {
        dbgP(num_odd, todo);
        edges.emplace_back(root, todo);
        while (dat[todo] != '1') {edges.emplace_back(todo, todo+1); ++todo;}
        ++todo; --num_odd;
    }
    dbgY(todo, edges);
    // now edit the last part.
    if (todo != N) {
        auto& [a, b] = edges.back();
        assert(b == todo - 1);
        b = N-1;
        dbgY(edges);
        FOR(k, todo, N-1) {edges.emplace_back(k, k+1);}
        edges.emplace_back(todo, todo-1);
    }
    YES;
    for(auto& [a, b] : edges) {ps1(a, b);}
    return;
}
