...
void brute() {
    ...
    for(auto& [L, R] : queries) {
        string curr = S;
        sort(curr.begin() + L, curr.begin() + R + 1);
        dbg(MP(L,R), curr);
        seen.insert(curr);
    }
...
void solve() {
    ...
    /*
        I need to quickly read off:
        First position in [l, r] that's a 1
        Last position in [l, r] that's a 0

        I guess I can just binary search on prefix sums?
    */

    V<ll> prefix_sums(N+1);
    FOR(k, 0, N) {
        prefix_sums[k+1] = prefix_sums[k] + (S[k] == '1');
    }
    dbgP(prefix_sums);

    auto first_one = [&](ll L, ll R) -> ll {
        // first position X such that ps[X+1] > ps[L].
        ll out = fstTrue(L, R, [&](ll x) -> bool {
            return prefix_sums[x+1] > prefix_sums[L];
        });
        // assert(L <= out && out <= R);
        // if (L <= out && out <= R && S[out] == '0') {--out;}
        return out;
    };

    auto last_zero = [&](ll L, ll R) -> ll {
        // last position X such that ps[X+1] - ps[L].
        dbg_only(
            FOR(x, L, R+1) {
                dbgB(x, prefix_sums[R+1], prefix_sums[x+1], (prefix_sums[R+1] - prefix_sums[x+1] != R-x));
            }
        );
        ll temp = lstTrue(L, R, [&](ll x) -> bool {
            return prefix_sums[R+1] - prefix_sums[x] != R-x+1;
        });
        dbgcP("last zero", MP(L,R), temp);
        ll out = temp;
        // assert(L <= out && out <= R);
        // if (L <= out && out <= R && S[out] == '1') {++out;}
        return out;
    };

    auto first_one_brute = [&](ll L, ll R) -> ll {
        FOR(k, L, R+1) {
            if (S[k] == '1') {return k;}
        }
        return R+1;
    };
    auto last_zero_brute = [&](ll L, ll R) -> ll {
        ROF(k, L, R+1) {
            if (S[k] == '0') {return k;}
        }
        return L-1;
    ...
    dbg_only(
        FOR(L, 0, N) {
            FOR(R, L, N) {
                ll fo = first_one(L, R);
                ll fob = first_one_brute(L, R);
                ll lz = last_zero(L, R);
                ll lzb = last_zero_brute(L, R);
                if (fo != fob || lz != lzb) {
                    dbgY(MP(L, R), MP(fo, lz), MP(fob, lzb));
                    assert(false);
                }
            }
        }
    ...
    for(auto& [L, R] : queries) {
        ...
        ll lz = last_zero(L, R);
        dbg(MP(L, R), MP(fo, lz));
        if (fo > lz) {seen.insert(MP(-1, -1));}
        else {seen.insert(MP(fo, lz));}
    }
