...
template<class T>
long long mergeSort(vector<T>& arr, int left, int right) {
    long long invCount = 0;
...
template<class T>
long long getInversions(vector<T> arr) {
...
void learn() {
    ...
    do {
        V<ll> scores(N+1);
        FOR(l, 0, N) FOR(r, l, N) {
            // find max position and min position in this range.
            auto lo = min_element(ph.begin() + l, ph.begin() + r+1);
            auto hi = max_element(ph.begin() + l, ph.begin() + r+1);
            // dbgP(MP(l,r), *lo, lo-ph.begin(), *hi, hi-ph.begin());

            if (lo >= hi) {continue;}
            ++scores[*lo];
        }
        ll S = sumv(all(scores));
        ll invs = N*(N-1)/2 - getInversions(ph);
        dbgBold(ph);
        dbgY(S, invs);
        dbg(pdh(scores));
        el;
