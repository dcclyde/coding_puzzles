...
void solve_old() {
    ...
    TrieDCC root(MAX_DEPTH + 1);
    for(auto& x : A) {
        TrieDCC* cur = &root;
        for(ll i = MAX_DEPTH; i >= 0; --i) {
            ll bit = (x >> i) & 1;
            if(cur->children[bit] == nullptr) {
                cur->children[bit] = new TrieDCC(i);
            }
...
void solve() {
    ...
    auto check = [&](ll T) -> bool {
        dbgY(T, sort_helper);
        // try to 2-color A.
        // x, y are connected if x ^ y < T. Only consider if indices are within 3 steps.

        color.assign(N, -1);
        FOR(base, 0, N) {
            if (color[base] != -1) {continue;}
            dbg(base);
            color[base] = 0;
            queue<ll> todo;
            todo.push(base);
            while(!todo.empty()) {
                ...
                for(ll i = -3; i <= 3; ++i) {
                    ...
                    if((sort_helper[nxt].f ^ sort_helper[cur].f) < T) {  // we have an edge.
                        dbgP(cur, nxt, color);
                        if(color[nxt] == -1) {
    ...
    ll final_score = lstTrue(0, (1LL << 30), check);
    check(final_score);
    dbg(color);
    dbgY(sort_helper);
    dbgY(final_score);
    ...
    string out(N, '.');
    FOR(k, 0, N) {
        out[sort_helper[k].s] = (color[k] == 0 ? '0' : '1');
    }
