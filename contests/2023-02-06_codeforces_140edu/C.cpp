...
void solve() {
    ...
    // find the one-color intervals.
    IntervalUnion<ll, false> ones;
    FOR(r, 0, N) FOR(c, r, N) {
        if (dat[r][c] == 1) {
            ones.insert(MP(r, c));
        }
    ...
    // auto a = ones.test_contained(MP(0, 1));
    // auto b = ones.missing_sections(MP(0, 1));
    // dbgP(a, b);

    // make sure there aren't any doomed 2s.
    V<pll> twos;
    FOR(r, 0, N) FOR(c, r, N) {
        if (dat[r][c] == 2) {
            if (ones.test_contained(MP(r,c)) == INF) {
                return ps(0);
            }
            twos.emplace_back(r, c);
        }
    }
    dbgP(twos);
    V<pll> ones_x;
    for(auto& [a, b] : ones.x) {ones_x.emplace_back(a, b);}

    ll B = N-1;  // our world is from 0 to B.
    ROF(k, 0, ones_x.size()) {
        auto& [a, b] = ones_x[k];
        // compress these points.

        for(auto& [c, d] : twos) {
            if (c > a) { c = max(a, c - b + a); }
            if (d > a) { d = max(a, d - b + a); }
        }
        B -= b-a;
    }
    ++B;
    ...
    V<ll> shortest_two(B, -1);
    for(auto& [a, b] : twos) {
        ckmax(shortest_two[b], a);
    }
    dbgR(shortest_two);
    FOR(k, 0, B) {if (shortest_two[k] == k) {return ps(0);}}
    el; el;

    // dp[b][x] = num ways to fill up through slot b with the current run starting at x.
    V<V<mi>> dp(B+1, V<mi>(B+1));
    dp[0][0] = 2;
    FOR(b, 1, B) {
        // what happens when filling slot b?
        ll run_start_min = shortest_two[b] + 1;
        FOR(x, run_start_min, b) { dp[b][x] = dp[b-1][x]; }
        FOR(x, 0, b) {dp[b][b] += dp[b-1][x];}
        dbgY(b, dp[b]);
    }

    mi out = 0;
    for(auto& s : dp[B-1]) {out += s;}
    return ps(out);
}
