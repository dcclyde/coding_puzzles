...
/**
    Vertex per contiguous vert or horiz line segment.
    Edge if they intersect.
    Graph is bipartite.

    I want to select smallest list of vertices s.t.
    if v is missing then all w touching v are selected.

    Conversely, biggest set s.t. can't pick 2 that touch.

    Seems like I need size of maximum independent set.
    Algorithm wise, seems like I should start with a max matching.
 */
...
void solve() {
    ...
    auto vedges = ndvec<ll>(R, C, ll(-1));
    auto hedges = ndvec<ll>(R, C, ll(-1));
    ...
    FOR(r, 0, R) FOR(c, 0, C) {
        ...
        if (r > 0 && dat[r-1][c] == '.') {vedges[r][c] = vedges[r-1][c];}
        else {vedges[r][c] = left; ++left;}

        if (c > 0 && dat[r][c-1] == '.') {hedges[r][c] = hedges[r][c-1];}
        else {hedges[r][c] = right; ++right;}

        graph.emplace_back(vedges[r][c], hedges[r][c]);
    }

    auto matching = maxMatch(left, right, graph);

    V<bool> vseen(left), hseen(right);
    for(auto& [l, r] : matching) { vseen[l] = true; hseen[r] = true; }
    ll cover_size = matching.size();
    FOR(k, 0, left) {cover_size += (1-vseen[k]);}
    FOR(k, 0, right) {cover_size += (1-hseen[k]);}

    // cover_size = max independent set size
    //      = "opposite" of the min I want?

    ll out = left + right - cover_size;
    ps(out);
    return;
}
