...
void solve() {
    ...
    FOR(k, 0, E) {
        ...
        G[a].emplace_back(b, MP(k, w));
        G[b].emplace_back(a, MP(k, w));
    }
    dbgY(pdh(G));
    ...
    for(auto& [a, b, w] : edges) { hld.ae(a, b); }
    hld.init(N);
    for(auto& [a, b, w] : edges) { hld.modifyPath(a, b, w); }

    for(auto& [qtype, x, y] : queries) {
        if (qtype == 1) {
            ll j = x-1; ll new_weight = y;
            auto [a, b, w] = edges[j];
            ll curr_weight = hld.queryPath(a, b);
            ll delta = new_weight - curr_weight;
            hld.modifyPath(a, b, delta);
        } else {
            ll a = x-1; ll b = y-1;
            ll out = hld.queryPath(a, b);
            ps(out);
        }
    }
