...
void solve() {
    ...
    auto binsearch_helper = [&](db C) {
        el; dbgcBold("binsearch_helper", C);
        sort(all(B), [&](auto x, auto y) {
            return C*x.s - x.f < C*y.s - y.f;
        });
        dbg(pdh(B));
        ll cnt = 0;
        for(auto& [s, t] : A) {
            ll num_here = 1 + lstTrue(0, B.size()-1, [&](ll n) {
                // dbgW(MP(s,t), n, B[n], (s + B[n].f) / (t + B[n].s));
                return db(s + B[n].f) / (t + B[n].s) >= C;
            });
            cnt += num_here;
            dbgP(MP(s,t), num_here, cnt);
        }
        dbgY(cnt);
        // timebomb(3);
        return cnt >= K;
    ...
    db lo = 0;  // definitely works
    db hi = 1;  // assume this doesn't work
    while (hi - lo > 1e-13) {
        db curr = (lo + hi) / 2;
        if (binsearch_helper(curr)) { lo = curr; }
        else { hi = curr; }
    }
