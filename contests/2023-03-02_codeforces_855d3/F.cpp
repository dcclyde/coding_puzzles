...
void solve() {
    ...
    V<umap<ll,ll>> seen(26);

    ll out = 0;
    for(auto& q : dat) {
        // get "odd/even" hash and list of zero positions.
        V<ll> cnt(26);
        for(auto& c : q) {++cnt[c-'a'];}
        ll hash = 0;
        V<ll> zeros;
        FOR(k, 0, 26) {
            if (cnt[k] % 2 == 1) {hash |= (1<<k);}
            if (cnt[k] == 0) {zeros.push_back(k);}
        }

        // add points for pairs involving stuff we've seen previously.
        ll complement = (1 << 26) - 1 - hash;
        for(auto& b : zeros) {
            // how many previous items also have a zero in slot b
            // and add up to odd everywhere else?
            ll need = complement ^ (1<<b);
            if (seen[b].find(need) != seen[b].end()) {out += seen[b][need];}
        }
