...
void solve() {
    ...
    V<V<ll>> verts_by_depth;
    auto& vbd = verts_by_depth;
    FOR(k, 0, N) {
        while (vbd.size() <= depth[k]) {vbd.emplace_back();}
        vbd[depth[k]].push_back(k);
    }

    V<ll> subtree_type(N, -1);
    ROF(d, 0, verts_by_depth.size()) {
        ll num_types_seen = 0;
        umap<V<ll>, ll> type_lookup;
        umap<ll,ll> type_to_count;
        for(auto& v : vbd[d]) {
            // decide what type this is.
            V<ll> child_types;
            for(auto& o : G[v]) {
                if (o == par[v]) {continue;}
                child_types.push_back(subtree_type[o]);
            }
            sort(all(child_types));
            if (type_lookup.find(child_types) == type_lookup.end()) {
                type_lookup[child_types] = num_types_seen++;
            }
            subtree_type[v] = type_lookup[child_types];
            ++type_to_count[subtree_type[v]];
        }
        ll num_odd = 0;
        for(auto& [t, c] : type_to_count) {num_odd += (c % 2);}
        if (num_odd > 1) {return NO;}
    }
