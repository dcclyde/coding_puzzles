...
void solve() {
    ...
    // I can freely swap stuff in region [first_near_end, N) and [0, last_near_start].
    ll first_near_end = K;
    ll last_near_start = N-1-K;
    ckmin(first_near_end, N);
    ckmax(last_near_start, -1);
    dbg(last_near_start, first_near_end);

    FOR(k, last_near_start+1, first_near_end) {
        if (A[k] != B[k]) {return NO;}
    }

    if (last_near_start >= first_near_end) {
        sort(all(A)); sort(all(B)); return YESNO(A == B);
    }

    string ta, tb;
    FOR(k, 0, last_near_start+1) {
        ta.push_back(A[k]);
        tb.push_back(B[k]);
    }
    FOR(k, first_near_end, N) {
        ta.push_back(A[k]);
        tb.push_back(B[k]);
    }
    dbgP(ta, tb);
