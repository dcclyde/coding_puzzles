...
void solve() {
    ...
    for(auto& c : dat) {c = tolower(c);}
    string targ = "meow";
    map<char, ll> tinv;
    FOR(k, 0, targ.size()) {tinv[targ[k]] = k;}
    dbg(tinv);

    set<char> seen;

    FOR(k, 0, dat.size()) {
        if (tinv.find(dat[k]) == tinv.end()) {return NO;}
        seen.insert(dat[k]);
    }
    dbg();
    if (seen.size() < 4) {return NO;}
    dbgY(N);

    FOR(k, 0, N-1) {if (tinv[dat[k]] > tinv[dat[k+1]]) {return NO;}}
    dbgB(N);
