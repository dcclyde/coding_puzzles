...
void brute() {
    ...
    do {
        ...
        if (ckmax(out, used.size())) { dbgY(out, A); }
...
void solve() {
    ...
    FOR(k, 0, B.size()) {
        ...
        dbgY(k, middle);
        /*
            If B[3] = 10, then there are 10-1 options to the left but 3 are already used.
            if N = 15 then there are 15-10 options to the right
        */
        ll left_score = middle - k;
        ll right_score = N-middle - (B.size()-2-k);
        dbg(left_score, right_score);

        ckmin(left_score, left);
        ckmin(right_score, right);
        dbgP(left_score, right_score);

        ckmax(left_score, 0);
        ckmax(right_score, 0);

        ll curr = left_score + right_score + B.size() - 2;
        ckmax(out, curr);
        dbgB(left_score, right_score, curr, out); el;
    }
