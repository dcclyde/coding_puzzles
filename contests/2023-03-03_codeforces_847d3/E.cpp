...
void learn() {
    FOR(n, 2, 256) {
        bool win = brute(n);
        dbgGeneral((win ? OUT_YELLOW : OUT_CYAN), n, bitset<8>(n));
        // if (win) {
        //     dbgY(n, bitset<8>(n))
        // }
    }
...
void solve() {
    ...
    ll c = 0;
    auto has_bit = [&](ll b) -> bool {return (N>>b) & 1;};
    FOR(b, 0, 30) {
        if (!has_bit(b)) {continue;}

        if (b == 0 || has_bit(b-1)) {return ps(-1);}
        c |= (1 << (b-1));
    }
