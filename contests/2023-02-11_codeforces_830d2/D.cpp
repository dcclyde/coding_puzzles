...
void brute() {
    lls(Q);
    V<pair<char,ll>> queries;
    rv(Q, queries);
    dbgcR("brute", Q, queries);
    el;

    set<ll> cur;
    for(auto& [qtype, x] : queries) {
        dbg(qtype, x, cur);
        if (qtype == '+') {
            cur.insert(x);
        } else if (qtype == '-') {
...
void solve() {
    lls(Q);
    V<tuple<char,ll>> queries;
    rv(Q, queries);
    dbgR(Q, queries);
    el;

    // helper[y] stores all x s.t. we've considered y while computing memo[x].
    // If we remove y, then anything in helper[y] wants y in its rem.
    uset<ll> cur; cur.insert(0);
    umap<ll,ll> memo;
    umap<ll,set<ll>> rem;
    // if x is in rem[y] then we should add y to helper[x].
    umap<ll,uset<ll>> helper;
    // umap<ll,uset<ll>> reminv;

    // maybe reminv and helper are the same except that reminv is empty
    //      if x is currently in cur?

    FOR(qid, 0, Q) {
        auto [qtype, x] = queries[qid];
        el; dbgY(MT(qid,qtype,x), cur, memo, rem);
        dbg(helper);
        if (qtype == '+') {
            cur.insert(x);
            // if x is in rem[y], then I should remove it.
            // dbgP(x, reminv[x], rem[x])
            for(auto& y : helper[x]) {rem[y].erase(x);}
            // reminv[x].clear();
        } else if (qtype == '-') {
            cur.erase(x);
            for(auto& y : helper[x]) {
            ...
            if (memo.find(x) != memo.end()) {
                if (!rem[x].empty()) {out = *rem[x].begin();}
            }
            if (out == -1) {
                ll wip = memo[x];
                while (cur.find(wip) != cur.end()) {
                    helper[wip].insert(x); wip += x;
                }
                memo[x] = wip; out = wip;
            }
            ps(out);
            ...
            // ll& out = memo[x];
            // if (out == 0) {out = x;}
            // // for(auto& y : rem[x]) { if (cur.find(y) == cur.end()) {ckmin(out, y);} }
            // // if (rem[x].size() > 0) {out = *rem[x].begin();}
            // if (rem[x].size() > 0) {out = *rem[x].begin();}
            // while (cur.find(out) != cur.end()) {
            //     helper[out].insert(x);
            //     out += x;
            // }
            // ps(out);
