...
/**
    Picking the last 2 elements works with cost 3.
    Just need to check whether we can pick nothing, or just 1 of the last 2.
 */
...
void solve() {
    ...
    ll g = dat[0];
    for(auto& x : dat) {g = gcd(g, x);}
    dbg(g);

    if (g == 1) {return ps(0);}
    if (gcd(g, N) == 1) {return ps(1);}
    if (gcd(g, N-1) == 1) {return ps(2);}
    return ps(3);
}
