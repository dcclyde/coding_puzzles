...
void brute() {
    ...
    V<pll> out(Q);
    FOR(qid, 0, Q) {
        auto& [L, R] = queries[qid]; --L; --R;
        auto& bp = out[qid]; bp = MP(L, R);
        ll S = 0; ll X = 0;
        FOR(k, L, R+1) {S += dat[k]; X ^= dat[k];}
        FOR(l, L, R+1) {
            ll running_sum = 0; ll running_xor = 0;
            FOR(r, l, R+1) {
                running_sum += dat[r]; running_xor ^= dat[r];
                if (running_sum - running_xor == S - X) {
                    if (bp.s - bp.f > r - l) {bp = MP(l,r);}
                }
            }
        }
    }
...
void solve() {
    ...
    V<V<ll>> bit_prefsums(30, V<ll>(N+1));
    auto& bpsums = bit_prefsums;
    // V<set<ll>> positions_per_bit(30);
    V<V<ll>> positions_per_bit(30);
    auto& ppb = positions_per_bit;
    FOR(k, 0, N) FOR(b, 0, 30) {
        bpsums[b][k+1] = bpsums[b][k];
        if ((dat[k]>>b) & 1) {
            ++bpsums[b][k+1];
            // ppb[b].insert(k);
            ppb[b].push_back(k);
    ...
    auto count_occ = [&](ll b, ll l, ll r) -> ll {return bpsums[b][r+1] - bpsums[b][l];};
    auto next_occ = [&](ll b, ll x) -> ll {
        // auto it = ppb[b].lower_bound(x);
        // if (it == ppb[b].end()) {return -1;}
        // else {return *it;}
        ll pos = fstTrue(0, ppb[b].size()-1, [&](ll y) {return ppb[b][y] >= x;});
        return ppb[b][pos];
    };
    auto prev_occ = [&](ll b, ll x) -> ll {
        // auto it = ppb[b].upper_bound(x);
        // if (it == ppb[b].begin()) {return -1;}
        // else {--it; return *it;}
        ll pos = lstTrue(0, ppb[b].size()-1, [&](ll y) {return ppb[b][y] <= x;});
        return ppb[b][pos];
    };
    ...
    FOR(qid, 0, Q) {
        auto& [L, R] = queries[qid]; --L; --R;
        auto& [bestL, bestR] = out[qid];
        el; dbgY(qid, MP(L,R));

        set<t4> constraints;
        FOR(b, 0, 30) {
            // how many times does this bit show up in the query interval?
            ll occ = count_occ(b, L, R);
            if (occ <= 1) {continue;}

            if (occ % 2 == 0) {
                ll first = next_occ(b, L);
                ll last = prev_occ(b, R);
                constraints.emplace(first, last, -1, -1);
            } else {
                ll first = next_occ(b, L);
                ll second = next_occ(b, first+1);
                ll last = prev_occ(b, R);
                ll secondlast = prev_occ(b, last-1);
                constraints.emplace(first, secondlast, second, last);
            }
        }
        if (constraints.size() == 0) {out[qid] = MP(L,L); continue;}
        dbg(pdh(constraints));
        ll l = R; ll r = L;
        for(auto& [a,b,c,d] : constraints) {
            ckmin(l, a); ckmax(r, b);
        }
        out[qid] = MP(l,r);
        while(get<2>(*constraints.begin()) != -1) {
            auto [a,b,c,d] = *constraints.begin();
            constraints.erase(constraints.begin());
            constraints.emplace(c,d,-1,-1);
            l = get<0>(*constraints.begin());
            ckmax(r, d);
            if (r-l < bestR-bestL) {out[qid] = MP(l,r);}
            dbgP(constraints, MP(l,r), out[qid]);
        }
    }
    return pvn1(out);
}
