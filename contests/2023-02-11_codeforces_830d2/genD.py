...
def gen_test():
    ...
    dat = []
    cur = set()
    notcur = {k for k in range(V_RANGE[0], V_RANGE[1]+1)}
    for _ in range(N):
        ok = False
        while not ok:
            op = random.choice(['+','+','-','?'])
            ok = (
                (op != '-' or len(cur) > 0)
                and (op != '+' or len(notcur) > 0)
                and (_ < N-1 or op == '?')
            )
        if op == '+':
            # add a random item that we don't already have.
            x = random.choice(list(notcur))
            cur.add(x)
            notcur.remove(x)
        elif op == '-':
            x = random.choice(list(cur))
            notcur.add(x)
            cur.remove(x)
        else:
            x = ri(V_RANGE)
