...
void solve() {
    ...
    umap<ll,ll> will_remove;
    for(auto& [qtype, x] : queries) {
        if (qtype == '-') {++will_remove[x];}
    }
    ...
    FOR(qid, 0, Q) {
        ...
        dbgY(memo, rem, reminv, helper);
        if (qtype == '+') {
            curr.insert(x);
            dbg(qid, qtype, x, "", reminv[x]);
            if (reminv.find(x) != reminv.end()) {
                // I tried to use x while computing memo[ri].
                for(auto& ri : reminv[x]) { rem[ri].erase(x); helper[x].insert(ri);}
                reminv.erase(x);
            }
        } else if (qtype == '-') {
            curr.erase(x);
            dbgP(qid, qtype, x, "", helper[x]);
            if (helper.find(x) != helper.end()) {
                for(auto& h : helper[x]) {
                    // I previously considered x as an option while computing memo[h].
                    rem[h].insert(x);
                    reminv[x].insert(h);
                    dbgcP("ins rem", h, x);
                    dbgcP("ins reminv", x, h);
                }
                helper.erase(x);
            }
        } else {
            ...
            ll& out = memo[x];
            if (out == 0) {out = x;}
            dbgB(qid, qtype, x, "", out, rem[x]);
            for(auto& r : rem[x]) {
                if (curr.find(r) == curr.end()) {ckmin(out, r);}
                else {helper[r].insert(x);}
            }
            while (curr.find(out) != curr.end()) {
                helper[out].insert(x);
                out += x;
                dbgcB("ins to helper", out, x);
            }
            rem[x].clear();
            ps(memo[x]);
        }
