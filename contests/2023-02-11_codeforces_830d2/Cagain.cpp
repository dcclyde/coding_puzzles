...
void solve() {
    ...
    IntervalUnion<ll> zeros;
    FOR(k, 0, N) {if (dat[k] == 0) {zeros.insert(MP(k,k));}}
    dbg(zeros.x);

    // prefs[k] = sum over [0, k).
    // ! UPDATE: try making it store xor instead of sum? But don't change var names because lazy?
    V<ll> prefix_sums(N+1);
    auto& prefs = prefix_sums;
    FOR(k, 0, N) {prefix_sums[k+1] = prefix_sums[k] ^ dat[k];}

    V<pll> out(Q);
    FOR(qid, 0, Q) {
        auto& [L, R] = queries[qid];
        --L; --R;  // make em zero indexed.
        el; dbgY(qid, MP(L,R));
        ll sum = prefs[R+1] - prefs[L];
        ll orig_sum = sum;
        // if (sum == 0) {out[qid] = MP(L,L); continue;}
        // sum %= (1LL << 30);  // I think it doesn't matter whether I do this.

        // value removed, score
        V<pll> remR = {{0, 0}};
        ll r = R;

        // first, move R back as far as I can.
        // (as long as it's OK to delete the current element R)
        while (r > L && (dat[r] & sum) == dat[r]) {
            if (dat[r] == 0) {
                auto it = zeros.query(r);
                r = maxll(it->first - 1, L);
            } else {
                sum -= dat[r];
                --r;
            }
            remR.emplace_back(orig_sum - sum, R-r);
        }
        if (r == L) {out[qid] = MP(L,L); continue;}
        sum = orig_sum;

        // now move L.
        V<pll> remL = {{0,0}};
        ll l = L;
        while (l < R && (dat[l] & sum) == dat[l]) {
            if (dat[l] == 0) {
                l = zeros.query(l)->second + 1;
                ckmin(l, R);
            } else {
                sum -= dat[l]; ++l;
            }
            remL.emplace_back(orig_sum - sum, l-L);
        }

        dbg(remL);
        dbgP(remR);

        // now I guess do some 2 pointers stuff?
        // maybe binary search is good enough...
        ll best = 0;
        pll& bp = out[qid];  // "best_pair"
        bp.f = L; bp.s = R;
        for(auto& [rL, scoreL] : remL) {
            ll remRpos = lstTrue(0, remR.size()-1, [&](ll x){
                return (remR[x].f & (orig_sum - rL)) == remR[x].f;
            });
            auto& [rR, scoreR] = remR[remRpos];
            if (ckmax(best, scoreL + scoreR)) {
                bp.f = L + scoreL; bp.s = R - scoreR;
            }
        }
        dbgY(qid, MP(L,R), out[qid]);
    }
    return pvn1(out);
}
