...
using qholder = tuple<ll,ll,ll,ll>;
...
void brute() {
    ...
    V<qholder> queries(Q);
    ll MAX_VALUE = 0;
    for(auto& a : A) {ckmax(MAX_VALUE, a);} for(auto& b : B) {ckmax(MAX_VALUE, b);}
    FOR(qid, 0, Q) {
        auto& [qtype, l, r, x] = queries[qid];
        cin >> qtype;
        if (qtype == 1) {cin >> l >> r >> x; ckmax(MAX_VALUE, x);}
        else {cin >> l >> r; x = -1;}
        --l; --r;
    }
    MAX_VALUE += 1;
    dbgR(N, A, B, MAX_VALUE);
    ll INF = MAX_VALUE * MAX_VALUE + 1;
    el;
...
void solve() {
    ...
    MAX_VALUE += 1;
    ll INF = MAX_VALUE * MAX_VALUE + 1;
    dbgR(N, A, B, MAX_VALUE, INF);
    ll C = pow(N, .5);
    C *= 1;
    ckmax(C, 1); ckmin(C, N);
    while (N % C != 0) {A.push_back(A.back()); B.push_back(B.back()); ++N;}
    ll num_blocks = N / C;
    dbgR(N, C, num_blocks);
    el;

    // break into blocks of size C.

    // do precomputation within each block.
    // I want dp[c][x] = answer if I set this whole block to x.
    // For each g, find smallest multiple among b values. Then x*b/g^2 is a valid answer.
    auto dp = ndvec<ll>(num_blocks, MAX_VALUE+1, INF+1);
    FOR(c, 0, num_blocks) {
        auto& dpc = dp[c];
        uset<ll> bvals; FOR(k, 0, C) {bvals.insert(B[C*c+k]);}
        FOR(g, 1, MAX_VALUE+1) {
            ll m = 1;
            while (g*m <= MAX_VALUE && bvals.find(g*m) == bvals.end()) {++m;}
            if (g*m > MAX_VALUE) {continue;}
            FOR(x, g, MAX_VALUE+1, g) {ckmin(dpc[x], x*m/g);}
        }
    }
    dbgR(pdh(dp));

    V<ll> lazy(num_blocks);
    V<ll> blockans(num_blocks);
    auto ans_one_pair = [](ll a, ll b) -> ll {ll g = gcd(a,b); return a*b/(g*g);};
    auto apply_lazy = [&](ll c) -> void {
        ll L = C*c; ll R = C*(c+1);
        if (lazy[c] > 0) {FOR(k, L, R) {A[k] = lazy[c];} lazy[c] = 0;}
    };
    auto update_block = [&](ll c) -> void {
        ll L = C*c; ll R = C*(c+1);
        auto& out = blockans[c]; out = INF;
        FOR(k, L, R) {ckmin(out, ans_one_pair(A[k],B[k]));}
    };
    FOR(c, 0, num_blocks) {update_block(c);}

    // ok, now start processing queries.
    FOR(qid, 0, Q) {
        auto [qtype, L, R, x] = queries[qid];
        el; dbgY(qid, MT(qtype, L, R, x), A, B, lazy);
        if (qtype == 1) {
            if (L % C != 0) {
                ll c = L / C;
                apply_lazy(c);
                ll end = minll(C*(c+1), R+1);
                FOR(k, L, end) {A[k] = x;}
                update_block(c);
                L = C*(c+1);
            }
            if ((R+1) % C != 0) {
                ll c = (R+1) / C;
                apply_lazy(c);
                ll start = maxll(L, C*c);
                dbg(R, C, c, start);
                FOR(k, start, R+1) {A[k] = x;}
                update_block(c);
                R = C*c - 1;
            }
            if (L <= R) {
            ...
            if (L % C != 0) {
                ...
                apply_lazy(c);
                ll end = minll(C*(c+1), R+1);
                FOR(k, L, end) {dbgP(k); ckmin(out, ans_one_pair(A[k], B[k]));}
                L = C*(c+1);
            }
            if ((R+1) % C != 0) {
                ll c = (R+1) / C;
                apply_lazy(c);
                ll start = maxll(L, C*c);
                FOR(k, start, R+1) {dbgB(k, A, B); ckmin(out, ans_one_pair(A[k], B[k]));}
                R = C*c - 1;
            }
            if (L <= R) { FOR(c, L/C, R/C + 1) {ckmin(out, blockans[c]);} }
            ps(out);
        }
    }
