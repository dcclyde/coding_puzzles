...
void solve() {
    ...
    if (num_parts == 2) {
        FOR(a, 1, S.size()) {
            V<string> dat = {S.substr(0, a), S.substr(a)};
            dbg_only(dbg(a, dat); continue;)
            if (subsolve(dat)) {continue;}
            dat = {S.substr(0, a), S.substr(a)};
            POSSIBLE;
            return pv(dat);
        }
        return IMPOSSIBLE;
        ...
        FOR(a, 1, S.size()) {
            FOR(b, a+1, S.size()) {
                V<string> dat = {S.substr(0, a), S.substr(a, b-a), S.substr(b)};
                dbg_only(dbg(MP(a,b), dat); continue;)
                if (subsolve(dat)) {continue;}
                dat = {S.substr(0, a), S.substr(a, b-a), S.substr(b)};
                POSSIBLE;
                return pv(dat);
            }
        }
