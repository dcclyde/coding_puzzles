...
void solve() {
    ...
    V<t3> nodes_and_scores;
    FOR(k, 0, N) {
        nodes_and_scores.emplace_back(score[k], -hld.depth[k], k);
    }
    sort(all(nodes_and_scores));
    dbgB(pdh(nodes_and_scores)); el;

    ll out = 0;
    ll update_idx = 0;
    FOR(n, 0, N) {
        auto [score, _depth, k] = nodes_and_scores[n];
        el; dbgY(n, MP(k, score));
        dbg(hld.tree);

        // Update the hld by adding 1s in new positions as needed.
        while (get<0>(nodes_and_scores[update_idx]) * C < score) {
            ll other = get<2>(nodes_and_scores[update_idx]);
            hld.modifyPath(other, other, 1);
            dbgcR("update", other, get<0>(nodes_and_scores[update_idx]));
            ++update_idx;
        }
        dbgP(hld.tree);

        // Query the hld and compute the score at this node.
        // ! Careful: If C == 1, will n itself count in the query?
        ll good_total = hld.querySubtree(0);
        ll good_in_subtree = hld.querySubtree(k);
        ll curr = good_in_subtree * (good_total - good_in_subtree);
        out += curr;
        dbgB(good_total, good_in_subtree, curr);
    }
