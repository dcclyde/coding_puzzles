...
void solve() {
    ...
    using t3 = tuple<ll,ll,char>;
    set<t3> vaccines;
    FOR(k, 0, N) {vaccines.emplace(pickup[k], k, 'V');}
    dbg(vaccines);

    V<ll> out;
    ll pos = 0;
    FOR(qid, 0, Q) {
        ll query = queries[qid];
        el; dbgY(qid, query);
        out.push_back(0);
        auto& curr = out.back();

        ll L = pos; ll R = pos + query;
        ll dir = 1;
        if (L > R) {dir = -1; swap(L, R);}
        dbgY(MP(L,R), dir);

        auto next_vaccine = [&]() -> pair<ll, char> {
            pair<ll, char> FAIL = {-1, '?'};
            if (dir == 1) {
                auto it = vaccines.lower_bound({L, -1, 0});
                if (it == vaccines.end()) {return FAIL;}
                if (get<0>(*it) > R) {return FAIL;}
                return MP(get<1>(*it), get<2>(*it));
            } else {
                auto it = vaccines.lower_bound({R+1, -1, 0});
                if (it == vaccines.begin()) {return FAIL;}
                --it;
                if (get<0>(*it) < L) {return FAIL;}
                return MP(get<1>(*it), get<2>(*it));
            }
        };
        ...
        while (true) {
            ...
            el;
            dbg(vaccines);
            auto [k, ktype] = next_vaccine();
            if (k == -1) {break;}
            ll p = pickup[k];
            ll d = dest[k];
            dbgP(k, ktype, p, d);

            // If it was a vaccine, pick it up.
            ll new_pos = -1;
            if (ktype == 'V') {
                vaccines.erase({p, k, 'V'});
                vaccines.emplace(d, k, 'D');
                new_pos = p;
            } else {
                vaccines.erase({d, k, 'D'});
                ++curr;
                new_pos = d;
            }
            if (dir == 1) {L = new_pos;}
