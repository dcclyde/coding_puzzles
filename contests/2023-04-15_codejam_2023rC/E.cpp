...
bool subsolve(auto& dat) {
    ...
    sort(all(dat[0]));
    FOR(n, 1, N) {
        // Find the first permutation of dat[n] that is >= dat[n-1].
        // If none exists, then it's impossible.
        string& prev = dat[n-1];
        string& cur = dat[n];
        sort(all(cur));
        el; dbgY(n, prev, cur);
        ...
        map<char, ll> cnt;
        for (char c : cur) { cnt[c]++; }

        ll bestK = -1;
        FOR(k, 0, prev.size()+1) {
            if (k == prev.size()) {bestK = k; break;}

            // Consider matching at positions < k and differing at k.
            char cmp = (k == prev.size()) ? ('A'-1) : (prev[k]);
            auto it = cnt.upper_bound(cmp);
            dbg(k, cmp, (it == cnt.end()) ? ('Z'+1) : (it->first));
            if (it != cnt.end()) {bestK = k;}

            // If we can match at this position, try that instead.
            if (cnt[prev[k]] == 0) {
                break;
            }
            --cnt[prev[k]];
        ...
        cur.clear();
        FOR(k, 0, bestK) {
            cur.push_back(prev[k]);
            --cnt[prev[k]];
            if (cnt[prev[k]] == 0) {cnt.erase(prev[k]);}
        }
        // Now break the tie.
        if (bestK < prev.size()) {
            auto it = cnt.upper_bound(prev[bestK]);
            char c = it->first;
            cur.push_back(c);
            --cnt[c];
...
void solve() {
    ...
    ll N = S.size();
    if (num_parts == 2) {
        map<char, ll> prefix, suffix;
        ll prefix_len = 0;
        for (char& c : S) { suffix[c]++; }
        /*
            Is it possible to arrange prefix and suffix into strings such that prefix <= suffix?
            If latest element of suffix is before first element of prefix, return false.
            If latest element of suffix is after first element of prefix, return true.
            If equal, then I need this to be the only element, and I need prefix size > suffix size.
        */
        auto check = [&]() -> bool {
            char first_prefix = -1;
            char last_suffix = -1;
            for(char c = 'A'; c <= 'Z'; ++c) {
                if (first_prefix == -1 && prefix[c] > 0) {first_prefix = c;}
                if (suffix[c] > 0) {last_suffix = c;}
            }
            if (first_prefix < last_suffix) {return true;}
            if (first_prefix > last_suffix) {return false;}
            if (prefix[first_prefix] == prefix_len && prefix[first_prefix] <= suffix[last_suffix]) {return true;}
            return false;
        };
        FOR(k, 0, N-1) {
            char c = S[k];
            ...
            ++prefix_len;
            if (check()) {
                V<string> dat = {S.substr(0, k+1), S.substr(k+1)};
                POSSIBLE;
                return pv(dat);
            }
        }
        ...
        // If the string isn't already sorted, I can do this.
        {
            FOR(k, 0, N-1) {
                if (S[k] > S[k+1]) {
                    POSSIBLE;
                    set<ll> cuts = {0, k, k+1, k+2, N};
                    FOR(j, 0, N) {
                        if (cuts.size() == num_parts+1) {break;}
                        cuts.insert(j);
                    }
                    V<string> dat;
                    ll prev = 0;
                    cuts.erase(0);
                    for (ll cut : cuts) {
                        dat.push_back(S.substr(prev, cut-prev));
                        prev = cut;
                    }
                    return pv(dat);
                }
        ...
        // If the string isn't already sorted, this is easy.
        {
            FOR(k, 0, N-1) {
                if (S[k] > S[k+1]) {
                    POSSIBLE;
        ...
        // We also win if any letter appears 3 times.
        {
            FOR(k, 0, N-2) {
                if (S[k] == S[k+2]) {
                    POSSIBLE;
                    set<ll> cuts = {0, k, k+1, k+2, k+3, N};
                    FOR(j, 0, N) {
                        if (cuts.size() == num_parts+1) {break;}
                        cuts.insert(j);
                    }
