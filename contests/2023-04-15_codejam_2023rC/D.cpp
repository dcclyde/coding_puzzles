...
void solve() {
    lls(N, E, Q);
    V<V<pll>> G(N);
    map<ll,ll> club_to_id;
    DSU overall;
    overall.init(N);
    FOR(k, 0, E) {
        ll1(a, b, club);
        G[a].emplace_back(b, club);
        // G[b].emplace_back(a, club);
        if (club_to_id.count(club) == 0) {
            club_to_id[club] = club_to_id.size();
        }
        overall.unite(a, b);
    }
    dbgY(pdh(G));
    ...
    // count the clubs in each connected component.
    map<ll,set<ll>> leader_to_clubs;
    FOR(k, 0, N) {
        ll leader = overall.get(k);
        for (auto& [v, club] : G[k]) {
            leader_to_clubs[leader].insert(club);
        }
    }

    // If there's an odd number of clubs *in this CC* then every query will succeed.
    // Otherwise, a query fails only if every possible route touches all clubs.

    ll NUM_CLUBS = club_to_id.size();
    V<DSU> dsu(NUM_CLUBS);
    FOR(i, 0, NUM_CLUBS) dsu[i].init(N);
    FOR(k, 0, N) {
        for (auto& [v, club] : G[k]) {
            ll bad_id = club_to_id[club];
            FOR(j, 0, NUM_CLUBS) {
                if (j == bad_id) {continue;}
                dsu[j].unite(k, v);
            }
        }
    }
    ...
    ll out = 0;
    FOR(qid, 0, Q) {
        auto& [a, b] = queries[qid];
        el; dbgY(qid, MP(a,b), out);

        // a, b need to be in same component.
        if (!overall.sameSet(a, b)) { continue; }

        ll leader = overall.get(a);
        if (leader_to_clubs[leader].size() % 2 == 1) {
            ++out;
            continue;
        }

        for(auto& club : leader_to_clubs[leader]) {
            ll id = club_to_id[club];
            if (dsu[id].sameSet(a, b)) {
                ++out;
                break;
            }
        }
