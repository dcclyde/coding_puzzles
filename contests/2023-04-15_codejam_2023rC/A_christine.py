...
def solve(testID):
    ...
    dbg(N, dat)
    result = []
    # first entry in result is the first string in dat sorted in alphabetical order
    result.append(sorted([x for x in dat[0]]))
    for i in range(1, N):
        sorted_word = sorted([x for x in dat[i]])
        dbgG(sorted_word)
        word_map = dd(int)
        for c in sorted_word:
            word_map[c] += 1
        result_word_map = dd(int)
        dbgB(result, i - 1)
        for c in result[i-1]:
            result_word_map[c] += 1
        # check if the result_word_map is a subset of word_map
        is_subset = True
        for c in result_word_map:
            if result_word_map[c] > word_map[c]:
                is_subset = False
                break
        if is_subset:
            result.append([x for x in result[i-1]])
            for c in result_word_map:
                word_map[c] -= result_word_map[c]
            for c in sorted_word:
                if word_map[c] > 0:
                    result[-1].append(c)
                    word_map[c] -= 1
            continue
        max_char = sorted_word[-1]
        max_char_idx = 0
        best_win_idx = -1
        for j in range(len(result[i-1])):
            ...
            if word_map[x] > 0:
                word_map[x] -= 1
                if x == max_char and word_map[x] == 0:
                    for k in range(max_char_idx, len(sorted_word)):
                        if word_map[sorted_word[-(k + 1)]] > 0:
                            max_char = sorted_word[-(k + 1)]
                            max_char_idx = k
                            break
            else:
                break
        dbgY(best_win_idx)
        if best_win_idx == -1:
            print(f"Case #{testID}: IMPOSSIBLE")
            return
        winning_word = []
        word_map_2 = dd(int)
        for c in sorted_word:
            word_map_2[c] += 1
        for j in range(best_win_idx):
            winning_word.append(result[i-1][j])
            word_map_2[result[i-1][j]] -= 1
        for j in range(len(sorted_word)):
            if sorted_word[j] > result[i - 1][best_win_idx] and word_map_2[sorted_word[j]] > 0:
                winning_word.append(sorted_word[j])
                word_map_2[sorted_word[j]] -= 1
                break
        for j in range(len(sorted_word)):
            if word_map_2[sorted_word[j]] > 0:
                winning_word.append(sorted_word[j])
                word_map_2[sorted_word[j]] -= 1
        result.append(winning_word)

    print(f"Case #{testID}: POSSIBLE")
