...
void solve() {
    lls(total_size, num_colors, K);
    V<ll> dat;
    rv(num_colors, dat);
    dbgR(total_size, num_colors, K, dat);
    el;

    map<ll,ll> cnts;
    for(auto& x : dat) {++cnts[x];}
    dbgR(cnts);

    ll big = *max_element(all(dat));
    if (big > cdiv(total_size, K)) {return NO;}
    if (big == cdiv(total_size, K) && total_size % K != 0 && cnts[big] > total_size % K) {return NO;}
    return YES;
}
