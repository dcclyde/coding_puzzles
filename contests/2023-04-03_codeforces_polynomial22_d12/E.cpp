...
void solve() {
    ...
    auto subsolve = [&]() -> void {
        uset<ll> todo;
        for(auto& x : A) {todo.insert(x);}
        for(auto& x : B) {todo.insert(lca.jmp(x, D));}

        // special_in_subtree[v] = num items from todo that occur in v's subtree.
        V<ll> special_in_subtree(N, 0);

        ll ROOT = 0;
        {
            auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
                // work on parent
                special_in_subtree[curr] = (todo.find(curr) != todo.end());
                ...
                for (auto& o : G[curr]) {
                    ...
                    special_in_subtree[curr] += special_in_subtree[o];
                }
