...
void solve() {
    ...
    if (total_ones % R != 0) {return ps(-1);}

    ll average_weight = total_ones / R;
    umap<ll,ll> light, heavy;
    FOR(r, 0, R) {
        ll weight = sumv(dat[r]);
        weight -= average_weight;
        if (weight < 0) {light[r] = -weight;}
        else if (weight > 0) {heavy[r] = weight;}
    }
    dbgB(light);
    dbgP(heavy);
    ...
    V<t3> out;
    FOR(c, 0, C) {
        V<ll> light_here, heavy_here;
        FOR(r, 0, R) {
            if (dat[r][c] == 0 && light.find(r) != light.end()) {light_here.pb(r);}
            if (dat[r][c] == 1 && heavy.find(r) != heavy.end()) {heavy_here.pb(r);}
        }
        dbgY(c, light_here, heavy_here);
        FOR(k, 0, minll(light_here.size(), heavy_here.size())) {
            ll r1 = light_here[k];
            ll r2 = heavy_here[k];
            out.emplace_back(r1, r2, c);
            if (--light[r1] == 0) {light.erase(r1);}
            if (--heavy[r2] == 0) {heavy.erase(r2);}
        }
    }
