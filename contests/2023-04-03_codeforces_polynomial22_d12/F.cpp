...
tcT, class S> struct LazySeg {
	const T idT{}; const S idS{1, -INF_ll};  // ! identity
    ...
    void lazy_seg(T& a, const S& b) {  // ! lazy * seg
        if (b.s != idS.s) {a = b.s;}
        else {a *= b.f;}
    }

    // apply b after a.
    void lazy_lazy(S& a, const S& b) {  // ! lazy * lazy
        if (b == idS) {return;}
        if (a == idS) {a = b; return;}
        if (b.s != idS.s) {a.f = 1; a.s = b.s;}
        else if (a.s != idS.s) {a.s *= b.f;}
        else {a.f *= b.f;}
    }
...
void brute() {
    lls(Q);
    V<pll> queries(Q, MP(-1, -1));
    ll B = 0;
    FOR(k, 0, Q) {
        cin >> queries[k].f;
        if (queries[k].f != 3) {cin >> queries[k].s;}
        if (queries[k].f == 1) {ckmax(B, queries[k].s);}
    }
    ++B;
    dbgR(Q, B, queries);
    el;

    ll total_subtracted = 0;
    multiset<ll> dat;
    FOR(qid, 0, Q) {
        el; dbgY(qid, queries[qid]);
        ll qtype = queries[qid].f;
        ll val = queries[qid].s;
        if (qtype == 1) {
            dat.insert(val);
        } else if (qtype == 2) {
            multiset<ll> new_dat;
            for(auto& x : dat) {if (x > val) {new_dat.insert(x - val);}}
            dat = new_dat;
            total_subtracted += val;
        } else if (qtype == 3) {
            multiset<ll> other_dat;
            for(auto& x : dat) {if (x > total_subtracted) {other_dat.insert(x - total_subtracted);}}
            for(auto& x : other_dat) {dat.insert(x);}
            total_subtracted *= 2;
            ckmin(total_subtracted, powll(10,6));
        }
    }
...
void solve() {
    ...
    LazySeg<mi,pair<ll,ll>> st;
    st.init(B);
    ll st_begin = 0;
    ll total_subtracted = 0;

    auto subtract_helper = [&](ll num_to_subtract) {
        // "subtract" by moving st_begin and setting some chunk(s) to zero.
        // if st_begin = 0 and num_to_subtract = 3, then [0, 2] is set to 0 and st_begin becomes 3.

        ll new_st_begin = (st_begin + num_to_subtract) % B;
        if (num_to_subtract >= B) {
            st.upd(0, B-1, MP(1, 0));
        } else if (st_begin + num_to_subtract >= B) {
            st.upd(st_begin, B-1, MP(1, 0));
            st.upd(0, new_st_begin, MP(1, 0));
        } else {
            st.upd(st_begin, new_st_begin, MP(1, 0));
        }
        st_begin = new_st_begin;
        total_subtracted += num_to_subtract;
        return;
    };
    ...
    FOR(qid, 0, Q) {
        ...
        ll qtype = queries[qid].f;
        if (qtype == 1) {
            ll pos = (queries[qid].s + st_begin) % B;
            ll val = (int)st.query(pos, pos);
            st.upd(pos, pos, MP(1, val + 1));
        } else if (qtype == 2) {
            ll num_to_subtract = queries[qid].s;
            subtract_helper(num_to_subtract);
        } else {
            if (total_subtracted == 0) {
                st.upd(0, B-1, MP(2, -INF_ll));
            } else if (total_subtracted >= B) {
                // nothing happens.
            } else {
                // query the whole array one by one I guess?
                FOR(k, total_subtracted + 1, B) {
                    ll ind = (st_begin + k) % B;
                    bak[k] = st.query(ind, ind);
                    dbgP(k, ind, bak[k]);
                }
                // subtract_helper(total_subtracted);
                FOR(k, 1, B - total_subtracted) {
                    ll ind = (st_begin + k) % B;
                    mi val = st.query(ind, ind);
                    st.upd(ind, ind, MP(1, int(val + bak[k+total_subtracted])));
                }
                total_subtracted *= 2;
            }
        }
        dbg(st_begin, total_subtracted, st);
    }

    mi out = st.query(0, B-1);
    return ps(out);
}
