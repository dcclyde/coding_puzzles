...
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{INF_ll, INF_ll};  // ! identity
...
void brute() {
    ...
    if (K == 0) {return ps(dat);}

    V<ll> combs_helper; auto& ch = combs_helper;
    FOR(k, 0, N) {combs_helper.push_back(k);}
    string best(N-K, '9');
    do {
        if (dat[ch[0]] == '0') {continue;}
        string wip;
        FOR(k, 0, N-K) {wip.push_back(dat[ch[k]]);}
        ckmin(best, wip);
    } while (next_combination(ch.begin(), ch.begin() + N-K, ch.end()));
    return ps(best);
}
...
void solve() {
    ...
    ll Korig = K;
    set<ll> zeros;
    SegTree<pll> st; st.init(N);
    FOR(k, 0, N) {
        if (dat[k] == '0') {
            zeros.insert(k);
            // st.seg[st.n + k] = MP(INF_ll, k);
        } else {
            st.seg[st.n + k] = MP(dat[k] - '0', k);
        }
    }
    st.build();
    zeros.insert(N);
    ...
    // handle the first step separately?
    {
        ll last_allowed = pos + K;
        auto [minval, minpos] = st.query(pos, last_allowed);
        K -= (minpos - pos);
        pos = minpos + 1;
        out.push_back(dat[minpos]);
        dbgcR("1st done", MP(minval,minpos), K, pos, out);
    }

    while (K > 0 && pos < N) {
        ll last_allowed = pos + K;
        el; dbgY(pos, K, out);
        // how many things do we want to delete before taking our first digit?
        ll zpos = *zeros.lower_bound(pos);
        dbgB(zpos);
        if (zpos-pos <= K) {
            // just delete everything until this zero.
            K -= (zpos - pos);
            pos = zpos + 1;
            dbgB(K, pos);
            out.push_back('0');
            ...
            // if zpos = 3, we could consider deleting 0, 1, or 2 items.
            // then our value would be dat[0], dat[1], dat[2] respectively.
            dbgB(pos, last_allowed, dat.substr(pos, last_allowed-pos+1));
            auto [minval, minpos] = st.query(pos, last_allowed);
            K -= (minpos - pos);
            pos = minpos + 1;
            out.push_back(dat[minpos]);
            dbg(MP(minval, minpos), K, pos, out);
        }
    }
    if (pos < N) {out += dat.substr(pos);}
    while (out.size() > N - Korig) {out.pop_back();}
    return ps(out);
}
