...
tcT> T kruskal(int N, vector<pair<T,pi>> ed) {
    ...
    FOR(k, 0, N) {if (!D.sameSet(0, k)) {return -1;}}
	return ans;
}
...
void solve() {
    lls(N, num_roads);
    V<ll> airport_cost; rv(N, airport_cost);
    V<ll> harbor_cost; rv(N, harbor_cost);

    using t3 = tuple<ll,ll,ll>;
    V<t3> roads; rv(num_roads, roads);

    ll best = numeric_limits<ll>::max();
    FOR(use_air, 0, 2) FOR(use_harbor, 0, 2) {
        V<pair<ll,pii>> edges;
        for(auto& [a, b, cost] : roads) {edges.emplace_back(cost, MP(a-1,b-1));}
        ll nodes = N;
        if (use_air) {
            ll air_node = nodes; ++nodes;
            FOR(k, 0, N) {edges.emplace_back(airport_cost[k], MP(air_node, k));}
        }
        if (use_harbor) {
            ll harbor_node = nodes; ++nodes;
            FOR(k, 0, N) {edges.emplace_back(harbor_cost[k], MP(harbor_node, k));}
        }
        ll cost = kruskal(nodes, edges);
        if (cost == -1) {continue;}
        ckmin(best, cost);
        dbgY(use_air, use_harbor, cost, best);
    }
    return ps(best);
}
