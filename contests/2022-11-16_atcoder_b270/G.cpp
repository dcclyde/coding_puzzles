...
/**
    ...
    this will be eventually periodic.
    the preperiodic part should be short, like bounded by power of 2 that's <= p?
    handle that separately.

    once we're periodic...?

    f(f(x)) = a^2*x + a*b + b
    f3(x) = a^3 x + a^2*b + a*b + b
*/
...
template<int id> struct mint {
	...
	friend str to_string(mint a) { return ts(a.v); }
...
struct DiscreteLog {
	...
	int query(mi x) {
		F0R(i,block) {
			if (u.find((int)x) != u.end()) return i*block+u[(int)x];
			x *= cur;
		}
		return -1;
	}
	void init(int r) { // gcd(m,r) = 1
		root = r; block = sqrt(mi::mod())+1;
		u.clear(); cur = mi(1);
		F0R(i,block) {
			if (!(u.find((int)cur) != u.end())) u[(int)cur] = i;
			cur *= root;
		}
		cur = 1/cur;
	}
...
void solve() {
    lls(p, a, b, start, goal);
    if (start == goal) {return ps(0);}
    if (a == 0) {
        if (b == goal) {return ps(1);}
        else {return ps(-1);}
    }
    mi::set_mod(p);

    if (a == 1) {
        ll need = (goal - start + p) % p;
        // want minimum k s.t. k*b = need mod p.
        /*
            if b is coprime to p, we'll just use k = need/b.
        */
        ll g = gcd(b, p);
        if (need % g != 0) {return ps(-1);}
    ...
    mi e = goal + r;
    dbg(r, s, e);
    // ask: what's minimum k s.t. s * A^k = e?
    if (s == 0) {return ps(-1);}
    mi need = e / s;

    DiscreteLog dl;
    dl.init(a);
    ll out = dl.query(need);
