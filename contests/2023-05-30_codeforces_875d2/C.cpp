...
void brute() {
    ...
    while (dsu.size(0) != N) {
        ...
        FOR(k, 0, E) {
            auto [a, b] = edges[k];
            if (dsu.sameSet(0, a)) {dsu.unite(0, b);}
            if (dsu.sameSet(0, b)) {dsu.unite(0, a);}
        }
    }
...
void solve() {
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        ...
        for (auto& o : G[curr]) {
            ...
            score[o] = score[curr];
            if (parent != -1 && eid[{parent, curr}] > eid[{curr, o}]) {
                ++score[o];
            }
        ...
        dbgP(curr, parent, score[curr]);
    };
    dfs(ROOT, -1, dfs);

    ll out = *max_element(all(score));
    return ps(out);
}
