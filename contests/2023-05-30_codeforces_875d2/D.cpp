...
void solve() {
    lls(N);
    vecs(ll, N, A, B);  // a1 * a2 == b1 + b2.
    el;

    ll RT = sqrtll(2*N);

    // auto cnt = ndvec<ll>(RT+1, N+1, 0LL);
    V<umap<ll,ll>> cnt(RT+1);
    FOR(k, 0, N) {
        if (A[k] < cnt.size()) { ++cnt[A[k]][B[k]]; }
    }

    // ~ Main idea: Iterate over (position of big guy, multiplier from small guy).
    ll out_times_2 = 0;
    FOR(k, 0, N) {
        ll a2 = A[k]; ll b2 = B[k];
        FOR(a1, 1, RT+1) {
            ...
            if (b1 > N) {break;}
            if (a1 > a2) {break;}
            if (cnt[a1].find(b1) == cnt[a1].end()) {continue;}
            ll v = cnt[a1][b1];
            if (a1 == a2) {
                // only count stuff with b1 <= b2.
                if (b1 > b2) {continue;}
                if (b1 == b2) {
                    ll curr = v-1;
                    out_times_2 += curr;
                    dbg(k, MP(a2,b2), MP(a1,b1), v, curr, out_times_2);
                } else {
                    ll curr = v;
                    out_times_2 += 2*curr;
                    dbgP(k, MP(a2,b2), MP(a1,b1), curr, out_times_2);
                }
            } else {
                // count everything.
                ll curr = v;
                out_times_2 += 2*curr;
                dbgB(k, MP(a2,b2), MP(a1,b1), curr, out_times_2);
            }
        }
    }

    ll out = out_times_2 / 2;
    return ps(out);
}
