...
auto solve(ll nn = -1) {
    ...
    V<ll> need;
    FOR(k, 0, N+1) {need.push_back(k);}

    V<ll> out;
    auto use = [&](ll x) -> void { out.push_back(x); --need[x]; };

    use(N);
    while (out.back() > 2) { use(out.back() - 2); }
    if (out.back() == 2) { use(1); }

    while (true) {
        ll prev = out.back();
        bool ok = false;
        for (auto o : {prev-1, prev+1, prev+2}) {
            if (o <= N && need[o]) { use(o); ok = true; break; }
        }
    ...
    // by now I expect everything is used up except for maybe some Ns.
    V<ll> ans;
    FOR(k, 0, out.size()) {
        ans.push_back(out[k]);
        if (
            ...
            && need[N] > 0
            && (
                (out[k] == N-2 && out[k+1] == N-1)
                || (out[k] == N-1 && out[k+1] == N-2)
            )
        ) {
            ans.push_back(N); --need[N];
        }
    }
...
void check() {
    FOR(N, 3, 1001) {
        ...
        V<ll> need(N+1);
        FOR(k, 0, N+1) { need[k] = k; }
        ans.push_back(ans.front());
        FOR(k, 0, ans.size() - 1) {
            assert(ans[k] != ans[k+1]);
            assert(abs(ans[k] - ans[k+1]) <= 2);
            assert(1 <= ans[k] && ans[k] <= N);
            --need[ans[k]];
        }
        FOR(k, 1, N+1) { assert(need[k] == 0); }
    }
}
