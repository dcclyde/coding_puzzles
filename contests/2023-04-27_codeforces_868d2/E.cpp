...
void learn() {
    ...
    FOR(n, 0, N) {
        ...
        FOR(start, 0, n) {
            FOR(len, L, R+1) {
                ...
                if (stop >= n) {break;}
                ll r1 = grundy[start];
                ll r2 = grundy[n-stop-1];
                mex_helper.insert(r1 ^ r2);
            }
        }
        while (mex_helper.count(grundy[n])) {grundy[n]++;}
    }
    ll M = *max_element(all(grundy));
    ...
    FOR(n, 0, N) {
        ...
        while (mex_helper.count(grundy_loop[n])) {grundy_loop[n]++;}
    }
    dbg(MP(L, R), pdh(grundy), M);
    dbgB(pdh(grundy_loop));
}
...
void solve() {
    ...
    umap<ll,ll> size_to_cnt;
    FOR(k, 0, N) { size_to_cnt[dsu.size(k)]++; }
    ll B = 0;
    for(auto& [size, cnt] : size_to_cnt) { cnt /= size; ckmax(B, size);}

    // Now do Grundy stuff.
    auto grundy_cycle = [&](ll n) -> ll {
        if (n < L) {return 0;}
        if (n >= R+L) {return 0;}
        return n / L;
    };
    ...
    ll g = 0;
    for(auto& [size, cnt] : size_to_cnt) {
        if (cnt % 2 == 1) {g ^= grundy_cycle(size);}
    }
