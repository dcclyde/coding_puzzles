...
void learn() {
    ...
    FOR(N, 2, MAX_N+1) {
        ...
        auto product_rec_helper = [&](auto&& self) {
            ...
            // ll n = curr.size();
            FOR(k, 0, N) {  // ! iterate over options at depth n
                curr.push_back(k);
                self(self);
                curr.pop_back();
            }
...
#pragma endregion  // learn



void solve() {
    lls(N, NUM_CONSTRAINTS);
    V<ll> positions, scores;
    rv(NUM_CONSTRAINTS, positions, scores);
    dbgR(NUM_CONSTRAINTS, positions);
    dbgB(scores);
    el;

    V<pll> constraints;
    FOR(i, 0, NUM_CONSTRAINTS) {
        constraints.push_back({positions[i], scores[i]});
    }
    sort(all(constraints));
    // constraints.emplace_back(N, constraints.back().s);
    dbgP(pdh(constraints));

    // First constraint is special maybe?
    // aaaaa b c
    if (constraints[0].s > constraints[0].f) {return NO;}
    string out(constraints[0].s - 2, 'a');
    out += "bc";
    string ABC = "abc";
    ll abc_idx = 0;
    while (out.size() < constraints[0].f) {
        out.push_back(ABC[abc_idx]);
        abc_idx = (abc_idx+1) % 3;
    }

    dbg_only(
        ll asdf = count_pals(out);
        dbgB(MP(constraints[0].f, constraints[0].s), asdf, out);
        assert(asdf == constraints[0].s);
    );

    FOR(k, 1, constraints.size()) {
        auto [pos, score] = constraints[k];
        ll dp = pos - constraints[k-1].f;
        ll ds = score - constraints[k-1].s;
        if (ds > dp) {return NO;}
        if (ds < 0) {return NO;}

        // Add a b c a b c a... until dp == ds.
        FOR(j, 0, dp-ds) {
            out.push_back(ABC[abc_idx]);
            abc_idx = (abc_idx+1) % 3;
        }
        out += string(ds, 'a'+2+k);
        dbg_only(
            ll curr = count_pals(out);
            dbgY(k, MP(pos, score), MP(dp, ds), curr, out);
            assert(curr == score);
            assert(out.size() == pos);
        )
    }
