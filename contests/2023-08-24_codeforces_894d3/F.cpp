...
void solve() {
    ...
    /**
        I need to find the first time where F*t + W*t >= S *and*...
            F*t >= f, W*t >= S-f for some f with dp[f] = true.
    ...
    while (true) {
        ...
        ll lo = S - W*t;
        ll hi = F*t;
        // check all values that are in [lo, hi] but not in [prev lo, prev hi].
        if (lo > hi) {continue;}
        if (lo + W > hi - F) {
            // check from lo to hi.
            if (check(lo, hi)) {return ps(t);}
        } else {
            // don't recheck the parts we checked previously.
            if (check(lo, lo + W)) {return ps(t);}
            if (check(hi - F, hi)) {return ps(t);}
        }
    }
