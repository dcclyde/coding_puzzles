...
void solve() {
...

    {
        ...
        while (it2 != sorted.end()) {
            gaps.insert(it2->f - it->f);
            ++it; ++it2;
        }
    }

    FOR(qid, 0, Q) {
        auto [qi, qx] = queries[qid];
        dbgY(qid, MP(qi, qx));
        dbgR(sorted, gaps);

        // remove the old gap.
        {
            auto it = sorted.find({A[qi], qi});
            auto it2 = next(it);
            if (it2 != sorted.end()) {
                dbgcP("remove gap", it2->f - it->f);
                gaps.erase(gaps.find(it2->f - it->f));
            }
            if (it != sorted.begin() && it2 != sorted.end()) {
                // insert the new gap now that this element is gone.
                gaps.insert(it2->f - prev(it)->f);
            }

            if (it != sorted.begin()) {
                it2 = prev(it);
                dbgcP("remove gap", it->f - it2->f);
                gaps.erase(gaps.find(it->f - it2->f));
            }
        }

        // remove the old value.
        sorted.erase({A[qi], qi});

        // insert the new value.
        A[qi] = qx;
        sorted.insert({A[qi], qi});
        ...
        // insert the new gap.
        {
            ...
            auto it2 = next(it);
            if (it != sorted.begin() && it2 != sorted.end()) {
                gaps.erase(gaps.find(it2->f - prev(it)->f));
            }
            if (it2 != sorted.end()) {
                gaps.insert(it2->f - it->f);
            }
            if (it != sorted.begin()) {
                it2 = prev(it);
                gaps.insert(it->f - it2->f);
            }
        }

        ll big = prev(sorted.end())->f;
        ll biggest_gap = *gaps.rbegin();
        ll out = big + biggest_gap;

        dbgB(sorted, gaps);
        dbgR(big, biggest_gap, out);
