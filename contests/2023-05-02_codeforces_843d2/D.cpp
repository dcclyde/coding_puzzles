...
void solve() {
    ...
    ll PRIMES = sieve.primes.size();
    umap<ll,V<ll>> primes_to_indices;
    V<V<ll>> indices_to_primes(N);
    FOR(k, 0, N) {
        auto facs = factor(A[k]);
        for(auto& [p, e] : facs) {
            primes_to_indices[p].push_back(k);
            indices_to_primes[k].push_back(p);
        }
    }
    ...
    depth[START] = 0;
    q.push(START);
    uset<ll> primes_tried;
    while (!q.empty()) {
        ll cur = q.front(); q.pop();
        if (cur == TARG) {
            break;
        }
        for(auto& p : indices_to_primes[cur]) {
            if (primes_tried.find(p) != primes_tried.end()) { continue; }
            primes_tried.insert(p);
            for(auto& nxt : primes_to_indices[p]) {
                if (depth[nxt] == -1) {
