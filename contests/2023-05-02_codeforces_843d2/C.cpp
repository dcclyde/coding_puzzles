...
void solve() {
    ...
    // if X has bits that L doesn't have, we lose.
    if ((L | X) != L) {return ps(-1);}

    ll hi = 6e18;  // not allowed to go this high.
    ll lo = L;  // need to go at least this high.

    const ll BITS = 61;
    FOR(b, 0, BITS) {
        el; dbgY(b, MP(lo,hi));
        ll first_nonset = -1;
        // Find the first value >= L where bit b is NOT set.
        if ((L & (1LL << b)) == 0) {
            first_nonset = L;
            dbg(L, b, first_nonset);
        } else {
            first_nonset = L;
            // Find the first 0 that's left of b.
            FOR(first_zero, b+1, BITS) {
                if ((L & (1LL << first_zero)) == 0) {
                    ll h1 = (1LL << first_zero);
                    ll h2 = ((~0LL) << (first_zero+1));
                    dbgB(bitset<10>(h1), bitset<10>(h2));
                    first_nonset |= (1LL << first_zero);
                    dbgP(first_nonset);
                    first_nonset &= ((~0LL) << (first_zero));
                    break;
                }
            }
            dbgP(L, b, first_nonset);
        }

        // Do we want this bit set though?
        if (X & (1LL << b)) {
            // Yes, we want it set.
            // We must not go as far as first_nonset.
            ckmin(hi, first_nonset);
        } else {
            // No, we don't want it set.
            // We need to go at least as high as first_nonset.
            ckmax(lo, first_nonset);
        }
    }

    if (lo < hi) {return ps(lo);}
    else {return ps(-1);}
}
