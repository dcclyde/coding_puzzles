...
void solve_wrong() {
    ...
    // is the string exactly 110?
    if (dat == "bba") {return ps(":(");}

    // is there a 0 that isn't in the last slot?
    FOR(k, 1, N-1) {
        if (dat[k] == 'a') {
            string A = dat.substr(0, k);
            string B = dat.substr(k, 1);
            string C = dat.substr(k+1);
            dbg(k, A, B, C);
    ...
    {
        string A = dat.substr(0, 1);
        string B = dat.substr(1, 1);
        string C = dat.substr(2);
        dbgP(A, B, C);
        return ps(A, B, C);
    }
}
...
void solve() {
    ...
    string A = dat.substr(0, 1);
    string B = dat.substr(1, N-2);
    string C = dat.substr(N-1);
    dbgP(A, B, C);
    return ps(A, B, C);
}
