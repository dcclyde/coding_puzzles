...
void learn() {
    dbg_only(
        ...
        FOR(n, 1, MAX_N+1) {
            ...
            ll R = rootll(n, 2);
            if (R*R < n) {++R;}
            // RxR is the smallest square that holds N items.
            ...
            ckmax(MAX_C, C);
            if (ckmax(big_num_pairs, num_pairs)) {
                dbgR(n, num_pairs, MAX_C, MP(R, C));
            }
        }
        dbgBold(MAX_C);
        assert(false);
...
template<int id> struct mint {
	...
	friend mint pow(mint a, ll p) {
		...
		return ans; }
	friend mint inv(const mint& a) { assert(a.v != 0); assert(mod_totient != -1);
...
template<int id> int mint<id>::mod_totient = -1;
...
template <int id>
string to_string(mint<id> modint) {
...
void solve_type2() {
    ...
    FOR(r, 1, MAX_G+1) {
        FOR(c, 1, MAX_G+1) {
            ...
            FOR(g, 1, MAX_G+1) {
                if (c > g) {continue;}
                if (c > r) {
                    dp[g][c][r] = get(r, c-1, g) + get(r-1, c, g-c);
                } else {
                    dp[g][c][r] = get(r-1, c, g) + get(r-1, c, g-c);
                }
                dbg(g, MP(r,c), dp[g][c][r]);
            }
        }
    }
    ...
    auto subsolve = [&]() -> mi {
        ...
        V<mi> gf(G+1);
        FOR(g, 0, G+1) {gf[g] = get(g, g, g);}
        dbgY(gf);

        // Now we need gf raised to 4th power.
        auto square_poly = [&](auto& P) -> V<mi> {
            V<mi> out(G+1);
            FOR(j, 0, P.size()) FOR(k, 0, P.size()) {
                if (j+k >= out.size()) {continue;}
                out[j+k] += P[j] * P[k];
            }
            return out;
        };
    ...
    return ps(2*half_perim, result);
}

void solve() {
    if (PROBLEM_TYPE == 1) { solve_type1(); } else {solve_type2();}
}
...
int main() {
    ...
    int T = 1;
        ...
        std::cin >> PROBLEM_TYPE; dbgBold(PROBLEM_TYPE);
        if (PROBLEM_TYPE == 2) {
            std::cin >> MOD;
            dbgBold(MOD);
            mi::set_mod(MOD);
