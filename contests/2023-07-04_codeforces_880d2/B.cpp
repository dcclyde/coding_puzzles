...
void solve() {
    ...
    // first N-1 people each get "small" number of coins. Actually they receive 0.
    // Last guy gets K*G - small coins assigned.
    ll last_guy = K*G - (N-1) * small;
    dbg(small, last_guy);
    if (last_guy <= 0) {return ps(K*G);}

    // how much will the last guy actually receive?
    ll last_guy_rounded = (last_guy + small + 1) / G * G;
    ll saved = K*G - last_guy_rounded;
    return ps(saved);
}
