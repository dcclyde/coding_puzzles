...
void solve() {
    ...
    strings(sa, sb);
    ll delta = 0;
    FOR(k, 0, N) {delta += (sa[k] != sb[k]);}
    dbgR(N, sa, sb);
    el;

    if (delta == 0) {return ps(0);}

    // f(k) = r * f(1) + s.
    mi a;
    {
        mi rprev = 0; mi sprev = 0;
        mi r = 1; mi s = 0; ll k = 1;
        while (k < N) {
            // f(k) = 1 + k/N * f(k-1) + (N-k)/N * f(k+1)
            // f(k+1) = N / (N-k) * (f(k) - 1 - k/N * f(k-1))
            mi mult = mi(N) / (N-k);
            mi rnext = r - mi(k)/N * rprev;
            mi snext = s - mi(k)/N * sprev - 1;
            rnext *= mult; snext *= mult;

            rprev = r; sprev = s; r = rnext; s = snext;
            ++k;
        }
        // Now I need r*a + s == rprev*a + sprev + 1.
        a = (sprev + 1 - s) / (r - rprev);
    }

    // now do it again but this time we actually know a.
    {
        mi rprev = 0; mi sprev = 0;
        mi r = 1; mi s = 0; ll k = 1;
        while (k < delta) {
