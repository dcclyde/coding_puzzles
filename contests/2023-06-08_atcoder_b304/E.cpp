...
void solve() {
    ...
    FOR(k, 0, K) {
        ll1(a, b);
        H[dsu.get(a)].emplace(dsu.get(b));
        H[dsu.get(b)].emplace(dsu.get(a));
    }
    dbgB(pdh(H));
    ...
    FOR(qid, 0, Q) {
        ll1(a, b);
        /*
            If I add an edge from a to b, will that connect any H edges?
        */
        bool result = !(H[dsu.get(a)].count(dsu.get(b)));
        YesNo(result);
    }
