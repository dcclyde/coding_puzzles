...
void solve() {
    ...
    vecs(pll, N, X);
    lls(nA);
    vecs(ll, nA, A);  // x values where we'll make vertical cuts
    lls(nB);
    vecs(ll, nB, B);  // y values where we'll make horiz cuts
    el;
    ...
    ll big = 0;
    for(auto& [x, y] : X) {
        ll xi = lower_bound(all(A), x) - A.begin();
        ll yi = lower_bound(all(B), y) - B.begin();
        ckmax(big, ++cnt[{xi, yi}]);
    }
