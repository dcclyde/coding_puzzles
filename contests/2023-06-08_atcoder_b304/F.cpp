...
void solve() {
    ...
    FOR(mask, 0, MASK_END) {
        /*
            Repeat every N/d days.
            Answer will be:
                (result where we repeat every N/2 days)
                + f(N/3)
                - f(N/6)
            Sign depends on
        */
        ...
        dbg(d, num);
        if (d == 1) {continue;}
        // how many slots are restricted here?
        uset<ll> used;
        FOR(k, 0, N) {
            if (S[k] == '.') {
                used.insert(k % (N/d));
        ...
        ll dofs = (N/d) - used.size();
        ll scale = ((num % 2) ? (1) : (-1));
        out += scale * pow(mi(2), dofs);
        dbgR(used, dofs, scale, out);
        el;
    }
