...
void solve() {
    ...
    FOR(pr, 6, SQRT+1) {
        ll p = sieve.sp[pr];
        ll r = pr / p;
        if (!sieve.is_prime[r]) {continue;}
        el; dbgY(pr);
        ll qmax = N / (pr * pr);

        ll curr = lstTrue(0, sieve.primes.size()-1, [&](ll x) -> ll {
            return sieve.primes[x] <= qmax;
        });
        ll pi = lower_bound(all(sieve.primes), p) - sieve.primes.begin();
        ll ri = lower_bound(all(sieve.primes), r) - sieve.primes.begin();
