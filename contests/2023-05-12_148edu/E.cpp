...
const ll GENFAC_MAX = powll(10, 7) + 1;
...
void solve_with_fft() {
    ...
    V<mi> A = {a0};
    A.reserve(N);
    FOR(k, 1, N) { A.pb(((int)A[k - 1] * x + y) % m); }

    V<mi> Y; Y.reserve(N);
    FOR(k, 0, N) { Y.pb(binom(k+1, K)); }
    // dbgY(A);
    // dbgB(Y);
    // el;
...
void solve() {
    ...
    V<mi> B = {a0};
    B.reserve(N);
    FOR(j, 1, N) { B.push_back(B.back() + A[j]); }
    dbgY(A);
    dbgB(B);

    V<mi> Bnew(N);
    FOR(k, 1, K+1) {
        Bnew[0] = binom(1,k) * A[0];
        FOR(n, 1, N) {
            Bnew[n] = binom(1, k) * A[n]
                + Bnew[n-1] + B[n-1];
        }
        swap(Bnew, B);
        dbgR(k, B);
