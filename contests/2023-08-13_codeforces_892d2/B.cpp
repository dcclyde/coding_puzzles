...
void solve() {
    ...
    /**
        In most places, I'll score the second-smallest element per array.
        In one array, I'll instead score the global min.
     */

    ll score = 0;
    ll global_min = INF_ll;
    ll worst_second_smallest = INF_ll;
    for(auto& AA : A) {
        // add the second-smallest element.
        score += AA[1];

        ckmin(global_min, AA[0]);
        ckmin(worst_second_smallest, AA[1]);
    }

    score += global_min - worst_second_smallest;
    return ps(score);
}
