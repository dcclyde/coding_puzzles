...
void solve() {
    ...
    if (K <= 20) {
        ...
        for(auto& [p, e] : facs) {
            FOR(ee, 1, e+1) {
                if (p == 2 && ee == e) {break;}
                auto local = solve_one(K / powll(p, ee));
                if (local == boring) {break;}
                d *= p;
            }
        }
        dbgY(d);
        ...
        // handle all the "boring" stuff at once.
        ll num_copies = ((K % 2 == 0) ? (K/2 - d) : (K - d));
        out += num_copies * boring;
        dbgR(TIME(), MP(K,d), num_copies, out);
    }
