...
void brute() {
    ...
    FOR(start, 0, N) {
        ...
        if (end > N) {continue;}
        V<ll> curr;
        FOR(k, start, end) {curr.push_back(dat[k]);}
        FOR(k, 0, K/2) {out += (curr[k] != curr[curr.size()-1-k]);}
    }
    return ps(out);
}
...
void solve() {
    ...
    ll score = 0;
    FOR(parity, 0, 2) {
        el; el; dbgBold(parity);
        umap<ll,ll> cnt;

        FOR(j, parity, N, 2) {
            el;
            dbg(j, dat[j], cnt[dat[j]], cnt);


            // is there a chunk with middle at j-1?
            if (j-1 + S < N && j-1 - S >= 0) {++cnt[dat[j-2]]; dbgW(dat[j-2]);}

            // do I need to add an extra because I'm near the start?
            {
                ll proposed_middle = S;
                ll o = j - 2*(j - proposed_middle);
                if (proposed_middle != j-1 && o < j && j - o <= 2*S) {
                    ++cnt[dat[o]];
                    dbgB(dat[o]);
                }
            }
            ...
            // was there a chunk ending at j-2?
            if (j-2 - 2*S >= 0) {--cnt[dat[j-2-2*S]];}
    ...
    ll default_cost = (K-1)/2 * (N-K+1);
    ll out = default_cost - score;
    return ps(out);
}
