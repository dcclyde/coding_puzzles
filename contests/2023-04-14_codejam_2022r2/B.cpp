...
ll rootll(ll a, ll n) {  // nth root of a, rounded down. Assumes n >= 2, a >= 0.
    ll x = pow(a, 1./n);
    while (powll(x, n) > a) {--x;}
    while (powll(x+1, n) <= a) {++x;}
    dbgcB("rootll", MP(a,n), x);
    return x;
}

/*
    Say x = rootll(a, 2). Then x^2 <= a < (x+1)^2.
    I want to know whether (x+.5)^2 < a.
        x^2 + x + .25 < a
*/
// round a^(1/n) to the nearest integer. Break ties by rounding down.
ll sqrtll(ll a) {
    ll x = rootll(a, 2);
    if (x*x + x < a) {++x;}
    return x;
