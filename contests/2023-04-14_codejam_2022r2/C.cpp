...
/*
    Actually, maybe it will be better to treat ph as a desired assignment
        of kids to candies.
    Then iterate:
    Find a kid who can take his desired candy, and take it.
    If we ever fail, this assignment doesn't work.
*/
...
void solve() {
    ...
    // I'm only intending to solve the Small test set.
    // Let's just try all possible orders.
    ...
    auto dist_memo = ndvec<ll>(N, N+1, 0LL);
    FOR(k, 0, N) FOR(c, 0, N+1) {
        dist_memo[k][c] = dist(kids[k], candy[c]);
    }

    V<ll> ph;
    FOR(k, 0, N) ph.push_back(k+1);
    V<set<pll>> kid_to_options(N);
    FOR(k, 0, N) {
        FOR(c, 0, N+1) {
            kid_to_options[k].insert({dist_memo[k][c], c});
        }
    }

    V<ll> kid_order(N, -1);
    auto check = [&]() -> bool {
        auto options = kid_to_options;
        FOR(step, 0, N) {
            // find some kid who can take his desired candy.
            bool did_something = false;
            FOR(kid, 0, N) {
                if (options[kid].empty()) {continue;}
                ll desired_candy = ph[kid];
                ll shortest_distance = options[kid].begin()->first;
                if (shortest_distance == dist_memo[kid][desired_candy]) {
                    // This kid can take his desired candy.
                    // Take it.
                    did_something = true;
                    kid_order[step] = kid;
                    options[kid].clear();
                    FOR(other_kid, 0, N) {
                        ...
                        options[other_kid].erase({dist_memo[other_kid][desired_candy], desired_candy});
                    }
                    break;
                }
            }
            if (!did_something) {return false;}
        }
        return true;
    };
    ...
    bool win = false;
    do {
        if (check()) {win = true; break;}
    } while (next_permutation(ph.begin(), ph.end()));

    if (!win) {return IMPOSSIBLE;}
