...
void solve() {
    ...
    if (K % 2 != (N-1) % 2) {return IMPOSSIBLE;}

    using t3 = tuple<ll, ll, ll>;
    V<t3> corners;
    corners.emplace_back(1, 0, 0);
    while (true) {
        ll sz = corners.size();
        auto [n, r, c] = corners.back();
        auto [dc, dr] = stepsOrthogonal[(sz-1) % 4];
        // how far should we step?
        ll step = -1;
        if (sz == 1) {step = N-1;}
        else {step = N - sz/2;}
        if (step == 0) {break;}
        corners.emplace_back(n+step, r+step*dr, c+step*dc);
        dbg(n,MP(r,c), "", step, corners.back());
    }
    el; dbgY(pdh(corners)); el; el;

    // F(n) = the row and column where n appears.
    auto F = [&](ll n) -> pll {
        // Find the last corner with get<0>(corner) <= n.
        ll k = lstTrue(0, corners.size()-1, [&](ll x) {
            return get<0>(corners[x]) <= n;
        });
        auto [n0, r0, c0] = corners[k];
        if (n0 == n) {return MP(r0, c0);}
        auto [dc, dr] = stepsOrthogonal[k % 4];
        return MP(r0 + dr*(n-n0), c0 + dc*(n-n0));
    };
    ...
    auto G = [&](ll r, ll c) -> ll {
        ...
        ll delta = 0;
        ckmax(delta, abs(r-CENTER));
        ckmax(delta, abs(c-CENTER));

        /*
            If delta == CENTER then we are on the outermost ring. Start with k=0.
            If delta == CENTER-1 then we are on the next ring. Start with k=4.
        */
        ll k = 4 * (CENTER - delta);
        while (true) {
            // Is this k good?
            auto [n0, r0, c0] = corners[k];
            auto [dc, dr] = stepsOrthogonal[k % 4];
            dbgP(MT(n0,r0,c0), MP(dr,dc));

            // Can I get to (r, c) by starting at (r0, c0) and moving in the direction (dr, dc)?
            ll deltaR = r - r0;
            ll deltaC = c - c0;
            if (dc == 0 && dr > 0 && deltaC == 0 && deltaR > 0) {return n0 + deltaR;}
            if (dc == 0 && dr < 0 && deltaC == 0 && deltaR < 0) {return n0 - deltaR;}
            if (dc > 0 && dr == 0 && deltaC > 0 && deltaR == 0) {return n0 + deltaC;}
            if (dc < 0 && dr == 0 && deltaC < 0 && deltaR == 0) {return n0 - deltaC;}
    ...
    // How far should I walk before turning toward the center?
    ll min_distance = N-1;
    ll k = 0;
    ll turning_point = -1;
    while (true) {
        // If I move to the next corner, what will the min distance be?
        auto [n0, r0, c0] = corners[k];
        auto [n1, r1, c1] = corners[k+1];
        ll delta = n1 - n0;
        ll outward_steps = delta / 2;
        ll new_min_distance = min_distance + 2*outward_steps;
        if (new_min_distance > K) {
            turning_point = n0;
            turning_point += delta - (delta/2);
            turning_point += (K - min_distance) / 2;
            break;
        }
    ...
    dbgR(turning_point, min_distance); el;

    // Now we know the turning point. We can just walk from there.
    ll n = turning_point;
    V<pll> out;
    while (n != N*N) {
        auto [r, c] = F(n);
        V<ll> options;
        if (r > CENTER) {options.push_back(G(r-1, c));}
        if (r < CENTER) {options.push_back(G(r+1, c));}
        if (c > CENTER) {options.push_back(G(r, c-1));}
        if (c < CENTER) {options.push_back(G(r, c+1));}
        dbg(n, options);

        for(auto& new_n : options) {
            if (new_n > n) {
                if (new_n - n > 1) {out.emplace_back(n, new_n);}
                n = new_n;
                break;
            }
        }
