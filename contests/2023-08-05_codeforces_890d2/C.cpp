...
void solve() {
    ...
    ll out = 0;
    FOR(k, 0, N) {
        // find the best height achievable here.
        auto check = [&](ll H) -> bool {
            if (H <= A[k]) {return true;}
            ll steps_remaining = STEPS;
            bool good = false;
            FOR(j, k, N) {
                ll cost_here = H - A[j];
                if (cost_here <= 0) {good = true; break;}
                if (cost_here > steps_remaining) {break;}
                steps_remaining -= cost_here;
                H -= 1;
            }
            return good;
        };
        ll best_height = lstTrue(0, 1e9, check);
        ckmax(out, best_height);
    }
