...
void solve() {
    ...
    FOR(base, 0, N) {
        el; dbgY(base);
        // V<ll> subtree_sizes;
        set<pll, greater<pll>> wip;
        ll S = 0;
        for(auto& x : children[base]) {
            // subtree_sizes.emplace_back(hld.sz[x]);
            wip.emplace(hld.sz[x], x);
            S += hld.sz[x];
        }
        dbgB(wip);
        if (wip.size() == 0) {continue;}

        // ll S = sumv(subtree_sizes);
        // dbgB(S, subtree_sizes);
        while (wip.size() > 1) {
            dbg(wip);
            auto [a, v] = *wip.begin(); wip.erase(wip.begin());
            auto [b, w] = *wip.begin(); wip.erase(wip.begin());
            wip.emplace(a-b, v);
        }
        ll delta = wip.begin()->first;
        // ll L = S/2 - delta/2;
        // ll R = S - L;
        ll R = (S + delta) / 2;
        ll L = S - R;
        ...
        out += score;
#if 0
        // Use DP to answer: which sums can be achieved using a subset of subtree_sizes?
        // dp[n][s] = can I make sum s using a subset of the first n elements of subtree_sizes?

        // dp[n][s] = dp[n-1][s] || dp[n-1][s-subtree_sizes[n-1]]
        // dp[0][0] = true
        // dp[0][s] = false

        auto dp = ndvec<bool>(subtree_sizes.size()+1, S+1, false);

        // base cases
        dp[0][0] = true;

        FOR(n, 1, subtree_sizes.size()+1) {
            FOR(s, 0, S+1) {
                dp[n][s] = dp[n-1][s];
                if (s >= subtree_sizes[n-1]) {
                    dp[n][s] = (dp[n][s] || dp[n-1][s-subtree_sizes[n-1]]);
                }
            }
            dbg(dp[n]);
        }
