...
void precompute() {
    FOR(k, 2, powll(10,6)) {
        ll curr = 1 + k + k*k + k*k*k;
        while (true) {
            seen.insert(curr);
            // dbgP(k, curr);
            // timebomb(100);

            auto fut = __int128_t(curr) * k + 1;
            curr = fut;
            if (curr != fut) {break;}
        }
    }
...
void solve() {
    ...
    if (seen.size() == 0) {precompute();}
    if (seen.find(N) != seen.end()) {dbg(N); return YES;}

    // what about 1+k+k^2 case?
    ll quadk = lstTrue(0, powll(10,9), [&](ll k) {
        return 1+k+k*k <= N;
    });
    if (quadk >= 2 && 1 + quadk + quadk*quadk == N) {dbgY(N, quadk); return YES;}
    return NO;
}
