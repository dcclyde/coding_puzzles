...
void solve() {
    ...
    ll special_value;
    while (true) {
        // remove nothing and see if agg changes.
        A = ask(V<ll>());
        dbg(A);
        auto aend = agg();
        if (astart != aend) {
            // find which count increased.
            FOR(k, 0, 10) {
                if (astart[k] < aend[k]) {
                    special_value = k;
                    break;
    ...
    dbgY(special_value);
    V<ll> unspecial_positions;
    FOR(k, 0, N) {
        if (A[k] != special_value) {
            unspecial_positions.pb(k);
        }
    }
    A = ask(unspecial_positions);
