...
void solve() {
    ...
    {
        ll n1 = 0;
        FOR(k, 0, N) {n1 += (dat[k] == 1);}
        if (n1 == N) {return ps(0);}
        if (n1 > 0 && n1 < N) {return ps(-1);}
    }

    map<ll,V<ll>> work;
    FOR(k, 0, N) {work[dat[k]].push_back(k);}

    V<pll> out;
    while (work.size() > 1) {
        el; dbgY(work);
        auto it2 = work.end(); --it2;
        auto it = it2; --it;
        auto& [a, Va] = *it;
        auto& [b, Vb] = *it2;
        ll x = Va.back(); ll y = Vb.back();
        ll z = cdiv(b, a);

        out.emplace_back(y, x);
        work[z].push_back(y);
        // Va.pop_back();
        // if (Va.size() == 0) {work.erase(it);}
        Vb.pop_back();
        if (Vb.size() == 0) {work.erase(it2);}
        dbg(work);
    }
