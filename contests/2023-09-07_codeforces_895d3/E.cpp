...
template<class T, class S>
string tsdbg(LazySegXor<T, S> st) {
...
tcT, class S> struct LazySeg {
	...
	void push(int ind, int L, int R) { /// modify values for current node
        ...
        dbgB(ind, MP(L,R), seg[ind], lazy[ind], xorA[ind]);
        seg[ind] ^= xorA[ind];
		if (L != R) F0R(i,2) lazy[2*ind+i] ^= lazy[ind]; // ! lazy * lazy
		lazy[ind] = idS;
	} // recalc values for current node
...
void solve() {
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            ll1(l, r);
            queries.emplace_back(qtype, l, r);
        } else {
            lls(b);
            queries.emplace_back(qtype, b, -1);
        }
    }
    el;

    LazySegXor<ll, ll> stx;
    stx.init(N);
    FOR(k, 0, N) { stx.seg[stx.n + k] = A[k]; }
    stx.build();

    // xorA.assign(2*N, 0);
    // FOR(k, 0, N) {xorA[k+N] = A[k];}
    // ROF(k, 0, N) { xorA[k] = xorA[2*k] ^ xorA[2*k+1]; }
    xorA = stx.seg;
    ...
    V<ll> out;
    FOR(qid, 0, Q) {
        auto& [qtype, x, y] = queries[qid];
        if (qtype == 1) {
            dbg(x, y);
            st.upd(x, y, 1);
        } else {
            ll ans_ones = st.query(0, N-1);
            ll ans;
            if (x == 1) {ans = ans_ones;}
            else {
                ll ans_xor = stx.query(0, N-1);
                ans = ans_xor ^ ans_ones;
            }
            out.push_back(ans);
        }
    }
