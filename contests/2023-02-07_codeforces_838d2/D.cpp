...
ll ask(ll a, ll b) {
    if (memo.find(MP(a,b)) != memo.end()) {
        dbgcP("free", MP(a,b), memo[MP(a,b)]);
        return memo[MP(a,b)];
    }
    --steps_allowed;
    assert(steps_allowed >= 0);
    cout << "? ";
    ps(a, b);
    cout << flush;
    lls(out);
    memo[MP(a,b)] = out;
    memo[MP(b,a)] = out;
    dbgc("paid", steps_allowed, MP(a,b), memo[MP(a,b)]);
    return out;
}
void finish(ll a, ll b) {
    cout << "! ";
    ps(a, b);
    cout << flush;
    lls(response);
    if (response == -1) {exit(0);}  // wrong answer
    return;
}
...
void solve() {
    ...
    V<ll> dat, datnew;
    FOR(k, 1, N+1) {dat.push_back(k);}
    random_shuffle(all(dat));
    datnew = dat;
    ...
    while (dat.size() > 2) {
        ...
        bool done = false;
        while (!done) {
            random_shuffle(all(datnew));
            FOR(k, 0, datnew.size() / 2) {
                x = datnew[2*k]; y = datnew[2*k+1];
                g = ask(x, y);
                if (ckmax(gmin, g)) {done = true; break;}
            }
        }
        dbgB("phase 2", x, y, g);
        // now wipe out everything that's not divisible by g?
        ll target_size = (N-1) / g + 1;
        datnew.clear();
        datnew.push_back(x); datnew.push_back(y);
        for(auto& z : dat) {
            if (z == x || z == y) {continue;}
            ll gg = ask(x, z);
            if (gg < gmin) {continue;}
            else if (gg > gmin) {datnew.clear(); datnew.push_back(x); datnew.push_back(z); gmin = gg;}
            else {datnew.push_back(z);}
            if (datnew.size() == target_size) {break;}
        }
        dat = datnew;
        dbgY(gmin, dat);
    }

    dbgcY("gonna submit", dat);
    finish(dat.front(), dat.back());
