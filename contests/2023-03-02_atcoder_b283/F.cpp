...
tcT, class S> struct LazySeg {
    ...
    void assign(int idx, T val) {
        ll curr = query(idx, idx);
        upd(idx, idx, val - curr);
    }
...
void brute() {
    ...
    FOR(k, 0, N) {
        FOR(j, 0, N) {
            if (j == k) {continue;}
            ckmin(out[k], abs(P[k]-P[j]) + abs(k-j));
        }
    }
    return pv(out);
}
...
void solve() {
    ...
    // look right.
    {
        LazySeg<ll,ll> pbig; pbig.init(N);
        LazySeg<ll,ll> psmall; psmall.init(N);
        ROF(k, 0, N) {
            // update out[k].
            ll qbig = pbig.query(P[k]+1, N-1);
            ll cbig = qbig - P[k] - k;
            ckmin(out[k], cbig);

            ll qsmall = psmall.query(0, P[k]-1);
            ll csmall = qsmall + P[k] - k;
            ckmin(out[k], csmall);

            // update the segtrees.
            pbig.assign(P[k], P[k] + k);
            psmall.assign(P[k], -P[k] + k);
            dbgP(k, cbig, csmall);
            dbgY(pbig);
            dbgW(psmall);
            el;
