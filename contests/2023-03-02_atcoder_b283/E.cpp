...
void solve() {
    lls(R, C);
    V<V<ll>> dat(R);
    FOR(r, 0, R) {rv(C, dat[r]);}
    dbgR(MP(R, C), pdh(dat));
    el;

    auto check_row = [&](ll r) {
        set<ll> good_strategies;
        FOR(strategy, 0, 4) {
            bool ok = true;
            bool prev_diff = strategy % 2;
            bool next_diff = strategy / 2;
            FOR(c, 0, C) {
                if (c > 0 && dat[r][c] == dat[r][c-1]) {continue;}
                if (c < C-1 && dat[r][c] == dat[r][c+1]) {continue;}
                // c has no friends in this row. Check above and below.
                if (r > 0 && (prev_diff != (dat[r][c] == dat[r-1][c]))) {continue;}
                if (r < R-1 && (next_diff != (dat[r][c] == dat[r+1][c]))) {continue;}
                ok = false; break;
            }
            if (ok) {good_strategies.insert(strategy);}
        }
    ...
    // dp[n][b][s]
    auto dp = ndvec<ll>(R, 2, 4, INF_ll);

    // base case: n = 0.
    auto strats = check_row(0);
    {
        if (strats.count(0) && strats.count(2)) {
            dp[0][0][3] = 0;
            dp[0][1][3] = 1;
        } else if (strats.count(0)) {
            dp[0][0][1] = 0;
            dp[0][1][2] = 1;
        } else if (strats.count(2)) {
            dp[0][0][2] = 0;
            dp[0][1][1] = 1;
        }
    }

    auto prev_strats = strats;
    FOR(r, 1, R) {
        strats = check_row(r);
        el; dbgY(r, strats, prev_strats);
        FOR(b, 0, 2) {
            FOR(prev_b, 0, 2) {
                FOR(prev_s, 0, 4) {
                    // are these choices going to ruin the previous row?
                    if (prev_s != 3 && b+1 != prev_s) {continue;}

                    // what are our restrictions for the next row?
                    bool prev_same = (prev_b == b);
                    ll next_s = 0;
                    for(auto& mask : strats) {
                        if (mask % 2 == prev_same) {continue;}
                        ll next_allowed = (mask/2) ^ b;
                        next_s |= (1 << next_allowed);
                    }
                    ckmin(dp[r][b][next_s], dp[r-1][prev_b][prev_s] + b);
                    dbgP(MT(r-1,prev_b,prev_s), MT(r,b,next_s));
                }
            }
        }
        prev_strats = strats;
    ...
    ll out = INF_ll;
    FOR(b, 0, 2) {ckmin(out, dp[R-1][b][3]);}
    if (out == INF_ll) {return ps(-1);}
    return ps(out);
}
