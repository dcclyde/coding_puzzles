...
void solve() {
    ...
    ll MAX_DEPTH = *max_element(all(depth));
    dbg(MAX_DEPTH, depth);

    V<ll> subtree_depth(N);
    V<ll> subtree_size(N);
    V<ll> subtree_total_score(N);

    V<ll> cost_ending_here(N);
    V<ll> cost_ending_bottom(N);

    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        subtree_size[curr] = 1;
        if (curr == ROOT) {depth[curr] = 0;} else {depth[curr] = depth[parent] + 1;}
        subtree_depth[curr] = depth[curr];
        subtree_total_score[curr] += Q[curr];

        // ! (subtree size * 2, subtree answer)
        V<pll> subcosts_ending_here;
        V<pair<pll,ll>> subcosts_ending_maxdepth;
        for (auto& o : G[curr]) {
            if (o == parent) {continue;}
            self(o, curr, self);
            ...
            ckmax(subtree_depth[curr], subtree_depth[o]);
            subtree_total_score[curr] += subtree_total_score[o];

            if (RULE_VERSION == 1 && subtree_depth[o] == MAX_DEPTH) {
                subcosts_ending_maxdepth.emplace_back(MP(2 * subtree_size[o], subtree_total_score[o]), o);
            } else {
                subcosts_ending_here.emplace_back(2 * subtree_size[o], subtree_total_score[o]);
            }
            // dbgB(curr, o, subtree_depth[o]);

            cost_ending_here[curr] += cost_ending_here[o];
        }
        // update parent after all children


        auto cost_helper = [&](V<pll>& X) -> ll {
            ll out = 0;
            ll running = 1;
            if (!X.empty()) {out += X[0].s;}
            FOR(k, 1, X.size()) {
                running += X[k-1].f;
                out += running * X[k].s;
            }
            dbgcW("cost_helper", X, out);
            return out;
        };


        // ! compute cost_ending_here.
        if (RULE_VERSION == 1 && subcosts_ending_maxdepth.size() > 0) {
            sort(all(subcosts_ending_maxdepth), [](pair<pll,ll> a, pair<pll,ll> b) {
                return a.f.f * b.f.s < a.f.s * b.f.f;
            });
            FOR(k, 0, subcosts_ending_maxdepth.size() - 1) {
                auto& x = subcosts_ending_maxdepth[k];
                subcosts_ending_here.push_back(x.f);
            }
            if (subcosts_ending_here.size() >= 3) {
                // ^ in particular, don't try to sort the first -1 elements if list has size 0.
                sort(subcosts_ending_here.begin(), subcosts_ending_here.begin() + subcosts_ending_here.size() - 1, [](pll a, pll b) {
                    return a.f * b.s < a.s * b.f;
                });
            }
            cost_ending_bottom[curr] += cost_ending_here[curr];
            dbgcR("ceb 1", cost_ending_bottom[curr]);

            ll ch = cost_helper(subcosts_ending_here);
            cost_ending_bottom[curr] += ch;
            dbg(subcosts_ending_here, ch, cost_ending_bottom[curr]);
            dbgP(cost_ending_here[curr], cost_ending_bottom[curr]);

            auto& special = subcosts_ending_maxdepth.back();
            cost_ending_bottom[curr] += cost_ending_bottom[special.s];
            dbgcR("ceb 3", cost_ending_bottom[curr], special, cost_ending_bottom[special.s]);
            ll delay = 2 * (subtree_size[curr] - subtree_size[special.s]) - 1;
            cost_ending_bottom[curr] += delay * special.f.s;
            cost_ending_bottom[curr] -= cost_ending_here[special.s];
            dbgB(delay, special, cost_ending_bottom[curr]);

            subcosts_ending_here.push_back(special.f);
        }
        sort(all(subcosts_ending_here), [](pll a, pll b) {
            return a.f * b.s < a.s * b.f;
        });
        cost_ending_here[curr] += cost_helper(subcosts_ending_here);
        dbgY(curr, subcosts_ending_here, cost_ending_here[curr]);

        dbgP(depth[curr], subtree_size[curr], subtree_total_score[curr]);
        el;
        // dbgP(curr, parent, subtree_size[curr]);
    };
    dfs(ROOT, -1, dfs);
    ...
    if (RULE_VERSION == 0) {
        ll out_steps = 2*N - 2;
        ll out_cost = cost_ending_here[ROOT];
        return ps(out_steps, out_cost);
    } else {
        ll out_steps = 2*N - 2 - MAX_DEPTH;
        ll out_cost = cost_ending_bottom[ROOT];
        return ps(out_steps, out_cost);
    }
}
