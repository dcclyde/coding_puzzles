...
void solve() {
    ...
    auto digit_count_helpers = ndvec<ll>(10, N+1, 0LL);
    auto& dch = digit_count_helpers;
    FOR(k, 0, N) {
        FOR(d, 0, 10) {dch[d][k+1] = dch[d][k];}
        dch[dat[k]][k+1] += 1;
    }
    auto digit_count = [&](ll d, ll l, ll r) -> ll {
        return dch[d][r+1] - dch[d][l];
    };

    if (B < 100) {
        dbgcBold("subproblem 1");
        V<ll> per_digit(10);
        FOR(qid, 0, Q) {
            ll1(l, r);
            FOR(d, 0, 10) {per_digit[d] = digit_count(d, l, r);}
            mi out = 0;
            FOR(X, A, B+1) {
                if (X < 10) {
                    // * 1 digit.
                    mi cur = comb(per_digit[X], 1) * 2;  // put it on top or bottom.
                    out += cur;
                    dbg(X, cur, out);
                } else if (X % 11 == 0) {
                    // * 2 digits that're the same
                    ll d = X / 11;
                    mi cur = comb(per_digit[d], 2) * 2*2;
                    out += cur;
                    dbgP(X, cur, out);
                } else {
                    // * 2 distinct digits
                    ll d = X / 10; ll e = X % 10;
                    mi cur = per_digit[d] * per_digit[e] * 2;
                    out += cur;
                    dbgB(X, cur, out);
                }
            }
        ...
        dbgcBold("subproblem 2");
        V<ll> digs;
        while (A > 0) {digs.push_back(A%10); A/=10;}
        reverse(all(digs));
        dbgY(digs);
        ll S = digs.size();

        // F(l, r, a, b) tells how many ways to build digs[a, b] from dat[l, r].
        auto F = ndvec<mi>(N+1, N+1, S+1, S+1, mi(0));
        // handle [l, l] separately as a base case.
        FOR(l, 0, N) {
            FOR(a, 0, S) {
                if (dat[l] == digs[a]) { F[l][l][a][a] = 2; }
            }
        }
        FOR(biglen, 2, N+1) {
            FOR(l, 0, N) {
                ...
                FOR(a, 0, S) {
                    FOR(b, a, S) {
                        ...
                        mi cur2 = 0; mi cur3 = 0;
                        if (a < b && dat[r] == digs[b]) {
                            // OK to add dat[r] as the last element of digs[a..b].
                            cur2 = F[l][r-1][a][b-1];
                        }
                        if (a < b && dat[r] == digs[b]) {
                            // OK to add dat[r] as the first element of digs[a..b].
                            cur3 = F[l][r-1][a+1][b];
                        }
                        F[l][r][a][b] = cur1 + cur2 + cur3;
                        dbg(MP(l,r), MP(a,b), MT(cur1,cur2,cur3), F[l][r][a][b]);
                    }
                }
            }
        }
        ...
        // I'm not attempting to solve the full problem right now.
        V<ll> out = {2, 18, 34};
        return pvn(out);
    }
    return;
