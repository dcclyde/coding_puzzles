...
void solve() {
    ...
    auto ways = [&](ll r0, ll c0, ll r1, ll c1) -> mi {
        return binom(r1-r0 + c1-c0, r1-r0);
    };

    // dp[r][c] = num ways to arrive at (r, c) having passed through a YY.
    // If I pass through multiple YY then it counts more times.
    auto dp = ndvec<mi>(R, C, mi(0));
    mi out = 0;
    ...
    FOR(r, 0, R) {
        FOR(c, 0, C) {
            ...
            if (r > 0) {
                dp[r][c] += dp[r-1][c];
                if (dat[r][c] == 'Y' && dat[r-1][c] == 'Y') {
                    cur += ways(0,0,r-1,c);
                    out += 2 * dp[r-1][c] * ways(r, c, R-1, C-1);
                }
            }
            if (c > 0) {
                dp[r][c] += dp[r][c-1];
                if (dat[r][c] == 'Y' && dat[r][c-1] == 'Y') {
                    cur += ways(0,0,r,c-1);
                    out += 2 * dp[r][c-1] * ways(r, c, R-1, C-1);
                }
            }
            out += cur * ways(r, c, R-1, C-1);
            dp[r][c] += cur;
            dbgY(MP(r, c), dp[r][c], cur, out);
        }
    }
