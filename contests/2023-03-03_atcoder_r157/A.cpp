...
void brute() {
    lls(N, XX, XY, YX, YY);
    dbgR(N, N, XX, XY, YX, YY);
    el;

    ll MASK_MAX = (1<<N);
    FOR(mask, 0, MASK_MAX) {
        auto bit = [&](ll b) {return (mask>>b) & 1;};
        ll xx = 0, xy = 0, yx = 0, yy = 0;
        FOR(k, 0, N-1) {
            if (bit(k) && bit(k+1)) {++yy;}
            else if (bit(k) && !bit(k+1)) {++yx;}
            else if (!bit(k) && bit(k+1)) {++xy;}
            else {++xx;}
        }
        if (xx == XX && xy == XY && yx == YX && yy == YY) {
            dbgR(mask, bitset<20>(mask));
            return Yes;
        }
    }
    return No;
...
void solve() {
    lls(N, XXo, XYo, YXo, YYo);
    dbgR(N, N, XXo, XYo, YXo, YYo);
    el;

    FOR(start, 0, 2) {
        el; dbgY(start);
        ll prev = start;
        ll XX = XXo; ll XY = XYo; ll YX = YXo; ll YY = YYo;
        while (true) {
            dbg(prev, MT(XX, XY, YX, YY));
            if (prev == 0 && XX > 0) {XX = 0; continue;}
            if (prev == 1 && YY > 0) {YY = 0; continue;}

            if (prev == 0 && XY == 0) {break;}
            if (prev == 1 && YX == 0) {break;}

            if (prev == 0) {--XY; prev = 1;}
            else {--YX; prev = 0;}
        }
        if (XX + XY + YX + YY == 0) {return Yes;}
    }
