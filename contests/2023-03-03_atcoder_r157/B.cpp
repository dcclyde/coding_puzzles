...
void brute() {
    lls(N, K);
    strings(dat_raw);
    V<ll> dat(N); FOR(k, 0, N) {dat[k] = (dat_raw[k] == 'Y');}
    dbgR(N, K, dat_raw, dat);
    el;
    ...
    V<ll> wip(N);
    FOR(mask, 0, MASK_MAX) {
        if (pct(mask) != K) {continue;}
        FOR(b, 0, N) {wip[b] = ((mask>>b) & 1) ^ dat[b];}
        ll cur = 0;
        FOR(k, 0, N-1) {cur += (wip[k] && wip[k+1]);}
        if (ckmax(out, cur)) {dbg(cur, mask, bitset<20>(mask));}
    }
    return ps(out);
}
...
void solve() {
    ...
    V<V<ll>> end_blocks(2);
    {
        ...
        while (L < N) {
            ll R = L;
            while (R+1 < N && dat[R+1] == dat[L]) {++R;}
            if (L == 0 || R == N-1) {
                end_blocks[dat[L]].push_back(R-L+1);
            } else {
                blocks[dat[L]].push_back(R-L+1);
            }
            L = R+1;
        }
    }
    // blocks[0].push_back(2*N);
    FOR(k, 0, 2) {sort(all(blocks[k]));}
    dbgY(pdh(blocks));

    ll ones_final = cnt[1] + K;
    if (ones_final > N) {ones_final -= 2 * (ones_final - N);}
    dbg(ones_final);
    if (ones_final <= 1) {return ps(0);}
    // now score will be ones_final - num connected components of 1s.
    // ! watch out: 0s at the start and end don't really count against us.

    if (K == 0) {ll out = ones_final - blocks[1].size() - end_blocks[1].size(); return ps(out);}

    if (cnt[0] > K) {
        dbgcP("abc");
        // we can't turn all 0s into 1s.
        ll blocks_fixed = 0;
        for(auto& block : blocks[0]) {
            if (K >= block) {K -= block; ++blocks_fixed;}
            else {break;}
        }
        ll out = ones_final - (blocks[0].size()+1) + blocks_fixed;
        return ps(out);
    } else {
        dbgcB("def");
        // turn all 0s into 1s. Then we need to turn some 1s back into 0s.
        K -= cnt[0];
        for(auto& blk : end_blocks[1]) {K -= blk;}
        ll blocks_ruined = 0;
        reverse(all(blocks[1]));
        for(auto& block : blocks[1]) {
            if (K <= 0) {break;}

            if (K >= block) {K -= block; ++blocks_ruined;}
            else {K = 0; ++blocks_ruined; break;}
        }
        ll out = ones_final - (blocks_ruined + 1);
        return ps(out);
    }
}
