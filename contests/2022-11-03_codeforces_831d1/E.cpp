...
void solve() {
    ...
    /**
        For each vertex, I can choose EITHER height(v) OR sum of children values.
     */
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        ...
        for (auto& o : G[curr]) {
            ...
            sum_child_values += score[o];

        }
        // update parent after all children
        // todo
        score[curr] = max({height[curr], sum_child_values});
