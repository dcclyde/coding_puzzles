...
void solve() {
    ...
    ll B = sqrt(N) + 1;
    map<ll,ll> mdat;
    for(auto& x : dat) {++mdat[x];}

    V<ll> start_state(B);
    for(auto& [k, v] : mdat) {
        if (v < B) {++start_state[v];}
        else {start_state.push_back(v);}
    }
    sort(start_state.begin() + B, start_state.end());
    reverse(start_state.begin() + B, start_state.end());
    ll nnz = mdat.size();
    dbgY(B, nnz, start_state);

    map<tuple<ll,ll,V<ll>>, mi> dp;

    auto subsolve = [&](auto&& ss, auto&& packed_outer) -> mi {

        auto ssinner = [&](auto packed) -> mi {
            auto& [NNZ, try_size, state] = packed;
            dbg_only(auto pkdbg = packed);
            dbgcB("START", pkdbg);
            if (NNZ == 0) {return 1;}
            if (try_size <= 0) {return 0;}

            mi out = 0;
            assert(try_size <= NNZ);

            // option 1: don't use try_size.
            --try_size;
            out += ss(ss, packed);
            ++try_size;
            ...
            // option 2: do use it.
            /**
                Need to subtract 1 each from the top "try_size" items.
             */
            ll todo = try_size;
            dbgc("2", packed);
            {
                ...
                ll ptr = B;
                ll from_above = 0;
                while (ptr < state.size() && todo > 0) {
                    if (state[ptr] == B) { state.pop_back(); ++state[B-1]; ++from_above; --todo; }
                    else { --state[ptr]; --todo; ++ptr; }
                }
                dbgP(ptr, todo, state);
                if (todo == 0) {
                    // fix the sorting.
                    if (ptr < state.size() && state[ptr-1] > state[ptr]) {
                        ll other = ptr;
                        while (other+1 < state.size() && state[other+1] == state[ptr]) {++other;}
                        swap(state[ptr-1], state[other]);
                    }
                } else {
                    ROF(b, 1, B) {
                        ll done_here = min({todo, state[b] - from_above});
                        todo -= done_here;
                        from_above = done_here;

                        state[b] -= done_here;
                        if (b == 1) {NNZ -= done_here;}
                        else { state[b-1] += done_here; }

                        if (todo == 0) {break;}
                    }
                }
                // always need try_size <= NNZ.
                ckmin(try_size, NNZ);
            }
            out += ss(ss, packed);

            dbgcR("END", pkdbg, packed);
            return out;
        };

        // timebomb(100);

        if (dp.find(packed_outer) == dp.end()) { dp[packed_outer] = ssinner(packed_outer); }
        dbgY(dp[packed_outer], packed_outer); el;
        return dp[packed_outer];
    };

    mi result = subsolve(subsolve, MT(nnz, nnz, start_state));
    ps(int(result));
    return;
}
