...
void solve() {
    ...
    mi prev = 1;
    FOR(n, 1, N+1) {
        if (sieve.is_prime[n]) {primorial *= n;}
        // watch out for overflow.
        if (primorial > B) {primorial = B+1;}

        ll options_here = B / primorial;
        mi curr = prev * options_here;
        out += pow(mi(B), n) - curr;
        prev = curr;
        dbgY(n, primorial, options_here, curr, out);
    }
