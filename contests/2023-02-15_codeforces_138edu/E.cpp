...
void brute() {
    ...
    V<string> dat(R);
    FOR(r, 0, R) {cin >> dat[r];}
    dbgcR("br", MP(R,C), pdh(dat));
    el;

    auto in_grid = [&](ll r, ll c) {return 0 <= r && r < R && 0 <= c && c < C;};
    auto flat = [&](ll r, ll c) {return R*c + r;};
    auto unflat = [&](ll f) -> pll {return MP(f % R, f / R);};

    ll N = R * C;
    ll best_cost = 2e18;
    FOR(mask, 0, (1LL << N)) {
        auto wip = dat;
        auto has_adjacent = [&](ll r, ll c) {
            for(auto& [dr, dc] : stepsOrthogonal) {
                ll rrr = r+dr; ll ccc = c+dc;
                if (!in_grid(rrr, ccc)) {continue;}
                if (wip[rrr][ccc] == '#') {return true;}
            }
            return false;
        };
        ll cost = 0;
        ...
        FOR(n, 0, N) {
            ...
            auto [r, c] = unflat(n);
            // is this an OK place to drop a #?
            if (wip[r][c] == '#') {ok = false; break;}
            if (has_adjacent(r, c)) {ok = false; break;}
            wip[r][c] = '#';
            ++cost;
        }
        if (!ok) {continue;}

        // is there now a valid path?
        DSU dsu; dsu.init(N);
        FOR(r, 0, R) FOR(c, 0, C) {
            if (wip[r][c] == '#') {continue;}
            for(auto& [dr, dc] : stepsOrthogonal) {
                ll rr = r+dr; ll cc = c+dc;
                if (!in_grid(rr, cc)) {continue;}
                if (wip[rr][cc] == '#') {continue;}
                dsu.unite(flat(r,c), flat(rr,cc));
            }
        }
        bool good = true;
        FOR(c1, 0, C) FOR(c2, 0, C) {
            if (dsu.sameSet(flat(0, c1), flat(R-1, c2))) {good = false; break;}
        }
        if (!good) {continue;}
        if (ckmin(best_cost, cost)) {
            dbg(cost, pdh(wip));
        }
    }
    if (best_cost >= 1e17) {return NO;}
    else {YES; return ps(best_cost);}
}
...
void solveOld() {
    ...
    V<Dijkstra<ll, false>> solvers(2);
    // first solve for "white" cells. (r+c) % 2 == 0
    ll N = R*C;
    ll SOURCE = N; ++N;
    ll TARG = N; ++N;
    const ll COST_SCALE = 100'000'000;
    FOR(parity, 0, 2) {
        auto& solver = solvers[parity];
        solver.init(N);
        FOR(c, 0, C-1) {
            FOR(r, (c+parity) % 2, R, 2) {
                if (has_adjacent(r, c)) {continue;}
                ...
                for(auto dir : {-1, 1}) {
                    ...
                    ll cc = c+1;
                    if (!in_grid(rr, cc)) {continue;}
                    // is this vertex even legal?
                    if (has_adjacent(rr, cc)) {continue;}
                    ll cost = (dat[rr][cc] == '.');
                    cost = (cost == 1) ? (COST_SCALE) : (1);
                    solver.ae(flat(r,c), flat(rr,cc), cost);
                }
            }
        }
        // add edges from source to left edge.
        {
            ll c = 0;
            FOR(r, (c+parity) % 2, R, 2) {
                if (has_adjacent(r, c)) {continue;}
                ll cost = (dat[r][c] == '.');
                cost = (cost == 1) ? (COST_SCALE) : (1);
                solver.ae(SOURCE, flat(r,c), cost);
            }
        }
        // add edges from right edge to end.
        {
            ll c = C-1;
            FOR(r, (c+parity) % 2, R, 2) {
                if (has_adjacent(r, c)) {continue;}
    ...
    // which parities work if any?
    ll parity = 0; ll total_cost = solvers[0].dist[TARG];
    if (ckmin(total_cost, solvers[1].dist[TARG])) {parity = 1;}
    if (total_cost >= 1e18) {return NO;}

    // ok, now I guess I need to build a good path.
    auto& solver = solvers[parity];
    V<ll> path = {TARG};
    while (path.back() != SOURCE) {
        // find an OK next step.
        ll cur = path.back();
        for(auto& [prop, stepcost] : solver.adj[cur]) {
            if (solver.dist[prop] + stepcost == solver.dist[cur]) {
                path.push_back(prop);
                break;
            }
        }
    ...
    // now start actually editing dat.
    FOR(k, 1, path.size() - 1) {
        auto& f = path[k];
        auto [r, c] = unflat(f);
        dat[r][c] = '#';
    ...
    YES;
    // return pvn(dat);
    ll tempout = solvers[parity].dist[TARG] / COST_SCALE;
    dbgcBold("fix output before submit!", tempout);
    return ps(tempout);
}
...
void solve() {
    ...
    DijkstraWithPath<ll, true> solver;
    ll N = R*C;
    ll SOURCE = N++;
    ll TARG = N++;
    solver.init(N);

    // edges to first column
    FOR(r, 0, R) {
        ll c = 0;
        if (has_adjacent(r, c)) {continue;}
        ll cost = (dat[r][c] == '.');
    ...
    FOR(r, 0, R) FOR(c, 0, C-1) {
        for(auto dir : {-1, 1}) {
            ll rr = r+dir; ll cc = c+1;
            if (!in_grid(rr, cc) || has_adjacent(r, c) || has_adjacent(rr, cc)) {continue;}
            solver.ae(flat(r,c), flat(rr,cc), (dat[rr][cc] == '.'));
            solver.ae(flat(rr,cc), flat(r,c), (dat[r][c] == '.'));
        }
    }

    solver.gen(SOURCE);

    if (solver.dist[TARG] >= 1e17) {return NO;}

    V<ll> path = {TARG};
    while (path.back() != SOURCE) { path.push_back(solver.prev[path.back()]); }
    FOR(k, 1, path.size() - 1) {
        auto [r, c] = unflat(path[k]);
        dat[r][c] = '#';
    }
    YES;
    for(auto x : path) {
        if (x == SOURCE || x == TARG) {dbgY(x);}
        else {dbg(x, unflat(x));}
    }
    // return ps(solver.dist[TARG]);
    return pvn(dat);
}
