...
/**
    K = 4.
    Stage 1: remove something <= 4, then add 4 to something.
    Stage 2: remove something <= 3, then add 3 to something.

    If I want K to work, then:
        Need at least 2*J - 1 elements <= J for J = 1, 2, ..., K?

    -----

    OK no that doesn't fully cover it.
    Enemy will just delete 1s the whole time.
    So if K is gonna work, then I need the condition above AND at least K 1s.

    Enemy deletes K-1 1s
    I delete:
        a 1
        something <= 2
        something <= 3
...
void solve() {
    ...
    //     ll prop = out+1;
    //     running += mdat[prop];
    //     if (running >= 2*(out+1) - 1 && mdat[1] >= out+1) {++out;}
    //     else {break;}
    // }
    // return ps(out);
    ...
    ROF(K, 0, N+1) {
        ...
        ROF(j, 1, K+1) {
            dbgP(j, wip);
            {
                // delete the largest element <= j.
                auto it = wip.upper_bound(j);
                if (it == wip.begin()) {ok = false; break;}
                --it;
                if (it->second == 1) {wip.erase(it);}
                else {--it->second;}
            }
            dbg(wip);
            {
