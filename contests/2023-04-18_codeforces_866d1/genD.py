...
def gen_test():
    ...
    SHOPS = ri(SHOPS_RANGE)
    # ATYPES = ri(ATYPES_RANGE)
    ATYPES = ri((1, SHOPS//2))
    ...
    num_mystery = ri(MYSTERY_RANGE)
    # randomly select num_mystery positions from 1..SHOPS, without replacement
    mystery_pos = set(random.sample(range(SHOPS), num_mystery))
    # mystery_pos = {10, 25}

    for shopid in range(SHOPS):
        # mystery = ri((1,5))
        # if mystery == 1:
        if shopid in mystery_pos:
            ps(0)
        else:
            contained = rv((0, 1), ATYPES)
            data = [k+1 for k in range(ATYPES) if contained[k] == 1]
            data = [len(data)] + data
