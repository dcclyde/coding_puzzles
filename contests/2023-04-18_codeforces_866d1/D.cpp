...
void brute() {
    lls(STALLS, APPLE_TYPES);
    V<V<ll>> dat(STALLS);
    V<ll> unk;
    FOR(k, 0, STALLS) {
        lls(N);
        if (N == 0) {unk.push_back(k);}
        else {rv1(N, dat[k]);}
    }
    dbgR(MP(STALLS, APPLE_TYPES), unk, pdh(dat));
    el;
    ...
    auto simulate = [&]() -> ll {
        ...
        FOR(n, 0, STALLS) {
            for(auto& x : dat[n]) {
                if (apples.find(x) != apples.end()) { apples.clear(); break;}
                else {apples.insert(x);}
            }
        }
        dbgc("sim", apples.size(), apples, pdh(dat));
        return apples.size();
    };

    ll BITS = unk.size() * APPLE_TYPES;
    ll MASK_MAX = (1LL << BITS);
    ll out = 0;
    FOR(mask, 0, MASK_MAX) {
        for(auto& u : unk) {dat[u].clear();}
        FOR(b, 0, BITS) {
            if (mask & (1LL << b)) {
                ll u = b / APPLE_TYPES;
                ll a = b % APPLE_TYPES;
                dat[unk[u]].push_back(a);
            }
        }
        ll local = simulate();
        ckmax(out, local);
...
void solve() {
    ...
    uset<ll> from_prev;
    ll initial_endpoint = STALLS;
    if (unk.size() > 0) { initial_endpoint = unk[0]; }
    FOR(n, 0, initial_endpoint) {
        for(auto& x : dat[n]) {
            if (from_prev.find(x) != from_prev.end()) { from_prev.clear(); break; }
            else {from_prev.insert(x);}
        }
    }

    if (unk.size() == 0) { return ps(from_prev.size()); }
    if (unk.back() == STALLS-1) {return ps(APPLE_TYPES);}

    // List positions where each apple occurs.
    V<V<ll>> apple_pos(APPLE_TYPES);
    FOR(s, 0, STALLS) {
        for(auto& x : dat[s]) {
            apple_pos[x].push_back(s);
        }
    }

    auto next_pos = [&](ll start, ll apple) -> ll {
        ll idx = fstTrue(0, apple_pos[apple].size()-1, [&](ll x) {
            return apple_pos[apple][x] > start;
        });
        if (idx == apple_pos[apple].size()) { return STALLS; }
        return apple_pos[apple][idx];
    };

    V<ll> last_reset(STALLS, STALLS);
    V<ll> first_reset(STALLS, STALLS);
    FOR(u, 0, unk.size()) {
        // Figure out what we should do at mystery stall u.
        ll n = unk[u];
        ll future = STALLS;
        if (u < unk.size()-1) {future = unk[u+1];}
        el; dbgY(u, MP(n, future), from_prev);

        first_reset[future-1] = future;
        last_reset[future-1] = future;
        ROF(k, n+1, future-1) {
            dbgB(k);
            // compute first_reset[k].
            first_reset[k] = first_reset[k+1];
            for(auto& x : dat[k]) {
                ll next = next_pos(k, x);
                dbgP(x, next);
                if (next < future) { ckmin(first_reset[k], next); }
            }
            // compute last_reset[k].
            if (first_reset[k] >= future) {last_reset[k] = first_reset[k];}
            else {
                if (first_reset[k] + 1 >= future || last_reset[first_reset[k]+1] >= future) {
                    last_reset[k] = first_reset[k];
                } else {
                    last_reset[k] = last_reset[first_reset[k]+1];
                }
            }
            dbg(k, first_reset[k], last_reset[k]);
        }

        if (u == unk.size()-1) {break;}

        ll latest_reset = n;
        FOR(k, n+1, future) {
            ll last_reset_helper = last_reset[k];
            if (last_reset_helper >= future) {last_reset_helper = k-1;}
            ckmax(latest_reset, last_reset_helper);
            dbgP(k, last_reset[k], last_reset_helper, latest_reset);

            // If there's a forced reset at k-1, then break.
            bool need_break = false;
            // test for a forced reset due to conflict with from_prev.
            for(auto& x : dat[k-1]) {
                if (from_prev.find(x) != from_prev.end()) {
                    need_break = true;
                    break;
                }
            }
            // test for a forced reset due to conflict within this block.
            if (first_reset[n+1] <= k-1) {need_break = true;}

            if (need_break) {break;}
            ckmax(latest_reset, k);
        }

        dbg(latest_reset, future);
        from_prev.clear();
        FOR(k, latest_reset+1, future) {
            for(auto& x : dat[k]) {from_prev.insert(x);}
        }
    }

    // OK, now we need separate handling for the very last mystery shop.
    dbgcBold("final chunk");
    ll n = unk.back();
    dbg(n, last_reset[n+1]);
    if (last_reset[n+1] >= STALLS) {
        dbgcR("NO reset");
        // We are NOT forced to reset during the final chunk.
        // Test whether from_prev overlaps with the final chunk.
        uset<ll> final_chunk;
        FOR(k, n+1, STALLS) {
            for(auto& x : dat[k]) {final_chunk.insert(x);}
        }
        for(auto& x : from_prev) {
            if (final_chunk.find(x) != final_chunk.end()) {
                return ps(final_chunk.size());
            }
        }
        return ps(APPLE_TYPES);
    } else {
        dbgcR("YES reset", pdh(last_reset));
        // We ARE FORCED to reset during the final chunk.
        // We want to reset as early as possible.
        ll earliest_last_reset = last_reset[n+1];
        FOR(k, n+1, STALLS) {
            ll last_reset_helper = last_reset[k];
            if (last_reset_helper >= STALLS) {last_reset_helper = k-1;}
            ckmin(earliest_last_reset, last_reset_helper);
            dbgP(k, earliest_last_reset);

            // If a forced apple would make us reset here, then break.
            bool need_break = false;
            for(auto& x : dat[k-1]) {
                if (from_prev.find(x) != from_prev.end()) {
                    need_break = true;
        ...
        dbgY(earliest_last_reset, dat[earliest_last_reset]);
        from_prev.clear();
        FOR(k, earliest_last_reset+1, STALLS) {
            for(auto& x : dat[k]) {
                from_prev.insert(x);
                dbg(k, x, from_prev);
            }
        }
        return ps(from_prev.size());
    }
}
