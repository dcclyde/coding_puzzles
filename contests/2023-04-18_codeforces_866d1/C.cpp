...
void solve() {
    ...
    bool ok = true;
    {
        auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
            ...
            for (auto& o : G[curr]) {
                ...
                // update parent per child
                ckmax(height[curr], height[o] + 1);
                if (height[o] > 0) {
                    if (fat_child[curr] == -1) {fat_child[curr] = o;}
    ...
    V<ll> chain = {ROOT};
    while (fat_child[chain.back()] != -1) {
        chain.emplace_back(fat_child[chain.back()]);
    }

    V<ll> out;
    FOR(k, 0, chain.size()) {
        ll curr = chain[k];
        if (k % 2 == 0) {out.push_back(curr);}
        else {
            for(auto& o : G[curr]) {
                if (o == par[curr]) {continue;}
                if (o == fat_child[curr]) {continue;}
                out.push_back(o);
            }
        }
    }
