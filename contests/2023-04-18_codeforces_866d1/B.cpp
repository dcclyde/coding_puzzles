...
void solve() {
    ...
    set<pll> options;
    if (area % rmax == 0) {options.emplace(rmax, area/rmax);}
    if (area % cmax == 0) {options.emplace(area/cmax, cmax);}
    if (options.size() == 1) {
        ps(1);
        ps(*options.begin());
        return;
    ...
    auto check = [&](ll R, ll C) -> bool {
        el; el; el; dbgcBold("check", MP(R, C));
        umap<ll, uset<ll>> r2rects;
        umap<ll, uset<ll>> c2rects;
        FOR(k, 0, N) {
            r2rects[dat[k].f].insert(k);
            c2rects[dat[k].s].insert(k);
        }
        dbgY(pdh(r2rects));
        dbgY(pdh(c2rects)); el;
        ll ctr = 0;
        while (true) {
            dbg(ctr, R, C, r2rects[R], c2rects[C]);
            if (r2rects[R].empty() && c2rects[C].empty()) {break;}
            for(auto& rect : r2rects[R]) {
                auto [r, c] = dat[rect];
                C -= c;
                ++ctr;
                c2rects[c].erase(rect);
                dbgP(rect, MP(r,c), MP(R,C), ctr);
            }
            r2rects[R].clear();
            for(auto& rect : c2rects[C]) {
                auto [r, c] = dat[rect];
                R -= r;
                ++ctr;
                r2rects[r].erase(rect);
                dbgB(rect, MP(r,c), MP(R,C), ctr);
            }
            c2rects[C].clear();
        }
        return ctr == N;
    ...
    V<pll> out;
    for(auto& [R, C] : options) {
        if (check(R, C)) {out.emplace_back(R, C);}
    }
