...
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{0, 0};  // ! identity
    T cmb(T a, T b) {
        T out;
        if (a.f == b.f) {out = MP(a.f, a.s+b.s);}
        else {out = max(a, b);}
        if (out.f == 0) {out.s = 1;}
        return out;
    }  // ! seg * seg
	...
	T query(int l, int r) {	// associative op on [l, r]
        if (l > r) {return ID;}
        dbg_only(ll lorig = l; ll rorig = r;);
		T ra = ID, rb = ID;
        ...
        dbg_only(
            auto out = cmb(ra,rb); dbgcP("query", MP(lorig,rorig), out);
        );
		return cmb(ra,rb);
	}
...
void brute() {
    ...
    auto dp = ndvec<pair<mi,bool>>(N, N+1, MP(mi(0),false));
    FOR(n, 0, N) {dp[n][1] = MP(1, true);}
    FOR(n, 0, N) {
        FOR(b, 1, N+1) {
            if (!dp[n][b].s) {continue;}
            el; dbgY(n, b, dp[n][b]);
            FOR(fut, n+1, N) {
                // Is this a legal next step?
                if (b % B != 0 && dat[fut] != dat[n]) {continue;}
                dbg(fut);

                dp[fut][b+1].f += dp[n][b].f;
                dp[fut][b+1].s = true;
            }
        }
    }
    ...
    FOR(n, 0, N) {
        FOR(b, 1, N+1) {
            if (dp[n][b].s) {ckmax(big_b, b);}
        }
    }
    // round big_b down to nearest multiple of B.
    big_b = (big_b / B) * B;

    if (big_b == 0) {return ps(1);}
...
void solve() {
    ...
    V<pair<ll, mi>> dp(N, MP(0, mi(1)));
    // handle dp[0] by hand.
    if (B == 1) {return ps(1);}  // edge case. Use every position.

    SegTree<pair<ll,mi>> st;
    st.init(N);
    dp[0] = MP(0, mi(1));

    // st.upd(0, dp[0]);
    FOR(k, 0, N) {st.seg[k + st.n] = dp[0];}
    st.build();

    FOR(n, 1, N) {
        // dp[n] = dp[n-1];
        // If I make a block ending here, what would be the start index?
        ll color = dat[n];
        ll ci = lower_bound(all(c2pos[color]), n) - c2pos[color].begin();
        el; dbgY(n, color, ci);
        if (ci >= B-1) {
            // // pair<ll,mi> option = {dp[start].f + 1, dp[start].s};
            // pair<ll,mi> option = {1, 1};
            // if (start >= 0) {option = dp[start]; ++option.f;}

            FOR(starti, 0, ci-B+2) {
                ll start = c2pos[color][starti];
                auto option = st.query(0, start-1);
                if (option.f == 0) {option.s = 1;}
                ++option.f;
                mi bnm = binom(ci - starti - 1, B-2);
                option.s *= bnm;
                dp[n] = st.cmb(dp[n], option);
                dbg(starti, start, bnm, option, dp[n]);
            }
            // ll start = c2pos[color][ci-B+1] - 1;
            // auto option = st.query(0, start);
            // option.s += (option.s == 0);
            // ++option.f;
            // dbg(ci, start, option);
            // dp[n] = option;
    ...
    // if (dp[N-1].f == 0) {dbgc("no blocks"); return ps(1);}
    // return ps(dp[N-1].s);

    auto out = st.query(0, N-1);
    if (out.f == 0) {dbgc("no blocks"); return ps(1);}
    return ps(out.s);
}
