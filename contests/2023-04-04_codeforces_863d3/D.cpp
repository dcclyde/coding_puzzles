...
bool precompute() {
    fib = {1, 1};
    while (fib.size() < FIB_SIZE) {
        fib.push_back(fib.back() + fib[fib.size() - 2]);
    }
    return true;
}
auto dummy = precompute();
...
void solve() {
    ...
    while (C > 1) {
        // I'm about to delete a square with sidelength R.
        if (c < R) {c = C-1-c;}

        if (c < R) {return NO;}

        // Delete this square, and rotate the new rectangle so it looks like the old one.
        ll newC = R;
        ll newR = C - R;
        ll newc = r;
        ll newr = c - R;
