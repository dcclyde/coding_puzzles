...
void solve() {
    ...
    ll K = 0;
    while (K*K < N) {++K;}
    if (K*K != N) {dbgc("no"); return NO;}
    if (K * (K+1) != E) {dbgc("no"); return NO;}
    if (K < 3) {dbgc("no"); return NO;}
    ...
    if (dsu.size(0) != N) {dbgc("no"); return NO;}

    V<V<ll>> two_neighbors(N);
    V<V<ll>> four_neighbors(N);
    FOR(k, 0, N) {
        for (ll o : G[k]) {
            if (deg[o] == 2) {two_neighbors[k].emplace_back(o);}
            else if (deg[o] == 4) {four_neighbors[k].emplace_back(o);}
            else {dbgc("no"); return NO;}
        }
        if (deg[k] == 4 && !(two_neighbors[k].size() == 2)) {dbgc("no"); return NO;}
        if (deg[k] == 2 && !(two_neighbors[k].size() == 1 || two_neighbors[k].size() == 2)) {dbgc("no"); return NO;}
    }

    bool found_4s = false;
    V<bool> vis(N);
    FOR(start, 0, N) {
        el; dbgY(start, found_4s, vis);
        if (vis[start]) {continue;}
        if (deg[start] == 4) {
            if (found_4s) {dbgc("no"); return NO;}
            found_4s = true;
            vis[start] = true;
            queue<ll> todo;

            {
                ll nxt = four_neighbors[start][0];
                todo.push(nxt);
                vis[nxt] = true;
            }
            ll loop_size = 1;
            ...
            while (!todo.empty()) {
                dbg(loop_size, todo);
                if (todo.size() != 1) {dbgc("no"); return NO;}
                ll cur = todo.front(); todo.pop();
                ++loop_size;
                recent = cur;
                for (ll nxt : G[cur]) {
                    if (deg[nxt] != 4) {continue;}
                    if (vis[nxt]) {continue;}
                    vis[nxt] = true;
                    todo.push(nxt);
                }
            }
            if (loop_size != K) {dbgc("no"); return NO;}
            if (recent != four_neighbors[start][1]) {dbgc("no"); return NO;}
        } else {
            queue<ll> todo;
            todo.push(start);
            vis[start] = true;
            ...
            while (!todo.empty()) {
                ...
                ++loop_size;
                for (ll nxt : G[cur]) {
                    if (deg[nxt] != 2) {others_found.push_back(nxt); continue;}
                    if (vis[nxt]) {continue;}
                    vis[nxt] = true;
                    todo.push(nxt);
                }
            }
            if (loop_size != K-1) {dbgc("no"); return NO;}
            if (others_found.size() != 2) {dbgc("no"); return NO;}
            if (others_found[0] != others_found[1]) {dbgc("no"); return NO;}
        }
    }
