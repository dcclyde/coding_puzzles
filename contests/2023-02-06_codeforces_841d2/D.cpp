...
/**
    Looks like binary search stuff.
    Is there an LxL square of 1s in some given binary matrix?
    That should just be DP.
    dp[r][c] = biggest square starting here.
 */
...
void solve() {
    ...
    dbgR(MP(R,C), pdh(dat));
    el;
    auto dp = ndvec<ll>(R+1, C+1, 0LL);

    auto binsearch_helper = [&](ll L) {
        // base case
        FOR(c, 0, C) {dp[R][c] = 0;}
        FOR(r, 0, R) {dp[r][C] = 0;}
        ...
        ROF(r, 0, R) {
            ROF(c, 0, C) {
                if (dat[r][c] < L) {dp[r][c] = 0; continue;}
                dp[r][c] = min({
                    dp[r+1][c] + 1,
                    dp[r][c+1] + 1,
                    dp[r+1][c+1] + 1
                });
                if (dp[r][c] >= L) {return true;}
            }
        }
        return false;
    };

    ll out = lstTrue(1, minll(R,C)+1, binsearch_helper);
