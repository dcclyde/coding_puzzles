...
// #pragma GCC optimize("Ofast")  // unsafe
...
/**
    A is big.
    How many contiguous subsets will XOR to a perfect square?

    Values are <= 2e5 too. That means xors are bounded by 2^18,
        so we only care about s^2 for s < 2^9 = 512.

    Seems like a variant of that interview problem we sometimes used.
    If the current running xor is X, then I wonder how many prefixes have
        Y ^ X = s^2 for some s.
        so, look up X in a hash table that stores all s^2 ^ Y?
 */
...
void solve() {
    ...
    FOR(k, 0, 1000) {
        if (k*k > big) {break;}
        if (k >= 600) {assert(false);}
        squares.push_back(k*k);
    }
    dbgY(squares);

    ll out = 0;
    // umap<ll,ll> dp;  // this version barreelllly passes, or sometimes gets TLE.
    V<ll> dp(big+1);  // this version passes easily.
    dp[0] = 1;
    ...
    FOR(k, 0, N) {
        ...
        // dbgP(k, running, dp);
        for(auto& s : squares) {
            // if (dp.find(running^s) != dp.end()) {
                out += dp[running ^ s];
                // dbgB(s, dp[running^s], out);
            // }
        }
        dbg(k, dat[k], running, out);
