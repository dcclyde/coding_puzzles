...
void solve() {
    ...
    vecs(t4, K, constraints_raw);
    el;
    ++R; ++C;
    dbgR(R, C);

    uset<pll> first_check;  // help check that we don't have "direct" contradictions.
    using t3 = tuple<ll,ll,ll>;
    V<t3> constraints_processed(K);
    FOR(k, 0, K) {
        auto& [r1, c1, r2, c2] = constraints_raw[k];
        auto& [r, c, b] = constraints_processed[k];

        r = min(r1, r2);
        c = min(c1, c2);
        b = ((r1 - r2) == (c1 - c2));
    }
    dbgY(pdh(constraints_processed)); el;

    V<V<t3>> constraints_processed_bycol(C);
    FOR(k, 0, K) {
        auto& [r, c, b] = constraints_processed[k];
        dbg(r, c, b);
        if (first_check.find(MP(r,c)) != first_check.end()) {
            return NO;
        }
        first_check.insert(MP(r,c));
        constraints_processed_bycol[c].eb(r, c, b);
    }
    dbg(pdh(constraints_processed_bycol));

    /*
        Make a graph where each node represents one row in the grid.
        My grid is (R-1) x (C-1) because I care about the slashes BETWEEN the letters.

        First build connected components of "same" stuff.
        For each column c, for each j in {0, 1}, join together all rows
            that have K_{r,c} = j.

        Next, make sure the "must be different" pieces are different.
    */
    ...
    auto representatives = ndvec<ll>(C-1, 2, -1LL);
    FOR(b, 0, 2) {
        FOR(c, 0, C-1) {
            ll r = -1;
            for(auto& [rr, cc, bb] : constraints_processed_bycol[c]) {
                if (b == 0 && c == 4) {
                    dbgR(rr, cc, bb, r);
                }
                if( bb != b ) { continue; }
                if (r == -1) { r = rr; continue; }
                dbgcW("unite", r, rr);
                dsu.unite(r, rr);
            }
            representatives[c][b] = r;
        }
    }
    dbgP(pdh(representatives));

    // Now build the final "must not equal" graph and check if it's 2-colorable.

    V<V<ll>> G(R-1);
    FOR(c, 0, C-1) {
        ll r0 = representatives[c][0];
        ll r1 = representatives[c][1];
    ...
    FOR(k, 0, R-1) {
        ...
        todo.push(k);
        while(!todo.empty()) {
            ll curr = todo.front(); todo.pop();
            for(auto o : G[curr]) {
                if (color[o] == -1) {
                    color[o] = 1 - color[curr];
                    todo.push(o);
                } else if (color[o] == color[curr]) {
                    return NO;
                }
            }
        }
