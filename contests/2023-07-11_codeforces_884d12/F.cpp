...
void brute() {
    ...
    do {
        ll curr_score = score();
        if (ckmin(best_score, curr_score)) { best = A; }
...
void solve() {
    ...
    while (L < N) {
        ll R = L;
        dbgY(L, R); timebomb(10);
        while (R < N-1 && A[R] - A[R+1] < D) {++R;}

        bool done = false;
        while (!done) {
            done = true;
            FOR(k, L+1, R-1) {  // left edge of the pair I'm considering.
                ll a = A[k-1];
                ll b = A[k];
                ll c = A[k+1];
                ll d = A[k+2];
