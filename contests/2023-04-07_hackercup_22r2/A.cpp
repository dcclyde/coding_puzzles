...
void solve() {
    ...
    // pc[n] tells about contents of 0..n-1
    V<a26> prefix_counts(N+1);
    for ( int i = 0 ; i < N ; ++i ) {
        prefix_counts[i+1] = prefix_counts[i];
        prefix_counts[i+1][dat[i]-'a']++;
    }

    auto count_range = [&](int L, int R) -> a26 {
        a26 out = prefix_counts[R+1];
        FOR(i, 0, 26) out[i] -= prefix_counts[L][i];
        return out;
    };

    auto first_has_one_extra = [&](a26& A, a26& B) -> bool {
        int diff = 0;
        FOR(i, 0, 26) {
    ...
    FOR(qid, 0, Q) {
        ...
        ll len = R-L+1;
        el; dbgY(qid, MP(L, R), out);
        if (len % 2 == 1) {
            // ~ Odd subcase.
            ll mid = L + len/2;  // e.g. L = 3, R = 5, len = 3, mid = 4
            auto left_half = count_range(L, mid-1);
            auto right_half = count_range(mid+1, R);
            ll c = dat[mid] - 'a';
            if (left_half[c] == right_half[c]) {
                // I need to remove c from one side. That means others should match.
                out += (left_half == right_half);
                continue;
            } else if (abs(left_half[c] - right_half[c]) >= 2) {
                // no hope.
                continue;
            } else {
                if (left_half[c] > right_half[c]) {swap(left_half, right_half);}
                // left_half will receive c and lose one other character.
                ++left_half[c];
                out += first_has_one_extra(left_half, right_half);
                continue;
            }
        } else {
            // ~ Even subcase.
            // ? Update: Apparently they want to just never count these.
            continue;

            ll right_of_mid = L + len/2;
            auto left_half = count_range(L, right_of_mid-1);
            auto right_half = count_range(right_of_mid, R);

            bool good = false;

            --right_half[dat[right_of_mid]-'a'];
            // is left half - something = right half?
            good |= first_has_one_extra(left_half, right_half);
            ++right_half[dat[right_of_mid]-'a'];
