...
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{-1,0,0,0};  // ! identity
    /*
        a is on the left of b.
    */
    T cmb(T a, T b) {
        //  return a+b;
        auto& [a00,a01,a10,a11] = a;
        auto& [b00,b01,b10,b11] = b;

        if (a00 == -1) {return b;}
        if (b00 == -1) {return a;}

        t4 out;
        auto& [c00,c01,c10,c11] = out;
...
void solve() {
    ...
    rv(N, dat);
    dbgR(N, dat);
    dbgR("edited N to N-1!");

    lls(Q);
    V<pll> queries;
    rv(Q, queries);
    for(auto& [i, v] : queries) {--i;}
    el;

    SegTree<t4> st;
    st.init(N);
    FOR(k, 0, N) {
        st.seg[st.n + k] = MT(0,0,0,dat[k]);
    }
    st.build();
    ...
    // {
    //     ll L = 0; ll R = 1;
    //     auto [q00,q01,q10,q11] = st.query(L, R);
    //     dbg(MP(L,R), MT(q00,q01,q10,q11));
    // }
    ...
    FOR(qid, 0, Q) {
        el;
        auto& [idx, val] = queries[qid];
        dat[idx] = val;
        dbgY(qid, MP(idx,val), dat);
        S += val - get<3>(st.seg[st.n + idx]);
        st.upd(idx, MT(0,0,0,val));
        auto [q00,q01,q10,q11] = st.query(0, N-1);
        ll helper = q00;
        ll result = 2 * (S - helper);
        dbg(qid, MP(idx,val), S, MT(q00,q01,q10,q11), helper, result);
        ps(result);
    }
