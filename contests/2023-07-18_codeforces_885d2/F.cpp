...
void solve() {
    ...
    if (sumv(A) == 0) {return ps(0);}

    ll out = 0;
    V<ll> proposed(N);
    for (ll step = N/2; step > 0; step >>= 1) {
        // build the new "proposed" array.
        bool zeros = true;
        FOR(k, 0, proposed.size()) {
            proposed[k] = A[k] ^ A[(k+step) % N];
            if (proposed[k] != 0) {zeros = false;}
        }
        dbgY(step, out, A, proposed);
        if (!zeros) {
            out += step;
            A = proposed;
        }
