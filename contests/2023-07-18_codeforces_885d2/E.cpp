...
void solve() {
    ...
    for (auto& q : queries) {
        ...
        for(auto& [p, e] : qfacs) {
            if (p == 2) {continue;}
            // need to multiply out by (facs[p]+e+1) / (facs[p]+1).
            ll num = facs[p] + e + 1;
            ll denom_inv = powmod(facs[p]+1, MOD-2, MOD);
            out = (out * num) % MOD;
            out = (out * denom_inv) % MOD;
            facs[p] += e;
            dbg(MP(p,e), num, denom_inv, out, facs);
        }
        ans.push_back(out);
    }
