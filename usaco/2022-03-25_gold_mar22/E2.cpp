...
#pragma region  // st first
#if 0
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{INF, -INF, 0};
    T cmb(T a, T b) {
        auto& [amin, amax, an] = a;
        auto& [bmin, bmax, bn] = b;
        return {min(amin, bmin), max(amax, bmax), an + bn};
    }
    ...
    int first_good(int lo, int hi, int apple_r, int ind=1, int l=0, int r=-1) {
        // return first leaf idx with value >= apple_r.
        if ( r == -1 ) {r = n-1;}

        // if this segment is doomed, return.
        auto& [mini, maxi, ni] = seg[ind];
        if ( r < lo || l > hi ) {
            return -1;  // we don't overlap the needed range.
        }
        if ( apple_r > maxi) ) {
            // everything in this node has cr too small.
            return -1;
        }
        // ok now find first (or any) node with cr =

    }

	int first_at_most(int lo, int val, int ind=1, int l=0, int r=-1) { // if seg stores min across range
		if ( r == -1 ) {
			r = n-1;
		}
		if (r < lo || val < seg[ind].first) return -1;
		if (l == r) return l;
		int m = (l+r)/2;
		int res = first_at_most(lo,val,2*ind,l,m); if (res != -1) return res;
		return first_at_most(lo,val,2*ind+1,m+1,r);
	}
...
tcT> struct SegTree { // cmb(ID,b) = b
	...
	void init(int _n) { // upd, query also work if n = _n
		...
		seg.assign(2*n,ID); orig_n = _n;}
	// void pull(int p) { seg[p] = cmb(seg[2*p],seg[2*p+1]); }
	void upd(int p, pii val) { // set val at position p
        p += n;
        auto& [crnew, nnew] = val;
        auto [crold, nold] = *seg[p].begin();
		seg[p].clear(); seg[p][crnew] = nnew;
        for (p /= 2; p; p /= 2) {
            // update seg[p].
            // pull(p);
            seg[p][crold] -= nold;
    ...
    const pii failout{-1, -1};
    pii first_good(int lo, int hi, int ar, int ind=1, int l=0, int r=-1) {
        if ( r == -1 ) {r = n-1;}
        if ( r < lo || l > hi ) {return failout;}
        auto best = seg[ind].lower_bound(ar);
        if ( best == seg[ind].end() ) { return failout; }
        if ( l == r ) { return MP(l, seg[ind][l]); }
        int m = (l+r)/2;
        if ( seg[2*ind].count(best->first) == 1 ) {
            return first_good(lo, hi, ar, 2*ind, l, m);
        } else {
            return first_good(lo, hi, ar, 2*ind+1, m+1, r);
        }
    }
	// T query(int l, int r) {	// associative op on [l, r]
	// 	T ra = ID, rb = ID;
...
/*
    pos - time = "power right". "How far right I could be at t=0".
        can be negative.
    pos + time = "power left".
    To make a catch, I need the cows' (pl, pr) to fully contain the apples' (pl, pr).
*/
...
void solve() {
    ...
    set<int> compress;
    for ( int q = 0 ; q < Q ; ++q ) {
        int type, time, pos, num;
        cin >> type >> time >> pos >> num;
        compress.insert(pos - time);
        compress.insert(pos + time);
        if ( type == 1 ) {
            cows.emplace_back(pos - time, pos + time, num);
        } else {
            apples.emplace_back(pos - time, pos + time, num);
        }
    }
    map<int,int> inverse;
    int chelper = 0;
    ...
    for ( auto& [a,b,c] : apples ) {
        a = inverse[a];
        b = inverse[b];
    }
    // ok, now x coords are all between 0 and... 4e5 I guess.
    ...
    SegTree<map<int,int>> st;
    st.init(inverse.size());
    for ( auto& [a, b, n] : cows ) {
        st.upd(b, MP(a, n));
    }

    ll out = 0;
    for ( int aptr = 0 ; aptr < apples.size() ; ++aptr ) {
        /*
            Find the weakest cow group that can grab these apples.
            I want to minimize cr among groups with cl <= al.
            Maybe that's a segment tree?
            nodes correspond to cl.
            Store num cows and cr. Aggregate by min cr?
            If there are no cows here, just set cr = INF.

            I'll query [-INF, al].
            I need first cr that's at least ar.
            I need SMALLEST cr that's at least ar.

            I need cl in [-inf, al] and then minimal cr in [ar, inf].

        */
        auto& [ar, al, an] = apples[aptr];
        while ( an > 0 ) {
            pii fg = st.first_good(0, al, ar);
            auto& [fgpos, fgnum] = fg;
            if ( fgpos == -1 ) {
                // can't match these apples.
                dbgc("break", aptr, apples[aptr]);
                break;
            }

            // match as many as we can.
            auto& [cr, cn] = *st.seg[st.n + fgpos].begin();
            int local = min(an, cn);
            dbg(aptr, apples[aptr], fg, MP(cr,cn), st);
            an -= local;
            out += local;
            dbgP(local, out);
            if ( cn == local ) {
                // delete this cow group.
                st.upd(fgpos, MP(-INF, 0));
            } else {
                cn -= local;
            }
        }
