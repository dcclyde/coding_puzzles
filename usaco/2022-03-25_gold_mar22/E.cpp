...
void solve() {
    ...
    for ( auto& [a,b,c] : cows ) {
        // if I already have
        dbgc("insert cow", a, b, c);
        dbg(a, inverse[a], ptrs[inverse[a]]);
        int ia = inverse[a];
        a = ia + ptrs[ia];
        ++ptrs[ia];
        dbg(b, inverse[b], ptrs[inverse[b]]);
        int ib = inverse[b];
        b = ib + ptrs[ib];
        ++ptrs[ib];
    }
    for ( auto& [a,b,c] : apples ) {
        dbgcP("insert apple", a,inverse[a], b,inverse[b], c);
        dbgP(ptrs[inverse[a]], ptrs[inverse[b]]);
        a = inverse[a];
        b = inverse[b] - ptrs[inverse[b]];
    }
    ...
    SegTree<t3> st;  // (min, min position, n)
    st.init(chelper);
    for ( auto& [a, b, n] : cows ) {
        st.upd(b, MT(a, b, n));
    }
    ...
    for ( int aptr = 0 ; aptr < apples.size() ; ++aptr ) {
        /*
            ...
            No, I need SMALLEST cr that's at least ar.
        ...
        */
        ...
        while ( an > 0 ) {
            int cmin, cminpos, cn;
            while ( true ) {
                tie(cmin, cminpos, cn) = st.query(0, al);
                if ( cmin == -INF ) {
                    break;
                }
                if ( cmin < ar ) {
                    // wipe out these cows, they're useless.
                    dbgc("wipe out cows", cmin, cminpos, cn);
                    st.upd(cminpos, st.ID);
                } else {
                    break;
                }
            }
            ...
            an -= local;
            out += local;
            st.upd(cminpos, MT(cmin, cminpos, cn - local));
            dbg(MT(ar, al, an), local, out, st);
