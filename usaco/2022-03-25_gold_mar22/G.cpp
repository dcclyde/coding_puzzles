...
void solve(bool verbose) {
    ...
    vector<int> vals(N);
    function<t3(int, int)> Dfs = [&](int curr, int rootval) {
        auto& [l, r] = ranges[curr];
        vals[curr] = min(max(rootval, l), r);
        t3 out;
        auto& [score, subtree_min, subtree_max] = out;
        score = abs(vals[curr] - rootval);
        subtree_min = subtree_max = vals[curr];

        for ( int prop : children[curr] ) {
            auto [pscore, pmin, pmax] = Dfs(prop, rootval);
            ckmin(subtree_min, pmin);
            ckmax(subtree_max, pmax);
            ckmax(score, pscore);
        }
        ckmax(score, vals[curr] - subtree_min);
        ckmax(score, vals[curr] - subtree_max);
        return out;
    };

    function<int(int)> get_score = [&](int rootval) {
        auto [score, tree_min, tree_max] = Dfs(0, rootval);
        dbgP("g_s", rootval, score);
        return score;
    };
    ...
    for ( int curr = lo ; curr <= hi ; ++curr ) {
        if ( ckmin(best, get_score( curr )) ) {bestVal = curr;}
    }
    dbg(bestVal);
    ps(best);
    if ( verbose ) {
