...
/**
    If I couldn't give cash, I'd just use a heap.
    Process both types of events and give out the cheapest possible gift.

    In this case, I think it's still a heap, except that
        if there are K 3s then present 3 could be given up to K+1 times.
 */
...
void solve() {
    ...
    el;
    V<ll> fv(P);
    for(auto& x : friends) {++fv[x];}

    map<ll,ll> working;
    ll out = 0;

    FOR(pidx, 0, P) {
        dbg(pidx, fv[pidx], out, working);
        // put these into the heap.
        working[presents[pidx]] += fv[pidx] + 1;

        // now start popping stuff.
        FOR(g, 0, fv[pidx]) {
            auto& [wcost, wnum] = *working.begin();
            dbgcP("give gift", wcost);
            out += wcost;
            if (wnum == 1) {
                working.erase(wcost);
            } else {
