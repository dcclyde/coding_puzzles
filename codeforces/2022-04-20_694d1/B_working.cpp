...
inline namespace Helpers {
    ...
    tcT> constexpr bool is_iterable_v = is_iterable<T>;
...
struct custom_hash {
    ...
    // template<typename A>
    // size_t operator()(const A& v) const {
    //     uint64_t out = 0;
    //     uint64_t offset = 1;
    //     for (const auto& x : v) {
    ...
    //     return out;
    // }
    // template<typename A, typename enable_if<is_iterable_v<A>>::type* = nullptr>
    // size_t operator()(A& v) const {
    // size_t operator()(enable_if<is_iterable_v<A>,A>::type& v) const {

    // template<typename A>
    // typename enable_if<is_iterable<A>,size_t>::type operator()(A& v) const {
    template<typename A>
    typename enable_if<is_iterable_v<A>,size_t>::type operator()(A& v) const {
        uint64_t out = 0;
        uint64_t offset = 1;
        for(const auto& x : v) {
            uint64_t curr = (*this)(x);
...
inline namespace ToString {
    ...
    tcT> typename enable_if<needs_output_v<T>,str>::type ts(T v) {
        return "{"+ts_sep(v,", ")+"}"; }
    // tcT> typename enable_if<needs_output_v<T>,ostream&>::type operator<<(ostream& os, T& v) {
    //     return os << ts(v);}
...
// template <typename T, typename enable_if<is_iterable_v<T>>::type* = nullptr>
// ostream& operator<<(ostream& os, const T& v) {
//     os << to_string(v); return os;
// }
// template<typename T, typename enable_if<!is_printable<T>::value,str>::type* = nullptr>
template<typename T, typename enable_if<!is_printable<T>::value,str>::type* = nullptr>
ostream& operator<<(ostream& os, T& v) {return os << ts(v);}
...
void brute() {
    ...
    auto good = [&](ll a, ll b) {
        ll G = gcd(a, b);
        ll L = a*b / G;
        // is L/G a perfect square?
        ll targ = (a/G) * (b/G);
        ll sqrt_maybe = fstTrue(0, 2e9, [&](ll x){return x*x >= targ;});
        bool out = sqrt_maybe * sqrt_maybe == targ;
        // dbgcP("g", a, b, out);
        return out;
    };
    ...
    FOR(qid, 0, Q) {
        ...
        FOR(step, 0, q) {
            wipnew.assign(N, 1);
            FOR(k, 0, N) FOR(j, 0, N) {
                if (good(wip[k], wip[j])) {wipnew[k] *= wip[j];}
            }
            swap(wip, wipnew);
        }
        dbg(wip);
...
void solve() {
    ...
    umap<V<int>, int> state;
    for(auto& x : dat) {
        map<ul,int> factors;
        factor_rec(x, factors);
        V<int> helper;
        for(auto& [p, e] : factors) {
            if (e % 2 == 1) { helper.push_back(p); }
        }
        ++state[helper];
    }
    // dbg(state);
    ...
    ll v0 = -1;
    ll v1 = 0;
    for(auto& [vvv, count] : state) {
        ckmax(v0, count);
        if (count % 2 == 0 || vvv.size() == 0) {v1 += count;}
    }
    dbg(v0, v1);
    ckmax(v1, v0);
    dbg(v0, v1);
