...
/**
    If N < 500:
    Just waste 500 guesses to hit equilibrium, then ask everybody.
    Find the biggest "jump" from dat[k] to dat[k+2] and then answer k+1.
    Remember the wraparound.

    If N >= 500:
    Two contaminated areas. One >K, one <K.
    Initially they're size 0, then 1, then 2, ...
    I think I can query 1, 1, 3, 6, 10, 15, 21, ...
    1 1  if it was originally 0 or 2 I'd know. I wouldn't know if it's 1 though.
    0 1  if it was originally 0 I'd know. I wouldn't know 1.
    0 1 3  now I'd know if it was 1.
    0 1 3 6
 */
...
void solve() {
    ...
    if (N <= 300) {
        ll wait = min(1000 - N - 1, N);
        rep(wait) {ask(0);}
        V<ll> dat(N);
        FOR(k, 0, N) { dat[k] = ask(k); }
        dat.push_back(dat[0]);
        dat.push_back(dat[1]);
        ll out = -1;
        ll big_jump = -2e18;
        FOR(k, 0, N) {
            ll curr_jump = dat[k+2] - dat[k];
            if (ckmax(big_jump, curr_jump)) {
                out = (k+1) % N;
            }
        }
        return finish(out);
    ...
    ll found = K;
    while (found == K) {
        asked.push_back(asked.back() + asked.size() - 1);
        found = ask(asked.back());
    }

    ll start = asked.back();
    if (found < K) {
        // jump forward from here.
        ll step_needed = fstTrue(0, N-1, [&](ll x) {
            return ask((start + x) % N) >= K;
        });
        return finish((start + step_needed) % N);
    } else {
        // jump back.
        ll step_needed = fstTrue(0, N-1, [&](ll x) {
            return ask((start - x + N)%N) <= K;
        });
        return finish((start - step_needed + N) % N);
    }
