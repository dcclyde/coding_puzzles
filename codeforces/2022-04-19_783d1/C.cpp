...
vector<pii> steps6dirs = {{1,0},{0,1},{-1,0},{0,-1},{1,1},{-1,-1}};
...
void learn() {
    ...
    auto do_something = [&]() {
        ...
        V<V<bool>> seen(N, V<bool>(N, false));
        for(auto& [r, c] : curr) {
            for(auto& [dr, dc] : steps6dirs) {
                int rr = r; int cc = c;
        ...
        int ctr = 0;
        FOR(r, 0, N) FOR(c, 0, N) {ctr += seen[r][c];}
        if (ctr == N*N) {
            dbgY(curr);
            solved = true;
            timebomb(20);
    ...
    function<void(void)> product_rec_helper = [&]{
        if (curr.size() == num_queens) {return do_something();}  // ! define max depth
        int n = curr.size();
        FOR(k, 0, N*N) {  // ! iterate over options at depth n
            curr.emplace_back(k/N, k%N);
            product_rec_helper();
            curr.pop_back();
        }
    };
...
void solve() {
    ...
    // Find Q such that 2*(N-Q) + 1 <= Q.
    ll Q = fstTrue(0, N, [&](ll q){return 2*(N-q) - 1 <= q;});
    dbgR(N, Q);
    ll K = N - Q;
    ll diags_total = 2*K - 1;
    ll diags_per_side = K;

    V<pll> out;
    FOR(q, 0, (Q+1)/2) {
        out.emplace_back(q, 2*q);
        dbg(q, out);
    }

    if (Q % 2 == 0) {
        FOR(q, 0, Q/2) {
            out.emplace_back(Q-1-q, Q-1-2*q);
            dbgR(q, out);
        }
    } else {
        FOR(q, 0, Q/2) {
            out.emplace_back(Q-1-q, Q-2-2*q);
            dbgP(q, out);
        }
    }
