...
ll subsolve_brute(ll v, ll targ) {
    // dbg(v, targ);
    ll out = 0;
    while (out * v <= targ) {
        ++out;
...
void brute() {
    ...
    FOR(center, 0, N) {
        ...
        ROF(k, 0, center) {
            /**
             * I need wip[k] < wip[k+1].
             * x * dat[k] > -wip[k+1]
             */
            ll v = dat[k];
            ll targ = -wip[k+1];
            ll x = subsolve_brute(v, targ);
            wip[k] = -x*v;
            cost += x;
        }
        FOR(k, center+1, N) {
...
ll subsolve_again(ll v, ll targ) {
    ll out = targ / v - 3;
    while (out * v <= targ) {
        ++out;
    }
...
void solve() {
// dbg_only(
//     std::uniform_int_distribution<int> v_dist(1, 1000);  // inclusive
//     std::uniform_int_distribution<int> targ_dist(0, 1'000'000'000);  // inclusive
//     rep(1'000'000) {
//         ll v = v_dist(rng);
//         ll targ = targ_dist(rng);
//         ll a = subsolve(v, targ);
//         ll b = subsolve_again(v, targ);
//         if (a != b) {
//             dbg(v, targ, a, b);
//             assert(false);
//         }
