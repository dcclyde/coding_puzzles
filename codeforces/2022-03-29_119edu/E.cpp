 ...
 * Verification: *
 */
umap<int,int> c2h;  // "color to head"
umap<int,int> h2c;  // head to color
...
void solve() {
    ...
    rep(Q) {
        ...
        if ( qtype == 1 ) {
            ++n1;
            ints(x);
            queries.emplace_back(qtype, x, -1);
            ckmax(MAX, x);
        } else {
            ints(x, y);
            queries.emplace_back(qtype, x, y);
            ckmax(MAX, x);
            ckmax(MAX, y);
        }
    }
    ...
    h2c.clear();
    /*
        Initially, let's say vertex k is assigned to color -k.
    */
    FOR(k, 0, n1) {
        h2c[k] = -k;
        c2h[-k] = k;
    ...
    for ( auto& [qtype, x, y] : queries ) {
        ...
        if ( c2h.find(x) == c2h.end() ) {
            dbgc("no x");
            continue;
        }
        if ( c2h.find(y) == c2h.end() ) {
            dbgc("no y");
            h2c[c2h[x]] = y;
            c2h[y] = c2h[x];
            c2h.erase(x);
        ...
        int curr_head = c2h[x];
        int target_head = c2h[y];
        dsu.unite(curr_head, target_head);
        int new_head = dsu.get(curr_head);
        dbgP(curr_head, target_head, new_head);
        if ( new_head == curr_head) {
            swap(curr_head, target_head);
        }
        c2h[y] = target_head;
        c2h.erase(x);
        h2c[target_head] = y;
