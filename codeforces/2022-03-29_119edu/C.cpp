...
void solve() {
    ...
    while ( idx >= 0 ) {
        ...
        int consec_stars = 1;
        while ( idx - consec_stars >= 0 && dat[idx - consec_stars] == '*' ) {
            ++consec_stars;
        }
        int max_bs_here = K * consec_stars;
        int nb = X % (max_bs_here + 1);
        orev += string(nb, 'b');
        X /= (max_bs_here + 1);
        dbg(consec_stars, max_bs_here, nb, X);
