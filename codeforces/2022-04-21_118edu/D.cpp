...
/**
    Repeats are allowed, but otherwise I want
    0 1 2 3 4.... k
    OR
    0 1 2 3 4.... k k+2 (and then we could repeat k and/or k+2 forever).
 */
...
void solve() {
    ...
    /**
        First handle the case without jumps.
        dp[k] = ways to end with a k based on what we've seen so far.
        f[n] = answer for the first n elements?

        How many ways are there if this is our "jump"?
        Should just be 2^(number of ks + number of k+2s in prefix)?
     */
    V<ll> count_per_value(N+1);
    auto& cpv = count_per_value;
    for(auto& x : dat) {++cpv[x];}
    ...
    FOR(n, 1, N+1) {
        ...
        // handle cases where this is our "jump".
        if (k >= 1) {
            dbg(k, dp);
            mi lhs = (k == 1) ? (1) : (dp[k-2]);
            ll rhs_exp = cpv[k];
            if (k > 1) { rhs_exp += cpv[k-2]; }
            mi rhs = pow(mi(2), rhs_exp);
            mi contrib = lhs * rhs;
            out += contrib;
        }

        dp[k] *= 2;  // if I already have 0..k, I could take this one or not.
        if (k == 0) {dp[k] += 1;}  // if I already have 0..k-1, I could take this.
        else {dp[k] += dp[k-1];}
    }
    el;
    dbgY(dp);
    FOR(k, 0, N) { out += dp[k]; } ps(out);
    return;
}
