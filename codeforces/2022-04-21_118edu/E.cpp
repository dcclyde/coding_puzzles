...
void solve() {
    ...
    pll root = {-1, -1};
    FOR(r, 0, R) FOR(c, 0, C) {
        if (dat[r][c] == 'L') {root = MP(r, c); break;}
    }

    auto good = [&](ll r, ll c) -> bool {
        if (r < 0 || r >= R || c < 0 || c >= C) {return false;}
        if (dat[r][c] != '.') {return false;}
        int steps_good = 0;
        int steps_bad = 0;
        for(auto& [dr, dc] : stepsOrthogonal) {
            ...
            if (dat[rr][cc] == '#') {continue;}
            if (dat[rr][cc] == 'L' || dat[rr][cc] == '+') {++steps_good;}
            if (dat[rr][cc] == '.') {++steps_bad;}
        }
        if (steps_good == 0) {return false;}
        if (steps_bad >= 2) {return false;}
        return true;
    };
    ...
    todo.push_back(root);
    while (!todo.empty()) {
        auto [curr_r, curr_c] = todo.back();
        todo.pop_back();
        for(auto& [dr, dc] : stepsOrthogonal) {
            if (!good(curr_r + dr, curr_c + dc)) { continue; }
            dat[curr_r + dr][curr_c + dc] = '+';
            todo.emplace_back(curr_r + dr, curr_c + dc);
        }
    }
