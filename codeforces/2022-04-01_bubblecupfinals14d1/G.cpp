...
bool three_colinear(auto& dat, auto& X) {
    auto& [x1, y1] = dat[X[0]];
    auto& [x2, y2] = dat[X[1]];
    auto& [x3, y3] = dat[X[2]];
    ll area_times_2 = x1*(y2-y3) + x2*(y3-y1) + x3*(y1-y2);
    return (area_times_2 == 0);
}
...
void solve() {
    ...
    rv(N, dat);
    pll pk = dat[K];
    // duplicates might be allowed but I don't like them.
    remDup(dat);
    K = 0;
    while ( dat[K] != pk ) {
        ++K;
    ...
    */
    ...
    if ( N == 3 ) {
        ...
        // 3/4 of p0, p1, p2, p3 must be colinear.
        V<int> on_line;
        FOR(exclude, 0, 4) {
            on_line.clear();
            FOR(k, 0, 4) {
    ...
    --N;
    dbgP(ncp_loc, K, dat[K], dat);


    pll L = dat[0];
    pll R = dat[N-1];
    if ( ncp == K ) {
        // step to one endpoint, then the other.
        db out = min(d(ncp_loc, L), d(ncp_loc, R));
        out += d(dat[0], dat[N-1]);
        return ps(out);
    }
    if ( K > ncp ) {
    ...
    // which endpoints should we visit before touching ncp?
    db out = 1e18;
    FOR(mask, 0, 8) {
        bool hi = mask % 2;
        bool lo = (mask/2) % 2;
        ...
        if ( !lo && !hi ) {
            db curr = d(dat[K], ncp_loc);
            curr += min(d(ncp_loc, L), d(ncp_loc, R));
            curr += d(L, R);
            ckmin(out, curr);
        } else if ( lo && !hi ) {
            if ( dir ) {
            ...
            FOR(lbd, 0, N-1) {  // "last before detour"
                dbg(lbd);
                int remnant = max(lbd+1, K+1);
                db curr = d(dat[K], L);
                curr += d(L, dat[lbd]);
                curr += d(dat[lbd], ncp_loc);
                curr += min(d(ncp_loc, dat[remnant]), d(ncp_loc, R));
                curr += d(dat[remnant], R);
                dbg(d(dat[K], L), d(L, dat[lbd]), d(dat[lbd], ncp_loc), min(d(ncp_loc, dat[remnant]), d(ncp_loc, R)), d(dat[remnant], R));
                dbg(curr);
                ckmin(out, curr);
            }
        } else if ( !lo && hi ) {
            ...
            FOR(lbd_helper, 0, N-1) {  // "last before detour"
                int lbd = N-1 - lbd_helper;
                int remnant = min(lbd-1, K-1);
                db curr = d(dat[K], R);
                curr += d(R, dat[lbd]);
                curr += d(dat[lbd], ncp_loc);
                curr += min(d(ncp_loc, dat[remnant]), d(ncp_loc, L));
                curr += d(dat[remnant], L);
                ckmin(out, curr);
            }
        } else if ( dir ) {
            // lo first
            db curr = d(dat[K], L) + d(L, R) + d(R, ncp_loc);
            ckmin(out, curr);
        } else {
            db curr = d(dat[K], R) + d(L, R) + d(L, ncp_loc);
            ckmin(out, curr);
        }
    }
