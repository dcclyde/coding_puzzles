...
void solve() {
    ...
    cdeg.push_back(1);  // ghost node that has root as a child
    ...
    auto tryit = [&](ll STEPS) {
        ...
        // I need to infect at least one child per vertex that has children.
        ll cost = n;
        FOR(k, 0, n) {
            /*
                item k will get extra help.
                If STEPS = 5 and k = 0 then there are 4 more steps after this one,
                so extra cost here is max(cdeg[k] - 5, 0).
            */
            ll extra_cost = maxll(cdeg[k] - STEPS + k, 0);
            cost += extra_cost;
        }
        return cost <= STEPS;
    };

    ll out = fstTrue(0, N, [&](ll steps) {return tryit(steps);});
    ps(out);
    return;
}
