...
template<int MXBIT> struct Trie {
    V<array<int,2>> nex; V<set<int>> val; int num = 0; // num is last node in trie
    Trie() {val.assign(1, set<int>());}
	void ins(ll x, int p, int start=0) { // insert or delete
        dbgcP("ins", x, p, start);
        // static int ctr = 0;
        // if (++ctr > 20) {
        //     exit(1);
	...
	ll query(ll l, ll r) { //
        ...
        auto get_count = [&](set<int>& Q) {
            auto it = Q.lower_bound(l);
            int out = 0;
            if (it == Q.end() || *it > r) {return out;}
            ++it; ++ out;
            if (it == Q.end() || *it > r) {return out;}
            return out + 1;
        };
		...
		R0F(i,MXBIT) {
			...
			// if (!nex[cur][t]) {t ^= 1;}
            // else {
            //     int c0 = get_count(val[nex[cur][0]]);
            //     int c1 = get_count(val[nex[cur][1]]);
            // }
            // else if (get_count(val[nex[cur][t]]) >= 2) {
            //     // pass
            // } else if
            // else if (val[nex[cur][t]].size() == 1 &&
            // else if (!has_2(val[nex[cur][t]])) {}


			if (!nex[cur][t] || !has_2(val[nex[cur][t]])) t ^= 1;
			cur = nex[cur][t]; if (t) out ^= 1LL<<i;
		}
		return out;
	}
