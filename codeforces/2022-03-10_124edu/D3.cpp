...
void solve() {
    lls(N);
    vector<pii> dat( N );
    map<pii, pair<int,pii>> cost;
    FOR(k,0,N) {
        auto& [a, b] = dat[k];
        cin >> a >> b;
        cost[dat[k]] = MP(INF, MP(-1,-1));
        ...
        for ( int dx = -1 ; dx <= 1 ; ++dx ) {
            for ( int dy = -1 ; dy <= 1 ; ++dy ) {
                ...
                pii neighbor( a+dx , b+dy );
                if ( ! cost.count( neighbor ) ) {
                    cost[neighbor] = MP(0, neighbor);
                }
            }
        }
    }
    ...
    while ( !todo.empty() ) {
        ...
        for ( int dx = -1 ; dx <= 1 ; ++dx ) {
            for ( int dy = -1 ; dy <= 1 ; ++dy ) {
                ...
                pii proposed(p.first + dx, p.second + dy);
                if ( ! cost.count(proposed) || cost[proposed].first <= c+1 ) {
                    continue;
                }
                cost[proposed] = MP( c+1 , closest );
