...
void solve() {
    ...
    /**
        dp[n] = list of possible states of the other guy?
        Don't need to hang onto more than 3 though.
     */

    map<ll, set<ll>> dpNext;
    ll best_score = -1;
    while (n < N) {
        dpNext.clear();
        for(auto& [score, states] : dp) {
            for(auto& q : states) {
                ll x = dat[n];
                if (x != dat[n-1]) { dpNext[score+1].insert(q); }
                else {dpNext[score].insert(q);}

                if (x != q) {dpNext[score+1].insert(dat[n-1]);}
                else {dpNext[score].insert(dat[n-1]);}
            }
        }
        best_score = dpNext.rbegin()->first;
        dpNext.erase(best_score - 2);
        dp.clear();
        for(auto& [score, states] : dpNext) {
            int ctr = 3;
            for(auto& q : states) {
                dp[score].insert(q);
                --ctr;
