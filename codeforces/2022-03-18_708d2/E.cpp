...
#pragma region  // small and precompute
vector<int> psq;
void precompute() {
    const int MAX = 3162;
...
void solve() {
    ...
    dbg(dat);
    // dp[ n ][ k ] = (cheapest price to get here with k skips, NEGATIVE start of current subarray)
    ...
    FOR(n, 0, N) {
        ...
        FOR(k, 0, K+1) {
            ...
            // can I just add the new element for free?
            // well... when's the last time I saw this element?
        ...
        appearances[dat[n]].first += 1;  // increment aptr
        dbg(n+1, dp[n+1]);
    }
