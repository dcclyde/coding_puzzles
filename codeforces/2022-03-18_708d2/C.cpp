...
void solve_small() {
    ...
    dbg(N, pow4);
    tuple<int,int,int> before_scaling;
    if ( N == 4 ) {
        before_scaling = MT(2, 1, 1);
    } else if ( N == 8 ) {
        before_scaling = MT(4, 2, 2);
    } else if ( N % 2 == 1 ) {
        before_scaling = MT(1, N/2, N/2);
    } else if ( N % 4 == 2 ) {
        before_scaling = MT(2, N/2-1, N/2-1);
    } else {
        assert(false);
    }
    dbg(before_scaling, pow4);

    auto& [a, b, c] = before_scaling;
    a *= pow4;
    b *= pow4;
    c *= pow4;
    ps(a, b, c);
