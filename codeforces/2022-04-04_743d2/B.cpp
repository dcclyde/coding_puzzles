...
void solve() {
    ...
    int out = INF;
    FOR(swaps_a, 0, N) {
        // what's the smallest value I can move to front of A in <= s_a swaps?
        // what's the cheapest item from B that loses to val_a?
        if ( A[swaps_a] < val_a ) {
            for(int check_b = A[swaps_a] + 1; check_b < min(val_a, 2*N+1); check_b += 2) {
                int curr_cost_b = v2p[check_b];
                int total_cost = curr_cost_b + swaps_a;
                dbg(swaps_a, check_b, curr_cost_b);
                ckmin(out, total_cost);
            }
            val_a = A[swaps_a];
        }
    }
