...
void solve() {
    ...
    V<set<int>> nt2pos(N);
    FOR(k,0,N) {
        nt2pos[num_todos[k]].insert(k);
    }
    int total_todo = N;

    int out = 0;
    auto& read_now = nt2pos[0];
    while ( !read_now.empty() ) {
        // do one pass through the book.
        ++out;
        dbg(read_now);
        int pos = -1;
        while ( true ) {
            // find the next page that I can read.
            auto it = read_now.lower_bound(pos+1);
            if ( it == read_now.end() ) {
                break;
            }
            pos = *it;  // page we're currently reading
            dbgc("read page", out, pos);
            read_now.erase(pos);
            --total_todo;
            for(auto& o : G[pos]) {
                dbgcP("update", pos, o);
                int curr_nt = num_todos[o];
                --num_todos[o];
                nt2pos[curr_nt].erase(o);
                nt2pos[curr_nt-1].insert(o);
            }
        }
    }
    if ( total_todo == 0 ) {
