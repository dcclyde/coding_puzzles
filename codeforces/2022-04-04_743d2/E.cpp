...
void solve() {
    ...
    rv1(N, dat);
    dbgY(N, dat);
    // first, kill all "streaks".
    V<int> kill_streaks;
    kill_streaks.push_back(dat[0]);
    FOR(k,0,N) {
        if ( dat[k] != kill_streaks.back() ) {
            kill_streaks.push_back(dat[k]);
        }
    }
    dat = kill_streaks;
    V<V<int>> c2pos(N, {-1});
    ...
    FOR(k, 0, N) {
        c2pos[dat[k]].push_back(k);
    }
    FOR(k,0,N) {
        c2pos[k].push_back(N);
    }

    dbgcBold("dp start", TIME());
    const int INF = 1e6;
    // dp[l][r] = cost to make [l...r] all match.
    ...
    dbgcBold("per color start", TIME());
    int out = INF;
    FOR(c, 0, N) {
        auto& c2p = c2pos[c];
        int n = c2p.size();
        dbg(c2p);
        FOR(mask_pre,0,1<<(c2p.size()-2)) {
            int mask = (mask_pre<<1) | 1 | (1<<(c2p.size()-1));
            dbg(mask, mask_pre);
            if ( ((mask & 1) == 0) || ((mask & (1<<(n-1))) == 0) ) {
                continue;
            }
            // 1 in mask means we'll keep that position
            // V<int> positions;
            // FOR(b, 0, c2p.size()) {
            //     if ( (mask>>b) & 1 ) {
            //         positions.push_back(c2p[b]);
            //     }
            // }
            // dbg(mask, positions);
            // int out_local = 0;
