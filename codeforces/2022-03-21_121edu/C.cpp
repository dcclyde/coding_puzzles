...
void solve() {
    ...
    ll out = 0;
    ll curr_start = T.back();
    ll curr_end = curr_start - H.back();
    for ( int k = N-2 ; k >= 0 ; --k ) {
        ll next_start = T[k];
        ll next_end = next_start - H[k];
        dbg(k, curr_start, curr_end, next_start, next_end);
        if ( curr_end >= next_start ) {
            // curr triangle can be processed.
            ll dx = curr_start - curr_end;
            ll contribution = (dx * (dx+1)) / 2;
            out += contribution;
            dbgY(contribution, out);

            curr_start = next_start;
            curr_end = next_end;
        } else {
            ckmin(curr_end, next_end);
        }
    }
    dbgc("ending");
    ll dx = curr_start - curr_end;
