...
void solve() {
    ...
    FOR(ai, 0, 20 ) {
        ...
        FOR(bi, 0, 20 ) {
            ...
            // how many of our existing players can be used?
            int free_S;
            if ( final_S >= N ) {
                free_S = N;
            } else {
            ...
            if ( free_S + final_M >= N ) {
                ...
                free_M = feq[final_M + free_S] - free_S;
            }

            int free_L = N - free_S - free_M;
            int final_L = 1;
            while ( final_L < free_L ) {
                final_L <<= 1;
            }
            // dbgP(free_L, final_L);
            dbg(final_S, final_M, free_S, free_M, final_L);
            int final_total = final_S + final_M + final_L;
            int cost = final_total - N;
            ckmin(out, cost);
            dbgY(final_total, cost, out);
            if ( final_M >= N ) {
                break;
            }
        }
