...
/*
    If 2 blacks are adjacent, we see everything.
    If everything is black or BA, done.

    If I am BA and one of my subtrees (incl parent) has a black,
        other subtrees are good.
*/

void see_everything(int N) {
    vector<int> out( N , 1 );
    return pv(out);
}
...
void solve() {
    ...
    // start from some black node.
    int root = 0;
    while ( color[root] != 2 ) {
        ++root;
    }
    dbgc("before dfs", root);

    int final_root = root;
    set<pii> good_edges;
    vector<int> subtree_num_black(N, 0);
    ...
    function<void(int,int)> Dfs = [&](int curr, int parent) {
        ...
        for ( auto& proposed : G[curr] ) {
            ...
            subtree_num_black[curr] += subtree_num_black[proposed];
        }
        if ( color[curr] == 2 ) {
            subtree_num_black[curr] += 1;
        }
        // check for good edges.
        if ( color[curr] == 1 ) {
            if ( subtree_num_black[curr] >= 2 ) {
                final_root = curr;
                good_edges.clear();
                good_edges.emplace(curr, parent);
            }
            for ( auto& o : G[curr] ) {
                if ( subtree_num_black[curr] - subtree_num_black[o] >= 2 ) {
                    good_edges.emplace(curr, o);
                }
            }
        }
    };
    Dfs(root, -1);
    dbg(final_root);
    dbg(subtree_num_black);
    dbg(good_edges);

    vector<int> out(N, false);
    function<void(int,int,bool)> DfsFinal = [&](int curr, int parent, bool good) {
        if ( good_edges.count(MP(parent,curr)) ) {
            good = true;
        }
        out[curr] = (good || (color[curr] > 0));
        dbgP(curr, good, color[curr], out[curr]);
        for ( auto& proposed : G[curr] ) {
            if ( proposed == parent ) {
                continue;
            }
            DfsFinal(proposed, curr, good || good_edges.count(MP(curr, proposed)));
        }
    };
    DfsFinal(final_root, -1, false);
