...
/**
 * Given a prefix, I need to count how many valid permutations start this way.
 * N up to 1e5.
 *
 * I guess good permutations <--> subsets of {1, 2, ..., N-1}?
 *
 */
...
void solve() {
    ...
    set<int> picked;
    auto helper = rangeint(1, N+1);
    picked.insert(all(helper));
    FOR(b, 0, 60) {
        if ((K>>b) & 1) {
            picked.erase(N-1 - b);
        }
    ...
    FOR(k, 1, N+1) {
        el;
        dbgY(k, out);
        if (picked.find(k) != picked.end()) {
            ROF(j, last_inserted + 1, k+1) {
                out.push_back(j);
            }
            last_inserted = k;
        }
