...
/**
    Fix b and c.
    Then I need count of a < b with dat[a] < dat[c]
    times count of d > c with dat[d] < dat[b].

    pre[x][y] = num items with idx < x and val < y
...
void solve() {
    ...
    V<V<int>> pre(N+1, V<int>(N+1, 0));
    FOR(x, 0, N) {
        // for values less than dat[x], it's just same as before.
        FOR(k, 0, dat[x]+1) {pre[x+1][k] = pre[x][k];}

        FOR(k, dat[x]+1, N+1) {pre[x+1][k] = pre[x][k] + 1;}
    }
    dbgY(pdh(pre));

    ll out = 0;
    FOR(b, 1, N) FOR(c, 0, N-1) {
        if (c <= b) {continue;}
        ll left = pre[b][dat[c]];
        ll right_helper = pre[c+1][dat[b]];
        // rh = num things to left of c and smaller than dat[b].
        // there are dat[b] things total smaller than dat[b]
        // TODO off by one???
        // so the others are to right of c.
        ll right = dat[b] - right_helper;
        dbg(b, c, left, right_helper, right);
        out += left * right;
    }
