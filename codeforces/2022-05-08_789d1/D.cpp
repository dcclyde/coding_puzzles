...
/**
    The initial K operations are basically part of a bubble sort?
    Basically just if K=N-1 then it's bubble sort.

    It feels like two separate steps.
    Understand stuff about the "sorted" permutation
        vs where could it have been before sorting.

    I mean maybe there'll be useful structure later but thinking as above
        seems fine for now.

    Can I say something useful like "intermediate bubble sort states always
        have property ABC"?


    If dat[N-1] is N-1 then the 1 is at the end.

    If dat[k] == k then the kth value is the smallest so far.
