...
/**
    10^6 students total.
    They will fully fill the matrix.

 *  ! COLUMNS
    How many good columns?
    Well, build the final matrix.
    Write down the list of turns when each column becomes good (if at all).
    Binary search in that list (or just pointer walk!)
    to get # good after k steps.

 *  ! ROWS
    I kinda want separate lists for each rotated version?
    Answer: What's the first k making row r good, assuming k % C = c?
...
void solve() {
    ...
    auto flat = [&](ll r, ll c) {return (R-1-r)*C + (C-1-c);};
    // auto coords = [&](ll k) -> pll {return MP(R-1-(k/C), C-1-(k%C));};

    // ! first handle columns.
    V<V<bool>> mat(R, V<bool>(C, false));
    FOR(r, 0, R) FOR(c, 0, C) {
        mat[r][c] = (dat[flat(r,c)] == '1');
    }
    V<ll> columns_become_good;
    auto& cbg = columns_become_good;
    FOR(c, 0, C) {
        ROF(r, 0, R) {
            if (!mat[r][c]) {continue;}
            cbg.push_back(flat(r,c));
            dbgc("build cbg", MP(r,c), flat(r,c));
            break;
        }
    }
    sort(all(cbg));
    ...
    // ! OK now rows...
    /**
        dp[r][c] = first k such that
     */
    V<ll> pref(N+1);
    FOR(k, 0, N) { pref[k+1] = pref[k] + (dat[N-1-k] == '1'); }
    auto has_good = [&](ll start, ll end) -> bool {
        bool out = ((pref[end+1] - pref[start]) > 0);
        dbgcP("has_good", MP(start, end), out);
        return out;
    };
    dbgR(pref);
    // V<V<ll>> dprow(R, V<ll>(C, -1));
    // V<V<ll>> rows_good_helper;
    // auto& rgh = rows_good_helper;
    // FOR(c, 0, C) {
    ...
    V<ll> good_rows(N);
    FOR(k, 0, C) {
        // handle the first row the hard way.
        good_rows[k] = has_good(N-1-k, N-1);
    }
    FOR(k, C, N) {
        good_rows[k] = good_rows[k-C] + has_good(N-1-k, N-1-k + C-1);
    }
    dbgY(good_rows);
    ...
    ll cbg_idx = 0;
    FOR(k, 0, N) {
        if (cbg_idx < cbg.size() && cbg[cbg_idx] == k) {
            ++num_good_cols;
            ++cbg_idx;
        }
        ll num_good_rows = good_rows[k];
        dbg(k, num_good_cols, num_good_rows);
        out.push_back(num_good_rows + num_good_cols);
    }
