...
/**
    N <= 2e5. Logs and/or graphs I guess.

    I'm given 2 permutations P, Q.
    Then I get to choose a third permutation R,
    and then I want R*P and R*Q to have very far-apart values.

    Is there some nice way to express "values of 2 permutations are far apart"?

    I can immediately discard any positions where P = Q.
 *!     I can't decrement N though! Dummy positions still make other values big.

    Do something per cycle?
    Maybe I basically want

 *! Maybe I have good ideas?
    I think I want to first break into cycles and iterate from shortest to longest.
        (or think more about iteration order)
    Cycle length 2 always contributes 2 * (big - little).
    Cycle length 3 contributes the same, doesn't matter what the middle guy is.
    Cycle length 4?
    I think just - for odd length cycles, we should burn one item in the middle.
    For even length, iterate inward from the two ends.
 */
...
void solve() {
    ...
    DSU dsu;
    dsu.init(N+1);
    FOR(k, 0, N) {dsu.unite(P[k], Q[k]);}
    // get the cycle sizes.
    umap<ll, ll> head_to_size;
    auto& hts = head_to_size;
    FOR(k, 1, N+1) {
        hts[dsu.get(k)] = dsu.size(k);
    }
    V<ll> cycles;
    for(auto& [head, csize] : hts) {cycles.push_back(csize);}
    sort(all(cycles));
    ...
    for(auto& c : cycles) {
        V<ll> cycle_values;
        if (c%2 == 1) {
            cycle_values.push_back(ord[iend]);
            --iend;
        }
        FOR(k, c%2, c) {
            cycle_values.push_back(ord[istart]);
            ++istart;
        }
        assert(cycle_values.size() == c);
        cycle_values.push_back(cycle_values.front());
        ll local = 0;
        FOR(k, 0, c) {local += abs(cycle_values[k+1] - cycle_values[k]);}
        out += local;
    }
    ps(out);
    return;
