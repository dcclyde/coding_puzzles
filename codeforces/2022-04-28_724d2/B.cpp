...
V<string> get_options() {
   ...
   auto do_something = [&]() {
    ...
    FOR(MAX_DEPTH, 1, 4) {
   auto product_rec_helper = [&](auto&& self) -> void{
      if (curr.size() == MAX_DEPTH) {return do_something();}  // ! define max depth
      FOR(k, 0, 26) {  // ! iterate over options at depth n
            curr.push_back('a' + k);
            self(self);
            curr.pop_back();
      }
...
void solve() {
    ...
    uset<string> seen;
    FOR(a, 0, N) FOR(len, 1, 5) {
        ll b = a + len;
        if (b > N) {break;}
        seen.insert(dat.substr(a, len));
    }
    // gen all strings with up to 5 chars.
    static V<string> options = get_options();
    // dbg(options);
    for(auto& s : options) {
        if (seen.find(s) == seen.end()) { return ps(s); }
    }
    return;
}
