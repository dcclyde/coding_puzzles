...
using mi = mint<5>; // 5 is primitive root for both common mods
using vmi = V<mi>;
...
template <int RT>
string to_string(mint<RT> modint) {
...
void solve() {
    ints(N, maxes_desired, good_desired);
    int M = maxes_desired;
    int D = good_desired;
    cin >> MOD;
    ...
    FOR(n, 0, max(N+1, 10)) {
        FOR(k, 1, n) {
            chelper[n][k] = chelper[n-1][k-1] + chelper[n-1][k];
        }
    }
    FOR(n, 0, 5) {
        FOR(k, 0, n+1) {
    ...
    V<V<V<mi>>> dp(N+1, V<V<mi>>(D+1, V<mi>(maxes_desired+1, 0)));
    dbg(N, D, maxes_desired);
    ...
    FOR(n, 2, N+1) {
        FOR(g, 0, D+1) {
            FOR(m, 2, M+1) {
                ...
                FOR(bigpos, 0, n) {
                    FOR(h, 0, g+1) {
                        int size_l = bigpos;  // space to left of bigpos
                        int size_r = n-1 - bigpos;
                        mi cl = dp[size_l][h][m-1];
                        mi cr = dp[size_r][g-h][m-1];
                        mi split = comb(n-1, size_l);
                        dbg(bigpos, h, size_l, size_r);
                        dbg(MT(size_l, h, m-1), cl);
                        dbg(MT(size_r, g-h, m-1), cr);
                        dbg(split);
                        dp[n][g][m] += cl * cr * split;
                    }
                }
                dbgY(MT(n,g,m), dp[n][g][m]);
            }
        }
    }
    exit(1);
    ...
    FOR(n, 1, N+1) {
        FOR(g, 0, D+1) {
            FOR(m, 3, maxes_desired+1) {
                if ( g + m-2 > n+1 ) {
                    break;
                }
                // place n somewhere arbitrary.
                FOR(bigpos, 0, (n+1)/2) {
                    int factor = 2;
                    if ( 2*bigpos + 1 == n ) {
                        factor = 1;
                    }
                    int size_left = bigpos;
                    int size_right = n - bigpos - 1;
                    int gin_lo = g + m-2-1 - (size_right+1);
                    ckmax(gin_lo, 0);
                    FOR(g_inner, gin_lo, g+1) {
                        dbg(bigpos, size_left, g_inner, m-1);
                        if ( g_inner > 0 && g_inner + m-2-1 > size_left+1 ) {
                            dbgc("skip");
                            continue;
                        }
                        // if ( g-g_inner + m-1 > size_right+1 ) {
                        //     break;
                        // }
                        mi contrib_left = dp[size_left][g_inner][m-1];
                        mi contrib_right = dp[size_right][g-g_inner][m-1];
                        mi pick_sizes = comb(n-1, size_left);
                        dbg(n-1, size_left, (int)pick_sizes);

                        dbg(size_right, g-g_inner, m-1);
                        dbgP(MT(n,g,m), bigpos, g_inner, (int)contrib_left, (int)contrib_right, (int)pick_sizes);

                        dp[n][g][m] += factor * contrib_left * contrib_right * pick_sizes;
                    }
                }
                dbgY(MT(n,g,m), (int)dp[n][g][m]);
                // if ( n == 3 && g == 1 && m == 4 ) {
                //     exit(1);
                // }
            }
