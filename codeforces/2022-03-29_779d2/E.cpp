...
/*
    Winner will be the first player to pick a position that blocks out
    all bigger numbers.
    Are there other winning positions? Probably I guess?

    1) Win if I block all higher numbers.
    2) Lose if I allow moving to a type (1) position.
    3) Win if I block all higher numbers NOT COUNTING type (2)
    ...and so on...

    N^2 is 4e6 so it maybe has to be linear?
*/
const int INF = 2e9;
...
void solve() {
    ...
    auto max_dist_to_winning = [&](pii& q) {
        auto& [r, c] = q;
        int a = r+c;
        int b = r-c;
        int result = max({
    ...
    for ( int k = N*N-2 ; k >= 0 ; --k ) {
        // check max distance to a known-winning position.
        int d2w = max_dist_to_winning(inv[k]);
        auto& [r, c] = inv[k];
        if ( d2w > K ) {
            out[r][c] = 'G';
        } else {
            out[r][c] = 'M';
            update_winning(inv[k]);
        }
    }
