...
/**
    F2:
    First, solve F1(?) or otherwise know the max inversions.
    Second, probably some range query type stuff to count cost to given array?
 */

/**
    I definitely want to put all 1s together, all 2s together, etc.
...
void learn() {
    ...
    // ll MAX_V = 3;
    // V<ll> dat;
    // FOR(k, 0, N) { dat.push_back(rng() % MAX_V + 1); }
    // V<ll> dat = {1, 2, 3, 2, 1};
    V<ll> dat = {1, 2, 2, 3, 3, 3, 3, 4, 1, 1};
    N = dat.size();
    ...
    ll max_cost = 0;
    do {
        // what's the cost from ph to dat?
        auto wip = ph;
        ll cost = 0;
        ROF(k, 0, N) {
            int pos = wip.size() - 1;
            while (wip[pos] != dat[k]) { --pos; }
            // dbgR(pos, wip, k, dat);
            ll cost_here = wip.size() - 1 - pos;
            cost += cost_here;
            wip.erase(wip.begin() + pos);
            // dbgP(k, dat, cost_here, wip, cost);
        }
        if (cost >= max_cost) {
            dbg(cost, ph);
            max_cost = cost;
...
void confirm(ll N, V<ll>& dat, V<ll>& proposed) {
    el;
    dbgcY("confirm", N, dat, proposed);
    V<ll> ph = dat;
    sort(all(ph));
    ll max_cost = 0;
    ll proposed_cost = -1;
...
void solve() {
    ...
    ll MAX = uncinv.size() - 1;
    dbg(uncompressed);
    dbgP(uncinv);
    dbg(MAX, dat);

    V<V<ll>> indices_per_num(MAX+1);
    auto& ipn = indices_per_num;
    FOR(k, 0, N) {
        ipn[dat[k]].push_back(k);
    }
    dbgP(pdh(ipn));

    auto sort_cost = [&](ll a, ll b) -> ll {
        ll cost = 0;
        int ib = 0;
        FOR(ia, 0, ipn[a].size()) {
            while (ib < ipn[b].size() && ipn[a][ia] > ipn[b][ib]) {
                dbgY(ia, ipn[a][ia], ib, ipn[b][ib]);
                ++ib;
            }
            cost += ib;
            dbg(ia, ib, cost);
        }
        dbgcP("s_c", a, b, cost, ipn[a], ipn[b]); el;
        return cost;
    };

    // V<ll> sort_helper;
    // FOR(k, 0, MAX+1) {sort_helper.push_back(k);}
    // // dbg(sort_helper);
    // sort(all(sort_helper), [&](ll a, ll b) {
    //     // I need to return true if a "<" b.
    //     return sort_cost(a, b) > sort_cost(b, a);
    // });

    // /**
    //     I think my remaining problem is:
    //     Some elements will compare = to each other, but equality is not transitive.
    //     Like if I have 1 2 3 1 then the sorter could give me 2 1 1 3. 1v2 and 1v3 are =, but 2v3 is not equal.
    //  */

    // V<ll> eq;
    // FOR(k, 0, N-1) {
    //     ll a = sort_helper[k];
    //     ll b = sort_helper[k+1];
    //     if (sort_cost(a, b) == sort_cost(b, a)) {eq.push_back(k);}
    // }
    // eq.push_back(N-1);
    // eq.push_back(N-2);
    // V<ll> merged;
    ...
    FOR(k, 0, MAX+1) {
        auto& indices = ipn[k];
        dbg(k, indices, indices.size(), indices[indices.size() / 2]);
        sort_helper.emplace_back(
            -indices[indices.size() / 2],
            k
        );
    }
    dbg(sort_helper);
    sort(all(sort_helper));


    V<ll> out;
    for(auto& [median, v] : sort_helper) {
    // for(auto& v : sort_helper) {
        ll n = ipn[v].size();
        ll unc_v = uncompressed[v];
        out.insert(out.end(), n, unc_v);
    }
    pv(out);
    dbg_only(
        for(auto& x : dat) {x = uncompressed[x];}
        confirm(N, dat, out)
    );
    return;
}
