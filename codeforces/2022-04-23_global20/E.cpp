...
/**
    N and L <= 2000.
    Can it mean I should do some quadratic computations? Probably not right?

    N+30 queries. That could be N + some binary search(es).
    I could definitely binary search for total length and max length.

    If I knew all the words, how would I solve the problem?
    Well, putting it all on one line is "nearly optimal".
    Call that length B.

    Then I should also consider like


 */

const ll MAX_L = 2'000;
...
void solve() {
    ...
    dbg(MAX_L * N);
    ll B = fstTrue(2*N - 1, MAX_L * N, [&](ll x) {
        return (ask(x) == 1);
    });
    dbgY(B);
    // spent log2(MAX_L * N) guesses so far.
    // definitely <= 22 guesses used.

    // V<ll> divisors;
    // for(ll d = 1; d*d <= B+1 ; ++d) {
    //     if ((B+1) % d == 0) {
    //         divisors.push_back(d);
    //         divisors.push_back((B+1)/d);
    //     }
    // }
    // remDup(divisors);
    // dbg(divisors);
    ...
    // for(auto& d : divisors) {
    //     if (d > N) {break;}
    //     if (d == 1 || d == B+1) {continue;}
    //     ll q = (B+1 - d) / d;
    //     ll result = ask(q);
    //     if (result == 0) {break;}
    //     if (ckmin(best_result, result * q)) {best_input = q;}
    // }
    FOR(d, 2, N+1) {
        ll q = B / d;
        if (q <= 0) {break;}
