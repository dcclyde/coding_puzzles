...
void solve() {
    ...
    map<ll,ll> extra_used;
    auto& eu = extra_used;
    ll bi = N-1;
    ll ai = N-1;
    while (bi >= 0 && ai >= 0) {
        while (ai >= 0 && A[ai] != B[bi] && extra_used[A[ai]] > 0) {
            --extra_used[A[ai]];
            --ai;
        }
        if (ai < 0 || A[ai] != B[bi]) {return NO;}
        ll bs = bi;
        while (bs > 0 && B[bs-1] == B[bi]) {--bs;}
        ll copies_b = bi - bs + 1;
        // we need to build that many copies in a row.
        extra_used[A[ai]] += copies_b;
        bi = bs - 1;
    }
    dbgY(extra_used);
    YES;
