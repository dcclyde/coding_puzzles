...
ll subsolve(auto& X, int N, int K) {
    int pos = N-1;
    ll out = 0;
    while ( pos >= 0 ) {
        out += 2 * X[pos];
...
void solve() {
    ...
    // try making last trip to the left.
    ll last_left = subsolve(A, max(0LL, (ll)A.size()-K), K) + subsolve(B, B.size(), K);
    if ( A.size() > 0 ) {
        last_left += A.back();
    }
    dbg(last_left);
    ll last_right = subsolve(B, max(0LL, (ll)B.size()-K), K) + subsolve(A, A.size(), K);
    if ( B.size() > 0 ) {
        last_right += B.back();
    }
    dbg(last_right);
    ll out = min(last_left, last_right);
    ps(out);
