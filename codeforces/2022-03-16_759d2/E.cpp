...
void solve() {
    ...
    FOR(k, 0, N-1) {
        int1(parent);
        hld.adj[k].push_back(parent);
        hld.adj[parent].push_back(k);
    }
    hld.init( N );
...
tcT> struct SegTree { // cmb(ID,b) = b
    ...
    void add(int p, T val) {
        // dbgc("add PRE", p, val, *this);
        upd(p, val + seg[p+n]);
        // dbgc("add POST", p, val, *this);
    }
    ...
    int first_at_least(int lo, int val, int ind=1, int l=0, int r=-1) { // if seg stores max across range
        if (r == -1) r = n-1;
        dbgc("fal", lo, val, ind, seg[ind], l, r);
        dbg((r < lo || val > seg[ind]), (r < lo), (val > seg[ind]));
        ...
        if (res != -1) return res;
        dbg();
        return first_at_least(lo,val - seg[2*ind],2*ind+1,m+1,r);
    }
};
...
void solve() {
    ...
    FOR(k, 0, N-1) {
        int1(parent);
        G[k+1].push_back(parent);
        G[parent].push_back(k+1);
    }
    dbg(G);

    // st[L, R] = num values with between L and R occurrences.
    SegTree<int> st;
    st.init(N + 1);
    st.upd(0, N);  // initially every value occurs 0 times.
    ...
    FOR(qid, 0, Q) {
        ints(pos, cutoff, idx);
        --pos; --idx;
        queries[pos].emplace_back(cutoff, idx, qid);
    }
    ...
    function<void(int, int)> Dfs = [&](int pos, int parent) {
        ...
        X[cnt].erase(val);
        ++cnt;
        dbgc("visit", pos, parent, X, st);

        // answer queries
        if ( queries.count(pos) == 1 ) {
            for ( auto& [cutoff, idx, qid] : queries[pos] ) {
                // find first R such that st.query(cutoff, R) >= idx.
                dbg(pos, cutoff, idx, qid);
                dbg(st);
                dbg(X);
                int num_ignored = st.query(0, cutoff-1);
                int count_to_use = st.first_at_least(0, idx+1 + num_ignored);
                dbg(num_ignored, count_to_use); el;
                if ( count_to_use == -1 ) {
                    out[qid] = -1;
                } else {
                    out[qid] = *X[count_to_use].begin();
                }
            }
        }

        // step to children
        for ( auto& proposed : G[pos] ) {
            if ( proposed == parent ) {
                continue;
            }
            Dfs(proposed, pos);
