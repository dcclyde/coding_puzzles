...
/**
    Assume dat is sorted.
    Total positive cost is just dat[N-1].
    For negatives, I can draw from dat[a] to dat[b] with a < b.
    So, sum of dat[b] - dat[a].
 */
...
void solve() {
    ...
    FOR(k, 0, N) {
        ll instances_left = k;
        ll instances_right = N-1-k;
        out += (instances_right - instances_left) * dat[k];
    }
    return ps(out);
}
