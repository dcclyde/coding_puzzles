...
/**
    Per pencil-and-paper, I think limiting A will have same sum as before but
        spaced out by at least B.
 **     Tricky part: What if some of the gaps were already too big?

    If I ignore the "tricky" piece, I get:
        Length of whole chain is sB
        Mean of chain is sA / N
        If I place A[0] at 0, I'd get mean of spB / N where spB = sum of prefix sums.
        So the correct A[0] will be (sA - spB) / N.

    It wouldn't be hard from there.
    So now what about the "tricky" part?

    Probably I'll end up running some kinda DP
    State = (position, sum so far), score = num ways?

    If sA is big enough, we'll always win.
    Could I choose careful spacing so I sometimes win even wtih smaller sA?
    Mayyybe no?
 */

/**
    Problem: If A is [0, 2] and B is [0] then A will never change.
    Even if sA is big enough, I can still lose if A[0] is too small.
    Probably I just need A[0] >= (something)?
    If A[1..N] will be at least 2*B[0] away from A[0] then A[0] won't move.

 */

bool eqish(db x, db y) {return abs(x - y) < 1e-12;}
...
void brute() {
    ...
    ll out = 0;
    {
        ...
        auto do_something = [&]() {
            ...
            V<db> A;
            for(auto& x : curr) {A.push_back(x);}
            bool done = false;
            el; dbgP(A);
            while (!done) {
                done = true;
                FOR(k, 0, B.size()) {
                    db res1 = min((A[k] + A[k+1] - B[k]) / 2., A[k]);
                    db res2 = max((A[k] + A[k+1] + B[k]) / 2., A[k+1]);
                    if (eqish(res1, A[k]) && eqish(res2, A[k+1])) {continue;}
                    done = false;
                    A[k] = res1;
                    A[k+1] = res2;
                }
        ...
        function<void(void)> product_rec_helper = [&]{
            if (curr.size() == C.size()) {return do_something();}  // ! define max depth
            int n = curr.size();
            FOR(k, 0, C[n] + 1) {  // ! iterate over options at depth n
                curr.push_back(k);
                product_rec_helper();
                curr.pop_back();
            }
...
void solve() {
    ...
    el;
    ll spB = 0;
    FOR(k, 0, N-1) {spB += (N-1-k) * B[k];}
    dbg(spB);
    ll SUM_MAX = sumv(C);

    V<V<mi>> dp(N+1, V<mi>(SUM_MAX+1, 0));
    dp[0][0] = 1;
    FOR(n, 0, N) FOR(a, 0, C[n]+1) FOR(base, 0, SUM_MAX+1-a) {
        if (n == 0 && a > 0) {continue;}
        dp[n + 1][base + a] += dp[n][base];
    }
    dbg(pdh(dp));
    dbgP(dp[N]);

    // pref[k] = sum of dp[0:k].
    // pref[0] = ways to get <= -1.
    V<mi> pref(SUM_MAX+2);
    FOR(k, 0, SUM_MAX+1) {pref[k+1] = pref[k] + dp[N][k];}
    dbg(pref);

    // ways to get >= x
    auto ways_above = [&](ll x) -> mi {
        mi out;
        if (x > SUM_MAX) {out = 0;}
        else if (x < 0) {out = pref.back();}
        else { out = pref.back() - pref[x]; }
        dbgcR("w_a", x, out);
        return out;
    };
    ...
    FOR(qid, 0, Q) {
        ...
        /**
            ...
            I also need some condition on A[0].
            What would happen to A[1..N]?
         */
        mi out = ways_above(N*q + spB);
        ps(out);
    }
    return;
}
