...
/**
    ...
    Seems like some linearity of expectation stuff.
    For specific a < b, what's the probability that a comes before vs after b?
    That should just depend on size of b subtree vs a subtree.

    Root the tree somewhere random.
    If a and b are in separate subtrees, just use those sizes.
    If b is in the subtree of a, then for a's size we should use sum over all
        edges from a that DON'T lead toward b.

    N is only 200, so it should be fine to waste a lot of time here.
    Maybe it's convenient to just rerun dfs for each (a, b)?
 */
...
void solve() {
    ...
    V<ll> dist_to_b(N);
    V<ll> count_per_dist(N);
    V<ll> subtree_size(N);
    ll aa, bb;
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        subtree_size[curr] = 1;
        if (curr == bb) {dist_to_b[curr] = 0;}
        ...
        for (auto& o : G[curr]) {
            ...
            ckmin(dist_to_b[curr], 1 + dist_to_b[o]);
        ...
        if (d < INF_ll) {
            ...
            for(auto& o : G[curr]) {
                if (o == parent) {continue;}
                if (dist_to_b[o] != INF_ll) {continue;}
                count_per_dist[d] += subtree_size[o];
            }
        }
    ...
    FOR(a, 0, N) FOR(b, a+1, N) {
        aa = a; bb = b;
        dist_to_b.assign(N, INF_ll);
        count_per_dist.assign(N, 0);
        dfs(a, -1, dfs);
        el;
        dbgY(a,b, dist_to_b);
        dbg(subtree_size);
        ll D = dist_to_b[a];
        count_per_dist.resize(D);
        dbgR(count_per_dist);
        if (D <= 0) {
            dbg(a, b, D);
            assert(false);
        ...
        ll size_b = subtree_size[b];
        ll size_a = 1;
        for(auto& o : G[a]) {
            if (dist_to_b[o] < INF_ll) {continue;}
            size_a += subtree_size[o];
        }
        local += size_b;
        ...
        FOR(k, 1, D) {
            /**
                k steps away from A, D-k steps away from b.
                Imagine flipping D-1 coins and needing at least D-k heads.
                num/denom already tells us how to get at least D-k-1 heads.
             */
            mi curr = comb(D-1, k-1);
            num += curr;
            mi prob = num / denom;
            mi contrib = prob * count_per_dist[D-k];
            local += contrib;
