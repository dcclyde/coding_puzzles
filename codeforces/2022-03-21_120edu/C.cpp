...
void solve() {
    ...
    ll out = sum - targ;  // upper bound
    ll base = 0;
    dbg(sum, out);
    for ( int k = N-1 ; k >= 1 ; --k ) {
        // k = number of things that get moved to match min.
        sum += dat[0] - dat[k];
        ++base;
        ll curr_cost = cdiv(sum - targ, base+1);
        curr_cost = maxll(curr_cost, 0) + base;
        ckmin(out, curr_cost);
        dbgP(k, base, sum, curr_cost, out);
    }
    ps(out);
    return;
}
