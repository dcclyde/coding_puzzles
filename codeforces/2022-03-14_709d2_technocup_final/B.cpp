...
void solve() {
    ...
    int neg_step = 0;
    for ( int k = 0 ; k < N-1 ; ++k ) {
        ll curr = dat[k+1] - dat[k];
        deltas.insert( curr );
        if ( curr > 0 ) {
            if ( pos_step == 0 or pos_step == curr ) {
                pos_step = curr;
            } else {
                cout << -1 << '\n'; return;
            }
        } else if ( curr < 0 ) {
            if ( neg_step == 0 or neg_step == curr ) {
                neg_step = curr;
            } else {
                cout << -1 << '\n'; return;
            }
    ...
    if ( has_zero && (pos_step != 0 || neg_step != 0 ) ) {
        cout << -1 << '\n'; return;
    }

    if ( pos_step == 0 || neg_step == 0 ) {
        cout << 0 << '\n'; return;
    }

    ll c = pos_step;
    ll m = c - neg_step;  // a -> a+c-m
    if ( dat[0] >= m ) {
        cout << -1 << '\n'; return;
    }
    for ( int k = 0 ; k < N-1 ; ++k ) {
        if ( (dat[k] + c) % m != dat[k+1] ) {
            cout << -1 << '\n'; return;
        }
    }
    cout << m << ' ' << c << '\n';
