...
/*
First handle the ones with no choice.
Then just iterate through, never choosing a "current outright winner".
Only need to look at 2 options per day so runtime is fine.
*/
...
void solve() {
    lls(F, D);
    vector<vector<ll>> dat( D );
    FOR(d, 0, D) {
        lls(num_available);
        rv(num_available, dat[d]);
        for ( auto& x : dat[d] ) {
            --x;
        }
        dbg(d, dat[d]);
    ...
    for ( auto& [f, count] : counts ) {
        if ( leader == -1 || counts[leader] < count ) {
            leader = f;
        }
    }

    if ( counts[leader] > cdiv(D,2) ) {
        cout << "NO\n";
        return;
    }
