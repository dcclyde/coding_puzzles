...
#define OUT_WHITE       "\033[97" << BOLD_MAYBE << "m"
...
/**
 * I'd like to run DFS on the first tree and update some data structure for other.
 * I need:
 * * Insert a vertex
 * * Remove a vertex
 * * Query max size of a subset with no ancestor/descendant pairs
 *
 * ? Naive strategy
 * Maintain a uset of special verts.
 * When inserting a vertex, just walk up the tree until we find an ancestor
 *  or hit root.
 * * This is not good enough.
 *      Tree is linked list
 *      Insert root
 *      Insert bottom, remove bottom
 *      Insert second-from-bottom, remove it
 *      etc
 *
 * I can't have too many distinct depths in my good subset though...
 *      (only sqrt(N) I guess)
 *
 * I could try using binary lifting to query the new guy's ancestor at each
 *      relevant depth?
 *
 */
...
void solve() {
    ...
    // depth --> (vid --> whoever he replaced on arrival)
    umap<int,umap<int,int>> special;

    // vid --> (depth, whoever he replaced)
    V<int> displaced;
    ll out = 0;
    ll score = 0;

    auto insert_vertex = [&](int v) -> void {
        dbgc("i_v", v, pdh(special));
        int d = lca.depth[v];
        for (auto& [od, oetc] : special) {
            if (od <= d) {
                int p = lca.jmp(v, d-od);
                if (oetc.find(p) != oetc.end()) { return; }
            } else {
                // do any of these vertices make me obsolete?
                // ! I guess this would be too slow.
            }
            auto& [ov, orep] = oetc;
            dbgW(v, d, od, ov, orep);
            if (od > d) {
                dbgcP("a");
                // other node is lower.
                if (lca.jmp(ov, od-d) == v) { return; }
            } else if (od < d) {
                dbgcP("b");
                if (lca.jmp(v, d-od) == ov) {
                    special[d] = MP(v, ov);
                    displaced[ov] = orep;
                    special.erase(od);
                    return;
        ...
        special[d] = MP(v, -1);
        ckmax(out, special.size());
    };

    auto remove_vertex = [&](int v) -> void {
        dbgcB("r_v", v, special);
        int d = lca.depth[v];
        auto it = special.find(d);
        if (it == special.end()) {return;}
        auto& [sv, srepl] = it->second;
        if (sv != v) {return;}
        if (srepl != -1) {
            special[lca.depth[sv]] = MP(sv, displaced[srepl]);
        }
        special.erase(d);
    };
    ...
    auto dfs = [&](int curr, int parent, auto&& self) -> void {
        ...
        for (auto& o : G[curr]) {
            ...
            // todo Insert this vertex into the data structure, update out
            insert_vertex(o);
            dbgcR("i_v done", o, special); el;
            self(o, curr, self);
            remove_vertex(o);
            dbgcP("r_v done", o, special); el;
            // todo remove this vertex from the data structure.
