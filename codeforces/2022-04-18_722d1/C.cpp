...
/**
 * What do I want a segtree to store here?
 * I already have hs and he.
 * I want to ask:
 *      Is v inside one of the subtrees I've already inserted? If so, which?
 *      Is any subtree fully inside v?
 * I guess I basically need the closest 1 on each side.
 */
...
void solve() {
    ...
    auto insert_vertex = [&](int v) -> void {
        ...
        if (before != -1 && (before == vs || (before_partner = he[hsinv[before]]) > vs)) {
            // is v a descendant of someone in our special set?
            if (before == vs) {
                dbgcR("here");
                before_partner = vs;
            ...
            // is someone in the special set a descendnt of v?
            // if so do nothing.
        } else {
            st.upd(vs, vs, +1);
            st.upd(ve, ve, +1);
