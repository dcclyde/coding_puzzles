...
void solve() {
    ...
    auto dfs = [&](int curr, int parent, auto&& self) -> void {
        ...
        for (auto& o : G[curr]) {
            ...
            score[curr][0] += max(
                score[o][0] + abs(dat[curr].first - dat[o].first),
                score[o][1] + abs(dat[curr].first - dat[o].second)
            );
            score[curr][1] += max(
                score[o][0] + abs(dat[curr].second - dat[o].first),
                score[o][1] + abs(dat[curr].second - dat[o].second)
            );
        }
        dbg(curr, score[curr]);
    ...
    dfs(0, -1, dfs);
    ll out = max(score[0][0], score[0][1]);
    ps(out);
    return;
}
