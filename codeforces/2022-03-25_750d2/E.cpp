...
void solve() {
    ...
    --MAX_K;
    V<V<ll>> dp(N+1, V<ll>(MAX_K+2, -INF));
    FOR(n, 0, N+1) {
        dp[n][0] = INF;
    }
    ...
    FOR(n, 1, N+1) {
        ...
        int ctr = 0;
        ll total = 0;
        while ( dp[n - ctr][ctr] >= total ) {
            ++ctr;
            total += dat[n - ctr];
        }
        // the last `ctr` items' sum is bigger than dp[n-ctr][ctr].
        if ( dp[n-ctr][ctr-1] > total ) {
            ckmax(dp[n][ctr], total);
        }
        dbg(n, dat[n-1], ctr, total, pdh(dp)); el;
    }
    int out = MAX_K + 1;
    while ( dp[N][out] == -INF ) {
        --out;
