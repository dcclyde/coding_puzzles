...
void solve() {
    ...
    vector<int> out(N);
    if ( N % 2 == 1 ) {
        // do we have 3 of a kind for a value with abs at least 10?
        int chosen_value = INF;
        map<int,int> seen;
        for ( auto& x : dat ) {
            ++seen[x];
            if ( seen[x] >= 3 ) {
        ...
        if ( chosen_value == INF ) {
            // we can't be near the max total. Just do a simple thing.
            int sum = 0;
            FOR(i, 0, 3) { sum += dat[i]; }
            int sidx = 0;
            while ( sum == dat[sidx] ) {
            ...
            FOR(i, 0, 3) {out[i] -= dat[sidx];}
            out[sidx] += sum;
            FOR(i,0,3) special.insert(i);
        } else {
            vector<int> positions;
            FOR(k, 0, N) {
                if ( dat[k] == chosen_value ) {
            ...
            out[positions[1]] = -1;
            out[positions[2]] = -1;
            special.insert(all(positions));
        }
    }
    int ptr = 0;
    while ( ptr < N ) {
        int a = ptr;
        while ( special.count(a) ) {
            ++a;
        }
        int b = a+1;
        while ( special.count(b) ) {
            ++b;
        }
        if ( b >= N ) {
            break;
        }
        // positions a, b are paired.
        out[a] = dat[b];
        out[b] = -dat[a];
        ptr = b + 1;
    }
