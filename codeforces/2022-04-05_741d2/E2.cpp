...
/*
    Given a string S.
    I need to build something(s) that let me answer:
        Given starting positions A and B, how long of a matching substr do we find?
...
function<bool(pii, pii)> compare = [](pii a, pii b) {
    ...
    if (la == lb) {
        ...
        // now comparison depends on the next character after the common ones.
        int idxa = la + num_common;
        int idxb = lb + num_common;
        dbg(num_common, idxa, idxb);
        ...
        int idxa = la + num_common;
        int idxb = lb + num_common;
        dbgB(idxa >= N || dat[idxa] < dat[idxb]);
        return (idxa >= N || dat[idxa] < dat[idxb]);
    }
};

function<bool(pii, pii)> compare_brute = [](pii a, pii b) {
    auto& [la, lena] = a;
    auto& [lb, lenb] = b;
    if (lb < 0) {return false;}
    if (la < 0) {return true;}
    return dat.substr(la, lena) < dat.substr(lb, lenb);
};
...
int lis(V<pii> v) {
	...
	for (pii& x : v) {
		...
		if (lo == sz(min_last)) min_last.emplace_back();
		min_last[lo] = x;
	}
	return sz(min_last)-1;
}
...
void solve() {
    ...
    FOR(a, 0, N) {
        ...
        FOR(offset, 0, N-a) {
            mla[offset] = lstTrue(0, N-offset, [&](int len) {
