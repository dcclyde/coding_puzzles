...
#define ROFint(i,a,b) for (int i = (b)-1; i >= (a); --i)
...
/*
    ...
    0 at position k in second half:
        Just choose [1 .. k-1] and [1 .. k].

    Second half is all 1s:
        multiplier needs to be odd.

    If first half has a zero: I guess just do same thing for multipler of 1?

    So now just what if the whole thing is 1s?

    In that case, use (2^N-1)*(2^N+1) = 2^(2N) - 1.
*/
...
void solve() {
    ...
    FOR(k, 0, N) {
        if (dat[k] == '0') {
            zero_pos.push_back(k);
        }
    }

    if (zero_pos.size() > 0) {
        if (zero_pos.front() <= N/2 - 1) {
            int x = zero_pos.front();
            V<int> out = {x, N-1, x+1, N-1};
            return pv1(out);
        } else if (zero_pos.back() >= (N+1)/2) {
            int x = zero_pos.back();
            V<int> out = {0, x, 0, x-1};
            return pv1(out);
        }
