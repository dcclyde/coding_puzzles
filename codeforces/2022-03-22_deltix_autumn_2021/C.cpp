...
void solve() {
    ...
    FOR(first, 0, E) {
        vector<int> working;
        for ( int m = 0 ; first + E*m < N ; ++m ) {
            working.push_back( dat[first + E*m] );
        }
        dbgY(first, working);
        FOR(one_pos, 0, working.size()) {
            if ( working[one_pos] != 1 ) {
                continue;
            }
            // how many 0s on each side?
            ll zeros_left = 1;
            while ( one_pos - zeros_left >= 0 && working[one_pos - zeros_left] == 0 ) {
                ++zeros_left;
            }
            --zeros_left;
            ll zeros_right = 1;
            while ( one_pos + zeros_right < working.size() && working[one_pos + zeros_right] == 0 ) {
                ++zeros_right;
            }
            --zeros_right;
            ll curr = (zeros_left + 1) * (zeros_right + 1) - 1;
            out += curr;
            dbgP(one_pos, zeros_left, zeros_right, curr, out);
        }
    }
    ps(out);
