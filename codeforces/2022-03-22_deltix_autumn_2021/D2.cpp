...
struct DSU {
	vi e;
    set<pii> comps_small;  // size, flag bearer
    set<pii> comps_big;
    int k = 0;
    ll sum = 0;  // sum of the k largest component sizes, in comps_big
    // set<pii> comps;
    void init(int N) {
        e = vi(N,-1);
        FOR(j,0,N) {comps_small.emplace(1, j);}
    }
    int increment_k() {
        dbgc("i_k", k, comps_small, comps_big);
        ++k;
        auto it = comps_small.rbegin();
        if ( it == comps_small.rend() ) {
            sum = e.size();
            return sum;
        }
        sum += it->first;
        comps_big.insert(*it);
        comps_small.erase(*it);
        dbgc("i_k END", k, comps_small, comps_big);
        return sum;
    }
	int get(int x) { return e[x] < 0 ? x : e[x] = get(e[x]); }
    void remove_comp(pii p) {
        dbgcP("r_c", p, k, comps_small, comps_big);
        auto it = comps_small.find(p);
        if ( it != comps_small.end() ) {
            comps_small.erase(it);
        } else {
            sum -= p.first;
            it = comps_big.find(p);
        ...
        dbgcP("r_c END", p, k, comps_small, comps_big);
        return;
    }
    void create_comp(pii p) {
        dbgcP("C_c", p, k, comps_small, comps_big);
        sum += p.first;
        comps_big.insert(p);
        auto it = comps_big.begin();
        sum -= it->first;
        comps_small.insert(*it);
        comps_big.erase(it);
        dbgcP("C_c END", p, k, comps_small, comps_big);
        return;
    }
	...
	bool unite(int x, int y) { // union by size
		...
		if (e[x] > e[y]) swap(x,y);
        remove_comp(MP(-e[y], y));
        remove_comp(MP(-e[x], x));
        create_comp(MP(-e[x]-e[y], x));
		e[x] += e[y]; e[y] = x;
