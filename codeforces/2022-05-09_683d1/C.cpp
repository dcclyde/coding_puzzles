...
/**
    Remove nums to make a good list.
    Each node is the source of 1 directed edge.
    But realistically we only get N-1 edges because the smallest XOR will
        yield the same edge in both directions.

    It's bad if there's a cycle.

    The two vertices with smallest XOR should collectively be the "root".
 */
    ...
    FOR(mask, 0, 1<<BITS) {
        ...
        FOR(xi, 0, dat.size()) {
            ...
            FOR(yi, 0, dat.size()) {
                auto& y = dat[yi];
                if (x == y) {continue;}
                if ((best == -1) || ((x ^ y) < (x ^ dat[best]))) {best = yi;}
            }
            // dbgP(x, dat[best]);
            dsu.unite(xi, best);
        }
