...
void solve() {
    ...
    FOR(a, 0, N) {
        FOR(b, a+1, N) {
            // how many terms are already on the arithmetic sequence passing through a and b?
            /*
                Think: a0 + c*k = ak
                c = (ak-a0)/k = (dat[b] - dat[a]) / (b-a).

                I need dat[a] + c*(d-a) = dat[d]
            */
            int c_num = dat[b] - dat[a];
            int c_denom = b-a;
            int num_hits = 0;
            FOR(d, 0, N) {
                int test = dat[a] * c_denom + c_num * (d-a) - c_denom * dat[d];
                if ( test == 0 ) {
                    ++num_hits;
                }
            }
            out = min( N-num_hits, out );
        }
    }
