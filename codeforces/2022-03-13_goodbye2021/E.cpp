...
/*
    I'm thinking about having 2 segtrees A and B.
    A[l..r] tells me how many elements from that range are still here.
    B[l..r] is a std::set of contained elements.
*/

tcT> struct SegTreeSum { // cmb(ID,b) = b
...
// T will be a set of pairs like (letter, idx).
tcT> struct SegTreeElements { // cmb(ID,b) = b
	const T ID{};
	...
	void init(int _n) { // upd, query also work if n = _n
		...
		seg.assign(2*n,ID); }
	// void pull(int p) { seg[p] = cmb(seg[2*p],seg[2*p+1]); }
	void upd(int p, char c) { // set val at position p
        p += n;
        char old = 'z'+2;
        if ( seg[p].size() > 0 ) {
            old = *(seg[p].begin());
	...
	bool query_lt(int l, int r, char c) {	// associative op on [l, r]
		// T ra = ID, rb = ID;
		for (l += n, r += n+1; l < r; l /= 2, r /= 2) {
			if (l&1) {
                // ra = cmb(ra,seg[l++]);
	...
	bool query(int l, int r, char c) {	// associative op on [l, r]
		// T ra = ID, rb = ID;
		for (l += n, r += n+1; l < r; l /= 2, r /= 2) {
			if (l&1) {
                // ra = cmb(ra,seg[l++]);
...
void solve() {
    ...
    SegTreeSum<int> stsum;
    SegTreeElements<set<char>> stelements;
    stsum.init(N);
    stelements.init(N);
    FOR(k, 0, N) {
        stsum.upd( k , 1 );
        stelements.upd( k , S[k] );
    }
    ...
    while ( tidx < size(T) ) {
        dbgc(OUT_MARK<<"START ITER");
        while ( sidx < size(S) && stsum.query_one(sidx) == 0 ) {
            ++sidx;
        }
        if ( sidx == size(S) ) {
            best_total = min(best_total, running_cost);
            break;
        }

        // get first position of a letter < c.
        char c = T[tidx];
        dbg(tidx, c);
        int first_lessthan_c = fstTrue( (int)0 , (int)N-1 , [&](int idx) {
            return stelements.query_lt( 0 , idx , c );
        });
        int first_c = fstTrue( (int)0 , (int)N-1 , [&](int idx) {
            return stelements.query( 0 , idx , c );
        });
        #ifdef DCCLYDE_LOCAL
        for ( int k = stsum.n ; k < stsum.seg.size() ; ++k ) {
            dbg(k-stsum.n, stsum.seg[k]);
        } el;
        for ( int k = stelements.n ; k < stelements.seg.size() ; ++k ) {
            dbg(k-stelements.n, stelements.seg[k]);
        } el;
        #endif
        dbg(first_lessthan_c, first_c);

        // if I collect first_lessthan_c then I win.
        // how many elements do I need to switch it past?
        if ( first_lessthan_c < N ) {
            ll local_cost = stsum.query(0, first_lessthan_c - 1);
            best_total = min(best_total, running_cost + local_cost);
            dbgc("win NOW", local_cost, running_cost, best_total);
        }

        // if I collect first_c then I can at least move onto the next step.
        if ( first_c == N ) {
            break;
        }
        ll local_cost = stsum.query(0, first_c - 1);
        running_cost += local_cost;
        dbgc("win LATER", local_cost, running_cost, best_total);

        // mark this element as used-up.
        stsum.upd( first_c , 0 );
        stelements.upd( first_c , 'z'+1 );
...
// TODO I think one mistake is: I shouldn't just delete my element from the st of std::set,
// TODO since there could be another copy in that range.
// TODO Use a map or otherwise check more carefully.
