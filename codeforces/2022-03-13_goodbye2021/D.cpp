...
// ll min_good_value(dat[k], 1, X);
ll min_good_value(ll running_total, ll running_len, ll X) {
    return X * (running_len + 1) - running_total;
}
...
void solve() {
    ...
    FOR(k, 0, N) {
        if ( k == 0 || !take[k-1] ) {
            ...
            take[k] = true;
            if ( dat[k] + dat[k-1] < 2*X ) {
                dbgc("F a");
                take[k] = false;
            }
            if ( k >= 2 && take[k-2] && dat[k] + dat[k-1] + dat[k-2] < 3*X ) {
                dbgc("F b");
                take[k] = false;
            }
        }
    ...
    // ll running_len = 0;
    // FOR(k,0,N) {
    //     if ( k == 0 || dat[k] + running_total >= X * (running_len + 1) ) {
    //         ++out;
    //         /*
    //             "Which version of running_total and len is more fragile?"
    //             proposals are total = total+dat[k], len = len+1
    //                 OR total = dat[k], len = 1
    //         */
    //         ll mgv1 = min_good_value(running_total+dat[k], running_len+1, X);
    //         ll mgv2 = min_good_value(dat[k], 1, X);
    //         if ( mgv1 > mgv2 ) {
    //             running_total += dat[k];
    //             running_len += 1;
    //         } else {
    ...
    //         running_len = 0;
    //     }
    //     dbg(k, dat[k], out, running_total, running_len);
    // }
