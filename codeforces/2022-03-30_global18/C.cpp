...
void solve() {
    ...
    int other_onesA = N+1 - onesA;
    if ( onesB != onesA && onesB != other_onesA ) {
        return ps(-1);
    }
    int option_main = N+1;
    int option_other = N+1;
    if ( onesB == onesA ) {
        // how many overlaps?
        int overlap = 0;
        FOR(k, 0, N) {
        ...
        option_main = 2 * (onesA - overlap);
    }
    if ( onesB == other_onesA ) {
        // imagine picking one "overlap" position and flipping everything
        // or I guess just do that...
        string otherA = A;
        int special = 0;
        while ( A[special] == '0' || B[special] == '0' ) {
            ++special;
        }
        dbg(special);
        FOR(k, 0, N) {
            ...
            otherA[k] = ((otherA[k] == '0') ? ('1') : ('0'));
        }
        dbg(otherA, B);
        int overlap = 0;
        FOR(k, 0, N) {
            if ( otherA[k] == B[k] && otherA[k] == '1' ) {
                ++overlap;
            }
        }
        option_other = 2 * (onesB - overlap) + 1;
    }
    int out = min(option_main, option_other);
    ps(out);
