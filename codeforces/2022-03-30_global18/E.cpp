...
tcT, class S> struct LazySeg {
	...
	void init(int _n) {
        ...
        SZ = n;
		seg.assign(2*n,idT); lazy.assign(2*n, idS);  // ! initialize
	}
	void push(int ind, int L, int R) { /// modify values for current node
		seg[ind].first += lazy[ind]; // ! lazy * seg
...
void solve() {
    ...
    V<int> parent(N, -1);
    V<int> a2t(N);  // "array to tree".
    V<pii> t2a(N);  // "tree to array". v maps to left and right endpoints of v's subtree.
    V<int> depth(N);  // initial value will be depth + 1.
    int ctr = 0;
    function<int(int)> traverse = [&](int curr) {
        if ( curr == 0 ) {
            depth[curr] = 0;
        } else {
    ...
    // now build a lazy segment tree I guess?
    LazySeg<pii, int> st;
    st.init(N);
    FOR(k, 0, N) {
        st.seg[st.n + k] = MP(depth[a2t[k]] + 1, k);
    }
    st.build();
    dbg_only(st.detailed_printouts());
    dbg(st);
    // FOR(k, 0, N) {
    //     // assign value 1 to every vertex.
    //     int v = a2t[k];
    //     auto& [l, r] = t2a[v];
    //     st.upd(l, r, +1);
    // }
    ...
    V<bool> devalued(N, false);
    FOR(k, 0, K+1) {
        // greedily pick the current best option.
        auto [value, idx] = st.query(0, N);
        remaining.push_back( remaining.back() - value );
        if ( remaining.back() == 0 ) {
            break;
        }

        // take away this option's value.
        int v = a2t[idx];
        while ( true ) {
            if ( devalued[v] ) {
                break;
    ...
    if ( remaining[K] > K ) {
        // Blue will win.
        // Red should just reduce `remaining` as much as possible.
        ll A = remaining[K];
        ll out = 0;
        FOR(b, K+1, A+1) {
            ll score = -(b-K)*(N-K-b);
