...
void precompute() {
    // while ( (1<<BIT_MAX) <= R_MAX ) {
    //     ++BIT_MAX;
    // }
    dbg(BIT_MAX);
    prefix_cpb.assign(BIT_MAX + 1, V<int>(R_MAX + 1, 0));
    FOR(k, 1, R_MAX+1) {
        FOR(b, 0, BIT_MAX + 1) {
            prefix_cpb[b][k] = prefix_cpb[b][k-1];
            if ( (k>>b) & 1 ) {
                ++prefix_cpb[b][k];
            }
        }
...
void solve() {
    ...
    int out = N + 1;
    FOR(b, 0, BIT_MAX) {
        int bit_count = prefix_cpb[b][R] - prefix_cpb[b][L-1];
        ckmin(out, N - bit_count);
    }
    dbg(TIME());
    ps(out);
}
...
void first() {
    ...
    int MAX_BIT = 20;
    V<int> cpb(MAX_BIT+1);  // "count per bit"
    FOR(x, L, R+1) {
        FOR(b, 0, MAX_BIT+1) {
            if ( (x>>b) & 1 ) {
                ++cpb[b];
