...
inline namespace Helpers {
    ...
    tcT> T increment(T t) { increment_one(t,+1); return t; }
    // void increment() {} // add one to each, for output. USE DEEP COPIES.
...
void solve() {
    ...
    auto legal = [&](pll& U, pll& V, pll& W) -> bool {
        // Can I step U --> V --> W?
        auto& [ux, uy] = U;
        auto& [vx, vy] = V;
        auto& [wx, wy] = W;
        pll d1 = {vx - ux, vy - uy};
        pll d2 = {wx - vx, wy - vy};
        ll dot = d1.first*d2.first + d1.second*d2.second;
        return dot < 0;
    };
    ...
    FOR(k, 2, N) {
        ...
        int B = wip.back();
        int C = k;
        bool ABC_ok = legal(dat[A], dat[B], dat[C]);
        if (ABC_ok) {
            wip.pop_back();
            wip.push_front(B);
            wip.push_front(C);
