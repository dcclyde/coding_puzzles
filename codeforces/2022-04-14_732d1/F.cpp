...
/*
    Given 2N arrays.
    Guaranteed possible to build a Latin square such that
    array A_j agrees in one position with array B_j.
    I need to know how many ways to make a latin square.

    What can I get out of the "agrees in at least one position" criterion?
...
void solve() {
    ...
    FOR(k, 0, N) {
        ...
        while (A[k] < B[k]) {
            out[i_second].second = k;
            ++i_second;
            ++A[k];
        }
    }
    dbg(i_first, i_second);
    assert(i_first == i_second);
    out.resize(i_first);

    ps(out.size());
    for(auto& x : out) {
        ps1(x.first, x.second);
    }
