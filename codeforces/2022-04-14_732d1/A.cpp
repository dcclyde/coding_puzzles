...
/*
    If all numbers are distinct, I could just sort
    then check if (source - dest) % 2 == 0.
*/
...
void solve() {
    ...
    V<umap<int,int>> pre(2);
    FOR(k, 0, N) {
        ++pre[k%2][dat[k]];
    }

    sort(all(dat));
    V<umap<int,int>> post(2);
    FOR(k, 0, N) {
        ++post[k%2][dat[k]];
    }

    // maybe they need to match now?
    FOR(m, 0, 2) {
        for(auto& [val, count] : pre[m]) {
            if (post[m][val] != count) {
                return no;
            }
        }
