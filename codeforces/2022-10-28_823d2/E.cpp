...
/**
    I want to do some "line sweep" thing where I add a new element at end
    and then I have some data structures that track "max so far" type stuff.

    state = S = list of tuples like (min val, max val, length).
    When adding a new element at the end, I may need to pop some stuff off the back of S.
    I think counting how many times each point gets added/removed shows it's still O(N).
 */
...
/**
    If curr < prev, then maxes will look the same as we extend back with len >= 2,
        and mins may need more updating.
...
void solve() {
    ...
    ll out = 1;
    V<pll> lo, hi;
    lo.emplace_back(dat[0], 1);
    hi.emplace_back(dat[0], 1);
    ll score = 1;  // "value of current state"
    FOR(n, 1, N) {
        ll v = dat[n]; ll prev = dat[n-1];
        if (v == prev) {
            ++score;
            ++lo.back().s; ++hi.back().s;
        } else if (v < prev) {
            ll len = 1;  // length of new chunk
            ll hk = hi.size() - 1; ll hlen = hi[hk].s;
            while (lo.size() > 0 && lo.back().f >= v) {
                // update score: lo is changing from lo.back().f to v on this range.
                ll dlen = lo.back().s;
                while (hlen < dlen) {
                    if (hi[hk].f % lo.back().f == 0) {score -= hlen;}
                    if (hi[hk].f % v == 0) {score += hlen;}
                    dlen -= hlen;
                    --hk;
                    hlen = hi[hk].s;
                }
            ...
            lo.emplace_back(v, len);
        } else {
            ll len = 1;  // length of new chunk
            while (hi.size() > 0 && hi.back().f <= v) {
                len += hi.back().s;
                hi.pop_back();
                // ! do something to score.
            }
            hi.emplace_back(v, len);
        }
        out += score;
    }
    ps(out);
