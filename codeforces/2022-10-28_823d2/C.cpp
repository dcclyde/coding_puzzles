...
/**
    ...
    Think: Repeatedly find first place where dat isn't sorted and delete 1st num.
    We'll insert all of them later.
 */
...
void solve() {
    ...
    V<ll> count_per_digit(10);
    auto& cpd = count_per_digit;

    auto incr = [&](char c) {return (c=='9') ? c : (c+1);};

    char lowest = '9';
    ROF(k, 0, dat.size()) {
        if (dat[k] > lowest) {++cpd[incr(dat[k]) - '0'];}
        else {
            ++cpd[dat[k] - '0'];
            ckmin(lowest, dat[k]);
        }
    ...
    string out;
    FOR(k, 0, 10) {out += string(cpd[k], '0'+k);}
    ps(out);
    return;
}
