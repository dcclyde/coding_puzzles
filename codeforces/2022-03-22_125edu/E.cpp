...
void solve() {
    ...
    FOR(k, 0, K) {
        FOR(a, 0, N-1) {
            ...
            FOR(b, 0, N-a) {
                /*
                    dp[ a ][ k ] = ways to draw first a edges from 1
                    with those weights <= k.
                    b = num edges we're about to draw with weight k+1.
                */
                dbg(a, k, b);

                int exp = (b*(b-1))/2 + a*b;
                mi base = K-k;
                mi curr = pow(base, exp);
                mi bin = comb(N-1-a, b);
                curr *= bin;
                mi contrib = dp[a][k] * curr;

                dbg(MP(a+b,k+1), (int)bin, (int)base, exp, (int)curr,  (int)contrib);
                dp[ a+b ][ k+1 ] += contrib;
                // if ( b > 0 && k+1 <= K ) {
                //     dbg(MP(a+b,k+1), (int)bin, (int)base, exp, (int)curr,  (int)contrib);
                //     dp[ a+b ][ k+1 ] += contrib;
                // } else if ( b == 0 ) {
                //     dbg(MP(a+b,k), (int)bin, (int)base, exp, (int)curr,  (int)contrib);
                //     dp[ a+b ][ k ] += contrib;
                // }
                el;
            }
