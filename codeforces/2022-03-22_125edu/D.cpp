...
void solve() {
    ...
    FOR(k, 0, num_enemies) {
        lls(d,h);
        enemies.push_back(d*h);
    }
    vector<ll> best_value_for_cost( C+1 );
    for ( auto& [cost, value] : friends ) {
        for ( int m = 0 ; m*cost <= C ; ++m ) {
            ckmax( best_value_for_cost[m*cost] , m*value - 1 );
        }
    }
    dbg(best_value_for_cost);

    FOR(k, 1, C+1) {
        ckmax(best_value_for_cost[k], best_value_for_cost[k-1]);
    }
    dbg(best_value_for_cost);

    dbgY(enemies);
    V<int> out;
    FOR(k, 0, enemies.size()) {
        int curr = fstTrue(0, (int)C, [&](int x){return best_value_for_cost[x] >= enemies[k];});
        out.push_back((curr>C) ? -1 : curr);
    }
    pv(out);
