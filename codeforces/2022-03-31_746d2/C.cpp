...
void solve() {
    ...
    int big = *max_element(all(dat));

    // count bits in each slot.
    int MAX_BITS = 0;
    while ( (1LL << MAX_BITS) <= big ) {
        ++MAX_BITS;
    }
    V<int> B(MAX_BITS);
    for ( auto& x : dat ) {
    ...
    // need to find a subtree with xor of x.
    V<int> subtree_xor(N);
    V<int> num_good_children(N);
    function<void(int,int)> Dfs = [&](int curr, int parent) {
        for ( auto& o : G[curr] ) {
            ...
            Dfs(o, curr);
            if (subtree_xor[o] == x || num_good_children[o] == 1) {
                ++num_good_children[curr];
            }
            if ( subtree_xor[o] == 0 && num_good_children[o] == 1) {
                num_good_children[curr] = 2;
            }
            subtree_xor[curr] ^= subtree_xor[o];
        }
        subtree_xor[curr] ^= dat[curr];
    };
    int root = 0;
    while ( G[root].size() > 1 ) {
        ++root;
