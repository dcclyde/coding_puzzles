...
void brute() {
    // N = array size
    // K = num binary digits in each number
    lls(N, K);
    dbgR(N, K);
    ...
    ll out = 0;
    {
        ...
        function<void(void)> product_rec_helper = [&]{
            if (curr.size() == N) {return do_something();}  // ! define max depth
            int n = curr.size();
            FOR(k, 0, 1<<K) {  // ! iterate over options at depth n
                    curr.push_back(k);
                    product_rec_helper();
                    curr.pop_back();
            }
...
/*
    What does it take to make bit b bigger?
    What does it take to make bit b match?

    Bigger: Need all 1s, AND n has to be even.

    Match with 1s: all 1s and n odd
    Match with 0s: any even number of 1s that's < n.
*/
...
void solve() {
    ...
    // ways to choose an even number of 1s out of N positions.
    // V<mi> even_ones_helper(N+1);
    // auto& ezh = even_ones_helper;
    // ezh[0] = 1;
    ...
    mi out = 0;
    FOR(b, 0, K) {
        el; dbgcY("start", b, ties, out);
        if (N % 2 == 0) {
            // we could just win here.
            // 1 way to set up this bit, then do anything we want from there.
            mi contrib = pow(mi(2), N*(K-b-1)) * ties;
            out += contrib;
            dbgc("win now", contrib, out);
        }
        // ok now how many ways to tie on this bit?
        mi new_ties = 0;
        if (N % 2 == 1) {
            // Tie at 1: choose 1 in all slots.
            new_ties += 1;
        }
        new_ties += eoh;
        ties *= new_ties;
