...
/*
    I start in bottom row.
    If he goes up, I do too.
    If he goes down, I reset my plans.
    I want to walk from col 2 to 7. That should force a step up if he was down here.

*/

map<string, pii> king_moves;

pii get_kingmove() {
    if (king_moves.size() == 0) {
        king_moves["Right"] = MP(0, 1);
        king_moves["Left"] = MP(0, -1);
        king_moves["Up"] = MP(-1, 0);
        king_moves["Down"] = MP(1, 0);
        king_moves["Down-Right"] = MP(1, 1);
        king_moves["Down-Left"] = MP(1, -1);
        king_moves["Up-Left"] = MP(-1, -1);
        king_moves["Up-Right"] = MP(-1, 1);
        king_moves["Done"] = MP(0, 0);
    }
    cout << flush;
    strings(q);
    return king_moves[q];
...
void solve() {
    int r = 0;
    while (r < 8) {
        // plan: visit (r, 1) through (r, 6).
        // if we see an "up" move, restart.
        // if we see a "down" move, step down and move to next r.
        // if we get to (r, 6) and don't see ups or downs, step down and next r.
        bool saw_up = true;
        while (saw_up) {
            saw_up = false;
            FOR(c, 1, 7) {
                auto [kdr, kdc] = move_to(r, c);
                dbgP(kdr, kdc);
                if (kdr == 0 && kdc == 0) {
                    dbgcY("win");
                    return;
        ...
        auto [kdr, kdc] = move_to(r+1, curr_c + 1);
        if (kdr == 0 && kdc == 0) {return;}
        ++r;
    }
