...
/*
    ...
    Query: min, plus "dest" which is probably problem-specific
*/
const int INF = 2e9;
...
/*
    Data structures: How to quickly test if rows j, k are connected?

    Graph: If everything is 1s then there'll be lots of possible edges.
        Need to avoid building all of them.

    Maybe I'm storing a segtree?
    st[k] = cheapest cost to get to position k and jump to current end.

    After processing each row, add 1 to the whole array.
...
void solve() {
    ...
    FOR(k, 0, N) {
        ...
        dbgcY("start", k);
        dbg(st);
        // what's the cheapest cost to get here?
        int cost = INF;
        for(auto& [l, r] : dat[k]) {
