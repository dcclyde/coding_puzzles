...
void brute() {
    ...
    re(spec);
    if (N == 1) {
        if (spec.f >= TARGET) {return YES;}
        else {return NO;}
    }

    rv(N-1, dat);
    dbgcR("BRUTE", N, TARGET, spec, dat);
    el;
    ...
    do {
        ...
        ll prop_final = min({c, spec.f}) + spec.s;
        ckmax(prop_final, spec.f);
        if (prop_final >= TARGET) {
            dbgcY("yes brute", dat);
            return YES;
        }
...
void solve() {
    ...
    ++global_ctr;
    // if (global_ctr == 222) {
    //     string helper = to_string(N) + "_" + to_string(TARGET);
    //     return ps(helper);
    // }
    V<pll> dat;
    pll spec;
    ...
    rv(N-1, dat);
    dbgcR("SO", N, TARGET, spec, dat);
    el;
    if (global_ctr == 222 && false) {
        dat.push_back(spec);
        string helper = "";
        for(auto& [a,b] : dat) {
            helper += ts(a) + "_" + ts(b) + "___";
        }
        helper.pop_back(); helper.pop_back(); helper.pop_back();
        return ps(helper);
    }

    sort(all(dat), [&](auto p, auto q) -> bool {return p.f + p.s < q.f + q.s;});
    ll c = 0;
    for (auto& [a, b] : dat) {
        if (c == 0) {c = a; continue;}
        ll prop = min({a, c}) + b;
    ...
    ll prop_final = min({c, spec.f}) + spec.s;
    ckmax(prop_final, spec.f);
    if (prop_final >= TARGET) {return YES;}
    else {
        dbgcY("no so", dat);
        return NO;
    }
