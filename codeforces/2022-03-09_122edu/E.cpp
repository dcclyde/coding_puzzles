...
template<typename T>
ll mst_cost(T& edges_global, int V, ll x) {
    static vector<tuple<ll,int,int>> edges( edges_global.size() );
    edges = edges_global;
...
void solve() {
    ints(V, E);
    vector<tuple<ll,int,int>> edges( E );
    FOR(k,0,E) {
        auto& [w,a,b] = edges[k];
        cin >> a >> b >> w;
    }
    ...
    FOR(k,Qe,Q) {
        queries[k] = (queries[k-1] * qa + qb) % qc;
    }
    dbg(edges);
    dbg(queries);
    el;
    ...
    /*
        I need to do a bunch of precomputation on the graph before worrying about the queries.
        I'll have 45k "transition points".
        For each TP, build an MST?
        Then costs will transition linearly between TPs?
    */
    map<int, ll> transition_points;
    for ( int j = 0 ; j < edges.size() ; ++j ) {
        int wj = get<0>( edges[j] );
        for ( int k = 0 ; k < edges.size() ; ++k ) {
            int wk = get<0>( edges[k] );
            // TP(s) are halfway in between.
            int tp = (wk+wj) / 2;
            transition_points[tp] = -1;
            if ( (wk-wj) % 2 == 1 ) {
                ++tp;
                transition_points[ tp ] = -1;
            }
    ...
    transition_points[0] = -1;
    transition_points[100'000'000] = -1;

    for ( auto& [tp, cost] : transition_points ) {
        // run an MST algorithm.
        cost = mst_cost(edges, V, tp);
    }
    dbg(transition_points); el;

    // now start actually handling queries.
    ll out = 0;
    #ifdef DCCLYDE_LOCAL
    vector<ll> results;
    results.reserve( Q );
    #endif DCCLYDE_LOCAL
    for ( auto& q : queries ) {
        auto it = transition_points.lower_bound( q );
        ll out_local;
        if ( it->first == q ) {
            out_local = it->second;
            dbgc("tp" , q , out_local );
        } else {
            // q must be between 2 transition points.
            ll x2 = it->first;
            ll y2 = it->second;
            --it;
            ll x1 = it->first;
            ...
            out_local = y1 + slope * (q - x1);
            dbgc("interpolate", q, MP(x1,y1), MP(x2,y2), out_local);
        }
    #ifdef DCCLYDE_LOCAL
        results.push_back( out_local );
    #endif
        out ^= out_local;
    }
    #ifdef DCCLYDE_LOCAL
