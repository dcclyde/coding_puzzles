...
bool test_win(ll health_me, ll damage_me, ll health_monster, ll damage_monster) {
    ll turns_to_win = cdiv(health_monster, damage_me);
    ll turns_to_lose = cdiv(health_me, damage_monster);
    bool out = (turns_to_win <= turns_to_lose);
    return out;
}
...
void solve() {
    ll health_me, damage_me, health_monster, damage_monster;
    cin >> health_me >> damage_me >> health_monster >> damage_monster;
    ll coins, dweapon, dhealth;
    cin >> coins >> dweapon >> dhealth;

    for ( ll coins_on_attack = 0 ; coins_on_attack <= coins ; ++coins_on_attack ) {
        ll coins_on_health = coins - coins_on_attack;
        ll upgraded_attack = damage_me + coins_on_attack * dweapon;
        ll upgraded_health = health_me + coins_on_health * dhealth;
        if ( test_win(upgraded_health, upgraded_attack, health_monster, damage_monster) ) {
            cout << "YES\n";
            return;
        }
    }
