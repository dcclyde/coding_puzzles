...
vector<int> costs( 1001 , -1 );
int MAX_COST = 0;
void init_costs() {
    deque<pii> todo;
    vector<bool> seen( costs.size() , false );
    seen[0] = true;
    seen[1] = true;
    todo.emplace_back(1, 0);
    while ( ! todo.empty() ) {
        ...
        for ( int k = 1 ; k <= pos ; ++k ) {
            int proposed = pos + (pos/k);
            if ( proposed >= costs.size() || seen[proposed] ) {
                continue;
            }
            seen[proposed] = true;
            todo.emplace_back( proposed , cost+1 );
        }
    }
    // dbg(MAX_COST, costs); el;
    return;
}
...
void solve() {
    ints(N, max_ops);
    vector<ll> B, C;  // B <= 1e3, C <= 1e6
    rv(N, B);
    rv(N, C);

    // I guess this is a knapsacky dp?
    /*
        f(n, k) = best score after considering first n options and using k ops.

        if object n+1 has cost C and value V, then f(n+1,k)
            can be same as before, or can be f(n, k-C) + V.

        ! I think I could bitset this if the speed would matter but it won't.

        I can't just run O(max_ops * N).
        But also the costs should always be <= 12,
        so we'll never spend more than N*12 anyway.
    */
    max_ops = min(max_ops, 12*N);
    vector<int> dp( max_ops + 1 , -1 );
    dp[0] = 0;
    for ( int n = 1 ; n <= N ; ++n ) {
        // consider adding the nth item.
        int cost = costs[B[n-1]];
        int value = C[n-1];
        for ( int k = dp.size()-1 ; k >= cost ; --k ) {
