...
void solve() {
    ...
    ll big = max_element(dat);
    for(auto& x : dat) {x = big - x;}
    ...
    auto subsolve = [&]() {
        ...
        FOR(k, 0, N) {
            sum += dat[k];
            twos_possible += dat[k] / 2;
        }

        auto check = [&](ll days) {
            // how many twos will we get?
            ll cycles = days / 2;
            ll twos = min(twos_possible, cycles);
            ll ones = days - cycles;
            dbgP(days, ones, twos, sum);
            if (ones + 2*twos >= sum) {
                return true;
            } else {
                return false;
        ...
        ll out = fstTrue(0, (ll)1e18, check);
        return out;
    };
