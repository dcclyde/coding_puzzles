...
void solve() {
    ...
    ll recent_cost = 0;
    ROF(slot, K-1, N) {
        ll need = dat[slot] - added_value;
        ckmax(need, 0);
        costs[slot] = cdiv(need, K);
        dbg(slot, added_value, need, costs[slot]);

        added_value += K * costs[slot];
        recent_cost += costs[slot];
        if (slot + K < N) {
            recent_cost -= costs[slot + K];
        }

        added_value -= recent_cost;
        dbgP(added_value, recent_cost);
        el;
    }

    // need to handle 0..K-2 separately.
    el;
    dbgcY("last part");
    V<ll> need_extra(K-1);
    ll cost_extra = 0;
    ROF(slot, 0, K-1) {
        ll need = dat[slot] - added_value;
        ckmax(need, 0);
        ll cost_here = cdiv(need, slot+1);
        ckmax(cost_extra, cdiv(need, slot+1));
        dbg(slot, added_value, need, cost_here, cost_extra);
