...
void brute() {
    ...
    V<V<bool>> dat(3, V<bool>(N));
    dbgR(dat_str, dat);
    FOR(r, 0, 3) FOR(c, 0, N) {
        dat[r][c] = (dat_str[r][c] == '1');
    }
        ...
        FOR(c, L, R+1) {
            FOR(r, 0, 3) {
                if (!dat[r][c]) {
                    ...
                    if (r > 0 && dat[r-1][c]) {
                        dsu.unite(3*(c-L) + r, 3*(c-L) + r-1);
                    }
                    if (c > L && dat[r][c-1]) {
                        dsu.unite(3*(c-L-1) + r, 3*(c-L) + r);
                    }
                }
            }
        }
        set<int> heads;
        FOR(c, L, R+1) FOR(r, 0, 3) {
            heads.insert(dsu.get(3*(c-L) + r));
        }
        int out = heads.size() - num_bad;
        ps(out);
    }
}

/*
    Segtree node state should be:
    * num components
    * bitmask for left edge, bitmask for right edge
    * if left edge is 101: are those same connected component?
    * same for right edge
    * Is this entire segment just 101s?

    All that last stuff could be a bitset<3+3+3> I guess?
*/

const V<int> lbits = {0, 1, 2};
const V<int> rbits = {3, 4, 5};
const int lsamebit = 6;
const int rsamebit = 7;
const int allspecbit = 8;

bool spec_r(bitset<9>& x) {
    return x[3] & (!x[4]) & x[5];
}
bool spec_l(bitset<9>& x) {
    return x[0] & (!x[1]) & x[2];
}
bool all_r(bitset<9>& x) {
    return x[3] & (x[4]) & x[5];
}
bool all_l(bitset<9>& x) {
    return x[0] & (x[1]) & x[2];
}

V<pair<int, bitset<9>>> mask_to_val(8);

tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{-1, -1};  // ! identity
    T cmb(T a, T b) {
        T c{0, 0};
        auto& [acomps, abits] = a;
        auto& [bcomps, bbits] = b;
        auto& [ccomps, cbits] = c;
        if (acomps == -1) {return b;}
        if (bcomps == -1) {return a;}
        dbgP(a, b);
        ...
        if (abits[allspecbit] & bbits[allspecbit]) {
            ccomps = 2;
            cbits[allspecbit] = 1;
            // cbits[lsamebit] = 0;
            // cbits[rsamebit] = 0;
        ...
        // handle c[?same].
        if (abits[lsamebit] || (abits[allspecbit] && (bbits[lsamebit] || all_l(bbits)))) {
            cbits[lsamebit] = 1;
            dbgcP("L same");
        }
        if (bbits[rsamebit] || (bbits[allspecbit] && (abits[rsamebit] || all_r(abits)))) {
            cbits[rsamebit] = 1;
            dbgcP("R same");
        }

        ccomps = acomps + bcomps;
        dbg(spec_r(abits), spec_l(bbits));
        if (spec_r(abits) && spec_l(bbits)) {
            dbgcR("both");
            if (abits[rsamebit] && bbits[lsamebit]) {
                ccomps -= 1;
            } else if (abits[rsamebit] || bbits[lsamebit]) {
                ccomps -= 2;
            } else {
                ccomps -= 2;
            }
        } else if (spec_r(abits) && all_l(bbits)) {
            if (abits[rsamebit]) {
                ccomps -= 1;
            } else {
                ccomps -= 2;
            }
        } else if (spec_l(bbits) && all_r(abits)) {
            if (bbits[lsamebit]) {
                ccomps -= 1;
            } else {
                ccomps -= 2;
            ...
            // there's no funny business, just check if anything touches.
            bool touches = false;
            FOR(k, 0, 3) {
                touches |= (abits[rbits[k]] & bbits[lbits[k]]);
            }
            if (touches) {
                --ccomps;
            }
	...
	void upd_base(int p, int mask) { // set val at position p
        T val = mask_to_val[mask];
		seg[p += n] = val;  // ! TODO I need to write this.
        // for (p /= 2; p; p /= 2) pull(p);
    }
	int query(int l, int r) {	// associative op on [l, r]
		T ra = ID, rb = ID;
...
void solve() {
    ...
    FOR(mask, 0, 8) {
        auto& [vcomps, vbits] = mask_to_val[mask];
        bitset<3> mbits(mask);
        // swap(mbits[0], mbits[2]);
        bool temp = mbits[0];
        mbits[0] = mbits[2];
        mbits[2] = temp;
        FOR(k, 0, 3) {
            vbits[lbits[k]] = mbits[k];
            vbits[rbits[k]] = mbits[k];
        }
        if (mask == 0) {
            // vcomps = 0;
        } else if (mask == 5) {
    ...
    V<int> bits(N);
    FOR(k, 0, N) {
        bits[k] = 4*(dat[0][k] == '1') + 2*(dat[1][k] == '1') + (dat[2][k] == '1');
    }
    dbg(bits);
    dat.clear();

    SegTree<pair<int, bitset<9>>> st;
    st.init(N);
    FOR(k, 0, N) {
        st.upd_base(k, bits[k]);
    }
    st.build();
    dbgY(st);
