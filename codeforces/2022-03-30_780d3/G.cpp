...
void solve() {
    ...
    V<int> timers(N);
    V<V<int>> G(N);
    umap<int, set<pii>> horiz;
    umap<int, set<pii>> vert;
    DSU dsu;
    dsu.init(N);
    FOR(vidx, 0, N) {
        ints(x, y, t);
        timers[vidx] = t;
        vert[x].emplace(y, vidx);
        horiz[y].emplace(x, vidx);
    }
    for ( auto& [y, xvals] : horiz ) {
        if ( xvals.size() == 1 ) {
            continue;
        }
        auto it1 = xvals.begin();
        auto it2 = xvals.begin(); ++it2;
        while ( it2 != xvals.end() ) {
            auto& [p1, v1] = *it1;
            auto& [p2, v2] = *it2;
            if ( p2 - p1 <= D ) {
    ...
    umap<int,int> h2t;  // "head to time"
    FOR(k, 0, N) {
        int head = dsu.get(k);
        if ( h2t.find(head) == h2t.end() ) {
            h2t[head] = timers[k];
        } else {
            ckmin(h2t[head], timers[k]);
        }
    }
    dbg(h2t);
    V<int> times;
    ...
    dbg(times);
    int out = 0;
    while ( times.size() > 0 && times.back() > out ) {
        ++out;
        times.pop_back();
    }
    ps(out);
