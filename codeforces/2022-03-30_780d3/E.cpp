...
void solve() {
    ...
    ll running_cost = 0;
    FOR(targ, 0, N+1) {
        out[targ] = running_cost + ctrs[targ];

        // now update running_cost.
        if ( ctrs[targ] > 0 ) {
            extra[targ] = ctrs[targ];
        }
        // find closest value to promote.
        if ( extra.empty() ) {
            break;
        }
        auto& [closest, num] = *extra.rbegin();
        --num;
        if ( num == 0 ) {
            extra.erase(closest);
        }
        running_cost += targ - closest;
    }
