...
/**
    I think I basically want
    1) topological sort
    2) longest chain where we only count vertices with indeg and outdeg >= 2

    Special cases if the answer is 1 or 2?
 */
...
struct TopoSort {
	...
	void ae(int x, int y) { dbgcP("ae", x, y); adj[x].pb(y), ++in[y]; }
	bool sort() {
		queue<int> todo;
...
void solve() {
    ...
    /**
        I guess just immediately delete any edges we won't be allowed to use?
     */
    V<pll> to_be_deleted;
    auto& tbd = to_be_deleted;
    FOR(a, 0, N) for(auto& b : G[a]) {
        if (G[a].size() == 1 || H[b].size() == 1) {tbd.emplace_back(a,b);}
    }
    for(auto& [a, b] : tbd) {
        G[a].erase(b);
        H[b].erase(a);
    ...
    TopoSort ts;
    ts.init(N);
    FOR(a, 0, N) for(auto& b : G[a]) { ts.ae(a, b); }
    ts.sort();

    /**
        Logic should be like:
        dp[k] = most steps we could take and end at k.
        At the end I want to know max_k dp[k].
        For updates, if k is good then its children should have dp >= dp[k]+1.
     */
    V<ll> dp(N);
    ll big = 0;
    dbgY(ts.res);
