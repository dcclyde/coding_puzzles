...
/**
    Just a bunch of casework, right?
    * Try 2 chunks that are far apart: Pay the two smallest costs / 2.
    * Try 2 chunks that are 2 apart: Pay smaller + leftover / 2.
    * Try 2 chunks that are 1 apart: Pay the fat guy till they're equal
          then decrease by 1, 2, 1, 2, 1, 2, ...
 */
...
void solve() {
    ...
    auto d2 = dat;
    sort(all(d2));
    ll far_apart = cdiv(d2[0], 2) + cdiv(d2[1], 2);

    ll two_apart = INF_ll;
    FOR(k, 1, N-1) {
        ll a = dat[k-1]; ll b = dat[k+1];
        if (a > b) {swap(a, b);}
        ll curr = a;
        b -= a;
        curr += cdiv(b, 2);
        ckmin(two_apart, curr);
    }

    ll one_apart = INF_ll;
    FOR(k, 0, N-1) {
        ll a = dat[k]; ll b = dat[k+1];
        if (a > b) {swap(a, b);}
        ll delta = b - a;
        ll initial_steps = min(delta, a);
        ll curr = initial_steps;
        a -= initial_steps;
        b -= 2*initial_steps;

        if (a == 0) {
            curr += cdiv(b, 2);
            ckmin(one_apart, curr);
            continue;
        }
        ll steps_2 = a / 3;
        curr += 2*steps_2;
    ...
    ll out = min({far_apart, two_apart, one_apart});
    ps(out);
    return;
}
