...
/**
    Bounds are very small. DP?
    They went out of their way to give a bound on the TOTAL num pancakes.....??

    Wait can I just simulate??
    I have plenty of time to simulate if I could be confident about strats.

    If I see 8 14
    then I need to end with A B
    all stuff to the left needs to become >= A
    all stuff to right becomes <= B.

    -----

    C suggests DP.
    State is (position, total pancakes across previous positions, prev plate height)
        --> cheapest cost to get here.

    dp[n][t][h]?
    well, start with dp[n-1][s][k].
