...
void solve() {
    ...
    /**
        Seems like normal tree DP.
        What do I want to write down per node?
        Root = START.
        I guess just record min time in my subtree
            if we start at me?
        Probably I should also mark whether this subtree contains END.
     */

    V<ll> subtree_cost(N);
    V<bool> needs_visit(N);
    for(auto& x : dat) {needs_visit[x] = 1;}
    V<bool> has_end(N);

    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        has_end[curr] = (curr == END);
        ...
        for (auto& o : G[curr]) {
            ...
            has_end[curr] = has_end[curr] || has_end[o];
            if (subtree_cost[o] > 0 || needs_visit[o] || has_end[o]) {
                if (has_end[o]) {
                    subtree_cost[curr] += 1 + subtree_cost[o];
                } else {
                    subtree_cost[curr] += 2 + subtree_cost[o];
                }
            }
            dbgP(curr, o, subtree_cost[o], subtree_cost[curr]);
        }
        // update parent after all children

        dbg(curr, parent, subtree_cost[curr], static_cast<bool>(has_end[curr]), static_cast<bool>(needs_visit[curr]));
    };
    dfs(START, -1, dfs);
    return ps(subtree_cost[START]);
