...
/**
    Find the first character that can't become 'a'.
    All the previous ones do become 'a'.
    That first bad one decreases as much as it can.
 */
...
void solve() {
    ...
    V<char> changes;
    FOR(k, 0, 128) {changes.push_back(k);}

    // Can I make the first character into 'a'?
    if (dat[0] - 'a' >= K) {
        char targ = dat[0] - K;
        FOR(c, targ, dat[0]+1) {changes[c] = targ;}
    } else {
        ll idx = 0;
        while (idx < N && dat[idx] - K <= 'a') {++idx;}
        char primary = *max_element(dat.begin(), dat.begin() + idx);
        ll used = primary - 'a';
        ll remaining = K - used;
        dbg(idx, used, remaining);
        FOR(k, 1, used+1) {changes['a'+k] = 'a';}
        if (idx < N) {
            FOR(k, dat[idx]-remaining, dat[idx]+1) {changes[k] = dat[idx]-remaining;}
        }
    }

    for(auto& c : dat) {c = changes[c];}
    ps(dat);
