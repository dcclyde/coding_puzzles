...
void solve() {
    ...
    DSU dsu;
    dsu.init(N);
    V<V<pair<int,bool>>> G(N);
    // V<int> parent(N);
    // V<V<int>> children(N);
    FOR(k, 0, E) {
        int1(a, b);
        strings(role);
        bool imp = (role == "imposter");
        // int1(a); int b = k+1;
    ...
    // get component heads from DSU.
    set<int> roots;
    FOR(k, 0, N) {
        roots.insert(dsu.get(k));
    }

    // now I need to 2-color the graph.

    V<bool> seen(N, false);
    V<int> global_color(N, -1);

    ll out = 0;
    for(auto& root : roots) {
        V<int> num_per_color(2, 0);
        ++num_per_color[0];
        global_color[root] = 0;
        seen[root] = true;
        deque<int> todo;
        ...
        while (!todo.empty()) {
            int curr = todo.back();
            todo.pop_back();
            for (auto& [o, flip] : G[curr]) {
                // if colors match and flip is false, we're good.
                int o_color = global_color[curr] ^ flip;
                if (global_color[o] != -1) {
                    if ( global_color[o] != o_color ) {
                        return ps(-1);
                    } else {
                        continue;
                    }
                }
                global_color[o] = o_color;
                ++num_per_color[o_color];
                todo.push_back(o);
            }
        }
        out += *max_element(all(num_per_color));
        // out += max(num_per_color[0], num_per_color[1]);
    }
    ps(out);
