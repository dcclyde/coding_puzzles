...
pair<bool,bool> brute(ll N, ll E, ll K, auto& G) {
    ll num_cliques = 0;
    ll num_good_sets = 0;
    FOR(mask, 0, 1<<N) {
        uset<ll> verts;
        FOR(b, 0, N) {
            if ((mask>>b) & 1) {verts.insert(b);}
        }
        if (verts.size() < K) {continue;}
        else if (verts.size() == K) {
            // is this a clique?
            bool clique = true;
            for(auto& a : verts) for(auto& b : verts) {
                if (a >= b) {continue;}
            ...
            for(auto& a : verts) {
                ll ctr = 0;
                for(auto& b : verts) { if (G[a].find(b) != G[a].end()) {++ctr;} }
                if (ctr < K) {good_set = false; break;}
            }
            if (good_set) {
                ++num_good_sets;
                dbgc("good set", verts);
    ...
    dbgR(num_cliques, num_good_sets);
    return MP(num_cliques > 0, num_good_sets > 0);
}

/**
    I guess I can throw out all vertices with degree < K-1.
    Need to update other vertices' degrees as I go.
    If I run out of vertices, obviously answer is -1.

    If I can run that elimination process for deg < K and still get nonempty,
        that's a "good" subset.

    Else, I need to check for a clique.
    E <= 1e5, so there are no cliques with size >= 448.

    Maybe just pick a vertex with degree K-1 and check if it's in a clique?
    If not, that lets us delete K-1 edges.
    So worst case we'll do this E/K times and each costs K^2.
    Seems maybe OK?
 */
...
void solve() {
    ...
    dbgR(N, E, K);
    dbgY(pdh(G));
    auto Gbak = G; auto Ebak = E;

#ifdef DCCLYDE_LOCAL
    auto [has_clique, has_good_set] = brute(N, E, K, G);
#else
    bool has_clique, has_good_set;
#endif

    auto clique_check = [&](auto& claimed) {
        for(auto& a : claimed) for(auto& b : claimed) {
            if (a >= b) {continue;}
            if (Gbak[a].find(b) == Gbak[a].end()) {return false;}
        }
        return true;
    };
    auto good_set_check = [&](auto& claimed_raw) {
        uset<ll> claimed;
        for(auto& x : claimed_raw) {claimed.insert(x);}
        for(auto& a : claimed) {
            ll ctr = 0;
            for(auto& b : Gbak[a]) {if (claimed.find(b) != claimed.end()) {++ctr;}}
            if (ctr < K) {return false;}
        }
        return true;
    };

    V<uset<int>> degree_to_indices(N);
    // map<int,int> degs;
    auto& dti = degree_to_indices;
    FOR(k, 0, N) {
        dti[G[k].size()].insert(k);
        // ++degs[G[k].size()];
    }

    auto delete_vertex = [&](ll a) {
        ll T = G[a].size();
        E -= T;
        // if (degs[T] == 1) { degs.erase(T); }
        // else {--degs[T];}
    ...
    ll low_deg = 0;
    while (E > 0 && low_deg <= K-1) {
    // while (!degs.empty() && degs.begin()->first <= K-1) {
        el;
        // dbg(degs);
        dbgP(pdh(dti));
        dbgR(pdh(G));
        if (E < K*(K-1)/2) {
        // if (degs.rbegin()->first < K-1) {
            assert(!has_good_set);
            assert(!has_clique);
            return ps(-1);
        }
        if (dti[low_deg].empty()) {++low_deg; continue;}
        // ll low_deg = degs.begin()->first;
        ll a = *dti[low_deg].begin();
        dbgY(a, low_deg, E, K*(K-1)/2, low_deg == K-1, K*(K-1)/2 <= E);
        dbg(low_deg, K-1, low_deg == K-1);
        if (low_deg == K-1 && K*(K-1)/2 <= E) {
            bool found_clique = true;
            dbgc("clique check", a);
            for(auto& b : G[a]) for(auto& c : G[a]) {
                if (b >= c) {continue;}
                dbgP(b, c, G[b].find(c) == G[b].end(), G[b]);
                if (G[b].find(c) == G[b].end()) {found_clique = false; break;}
            }
            if (found_clique) {
                ps(2);
                V<ll> out = {a};
                out.insert(out.end(), all(G[a]));
                pv1(out);
                // cout << a+1 << ' ';
                // for(auto& b : G[a]) {cout << b+1 << ' ';} cout << '\n';
                // cout << flush;
                // // pv1(G[a]);
                assert(has_clique);
                assert(clique_check(out));
                return;
            }
        }
        delete_vertex(a);
    ...
    V<ll> out;
    FOR(k, 0, N) {if (!G[k].empty()) {out.push_back(k);}}
    ps(1, out.size());
    pv1(out);
    assert(has_good_set);
    assert(good_set_check(out));
    return;
}
