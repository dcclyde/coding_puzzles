...
/**
    First pass: For each pair of columns, push problems down to bottom 2 rows.
    Costs n*m/2. (minus smaller)
    Now bottom 2 rows and rightmost 1-2 columns are messed up.

    Now I guess push those problems to bottom-right.
    Costs n+m ish (minus smaller)

    Now it's just the very last 2x2. Presumably this is solvable in 4 steps.
...
void solve() {
    lls(R, C);
    V<V<bool>> dat(R, V<bool>(C, false));
    FOR(r, 0, R) {
        strings(row);
        FOR(c, 0, C) {
            if (row[c] == '1') {dat[r][c] = true;}
        }
    }
    dbgR(R, C, pdh(dat));
    el;
    ...
    V<V<pll>> out;
    // step 1.
    FOR(r, 0, R-2) FOR(c, 0, C-2, 2) {
        if (!dat[r][c] && !dat[r][c+1]) { continue; }
        out.emplace_back();
        ll num_to_flip = 3;
        FOR(k, 0, 2) {
            if (dat[r][c+k]) {
                out.back().emplace_back(r, c+k);
                dat[r][c+k] = !dat[r][c+k];
                --num_to_flip;
            }
        }
        while (num_to_flip > 0) {
            out.back().emplace_back(r+1, c+num_to_flip-1);
            dat[r+1][c+num_to_flip-1] = !dat[r+1][c+num_to_flip-1];
            --num_to_flip;
        }
    }
    dbgcY("end 1", pdh(out), pdh(dat)); el;

    // push the last 2 rows of problems to the right.
    {
        ll r = R-2;
        FOR(c, 0, C-2) {
            if (!dat[r][c] && !dat[r+1][c]) {continue;}
            ll num_to_flip = 3;
            out.emplace_back();
            FOR(k, 0, 2) {
                if (dat[r+k][c]) {
                    out.back().emplace_back(r+k, c);
                    --num_to_flip;
                    dat[r+k][c] = !dat[r+k][c];
                }
            }
            while (num_to_flip > 0) {
                out.back().emplace_back(r+num_to_flip-1, c+1);
                dat[r+num_to_flip-1][c+1] = !dat[r+num_to_flip-1][c+1];
                --num_to_flip;
            }
        }
    }
    dbgcP("end 2a", pdh(out), pdh(dat)); el;

    // push the last 2 columns of problems to bottom.
    {
        ll c = C-2;
        FOR(r, 0, R-2) {
            if (!dat[r][c] && !dat[r][c+1]) { continue; }
    ...
    dbgcR("end 2b", pdh(out), pdh(dat)); el;

    // handle the last 2x2 square.
    ll todo = 0;
    FOR(r, R-2, R) FOR(c, C-2, C) {todo += dat[r][c];}

    auto finish_helper = [&](ll flip_0s, ll flip_1s) {
        out.emplace_back();
        FOR(r, R-2, R) FOR(c, C-2, C) {
            if (dat[r][c] && flip_1s) {
                out.back().emplace_back(r, c);
                dat[r][c] = 0;
                --flip_1s;
            } else if (!dat[r][c] && flip_0s) {
                out.back().emplace_back(r, c);
                dat[r][c] = 1;
                --flip_0s;
            }
    ...
    dbgcY("done", pdh(out), pdh(dat)); el;

    ps(out.size());
    FOR(k, 0, out.size()) {
        FOR(j, 0, 3) {
            cout << out[k][j].first+1 << ' ' << out[k][j].second+1 << ' ';
        }
        cout << '\n';
    }
    // pvn1(out);
