...
void solve() {
    ...
    // for ( auto& x : A ) { compress[x] = -1; }
    // for ( auto& x : B ) { compress[x] = -1; }
    // int max_el = 0;
    // for ( auto& [v, k] : compress ) {
    //     k = max_el;
    //     ++max_el;
    // }
    // for ( auto& x : A ) { x = compress[x]; }
    // for ( auto& x : B ) { x = compress[x]; }
    // dbg(A);
    // dbg(B); el;

    // vector<int> prefix_counts(

    // first, need to count inversions in the starting array A.

    ll out = 0;
    dbg(out);
    deque<tuple<int,int,int,int>> todo;
    todo.emplace_back( 0 , nB , -1, nA-1 );
    while ( ! todo.empty() ) {
        auto& [sB, eB, sA, eA] = todo.front();
        todo.pop_front();
        // do a bit of work.
        int mB = (sB + eB) / 2;
        int curr = B[mB];
        // place this one.
        int inversions = 0;
        dbg(sB, eB, sA, eA, mB, curr);
        for ( int k = sA + 1 ; k <= eA ; ++k ) {
            // dbg(k, curr, A[k]);
            if ( curr > A[k] ) {
                ++inversions;
            }
        }
        int best_cost = inversions;
        int best_pos = sA;
        dbgc("initial", best_pos, best_cost);
        for ( int k = sA+1 ; k <= eA ; ++k ) {
            if ( curr > A[k] ) {
                --inversions;
            } else if ( curr < A[k] ) {
            ...
            if ( inversions < best_cost ) {
                best_cost = inversions;
                best_pos = k;
            }
        }
        dbg(sB, eB, sA, eA, mB, curr, "", best_pos, best_cost);
        out += best_cost;
        if ( mB > sB ) {
            todo.emplace_back( sB , mB , sA , best_pos );
            dbgc("L", todo.back());
        }
        if ( mB+1 < eB ) {
            todo.emplace_back( mB+1 , eB , best_pos , eA );
            dbgc("R", todo.back());
        }
    }

    ll inversions_in_A = getInversions(A);
    out += inversions_in_A;
