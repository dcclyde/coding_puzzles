...
void solve() {
    ...
    vector<int> seen_from(N+1 , -1 );
    vector<pii> ended_from(N+1);
    vector<bool> ended(N+1, false);
    deque<int> todo;
    IntervalUnion<int, true> S;  // places I've LANDED (not stopped)
    ...
    while ( !todo.empty() ) {
        ...
        // what new places can I hit from here?
        pii landings_from_here( curr - A[curr] , curr );
        auto new_landings = S.missing_sections( landings_from_here );
        for ( auto& [a, b] : new_landings ) {
            FOR(x, a, b+1) {
                seen_from[x] = curr;
                int ending = x + B[x];
                if ( ending >= N || ended[ending] == true ) {
                    // do nothing;
                } else {
                    ended[ending] = true;
                    ended_from[ ending ] = MP(curr, x);
                    todo.push_back( ending );
                }
            }
        }
        S.insert( landings_from_here );
    }
    if ( seen_from[0] == -1 ) {
        cout << -1 << '\n';
        return;
    ...
    vector<pii> path;  // where I was standing, where I jumped to
    // path.emplace_back(0, -1);
    path.emplace_back(seen_from[0], 0);
    while ( path.back().first != N ) {
        auto& [curr, dummy] = path.back();
        auto& [prev_start, prev_jumpedto] = ended_from[curr];
        path.emplace_back(prev_start, prev_jumpedto);
    }
    ps(path.size());
    for ( auto& [start, jumpto] : path | views::reverse ) {
        cout << jumpto << ' ';
    }
    cout << '\n';
    return;
