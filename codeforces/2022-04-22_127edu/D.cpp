...
/**
    I need to insert 1, 2, ..., X.

    Whatever chunk that's contained in [min_dat, max_dat] costs nothing.
    What about the others?
    Probably I put them either at one end or next to the smallest existing element?
 */
...
void solve() {
    ...
    ll out = 0;
    FOR(k, 0, N-1) { out += abs(dat[k+1] - dat[k]); }
    dbgY(out);

    ll small = *min_element(all(dat));
    ll big = *max_element(all(dat));

    if (small > 1) {
        // placing the 1 will cost us something.
        V<ll> place_1_options;
        // I could place it next to the smallest element.
        place_1_options.push_back(2 * (small - 1));
        // I could place it at start or end.
        place_1_options.push_back(min(dat[0], dat[N-1]) - 1);

        ll place_1_cost = *min_element(all(place_1_options));
        out += place_1_cost;
        dbgc("place 1", place_1_options, place_1_cost, out);
    }

    if (big < X) {
        // placing the X will cost us something.
        V<ll> place_X_options;
        // I could place it next to the biggest element.
        place_X_options.push_back(2 * (X - big));
        // I could place it at start or end.
        place_X_options.push_back(X - max(dat[0], dat[N-1]));

        ll place_X_cost = *min_element(all(place_X_options));
        out += place_X_cost;
        dbgc("place X", place_X_options, place_X_cost, out);
    }
    ps(out);
    return;
}
