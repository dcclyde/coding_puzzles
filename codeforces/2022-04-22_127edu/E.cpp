...
void solve() {
    ...
    dat = string(1, '.') + dat;
    ll N = (1<<D);
    dbgR(D, N, dat.size(), dat);
    el;

    assert(dat.size() == (1<<D));
    assert(dat.size() == N);
    ll LEAF_CUTOFF = N/2;  // anything >= this is a leaf.
    dbgY(LEAF_CUTOFF);


    auto check_ic = [&](ll a, ll b, auto&& self) -> bool {
        dbgP(a, b);
        if (dat[a] != dat[b]) {return false;}
        if (max(a, b) >= LEAF_CUTOFF) {return true;}
        FOR(k, 0, 2) {
            if (self(2*a, 2*b+k, self) && self((2*a)^1, (2*b+k)^1, self)) {
                return true;
            }
        }
        return false;
    ...
    V<bool> isomorphic_children(dat.size());
    auto& ic = isomorphic_children;
    FOR(n, 1, LEAF_CUTOFF) {
        ic[n] = check_ic(2*n, 2*n+1, check_ic);
        dbg(n, static_cast<bool>(ic[n]));
    }
    dbgY(ic);
    ...
    ROF(n, 1, LEAF_CUTOFF) {
        mi lc = dp[2*n];
        mi rc = dp[2*n+1];
        mi sc = (isomorphic_children[n]) ? (1) : (2);
        mi contrib = lc * rc * sc;
        dp[n] = contrib;
    }
    dbg(dp);
