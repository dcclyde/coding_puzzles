...
#pragma region  // count inversions
template<class T>
...
#pragma endregion  // count inversions

ll brute(ll N, ll Ktarg, ll Xtarg) {
    V<int> ph = rangeint(1, N+1);
    ll out = 0;
    do {
        // K = inversions
        V<int> dummy = ph;
        ll K = getInversions(dummy);
        // X = adjacent inversions
        ll X = 0;
        FOR(k, 0, N-1) {
            if (ph[k] > ph[k+1]) {++X;}
        }
