...
/**
    P[n] = prefix.
    I want last n such that P[n] <= x.
    Then last n such that P[n] + n <= x.
    Then last n such that P[n] + 2n <= x.
    and so on....

    Do some 2 pointers thing?
    Fast forward to the next time when adding t*n would make it cost too much.
 */
...
void solve() {
    ...
    while (curr > 0) {
        dbg(T, curr, out);
        // fast forward as long as we can.
        // Cost is P[curr] + T*curr <= X
        // what's the last day when we can afford that?
        ll new_T = (X - P[curr]) / curr;
        ll contrib = (new_T - T) * curr;
        ckmax(contrib, 0);
        out += contrib;
        T = new_T;
        --curr;
