...
#define FORll(i,a,b) for (ll i = (a); i < ((ll)b); ++i)
...
tcT, class S> struct LazySeg {
	const T idT{}; const S idS{INF};  // ! identity
...
/*
    stR maintains anything that could be in R while L holds most recent.
    New item is c.
    If c is not legal for L, then stR gets murdered.
    If c is legal for L, then stR can just get clamped.
        Also, if R could have held c[qid - 1] then stR receives that value.
*/
...
void solve() {
    ...
    stL.detailed_printouts();

    V<set<pll>> times_added_L(MAX_VALUE + 1);
    V<set<pll>> times_added_R(MAX_VALUE + 1);
    ll prev_c = 0;

    /**
     * ! Will I have problems where times_added_X should have updated
    ...
    FOR(qid, 0, N) {
        ...
        int stL_first_nonzero = stL.first_nonzero();
        int stR_first_nonzero = stR.first_nonzero();
        bool stL_empty = (stL_first_nonzero == -1);
        bool stR_empty = (stR_first_nonzero == -1);
        dbg(stL_first_nonzero, stR_first_nonzero);

        if (stL_empty && stR_empty) {return no;}
        ...
        if (c < min_l || c > max_l) {
            ...
            if (!stL_empty) {
                ...
                times_added_R[prev_c].emplace(qid-1, stL_first_nonzero);
            }
            stR.upd(0, min_r-1, 0);
        ...
        if (c < min_r || c > max_r) {
            ...
            if (!stR_empty) {
                ...
                times_added_L[prev_c].emplace(qid-1, stR_first_nonzero);
            }
            stL.upd(0, min_l-1, 0);
    ...
    bool stL_empty = (stL.query(0, MAX_VALUE) == 0);
    bool stR_empty = (stR.query(0, MAX_VALUE) == 0);
    if (stL_empty && stR_empty) {
        return no;
    }

    yes;
    // arbitrarily choose any valid end state.
    string out(N, '.');
    ll L, R;
    char holding;
    ...
    dbg(times_added_L[last_card])
    dbg(times_added_R[last_card])

    if (stL_empty) {
        L = get<0>(dat[N-1]);
        R = stR.first_nonzero();
        // R = fstTrue(0, MAX_VALUE, [&](ll x) {return stR.query(0, x) > 0;});
        // out[N-1] = 'L';
        holding = 'R';
        // hold_until should be last time we added `R` to stR.
        dbg(holding, L, R, out, times_added_R[R]);
        tie(hold_until, change_for) = *times_added_R[R].rbegin();
    } else {
        R = get<0>(dat[N-1]);
        L = stL.first_nonzero();
        // L = fstTrue(0, MAX_VALUE, [&](ll x) {return stL.query(0, x) > 0;});
        // out[N-1] = 'R';
        holding = 'L';
        dbg(holding, L, R, out, times_added_L[L]);
        tie(hold_until, change_for) = *times_added_L[L].rbegin();
    }
    dbgP(pdh(times_added_L));
    dbg(pdh(times_added_R));
    ...
    ROF(qid, 1, N) {
        if (qid == hold_until) {
            if (holding == 'L') {
                ...
                // L = change_for;
                R = change_for;
                // when did we get this L?

                auto it = times_added_R[R].lower_bound(MP(qid, -INF));
                if (it == times_added_R[R].begin()) {
                    dbgcBold("not found", R, times_added_R[R]);
                    assert(false);
                } else {
                    --it;
                    tie(hold_until, change_for) = *it;
                    dbgP(qid, times_added_R[R], MP(hold_until, change_for));
                }
            } else {
                holding = 'L';
                L = change_for;
                // L = get<0>(dat[qid-1]);
                // R = change_for;
                auto it = times_added_L[L].lower_bound(MP(qid, -INF));
                if (it == times_added_L[L].begin()) {
                    dbgcBold("not found", L, times_added_L[L]);
                    assert(false);
                } else {
                    --it;
                    tie(hold_until, change_for) = *it;
                    dbgR(qid, times_added_L[L], MP(hold_until, change_for));
                }
            }
        }
