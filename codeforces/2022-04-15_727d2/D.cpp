...
/*
    Just buy the things I need.
    I want as many discounts as possible though.
    I guess - if I can buy something at a discount then do that,
    otherwise buy something with a hard-to-get discount?

    1e5 product types, but total num to buy is 1e14.
*/
...
void solve() {
    ...
    ll out = 0;
    ll bought = 0;
    for(auto& [num_needed, num_for_discount] : dat) {
        todo[num_for_discount] += num_needed;
    }
    while (!todo.empty()) {
        /*
            If the stuff on the front is cheap, take that and reloop.
            Else, buy some of the last item.
                Buy min(amount we need of that item, amount to get discount on front).
        */
        el; dbgcY("loop", bought, out, todo);
        auto [start_nd, start_nn] = *todo.begin();
        if (start_nd <= bought) {
            // buy this stuff.
            dbgc("start is cheap", start_nn, start_nn);
            out += start_nn;
            bought += start_nn;
            todo.erase(start_nd);
            continue;
        }
        auto [end_nd, end_nn] = *todo.rbegin();
        ll amount_to_buy = end_nn;
        ckmin(amount_to_buy, start_nd - bought);
        out += 2*amount_to_buy;
        bought += amount_to_buy;
        if (amount_to_buy == end_nn) {
            todo.erase(end_nd);
        } else {
            todo[end_nd] -= amount_to_buy;
        }
        dbgcP("buy from end", amount_to_buy, 2*amount_to_buy);
    }
    ps(out);
