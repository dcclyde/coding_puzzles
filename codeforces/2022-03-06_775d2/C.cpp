...
ll get_cost(vector<int>& dat) {
	...
	for ( int k = 1 ; k < dat.size() ; ++k ) {
		cost_to_even += k * (dat[k] - dat[k-1]);
		out += cost_to_even;
		dbg( k , dat[k] , dat[k] - dat[k-1] , cost_to_even , out );
	}
	return out;
}
...
void solve() {
	ints(R, C);
	map<int,vector<pii>> dat;
	for ( int r = 0 ; r < R ; ++r ) {
		for ( int c = 0 ; c < C ; ++c ) {
			ints(color);
			dat[color].emplace_back(r, c);
		}
	}

	ll out = 0;
	for ( auto& [color, positions] : dat ) {
		vector<int> horiz;
		vector<int> vert;
		for ( auto& pr : positions ) {
			vert.push_back( pr.first );
			horiz.push_back( pr.second );
		}
		el;
		dbg( color , positions );
		ll vert_cost = get_cost( vert );
		dbg( vert_cost , vert );
		out += vert_cost;
		ll horiz_cost = get_cost( horiz );
		dbg( horiz_cost , horiz );
		out += horiz_cost;
	}
	cout << out << '\n';
	return;
