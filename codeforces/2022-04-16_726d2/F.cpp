...
void solve() {
    ...
    auto dfs = [&](int curr, auto&& self) -> bool {
        // work on parent
        // todo
        dbg(curr, color[curr], color);

        for (auto& o : G[curr]) {
            dbgP(curr, o, color[curr], color[o]);
            if (color[o] != -1) {
                if (color[o] == color[curr]) { return false; }
                continue;
            }
            dbgR();
            color[o] = color[curr] ^ 1;
            if (!self(o, self)) { return false; }
    ...
    color[0] = 0;
    bool two_colorable = dfs(0, dfs);
    el;
    dbgY(two_colorable, color);

    if (two_colorable) {
        V<ll> spc(2);  // "sum per color"
        FOR(k, 0, N) {
            spc[color[k]] += dat[k];
        }
        if (spc[0] == spc[1]) {
