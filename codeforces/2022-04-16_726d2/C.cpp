...
void solve() {
    ...
    int min_pos = -1;
    FOR(k, 0, N-1) {
        if (ckmin(min_delta, dat[k+1] - dat[k]) ) {
            min_pos = k;
        }
    }
    V<ll> out;
    out.push_back(dat[min_pos]);
    out.insert(out.end(), dat.begin() + min_pos + 2, dat.end());
    out.insert(out.end(), dat.begin(), dat.begin() + min_pos);
    out.push_back(dat[min_pos+1]);
