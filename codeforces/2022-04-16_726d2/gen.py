...
def gen_test():
    N, edges = rtree(N_RANGE, mute=True)
    edges = [tuple(q) for q in edges]
    # add a few extra edges.
    max_edges = N*(N-1) // 2
    num_extra = ri(0, max_edges - len(edges))

    edge_options = [
        (a, b) for a in range(N) for b in range(a+1, N)
            if (a,b) not in edges and (b,a) not in edges
    ]
    extra_edges = random.sample(edge_options, num_extra)
    assert(len(set(edges)) == len(edges))
    assert(len(set(extra_edges)) == len(extra_edges))
    edges += extra_edges
