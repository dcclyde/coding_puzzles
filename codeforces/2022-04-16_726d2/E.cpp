...
void solve() {
    ...
    // Chop off everything after first char that's strictly worse than dat[0].
    FOR(k, 1, N) {
        if (dat[k] > dat[0]) {
            dat.resize(k);
            N = k;
    ...
    int best_len = 1;
    FOR(curr_len, 2, N+1) {
        // is starting from here better than starting from best_start?
        auto helper = [&]() {
            FOR(pos, 0, K) {
                char a = dat[pos % best_len];
                char b = dat[pos % curr_len];
                if (a == b) {continue;}
                if (a < b) {return false;}
                return true;
            }
    ...
    string out;
    FOR(pos, 0, K) {
        out.push_back(dat[pos % best_len]);
    }
    ps(out);
