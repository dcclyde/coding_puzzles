...
// string to_string(__Bit_reference b) {
//     return to_string(static_cast<bool>(b));
// }
...
string print_details_helper(V<bool>& q) {
    ...
    FOR(k, 0, q.size()) {
        out.push_back('\t');
        out += to_string(ctr) + ":\t" + to_string(static_cast<bool>(q[k])) + "\n";
        ++ctr;
    }
    return out;
}
...
void learn() {
    ...
    ll p2 = 1;
    while (p2 <= N) {
        dbg(p2, static_cast<bool>(winning[p2]));
        p2 *= 2;
    }
    exit(1);
}
