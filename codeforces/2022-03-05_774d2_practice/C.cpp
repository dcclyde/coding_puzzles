...
vector<ll> facts = {6};
void setup() {
	while ( facts.back() <= 1e12 ) {
		facts.push_back( facts.back() * (facts.size() + 3) );
	}
	dbg(facts);
}
...
void solve() {
	...
	for ( int mask = 0 ; mask < (1<<(facts.size())) ; ++mask ) {
		ll N_working = N;
		int mask_working = mask;
		for ( int bit = 0 ; bit < facts.size() ; ++bit ) {
			N_working -= facts[bit] * (mask_working & 1);
			mask_working >>= 1;
			if ( N_working < 0 || mask_working == 0 ) {
				break;
			}
		}
		if ( N_working < 0 ) {
		...
		int curr = count_bits( N_working ) + count_bits( mask );
		best = min(curr, best);
	}
	cout << best << '\n';
