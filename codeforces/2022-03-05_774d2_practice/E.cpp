...
void solve() {
	...
	ll out = 1;  // let 1 be a special case.
	set<int> all_powers;

	for ( int k = 2 ; k <= max_N ; ++k ) {
		// if this is a power, get out of here.
		if ( all_powers.count( k ) ) {
			continue;
		}
		...
		int exp = 2;
		while ( pow <= max_N ) {
			all_powers.insert( pow );
			pow *= k;
			++exp;
		}
		--exp;

		// how many powers of k can we achieve? Don't count k^0 = 1.
		// it should just be ( 1 .. exp ) * ( 1 .. max_pow ).
		// can I just simulate? Nah max_pow is big.
		// exp <= 19.
	}
