...
void solve() {
    ...
    for ( int pos = N ; pos >= 1 ; --pos ) {
        // how many times does the "pos" operation need to happen?
        // well, where is the last remaining value?
        int value_pos = 0;
        for ( int k = 0 ; k < pos ; ++k ) {
            if ( dat[k] == pos ) {
                value_pos = k;
        ...
        out[pos-1] = (value_pos+1) % pos;
        int shift = (pos - out[pos-1]) % pos;
        // now need to actually do it.
        dbgc("PRE", shift, dat);
        auto shift_helper = dat;
        for ( int k = 0 ; k < pos ; ++k ) {
            dat[k] = shift_helper[(k-shift+pos)%pos];
        }
        dbgc("POST", pos , value_pos , out[pos-1] , dat );
        el;
    }
    for ( auto& x : out ) {
        cout << x << ' ';
