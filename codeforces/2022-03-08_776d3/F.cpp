...
void solve() {
    ints(num_jobs, num_options);
    vector<ll> deadlines;
    rv(num_jobs, deadlines);
    // vector<tuple<ll,ll,ll>> options;
    // for ( int k = 0 ; k < num_options ; ++k ) {
    //     ints(a, b, c);
    //     options.emplace_back( a,b,c );
    // }
    dbg(deadlines);
    vector<vector<tuple<int,int,int>>> options(num_jobs);
    for ( int k = 0 ; k < num_options ; ++k ) {
        ints(taskID, hrs, progress);
        options[taskID-1].emplace_back( hrs , progress , k+1 );
    }
    dbg(options);
    el;

    ll time_spent_total = 0;
    vector<int> options_used;
    for ( int taskID = 0 ; taskID < num_jobs ; ++taskID ) {
        el;
        auto& opts = options[taskID];
        int N = opts.size();
        // I need to find the cheapest collection of options that gives progress >= 100.
        // I guess dp[k][p] = cheapest way to get progress p using first k?
        vector<pair<ll, vector<int>>> dp( 101 , MP(3e18, vector<int>()) );
        dp[0].first = 0;  // with 0 options examined, we've made 0 progress.
        for ( int k = 1 ; k <= N ; ++k ) {
            // if we don't use k, everything stays the same.
            // if we do use it, we get (p, cost) --> (p+p_k, cost+cost_k).
            auto& [hrs, progress, idx] = opts[k-1];
            dbg(k, hrs, progress, idx);
            for ( int p = 100 ; p >= 0 ; --p ) {
                if ( progress >= p ) {
                    if ( dp[0].first + hrs < dp[p].first ) {
                        dp[p] = MP(dp[0].first + hrs, dp[0].second);
                        dp[p].second.push_back( idx );
                    }
                } else {
                    if ( dp[p-progress].first + hrs < dp[p].first ) {
                        dp[p] = MP(dp[p-progress].first + hrs, dp[p-progress].second);
                        dp[p].second.push_back( idx );
                    }
                }
            }
        ...
        auto& [time_spent_here, indices] = dp[100];
        time_spent_total += time_spent_here;
        options_used.insert( options_used.end() , all(indices) );
        dbg(taskID+1 , time_spent_total , indices );
        if ( deadlines[taskID] < time_spent_total ) {
            cout << "-1\n";
            return;
        }
    }
    cout << options_used.size() << '\n';
    for ( auto& x : options_used ) {
        cout << x << ' ';
    }
    cout << '\n';
