...
template<typename T>
vector<pii> get_rests(T& dat) {
    vector<pii> rests;
    auto it = dat.begin();
    auto it2 = dat.begin(); ++it2;
    int ctr = 0;
    while ( it2 != dat.end() ) {
        rests.emplace_back( *it2 - *it - 1 , ctr );
        ++ctr; ++it; ++it2;
    }
    sort(all(rests));
    return rests;
...
int result_adding_one_exam(vector<int>& dat, int T) {
    // we can never do better than the worst interval in the current list.
    auto rests = get_rests(dat);
    auto& [worst_rest_len, worst_rest_idx] = rests.front();
    int curr_worst = worst_rest_len;

    // now, how should we place the new exam?
    // at the end is an option.
    int new_exam_cost = T - 1 - dat.back();

    // now consider each possible interval.
    for ( int k = 0 ; k < dat.size()-1 ; ++k ) {
        int cost_local = (dat[k+1] - dat[k]) / 2 - 1;
        new_exam_cost = max(new_exam_cost, cost_local);
    }
    int out = min(curr_worst, new_exam_cost);
    dbgc("ra1e", out, curr_worst, new_exam_cost, dat);
    return out;
}
...
void solve() {
    ...
    // remove either first or last endpoint of the worst rest.
    int out = 0;
    FOR( endpoint_removed , 0 , 2 ) {
        // check if we're about to remove 0, since that's not allowed.
        if ( worst_rest_idx == 0 && endpoint_removed == 0 ) {
            continue;
        }
        int idx_to_delete = worst_rest_idx + endpoint_removed;
        auto without_one = dat;
        without_one.erase( without_one.begin() + idx_to_delete );
        dbg(worst_rest_idx, endpoint_removed, without_one);

        int out_local = result_adding_one_exam( without_one, T );
        out = max(out, out_local);
    }
