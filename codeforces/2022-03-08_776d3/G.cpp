...
void solve() {
    ...
    map<pii, int> edge_usage;
    map<int, int> vertex_usage;
    map<int, int> vertex_usage_real;

    /*
        1) compute distance to goal for every node.
        Just use BFS.

        2) Compute (ways from here to goal wasting 0 time, ways from here to goal wasting 1 time) per node.
        Goal is (1, 0).
        Distance N stuff is
    */

    // ntw[k] = (distance to goal, ways to accomplish that trip).
    vector<pair<int, mi>> no_time_wasted( V , {INF, 0} );
    no_time_wasted[v_end] = MP(0, 1);
    {
        deque<pii> todo;
        vector<bool> seen( V , false );
        seen[v_end] = true;
        todo.emplace_back(v_end, 0);  // vertex id, distance from end
        while ( !todo.empty() ) {
            auto [vidx, cost] = todo.front();
            todo.pop_front();
            vertex_usage[vidx] += 1;
            no_time_wasted[vidx].first = cost;

            for ( auto& proposed : G[vidx] ) {
                if ( !seen[proposed] ) {
                    todo.emplace_back( proposed , cost+1 );
                    seen[proposed] = true;
                } else if ( no_time_wasted[proposed].first == cost-1 ) {
                    no_time_wasted[vidx].second += no_time_wasted[proposed].second;
                }
                edge_usage[MP(vidx, proposed)] += 1;
                edge_usage[MP(proposed, vidx)] += 1;
            }
        }
    }
    dbg(no_time_wasted);
    dbg(edge_usage);
    dbg(vertex_usage);
    dbg(vertex_usage_real);
    el;
    edge_usage.clear();
    vertex_usage.clear();

    dbgc(OUT_MARK<<"STEP 2");

    vector<mi> one_time_wasted( V , 0 );
    {
        vector<bool> seen( V , false );
        deque<int> todo;
        todo.push_back( v_end );
        seen[v_end] = true;
        while ( !todo.empty() ) {
            int vidx = todo.front();
            todo.pop_front();
            auto& [v_dist, v_ways0] = no_time_wasted[vidx];
            mi& v_ways1 = one_time_wasted[vidx];
            vertex_usage[vidx] += 1;

            // Ways to waste 1 time = (
            //    ways to move to same distance then waste 0 time
            //  + ways to move to smaller distance then waste 1 time )

            for ( auto& proposed : G[vidx] ) {
                auto& [prop_dist, prop_ways0] = no_time_wasted[proposed];
                edge_usage[MP(vidx, proposed)] += 1;
                edge_usage[MP(proposed, vidx)] += 1;
                mi contrib = 0;
                if ( prop_dist == v_dist - 1 ) {
                    contrib = one_time_wasted[proposed];
                } else if ( prop_dist == v_dist ) {
                    contrib = prop_ways0;
                }
                v_ways1 += contrib;
                if ( !seen[proposed] ) {
    ...
    cout << (int)(one_time_wasted[v_start] + no_time_wasted[v_start].second) << '\n';
