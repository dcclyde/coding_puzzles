...
void solve() {
    ints(N, M);
    vector<tuple<ll, int, int>> dat;
    for ( int k = 0 ; k < M ; ++k ) {
        ints(a, b);
        dat.emplace_back( b , a , k+1 );
    }
    dbg(N, dat);
    ...
    ll out_sum = 0;
    for ( int k = 0 ; k < 2*N ; ++k ) {
        auto& [weight, pos, idx] = dat[k];
        usethese.emplace_back( pos , idx );
        out_sum += weight;
    }
    sort(all(usethese));
    ...
    cout << out_sum << '\n';
    for ( int k = 0 ; k < N ; ++k ) {
        cout << usethese[k].second << ' ' << usethese[2*N-1-k].second << '\n';
    }
