...
/**
 * c_k = k*a_k + [(index of k+1th zero) - (k-1)].
 *     second term is 0 if there is no k+1th zero.
 *
 * First term is 0 or k.
 * Second term can be 0 through N-k?
 */
...
void solve() {
    ...
    V<ll> zero_positions(N, -1);
    auto& zp = zero_positions;
    ...
    FOR(k, 1, N) {
        el; dbgY(k, zp, out);
        if (zp[k-1] >= k) {
            // we already know whether out[k] is 0.
            ll tricky = dat[k] - k*out[k];
            zp[k] = tricky + k;
            dbgcP("111", k, zp[k-1], tricky, zp[k]);
        } else {
            // I guess all preceding entries were 0s?
            if (dat[k] == 0) {
                out.push_back(0);
                zp[k] = k;
                dbgcP("a", out[k], zp[k]);
                ...
                ll tricky = dat[k] - k*out[k];
                zp[k] = tricky + k;
                dbgcP("B", out[k], zp[k], tricky);
            }
            dbgc("2 2 2", k, zp[k-1], zp[k]);
        }
        if (zp[k] == k) {continue;}
        FOR(j, out.size(), zp[k]) { out.push_back(1); }
        if (zp[k] == N) { break; }
        out.push_back(0);
        dbg(k, zp, out);
    }
    ...
    FOR(k, 0, N) {
        if (k*2 >= N) {
            out[k] = (int)(dat[k] >= k+1);
            zero_positions[k] = dat[k] - k*out[k] + k;
            ckmin(last_todo, k);
        }
    }
    dbg(last_todo, out);
