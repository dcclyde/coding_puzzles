...
/**
 * TODO strats
 * The answer is never more than 2. You can't get 2 then 1 by ANDing.
 *
 * "Is it 0?" Store 30 separate DSUs, one per bit.
 *     Check if we're in same component in any of those.
 *
 * "Is it 1?" Only depends on the start vertex.
 * Can I get a zero in 1 slot before losing all my other bits?
 * Need to get to any vertex that touches an edge with e&1 == 0.
 * I can do that using the DSUs above. Write down which components have a "good"
 * vertex in each DSU. Remember to exlude the 0bit DSU since the whole point is
 * to arrive with any bit BESIDES the 0bit intact.
 *
 */
...
void solve() {
    ...
    FOR(k, 0, E) {
        ...
        if ((w&1) == 0) { bz.insert(x); bz.insert(y); }
    }
    dbgY(pdh(G));
    el;

    V<uset<int>> good_components_per_dsu(BITS);
    auto& gc = good_components_per_dsu;
    for(auto& x : bz) {
        FOR(b, 1, BITS) {
            gc[b].insert(dsu[b].get(x));
        }
    }
    ...
    FOR(qid, 0, Q) {
        ...
        int result = 2;
        FOR(b, 0, BITS) {
            if (dsu[b].sameSet(x, y)) {
                result = 0;
                break;
            }
        }
        if (result != 2) { ps(result); continue; }

        FOR(b, 1, BITS) {
            int comp = dsu[b].get(x);
            if (gc[b].find(comp) != gc[b].end()) {
                result = 1;
                break;
            }
        }
