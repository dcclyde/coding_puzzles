...
/**
 * dp[n] = cost to set up my capital at position n?
 * dp[0] = 0
 * answer takes an extra step, since I could stop at any point
 *     and just conquer the rest.
 *
 * dp[n+1] = min_k ( dp[k] + A * (all costs from k to n+1) + B*(dat[n+1]-dat[k])).
 */

/**
 * Can I do it without DP?
 *
 */
...
void solve() {
    ...
    V<ll> suffix_cost(N);
    ROF(k, 0, N-1) {
        suffix_cost[k] = suffix_cost[k+1] + (N-k-1) * diffs[k+1];
        dbgP(k, suffix_cost);
    }
    dbgY(suffix_cost);
    ...
    el;
    FOR(k, 0, N) {
        ll cost_left = dat[k] * (A+B);
        ll cost_right = suffix_cost[k] * A;
        dbgP(dat[k], suffix_cost[k])
        ll cost_total = cost_left + cost_right;
        ckmin(out, cost_total);
        dbg(k, cost_left, cost_right, cost_total, out);
    }
    ps(out);
    return;
}
