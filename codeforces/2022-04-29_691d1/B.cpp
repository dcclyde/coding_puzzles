...
/**
    N = 100.
    S = sum(a_j) = total water available for pouring.

    I pick k glasses.
    I start with sum_chosen(a_j) points.
    Then I add S-same / 2 points, unless that goes above my total capacity.

    I could binary search if I had a way to find the K fullest glasses
        that have total capacity >= C.

    I'd like to know best total water for k cups with capacity >= C
    OR
    best total capacity for k cups with total water >= A.

    This feels knapsacky.
    dp state is like (n, k, cap)
    dp value is best amount of water contained with these values
    n <= 100, k <= 100, cap <= n*100.

    What's my update?
    Considering a new glass.
    Use this one:
        k += 1
        cap += glass cap
        dp[n+1][k+1][cap+new_cap] = max(itself, dp[n][k][cap] + fullness)
        dp[n+1][k][cap] = max(itself, dp[n][k][cap])

    dp[k][cap]
    (n, k, cap)

    I think there are only N^2 distinct (k, cap) so N^3 updates we'd like to make.


    Any chance I can just get away with that?
    There are 2^N subsets total, but more like 100*N distinct capacities.
    What we really care about is more like the count of distinct (k, total cap)
 */
...
void solve() {
    ...
    FOR(n, 0, N) {
        // consider dat[n].
        auto& [curr_cap, curr_vol] = dat[n];
        ROF(k, 0, N) {
            for(auto& [cap, vol] : dp[k]) {
                ckmax(dp[k+1][cap + curr_cap], vol + curr_vol);
            }
        }
        dbgY(n, pdh(dp));
    }
    ...
    // now build output?
    ll S = 0;
    for(auto& [cap, vol] : dat) {S += vol;}

    V<db> out2(N);
    FOR(k, 1, N+1) {
        ll best_times_2 = 0;
        for(auto& [cap, vol] : dp[k]) {
            // If I choose glasses with total `vol`, I could get S/2 + vol/2.
            ll curr_times_2 = min(S + vol, 2*cap);
            ckmax(best_times_2, curr_times_2);
        }
        out2[k-1] = best_times_2 / 2.;
    }
    pv(out2);
