...
void solve() {
    ...
    FOR(a, 0, N) {
        FOR(b, a+1, N) {
            // is the number of vertices on this edge 1 or 3 mod 4?
            // store 0 if 1, 1 if 3.
            int dx = dat[a].first - dat[b].first;
            int dy = dat[a].second - dat[b].second;
            int g = gcd(dx, dy);
            int parity = (g / 2) % 2;
            dbg(MP(a,b), MP(dx,dy), g, parity);
            bs[a][b] = parity;
            bs[b][a] = parity;
        }
    }
    ...
    FOR(a, 0, N) {
        FOR(b, a+1, N) {
            int pab = bs[a][b];
            // Need the whole triangle to have total parity 0.
            auto z = bs[a] ^ bs[b];
            int count = z.count();
            dbg(MP(a,b), pab, count, z);
            out += (pab) ? (count) : (N-count);
            out -= 2;
            // if (pab) {
            //     out += count;
            //     out -= 2;
