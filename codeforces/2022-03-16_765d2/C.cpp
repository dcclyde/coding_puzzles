...
void solve() {
    lls(N, L, K);
    vector<ll> positions, costs;
    rv(N, positions, costs);
    positions.push_back( L );
    costs.push_back( -1 );
    ++N;
    dbg(N, K, positions, costs);
    vector<ll> dpos = {INF};  // "delta pos"
    for ( int k = 1 ; k < N ; ++k ) {
        dpos.push_back( positions[k] - positions[k-1] );
    }
    dbg(dpos);

    // dp[n][k][s] = answer as we approach signpost n,
    //     already skipped k, current speed costs[s]
    // V<V<V<int>>> dp( N , V<V<int>>(K+1 , V<int>( N+1 , INF ) ) );
    V<V<int>> dpPrev( K+1 , V<int>( N+1 , INF ) );
    V<V<int>> dp( K+1 , V<int>( N+1 , INF ) );
    dpPrev[0][0] = costs[0] * dpos[1];  // n = 1
    FOR( n , 1 , N-1 ) {
        FOR( k , 0 , K+1 ) {
            FOR ( s , 0 , N ) {
                dbg(n,k,s);
                auto& curr = dpPrev[ k ][ s ];
                auto& take = dp[ k ][ n ];
                take = min( (ll)take , curr + dpos[n+1] * costs[n] );
                if ( k+1 <= K ) {
                    auto& skip = dp[ k+1 ][ s ];
                    dbg(n+1, dpos.size(), s, costs.size());
                    skip = min( (ll)skip, curr + dpos[n+1] * costs[s] );
                }
            }
        }
        dpPrev = dp;
        dbg(n, dp); el;
        dp.assign( K+1 , V<int>( N+1 , INF ) );
    }
    int out = INF;
    FOR(k, 0, K+1) FOR(s, 0, N+1) {
        out = min(out, dpPrev[ k ][ s ]);
    }
    ps(out);
