            ...
            for(auto dx : {-1, 1}) {
                ...
                for(auto odx : {-1, 1}) {
                    auto check = [&]() -> bool {
                        ...
                        bool out;
                        while(true) {
                            if (p+dx < 0 || p+dx >= N || p+dx == o || dat[p+dx] > dat[p]) {
                                out = false;
                                break;
                            }
                            p += dx;
                            if (o+odx < 0 || o+odx >= N || o+odx == p || dat[o+odx] > dat[o]) {
                                out = true;
                                break;
                            }
                            o += odx;
                        }
                        dbgcP("check", s, dx, o, odx, out);
                        return out;
                    };
                    if (!check()) {
                        ok = false;
...
void solve() {
    ...
    int ml = *max_element(all(pl));
    int mr = *max_element(all(pr));
    dbg(ml, mr);
    if (ml != mr) {return ps(0);}
    int cl = 0; int cr = 0;
    V<int> posl, posr;
    FOR(k, 0, N) {
        if(pl[k] == ml) {++cl; posl.push_back(k);}
        if(pr[k] == mr) {++cr; posr.push_back(k);}
    }
    dbgP(cl, cr);

    if (cl >= 2 || cr >= 2) {dbg(); return ps(0);}
    if (posl[0] != posr[0]) {dbg(); return ps(0);}
    if (ml % 2 == 1) {dbg(); return ps(0);}
    dbg(); return ps(1);
    /*
    if (cl == 2 && cr == 1) {
        if (posl[0] + 2*ml == posl[1] && posl[0] == posr[0]) {
            return ps(1);
        } else {
            return ps(0);
        }
    ...
    if (cl == 1 && cr == 2) {
        if (posr[0] + 2*ml == posr[1] && posr[1] == posl[0]) {
            return ps(1);
        } else {
            return ps(0);
        }
    ...
    if (cl == 2 && cr == 2) {
        if (posr[1] == posl[0] && posr[0] == posr[1] - 2*ml && posl[1] == posl[0] + 2*ml) {
            return ps(1);
        } else {
            return ps(0);
        }
    }
    if (cl > 1 || cr > 1) { return ps(0); }
    if (posl != posr) {return ps(0);}
    return ps(1);
    */
}
