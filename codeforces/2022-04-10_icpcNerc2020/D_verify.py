...
def verify():
    ...
    with open("fast.out", "r") as f:
        fast_answer = [q.strip() for q in f.readlines()]
    if fast_answer[0] != 'Possible':
        dbg("don't verify impossible")
        sys.exit(0)

    tests_active = list(map(int, fast_answer[1].split()))
    G = [[] for _ in range(N)]
    E = int(fast_answer[2])
    if E > 264:
        dbgR("E too big", E, 264)
        sys.exit(1)
    for k in range(E):
        a, b = list(map(int, fast_answer[3+k].split()))
        a -= 1
        b -= 1
        G[a].append(b)
        # G[b].append(a)
