...
void brute() {
    ...
    ll latest = 0;
    FOR(k, 0, N) {
        ckmax(latest, get<1>(dat[k]));
    }
    dbgR(N, latest, dat);

    V<ll> min_cost(latest, INF);
    for(auto& [s, e, dt] : dat) {
        FOR(k, s, e) {
            if (k + dt > e) {
                break;
    ...
    while (true) {
        ll next_sword = INF;
        FOR(s, t, latest) {
            ckmin(next_sword, s + min_cost[s]);
        }
        if (next_sword == INF) {break;}
        ++out;
        dbg(t, next_sword);
        t = next_sword;
    }
...
void solve() {
    ...
    set<tuple<ll,ll,bool,int>> events;
    FOR(k, 0, dat.size()) {
        auto& [s, e, dt] = dat[k];
        if (s + dt > e) {continue;}
        events.emplace(s, dt, false, k);
        events.emplace(e-dt, dt, true, k);
    }
    dbgP(events);
    el;
    ...
    while (true) {
        el; el;
        auto it = events.lower_bound(MT(t,0,0,0));
        if (it == events.end()) {break;}
        auto [nt, ndt, ndir, nk] = *it;
        events.erase(it);
        dbgcY("Process event", nt, ndt, ndir, nk)
        dbg(t, next_sword, out);
        ...
        if (available.empty() || (next_sword > nt && next_sword < INF)) {
            t = nt;
            if (ndir == false) {
                available.emplace(ndt, nk);
                ckmin(next_sword, nt + ndt);
            } else {
                available.erase(MP(ndt, nk));
            }
            dbgcP("no swords till nt", t, out, next_sword, available);
            continue;
        }

        int best_available = available.begin()->second;
        auto [cs, ce, cdt] = dat[best_available];
        ll dt = nt - t;
        ll swords = dt / cdt;
        out += swords;
        t += swords * cdt;
        ckmin(next_sword, t + cdt);
        t = nt;
        dbgcP("ffwd", cs, ce, cdt);
        dbg(dt, swords);
        dbgP(t, next_sword, out);

        if (ndir == true) {
            available.erase(MP(ndt, nk));
        }
        if (ndir == false) {
            // would it be better to use the new guy instead?
            available.emplace(ndt, nk);
            ckmin(next_sword, nt + ndt);
        }
        dbg(available, t, next_sword, out);
    }
    if (next_sword < INF) {
        ++out;
    }
