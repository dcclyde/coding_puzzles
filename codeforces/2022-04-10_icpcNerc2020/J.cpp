...
void brute() {
    ...
    FOR(s, 2, biggest+2) {
        ...
        for(auto& x : dat) {
            ok = false;
            FOR(n, 1, x+1) {
                // what happens if we try to use n screens for x items?
                if (n*(s-1) <= x && x <= n*s) {
                    ok = true;
                    cost_local += n;
                    dbgP(x, n);
...
void solve() {
    ...
    map<int,int> c2o;  // count --> num occurrences
    FOR(k, 0, dat.size()) {
        ++c2o[dat[k]];
    }

    remDup(dat);
    dat.erase(dat.begin(), dat.begin()+1);
    N = dat.size();
    dbgR(N, dat);
    int MAX_S = dat[0] + 1;
    int best_s = 0;
    ...
    FOR(s, 2, MAX_S + 1) {
        ...
        ll out_local = 0;
        for(auto& x : dat) {
            int Q_SMALL = cdiv(x, s);
            int Q_BIG = x / (s-1);
            ok = false;
            dbg(x, Q_SMALL, Q_BIG);
            FOR(q, Q_SMALL, Q_BIG+1) {
                int r = abs(x - q*s);
                dbg(q, r);
                if (r <= q) {
                    ok = true;
                    out_local += q*c2o[x];
                    dbgcY("good", out_local);
                    break;
                }
            }
            if (!ok) {break;}
