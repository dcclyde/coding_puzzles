...
def solve_better(testID):
    N = nn()
    deployment = []
    for _ in range(N):
        deployment.append(lm())
    tests = []
    for _ in range(N):
        tests.append(lm())
    dbg(N, deployment, tests)
    entrypoints = {"111": [0]}
    found = dd(set)
    for i in range(N):
        if sum(deployment[i]) == 0:
            continue
        needed_key = "".join([str(x) for x in deployment[i]])
        found[needed_key].add(i)
    to_process = ["111"]
    while len(to_process) > 0:
        curr_key = to_process.pop()
        for i in found[curr_key]:
            test_product = []
            for x in range(3):
                test_product.append(deployment[i][x] * tests[i][x])
            test_product_key = "".join([str(x) for x in test_product])
            if test_product_key not in entrypoints:
                entrypoints[test_product_key] = [i]
                to_process.append(test_product_key)
    for key in found:
        if key not in entrypoints:
            key_vec = [0 if x == "0" else 1 for x in key]
            if sum(key_vec) != 2:
                print("Impossible")
                return
            zero_idx = 0
            ...
            if zero_idx == 1:
                ...
                key1 = "100"
            if key1 in entrypoints and key2 in entrypoints:
                entrypoints[key] = [entrypoints[key1][0], entrypoints[key2][0]]
            else:
                print("Impossible")
                return
    print("Possible")
    ...
    for x in entrypoints:
        for y in entrypoints[x]:
            entrypoint_vals.add(y)
    for i in range(N):
        if i in entrypoint_vals:
            ct_config.append("1")
        else:
            ct_config.append("0")
    print(" ".join(ct_config))
    cd_pairs = []
    for key in entrypoints:
        for entry in entrypoints[key]:
            for idx in found[key]:
                if idx == entry:
                    continue
                cd_pairs.append(f"{entry + 1} {idx + 1}")
    print(len(cd_pairs))
    print("\n".join(cd_pairs))
