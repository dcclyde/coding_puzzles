...
void solve() {
    ...
    V<string> out(N);
    int ctr = 0;
    int last_special = K-1;
    while (out[0].size() < L) {
        ROF(k, 0, last_special+1) {
            out[k].push_back(dat[ctr]);
            if (k < last_special && out[k+1].back() != out[k].back()) {
                last_special = k;
            }
            ++ctr;
        }
    }
    // now fill in the others at random.
    FOR(n, 0, N) {
        while (out[n].size() < L) {
            out[n].push_back(dat[ctr++]);
        }
    }
