...
void brute() {
    ...
    set<char> seen;
    for(auto& c : dat) {seen.insert(c);}
    FOR(start, 0, N) FOR(end, start, N) for(auto& a : seen) for(auto& b : seen) {
        ll ca = 0; ll cb = 0;
        FOR(k, start, end+1) {
            if (dat[k] == a) {++ca;}
            if (dat[k] == b) {++cb;}
...
void solve() {
    ...
    // dbg(cycle_len, seen);
    FOR(k, cycle_len, N) {
        dbgP(k, dat[k-cycle_len], dat[cycle_len]);
        if (dat[k-cycle_len] != dat[k]) {
            return NO;
        }
    }
    return YES;
