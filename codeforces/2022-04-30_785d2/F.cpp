...
//     // build+output horizontal costs.
//     ll S = 0;
//     FOR(r, 0, R) {
//         V<ll> wip;
//         FOR(c, 0, C-1) {
//             wip.push_back(flat(r,c) ^ flat(r,c+1));
//         }
//         pv(wip);
//         S += sumv(wip);
//     }
...
//             ll f1 = flat(r,c);
//             ll f2 = flat(r+1,c);
//             ll cost_here = flat(r,c) ^ flat(r+1,c);
//             dbgP(r, c, f1, f2, cost_here);
//             wip.push_back(cost_here);
//         }
//         pv(wip);
//         S += sumv(wip);
//     }
...
void solve() {
    ...
    FOR(r, 0, R) FOR(c, 1, C) {
        flat_helper[MP(r,c)] = flat_helper[MP(r,c-1)] ^ all_horiz[r][c-1];
    }
    el;el;
    dbg(pdh(flat_helper));
    ...
    FOR(qid, 0, Q) {
        lls(delta);
        // delta = prev ^ curr, so curr = delta ^ prev.
        ll curr = delta ^ prev;
