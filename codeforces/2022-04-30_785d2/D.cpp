...
/**
    ...
    OK, so we definitely need C a subset of B.
    If that holds, there's at least one solution since A could just be C.

    C always has at least 2 terms.
    We need dA to be a divisor of dC.

    We need LCM(dA, dB) = dC.

    After that check, I think each divisor should contribute 1 or infinity.
    Just test whether the "before the beginning" and "past the end" points
        are in B's range. If so, that constrains A appropriately.
 */
...
void solve() {
    ...
    // Is C a subset of B?
    if (dC % dB != 0) {return ps(0);}
    ll B_before_start = (sC - sB) / dB;
    ll B_after_end = (eB - eC) / dB;
    if (B_before_start < 0 || B_after_end < 0) {return ps(0);}
    if (sB + B_before_start*dB != sC) {return ps(0);}
    bool B_covers = true;
    B_covers &= (sB <= sC - dC);
    B_covers &= (eB >= eC + dC);
    if (!B_covers) {return ps(-1);}

    auto divs = divisors(dC);
    mi out = 0;
    for(auto& dA : divs) {
        ll g = gcd(dA, dB);
        ll lcm = dA * dB / g;
        ...
        // ok, dA works.
        // how many ways to extend past the ends without messing up C?
        ll ways_1_side = dC / dA;
        out += ways_1_side * ways_1_side;
    }
    ps(out);
    return;
}
