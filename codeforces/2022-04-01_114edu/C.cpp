...
ll maxll(ll a, ll b) {dbg(a, b, (ll)a, (ll)b, max((ll)a, (ll)b)); return max((ll)a, (ll)b);}
ll minll(ll a, ll b) {return min((ll)a, (ll)b);}

// ! ---------------------------------------------------------------------------

/*
    Attacker should either be barely good enough to win,
    or barely bad enough to lose.
*/
...
void brute() {
    ...
    FOR(qidx, 0, Q) {
        auto& [dragon_def, dragon_att] = queries[qidx];
        ll best = 8e18;
        FOR(aidx, 0, N) {
            ll attack_cost = dragon_def - dat[aidx];
            ckmax(attack_cost, 0LL);
            ll total_def = 0;
            FOR(k, 0, N ) {
                if ( k == aidx ) {
                    continue;
            ...
            ll def_cost = dragon_att - total_def;
            ckmax(def_cost, 0LL);
            ll total_cost = attack_cost + def_cost;
            ckmin(best, total_cost);
        }
        ps(best);
    }
    return;
...
void solve() {
    ...
    sort(all(dat));
    ll sum = 0; for(auto& x: dat) {sum += x;}

    FOR(q, 0, Q) {
        dbgY(queries[q]);
        auto& [dragon_def, dragon_att] = queries[q];
        int first_winner = fstTrue(0, (int)N-1, [&](int j){return dat[j] >= dragon_def;});
        int last_loser = first_winner - 1;

        ll out = 2e18;
        if ( first_winner < N ) {
            ll total_str = sum - dat[first_winner];
            ll cost = maxll(0, dragon_att - total_str);
            dbgc("first_winner", total_str, cost, dragon_att, dragon_def);
            dbg(dragon_att - total_str);
            dbg(maxll(0, dragon_att - total_str));
            ckmin(out, cost);
        }
        if ( last_loser >= 0 ) {
            ll total_str = sum - dat[last_loser];
            ll cost = maxll(0, dragon_att - total_str);
            cost += dragon_def - dat[last_loser];
            dbgc("last_loser", total_str, cost);
            ckmin(out, cost);
        }
        ps(out);
    }
