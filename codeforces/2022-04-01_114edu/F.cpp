...
void solve() {
    ...
    // set up links between adjacent numbers.
    umap<int,int> link;
    umap<int,int> link_back;
    uset<int> broken;
    DSU dsu;
    dsu.init(MAX+1);
    FOR(k, 0, N) {
    ...
    uset<int> broken_heads;
    for ( auto& x : broken ) {
        broken_heads.insert(dsu.get(x));
    }

    // check for cycles.
    V<bool> cycle_check_seen(MAX+1, false);
    FOR(k, 1, MAX+1) {
        if ( cycle_check_seen[k] ) {
            continue;
        }
        int curr = k;
        uset<int> cycle_check_curr;
        while ( true ) {
            if ( cycle_check_curr.find(curr) != cycle_check_curr.end() ) {
                // found a cycle.
                broken_heads.insert(dsu.get(curr));
                break;
            }
            if ( cycle_check_seen[curr] ) {
                break;
            }
            cycle_check_curr.insert(curr);
            if ( link.find(curr) == link.end() ) {
                break;
            } else {
                curr = link[curr];
            }
        }
        for ( auto& x : cycle_check_curr ) {
            cycle_check_seen[x] = true;
        }
    }


    // get the component sizes from dsu.
    uset<int> heads_seen;
    swap(heads_seen, broken_heads);
    umap<int, int> size_to_count;
    FOR(k, 1, MAX+1) {
        int head = dsu.get(k);
        if ( heads_seen.find(head) != heads_seen.end() ) {
            continue;
        } else {
            heads_seen.insert(head);
            ++size_to_count[dsu.size(head)];
        }
    }
    V<pii> s2c;
    s2c.insert(s2c.end(), all(size_to_count));
    sort(all(s2c));
    dbg(s2c);
