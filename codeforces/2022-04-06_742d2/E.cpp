...
void solve() {
    ...
    auto do_update = [&](int p, int v) {
        ...
        if (p == N-1 || ((dat[p] <= dat[p+1]) == (v <= dat[p+1]))) {
            // right side is already fine.
            dbgcB("no change right");
        } else if (dat[p] <= dat[p+1]) {
            // it was working before but now we broke it.
            int gl = good[l];
            good[l] = p+1 - l;
            good[p+1] = gl + l - (p+1);
            st.upd(l, good[l]);
            ...
            // it was broken before but now we fixed it.
            good[l] += good[p+1];
            good.erase(p+1);
            st.upd(p+1, 0);
            st.upd(l, good[l]);
        ...
        if (p == 0 || ((dat[p-1] <= v) == (dat[p-1] <= dat[p]))) {
            dbgc("no change left");
            // don't need to change left endpoint.
        } else if (dat[p-1] <= dat[p]) {
            // l < p, but now we've broken that chunk.
            good[p] = good[l] + l-p;
            st.upd(p, good[p]);
            good[l] = p - l;
            st.upd(l, p - l);
