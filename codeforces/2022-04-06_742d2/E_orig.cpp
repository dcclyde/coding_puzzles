...
tcT, class S> struct LazySeg {
	...
	void upd(int lo,int hi,S inc,int ind=1,int L=0, int R=-1, bool pr=true) {
        if (pr) {dbgcP("upd", lo, hi, inc);}
        if ( R == -1 ) {R = SZ-1;}
		push(ind,L,R); if (hi < L || R < lo) return;
		if (lo <= L && R <= hi) {
			lazy[ind] = inc; push(ind,L,R); return; }
		int M = (L+R)/2; upd(lo,hi,inc,2*ind,L,M, false);
		upd(lo,hi,inc,2*ind+1,M+1,R,false); pull(ind);
	}
...
void solve() {
    ...
    FOR(qid, 0, Q) {
        ...
        if (qtype == 1) {
            ...
            /*
                Need:
                First L such that L + seg[L] >= p
                p + seg[p] tells me last R

                same thing afterward

                then that will tell me which range queries to run.
            */
            int lpre = fstTrue(0, p, [&](int x){
                return x + st.query(x, x) >= p;
            });
            int segp = st.query(p, p);
            int rpre = p + segp;
            int lpost = p;
            int rpost = p;
            ...
            if (p > 0) {
                ...
                if (prev_v <= v) {
                    if (prev_v <= old_v) {
                        ...
                        lpost = fstTrue(0, p-1, [&](int x){
                            return x + st.query(x, x) >= p-1;
                        });
                    }
                }
            }
            if (p < N-1) {
                ...
                if (v <= next_v) {
                    if (old_v <= next_v) {
                        ...
                        rpost = st.query(p+1, p+1) + p+1;
                    }
                }
            }

            dbg(MP(lpre, rpre), MP(lpost, rpost));
            int new_segp = segp;
            if (rpre != rpost) {
                if (rpost == p) {
                    st.upd(p, p, -segp);
                    ...
                    st.upd(p, p, -segp + rpost - p);
                    new_segp = rpost - p;
                }
            }
            if (lpost > lpre) {
                // L used to extend further, but now it's p.
                st.upd(lpre, lpost-1, -(segp + 1));
            } else if (lpost < lpre) {
                // L used to be just p, but now it extends further.
                st.upd(lpost, lpre-1, (segp + 1));
            } else {
                st.upd(lpre, p-1, new_segp - segp);
            }
            dat[p] = v;
            dbgY(qid, p, v, st);
            dbgR(dat);
            ...
            int1(l, r);
            int result = st.query(l, r) + r-l+1;
            // Will this range try to reach past R?
            int r_extent = st.query(r, r);
            if (r_extent > 0) {
                int start_last_chunk = fstTrue(0, r, [&](int x){
                    return x + st.query(x, x) >= r;
                });
                ckmax(start_last_chunk, l);
                result -= (r - start_last_chunk + 1) * r_extent;
            }
