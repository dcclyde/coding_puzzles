...
void solve() {
    lls(MEX, XOR);
    dbgR(MEX, XOR);
    // Start by writing down all ints in [0, MEX).
    while (MEX-1 >= xor_prefixes.size()) {
        xor_prefixes.push_back(
            xor_prefixes.back() ^ xor_prefixes.size()
        );
    }
    int xsum = xor_prefixes[MEX-1];

    if (xsum == XOR) {
        return ps(MEX);
    }
    // now we'd like to add C such that xsum ^ C = XOR, or C = XOR ^ xsum.
    // This is fine unless C == MEX.
    int C = XOR ^ xsum;
    dbg(xsum, C);
    if (C == MEX) {
        return ps(MEX + 2);
