...
void solve() {
	...
	vector<pll> attempts;
	attempts.emplace_back( dat[0] , dat[1] - (dat[2]-dat[1]) );  // mult 0
	attempts.emplace_back( dat[2] , dat[1] + (dat[1] - dat[0]) );  // mult 2
	if ( (dat[2] - dat[0]) % 2 == 0 ) {
		attempts.emplace_back( dat[1] , dat[0] + (dat[2]-dat[0]) / 2 );
	}
	for ( auto& [source, targ] : attempts ) {
		if ( source != 0 && targ % source == 0 && targ / source > 0 ) {
			cout << "YES\n";
			return;
		}
	}
