...
void solve() {
    ints(N);
	int lo = 1;  // could be right
	int hi = N;  // too big
	int total_added = 0;
	while ( hi - lo > 1 ) {
		int mid = (lo + hi) / 2;
		// need to know: Is x currently < mid + total_added?
		int c = N - ((mid + total_added) % N);
		dbg(lo, hi, total_added, mid, c);
		cout << "+ " << c << '\n' << flush;
		int feedback;
		cin >> feedback;
		dbg( feedback , (lo + total_added)/N , feedback==(lo + total_added) / N );
		if ( feedback > (lo + total_added) / N ) {
			dbgc("update lo");
			lo = mid;
		} else {
			dbgc("update hi");
