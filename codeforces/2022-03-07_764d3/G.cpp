...
void solve() {
	...
	for ( int bit = MAX_BITS-1 ; bit >= 0 ; --bit ) {
		// Can we get away with setting this bit in the mask?
		int proposed_mask = (mask | (1<<bit));
		seen.assign( V , false );
		todo.clear();
		todo.push_back( 0 );
		seen[0] = true;
		int num_seen = 1;
		dbg(bit, mask, proposed_mask);
		while ( !todo.empty() ) {
			dbg(num_seen, seen, todo);
			int curr = todo.back();
			todo.pop_back();
			for ( auto& [proposed, weight] : G[curr] ) {
				// dbgc("propose move" , curr, proposed, weight, mask, weight&proposed_mask);
				if ( weight & proposed_mask ) {
					// dbgc("bad weight");
					continue;
				}
				if ( seen[proposed] ) {
