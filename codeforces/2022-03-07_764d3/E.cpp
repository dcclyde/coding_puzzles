...
void solve() {
	...
	map<string, tuple<int,int,int>> chunks;
	for ( int sidx = 0 ; sidx < dat.size() ; ++sidx ) {
		auto& s = dat[sidx];
		for ( int start = 0 ; start < length ; ++start ) {
			for ( int chunk_size = 2 ; chunk_size <= 3 ; ++chunk_size ) {
				if ( start + chunk_size > length ) {
					continue;
				}
				chunks[ s.substr(start, chunk_size) ] = MT(start, start+chunk_size, sidx);
			}
		}
	}
	dbg(chunks); el;

	vector<vector<tuple<int,int,int>>> dp( length+1 , vector<tuple<int,int,int>>( 1 , MT(-2,-2,-2)));
	// dp[0][0] = MT( -1,-1,-1 );
	for ( int k = 1 ; k < length ; ++k ) {
		for ( int chunk_size = 2 ; chunk_size <= 3 ; ++chunk_size ) {
			...
			int start_maybe = k + 1 - chunk_size;
			string chunk = goal.substr(start_maybe, chunk_size);
			dbg(k, chunk_size, chunk, chunks.count(chunk), ((k-chunk_size==-1)|| get<0>( dp[k-chunk_size][0] ) != -2));
			if ( chunks.count( chunk ) && ((k-chunk_size==-1) ||get<0>( dp[k-chunk_size][0] ) != -2) ) {
				if ( k - chunk_size <= 0 ) {
					dp[k].clear();
				} else {
					dp[k] = dp[k - chunk_size];
				}
				dp[k].push_back( chunks[chunk] );
				dbgc("update dp", k, dp[k]);
				break;
			}
		}
	}
	if ( get<0>( dp[length-1][0] ) == -2 ) {
		cout << "-1\n";
		return;
	} else {
		cout << dp[length-1].size() << '\n';
		for ( auto& [L, R, source] : dp[length-1] ) {
			cout << L+1 << ' ' << R << ' ' << source+1 << '\n';
		}
	}
