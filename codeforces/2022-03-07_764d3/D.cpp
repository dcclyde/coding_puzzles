...
void solve() {
	...
	string dat; cin >> dat;
    dbg(N, K, dat);
	map<int,int> char_counts;
	for ( auto& c : dat ) {
		++char_counts[c];
	}
	int pairs = 0;
	int alone = 0;
	for ( auto& [c, n] : char_counts ) {
		pairs += n / 2;
		alone += n % 2;
	}

	int pairs_per_color = pairs / K;
	/*
		I can definitely get at least pairs_per_color * 2 + (maybe 1 from alone).
	*/
	int out = pairs_per_color * 2;
	if ( alone >= K ) {
		++out;
	} else if ( alone + 2 * (pairs%K) >= K ) {
		++out;
	}
	cout << out << '\n';
