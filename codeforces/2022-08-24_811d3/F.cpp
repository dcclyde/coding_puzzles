...
inline namespace Output {
    ...
    template<class A, class B> string tsish(const pair<A,B>& p);
...
void solve() {
    ...
    ll S = d12 + d23 + d13;
    if (max({d12, d23, d13}) * 2 > S) {return NO;}

    ll twoa = d12 + d13 - d23;
    if (twoa % 2 != 0) {return NO;}
    ll a = twoa / 2;
    ll c = d13 - a;
    if (d12 + c > N-1) {return NO;}
    dbg(a, c);
    ...
    if (d12 == 1) {
        ...
        G.emplace_back(1, 4); used = 4;
        FOR(k, 0, d12-2) {G.emplace_back(4+k, 5+k); used = 5+k;}
        G.emplace_back(used, 2);
    }
    dbgcY("stage 1", used, G);

    ll c_launch;
    if (a == 0) {c_launch = 1;}
    else if (a == d12) {c_launch = 2;}
    else {c_launch = a + 3;}

    if (c == 0) {
        auto adj = [&](ll pre, ll post) {
            for(auto& [x, y] : G) {
                if (x == pre) {x = post;}
                if (y == pre) {y = post;}
            }
        ...
        while (c > 1) {
            G.emplace_back(c_launch, used + 1);
            ++used; c_launch = used; --c;
        }
        G.emplace_back(c_launch, 3);
    }
    dbg(used, G);
    FOR(k, used + 1, N+1) {G.emplace_back(1, k);}
    YES;
    pvn(G);
    return;
}
