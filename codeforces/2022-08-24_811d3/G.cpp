...
void solve() {
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        // work on parent
        running_A += A[curr];
        pref_B.push_back(pref_B.back() + B[curr]);

        // find largest k such that pref_B[k] <= running_A.
        out[curr] = lstTrue(0, pref_B.size()-1, [&](ll k){return pref_B[k] <= running_A;}) - 2;
        dbg(curr, running_A, pref_B, out[curr]);
