...
void solve() {
    ...
    const ll INF = 2e18;
    ll best_prod = INF;
    db best_prod_log = INF;
    V<int> best_omitted;
    int MAX_OMITTED = min(15, (int)dat.size() - 1);
    dbg(MAX_OMITTED);
    FOR(num_omitted, 1, MAX_OMITTED + 1) {
        ...
        do {
            ...
            FOR(d, 0, 10) {
                ...
                FOR(k, 0, omit_counts[d]) {
                    prod *= per_digit[d][k];
                    prod_log_local += log(per_digit[d][k]);
                }
            ...
            if ( ckmin(best_prod_log, prod_log_local) ) {
                dbgc("update")
                dbg(prod, prod_log_local, omit_counts);
                best_omitted = omit_counts;
            }
        } while (next_permutation(all(comb_helper)));
    }
    if ( best_prod_log > INF/2 ) {
        return ps(-1);
    }
    V<int> out;
    FOR(d, 0, 10) {
