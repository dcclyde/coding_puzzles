...
/*
    Is the number of terms forced?
    Let k be the number of Ds I *could* make.
    Can I split up leftovers + one D across the other k-1?

    If k is 2, definitely forced.
    Else, if D has 2 different prime factors, there should be no problem.
    Else, say D = p^r. If leftover has p^l, then we need to split up r+l factors into k-1 piles and stay < r.
        That is, need (k-1) * (r-1) >= r+l.

    If the number of terms is forced, I think it really is just bad if leftover is 1 or p.
*/
...
void solve() {
    ...
    if ( k == 2 ) {
        ...
        map<ul,int> factors_d;
        factor_rec( d , factors_d );
        if ( factors_d.size() >= 2 ) {
            forced = false;
        } else {
            int r; int p;
            for ( auto& [q, pow] : factors_d ) {
                p = q;
                r = pow;
            }
            int l = 0;
    ...
    // is leftover 1 or a prime?
    if ( num_factors( leftover ) <= 1 ) {
        cout << "NO\n";
    } else {
        cout << "YES\n";
    }
