...
#pragma region  // segtree
    ...
    while ( true ) {
        ...
        // what's the best place I can land from here?

        // first, what's the highest index I can reach?
        auto it = starts.lower_bound(MP(curr_height, N+1));
        --it;  // always valid because the place I'm standing is smaller
        int max_reachable_index = it->second;

        // now, what's the best right endpoint from any of those indices?
        int highest_reachable = st.query(idx, max_reachable_index);

        // starting there, how far do I need to walk to reach the next city?
        it = starts.lower_bound(MP(highest_reachable, -1));
        /*
            ! No. If highest reachable starts with this city then we walk.
            ! Otherwise we just go to that reachable city.
        */
    }
#endif
#pragma endregion
