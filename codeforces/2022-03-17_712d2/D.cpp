...
void solve() {
    ...
    vector<pii> red_positions, blue_positions;
    FOR(j, 0, N) FOR(k, 0, N) {
        if ( (j+k) % 2 == 0 ) {
            red_positions.emplace_back(j+1, k+1);
        } else {
            blue_positions.emplace_back(j+1, k+1);
        }
    }
    ...
    FOR(turn, 1, N*N+1) {
        ...
        if ( banned == 1 ) {
            // can't place red.
            if ( bluepos_ptr < blue_positions.size() ) {
                auto& [a, b] = blue_positions[bluepos_ptr];
                ps(2, a, b);
                ++bluepos_ptr;
            } else {
                auto& [a, b] = red_positions[redpos_ptr];
                ps(3, a, b);
                ++redpos_ptr;
            }
        } else if ( banned == 2 ) {
            if ( redpos_ptr < red_positions.size() ) {
                auto& [a, b] = red_positions[redpos_ptr];
                ps(1, a, b);
                ++redpos_ptr;
            } else {
