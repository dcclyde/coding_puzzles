...
/*
    Need num 0s on the left to equal num 1s on the right, and vice versa.
...
void solve() {
    ...
    dbgP(num_zeros, num_ones, num_spaces); el;

    mi out = 0;
    FOR(left_zeros, 0, N+1) {
        ll left_zeros_needed = left_zeros - num_zeros[0];
        ll right_zeros_needed = (N - left_zeros) - num_zeros[1];
        dbg(left_zeros, left_zeros_needed, right_zeros_needed);
        mi c1 = comb(num_spaces[0], left_zeros_needed);
        mi c2 = comb(num_spaces[1], right_zeros_needed);
        mi curr = c1 * c2;
        out += curr;
        dbgP(c1, c2, curr, out);
    }

    // special handling when all given dominos can match 01 or 10.
    ll num_fully_blank = 0;
    V<ll> alt_type(2);
    bool edgecase = true;
    for ( auto& S : dat ) {
        if ( S[0] == S[1] ) {
    ...
    dbg(edgecase, num_fully_blank);

    if ( edgecase ) {
        // any subset of the fully-blank ones might have gotten 01 vs 10.
        // all of those combination are actually invalid.
        mi offset = pow(mi(2), num_fully_blank);
        out -= offset;

        // ...but 10 10 10 or 01 01 01 are actually valid after all.
        FOR(k,0,2) {
            if ( alt_type[k] == 0 ) {
                dbgc("special", k);
                out += 1;
