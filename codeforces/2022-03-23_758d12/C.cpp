...
void add(auto& dat, int p, int v) {
    dbgcY("add", p, v, dat);
    dat[p] += v;
    if ( dat[p] == 0 ) {
        dat.erase(p);
    }
    dbgcY("add END", p, v, dat);
    return;
}
...
void solve() {
    ...
    vector<pii> hA, hB;
    FOR(k, 0, N) {
        hA.emplace_back(A[k], k);
        hB.emplace_back(B[k], k);
    }
    sort(all(hA));
    sort(all(hB));
    dbgP(hA);
    ...
    while ( true ) {
        dbg(ctr, dat);
        add(dat, hA[N-1-ctr].second, 1);
        add(dat, hB[N-1-ctr].second, -1);
        ++ctr;
        if ( dat.empty() ) {
            break;
        }
    ...
    // dat[hA.back().second] += 1;
    // dat[hB.back().second] -= 1;
    // if ( dat[hB.back().second] == 0 ) {
    //     dat.erase(dat[hB.back().second]);
    // }

    // using t3 = tuple<int,int,int>;
    // vector<t3> dat( N );
    // FOR(k,0,N) {
    //     dat.emplace_back(A[k], B[k], k);
    // }
    // sort(all(dat));

    // ll running_min_idx = get<2>( dat[N-1] );
    // int pos = N-1;
    // while ( running_min_idx < pos ) {
    //     --pos;
    //     ckmin(running_min_idx,
    // }
