...
void solve() {
    ...
    /**
        Pairs with first char matching + pairs with 2nd char matching - pairs with both matching
     */

    V<ll> per_c1(26), per_c2(26), per_both(26*26);
    for(auto& q : dat) {
        char c1 = q[0]; char c2 = q[1];
        ++per_c1[c1 - 'a'];
        ++per_c2[c2 - 'a'];
        ++per_both[(c1-'a') * 26 + (c2-'a')];
    }
    ll out = 0;
    for(auto& x : per_c1) { out += x * (x-1) / 2; }
    dbg(out);
    for(auto& x : per_c2) { out += x * (x-1) / 2; }
    dbgP(out);
    for(auto& x : per_both) { out -= x * (x-1); }
    ps(out);
