...
void solve() {
    ...
    // first find the interval starts/ends.
    // first and last occurrences of each value
    map<int,pii> value_to_interval;
    map<int,int> value_to_count;
    int num_deleted = 0;
    FOR(k, 0, N) {
        int curr = dat[k];
        if ( ! value_to_interval.count( curr ) ) {
            value_to_interval[curr].first = k;
        }
        value_to_interval[curr].second = k;

        // delete random interior occurrences for every value.
        if ( value_to_count[curr] >= 2 ) {
            ++num_deleted;
        } else {
            ++value_to_count[curr];
        }
    }
    dbg(num_deleted, value_to_interval);
    SegTree<int> st;
    st.init( N );
    // first insert all intervals.
    for ( auto& [val, interval] : value_to_interval ) {
        auto& [L, R] = interval;
        st.upd( L , R );
    }

    // now check for obsolete intervals.
    vector<pii> useful_intervals;
    for ( auto& [val, interval] : value_to_interval ) {
        auto& [L, R] = interval;
        int rightmost_end = st.query(0, L-1);
        if ( rightmost_end > R ) {
            if ( L == R ) {
                num_deleted += 1;
            } else {
    ...
    sort(all(useful_intervals));
    dbg(num_deleted, useful_intervals);
    auto& ui = useful_intervals;  // nickname

    vector<bool> ru( ui.size() , true );
    vector<pii> useful_v2;
    useful_v2.push_back(ui[0]);
    if ( ui.size() > 1 ) {
        useful_v2.push_back(ui.back());
    }
    pii last_good = ui[0];
    ...
    for ( int end = 1 ; end <= ui.size() ; ++end ) {
        // are we starting a new chunk or continuing the old one?
        if ( end < ui.size() && ui[end].first < ui[end-1].second ) {
            // do nothing I guess?
        } else {
            int num_in_chunk = end - prev_end;
            // if ( num_in_chunk == 1 && ui[end-1].first == ui[end-1].second ) {
            //     num_deleted += 0;  // without special handling we would add...??
            int contrib = num_in_chunk - 1;
            dbg(prev_end, end, num_in_chunk, contrib);
            num_deleted += contrib;
            prev_end = end;
        }
    }
