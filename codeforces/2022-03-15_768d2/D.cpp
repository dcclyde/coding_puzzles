...
void solve() {
    ...
    vector<ll> working = dat;
    sort(all(working));
    int min_in_range = cdiv( N + K, 2);
    int best_range_size = 2e9;
    int X, Y;
    dbg(min_in_range);
    for ( int l = 0 ; l+min_in_range-1 < N ; ++l ) {
        int curr = working[ l+min_in_range-1 ] - working[ l ];
        if ( curr < best_range_size ) {
            best_range_size = curr;
            // X = l; Y = l+min_in_range-1;
            X = working[l]; Y = working[ l+min_in_range-1 ];
        }
    }
    ...
    int num_true = 0;
    int num_false = 0;
    int intervals_printed = 0;
    int j = 0;
    for ( ; intervals_printed < K-1 ; ++j ) {
        if ( dat[j] >= X && dat[j] <= Y ) {
            ++num_true;
        } else {
            ++num_false;
        ...
        if ( num_true > num_false ) {
            int interval_start = j+1 - num_true - num_false;
            ps( interval_start+1 , j+1 );
