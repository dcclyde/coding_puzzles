...
void solve() {
    ...
    auto subsolve = [&](int l, int r) {
        ...
        dbgP(l, r, neg, twos);
        if ( neg % 2 == 0 ) {
            return MP(twos, MP(0, 0));
        }
        // try deleting stuff from one end.
        int twos_deleted = 0;
        pii proposed_left(0, 0);
        FOR(k, l, r+1) {
            if ( abs(dat[k]) == 2 ) {
                ++twos_deleted;
            }
            if ( dat[k] < 0 ) {
                proposed_left.first = k+1-l;
                break;
            }
        }
        int del_left = twos - twos_deleted;

        twos_deleted = 0;
        pii proposed_right(0, 0);
        ROF(k, l, r+1) {
            if ( abs(dat[k]) == 2 ) {
                ++twos_deleted;
            }
            if ( dat[k] < 0 ) {
                proposed_right.second = r+1-k;
                break;
            }
        }
        int del_right = twos - twos_deleted;
        dbg(del_left, proposed_left);
        dbg(del_right, proposed_right);

        if ( del_left > del_right ) {
            return MP(del_left, proposed_left);
        } else {
            return MP(del_right, proposed_right);
        }
    };

    int start = 0;
    while ( true ) {
        while ( start < dat.size() && dat[start] == 0 ) {
            ++start;
        }
        if ( start == dat.size() ) {
            break;
        ...
        int end = start;
        while ( end+1 < dat.size() && dat[end+1] != 0 ) {
            ++end;
        }
        auto [curr, extra_deletions] = subsolve(start, end);
        if ( ckmax(best, curr) ) {
            auto& [extra_left, extra_right] = extra_deletions;
            best_out = MP(start + extra_left, N-1-end + extra_right);
            dbg(MP(start,end), best, best_out);
        }
        start = end + 1;
    }
    auto& [a, b] = best_out;
