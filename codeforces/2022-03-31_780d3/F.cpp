...
void solve() {
    ...
    SegTree<int> st[3];  // st[a][b] is 0 if b%3 != a. Otherwise it tells how many times we've seen score b.
    FOR(i,0,3) st[i].init(N*2+1);
    int curr = 0;  // current running score
    ll out = 0;
    st[0].add(N, 1);
    FOR(ci, 0, dat.size()) {
        char c = dat[ci];
        if ( c == '+' ) {
            ++curr;
            int sti = (curr + 3*N) % 3;
            int contrib = st[sti].query(curr+N, 2*N);
            dbg(ci, c, curr, sti, contrib, st[sti]);
            out += contrib;
            st[sti].add(curr+N, 1);
            dbgP(st[sti]);
        } else {
