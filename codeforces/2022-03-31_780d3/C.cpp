...
void solve() {
    ...
    umap<char, pair<int,V<int>>> occ;  // occurrences
    FOR(k, 0, N) {
        auto c = dat[k];
        occ[c].second.push_back(k);
    }
    ...
    dp[0] = 0;
    FOR(k, 0, N) {
        // how should I handle character k?
        // 1) I could delete it.
        ckmin(dp[k+1], dp[k] + 1);

        // 2) I could delete stuff after this character until he has a friend.
        auto c = dat[k];
        if ( occ.find(c) != occ.end() ) {
            auto& [optr, odat] = occ[dat[k]];
            if ( optr < odat.size() - 1 ) {
                int num_deleted = odat[optr + 1] - odat[optr] - 1;
                ckmin(dp[odat[optr+1] + 1], dp[k] + num_deleted);
            }
            ++optr;
        }
    }
