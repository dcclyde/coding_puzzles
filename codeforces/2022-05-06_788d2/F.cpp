...
/**
    ...
    There are 60 bits for me to worry about.
    SUM is in [l, r]
    XOR is exactly z.

    XOR is just SUM with carries ignored.
    So we'll definitely have S > X.

    Feels like this is gonna end with combinatorics.

    How about the subproblem where X is 0?

    I'd like to do something like:
    DP state = (n, s, x)
    DP score = ways

    There are way too many DP states.
    Maybe I can reduce the state space by handling lots of low numbers at once?
    Or just adjust what I'm storing s.t. I really can apply those updates...

    What would those updates look like?
 */
...
void brute() {
    ...
    V<V<V<ll>>> dp(N+1, V<V<ll>>(R+1, V<ll>(XMAX)));
    dp[0][0][X] = 1;
    FOR(n, 0, N) FOR(s, 0, R+1) FOR(x, 0, XMAX) {
        FOR(curr, 0, R-s+1) {
            dp[n+1][s+curr][x^curr] += dp[n][s][x];
        }
    }
    ll out = 0;
    FOR(k, L, R+1) {out += dp[N][k][0];}
    ps(out);
}

/**
    OK that seems to work, just not fast enough.

    I for sure don't care about the [L, R] stuff. Just solve it for R and then
        run the algorithm twice and return ans(R) - ans(L-1).

    Basically I want to know how many ways to pass out certain collections of bits.
    I definitely use the bits from X, then I can pass out a multiple of 2 extras
        in any slot.

    How about some DP where state is (bit position, sum remaining)?
    Handle N just through combinatorics.
    How many sums will there be?
    1000 * (1+2+4+8) but then I only keep 1/16 of them...
    maybe it goes?
        nah I can't throw them all out. I need everything with sum <= R,
        not sum = R.

    Maybe I could use the high bits first?
    Then there's a limited space of sums that actually matter,
        and then we separately track how many ways create a sum that's small
        --> can be filled in with anything?
 */

mi subsolve(ll N, ll R, ll X) {
    ll curr_bit = 0;

}

void solve() {
    return brute();  // just for fun, clearly not fast enough.

    lls(N, L, R, X);
    dbgR(N, L, R, X);
    el;

    mi out = subsolve(N, R, X) - subsolve(N, L-1, X);
    ps(out);
    return;
}
