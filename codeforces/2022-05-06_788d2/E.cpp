...
/**
    ...
    I think the best cost should be N = 2^P.

    Say w is "big" if w >= N.
    I have N big things and N-1 small things.
    I have N vertices and N-1 edges.

    Maybe - assign N at root, then otherwise assign big at edges and small at vertices?
 */
...
void solve() {
    ...
    V<ll> vertex_weights(N, -1);
    V<ll> edge_weights(E, -1);
    auto& vw = vertex_weights;
    auto& ew = edge_weights;
    V<ll> depth(N, -1);
    ...
    auto dfs = [&](ll curr, ll parent, auto&& self) -> void {
        ...
        for (auto& [o, ei] : G[curr]) {
            if (o == parent) {continue;}

            depth[o] = depth[curr] + 1;
