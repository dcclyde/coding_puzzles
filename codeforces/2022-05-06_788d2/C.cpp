...
/**
    ...
    Maybe looking at cycle structure of P^-1 * Q?
    I think answer is just 2 ^ (num unconstrained cycles)
 */
...
void solve() {
    ...
    // get rid of bad heads though.
    FOR(k, 0, N) {
        if (D[k] != 0) {
            heads.erase(dsu.get(D[k]));
        }
    }

    mi out = pow(mi(2), heads.size());
    ps(out);
    return;
}
