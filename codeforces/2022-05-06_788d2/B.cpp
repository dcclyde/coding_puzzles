...
/**
    String might as well be binary.
    0010110
    .*.**..
    0110
    ...
    Something like - max consecutive 0s?
    But 1111 is different from 1...
    Maybe +1 if the max is a tie?
 */
...
void solve() {
    ...
    el;
    uset<char> spec;
    for(auto& c : special) {spec.insert(c);}

    V<bool> bin(N);
    FOR(k, 0, N) {
        if (spec.find(dat[k]) != spec.end()) {
            bin[k] = 1;
        }
    }
    while (bin.size() > 0 && bin.back() == false) {
        bin.pop_back();
        --N;
    }
    dbg(bin);
    ...
    pll best = {0, 0};
    auto& [best_val, best_start] = best;
    ll start = 0;
    while (start < N) {
        // find the end of the current run of 0s.
        ll end = start;
        while (end < N && bin[end] == false) {++end;}
        ll curr = end - start;
        ckmax(best, MP(curr, start));
        dbg(start, end, curr, best);
        start = max(end, start+1);
    }

    if (best_start > 0) {++best_val;}
    ps(best_val);
    return;
}
