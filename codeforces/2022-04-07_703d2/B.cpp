...
void brute() {
    ...
    int min_x = *min_element(all(vx));
    int max_x = *max_element(all(vx));
    int min_y = *min_element(all(vy));
    int max_y = *max_element(all(vy));

    ll best = 2e9;
    V<pll> best_points;
    FOR(x, min_x, max_x + 1) FOR(y, min_y, max_y + 1) {
        ll curr = 0;
        for(auto& [xo, yo] : dat) {
            curr += abs(x-xo) + abs(y-yo);
        }
        if (curr < best) {
            best_points.clear();
        }
...
void solve() {
    ...
    ll out = subsolve(vx) * subsolve(vy);
    ps(out);
