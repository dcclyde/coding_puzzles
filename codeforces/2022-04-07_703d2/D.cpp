...
void brute() {
    ...
    FOR(a, 0, N) {
        FOR(b, a+K-1, N) {
            V<ll> curr;
            curr.insert(curr.end(), dat.begin() + a, dat.begin() + b+1);
            // dbg(a, b, curr);
            int med = median(curr);
            if (ckmax(out, med)) {
                dbgcY("new best", a, b, med);
            }
        }
    }
    return ps(out);
...
void solve() {
    ...
    auto test = [&](int goal) {
        ...
        int min_start = 0;
        FOR(k, K, N) {
            if (running - min_start > 0) {
                return true;
            }
            running += score(dat[k]);
            running_start += score(dat[k-K]);
            ckmin(min_start, running_start);
        }
        return running > min_start;
    };

    int out = lstTrue(1, (int)big, [&](int m) {
        return test(m);
    });
    ps(out);
