...
void solve() {
    ...
    // I want to run Dijkstra on an augmented graph.
    // newG nodes are (oldG node, recent weight or -1).

    auto p2f = [&](int v, int w) {  // "pair to flat"
        return (w+1)*N + v;
    };

    auto sanity_check_p2f = [&]() {
        set<ll> flat_indices;
    ...
    dbg_only(assert(sanity_check_p2f()));

    Dijkstra<ll, true> dij;
    dij.init(N*(W+2));
    FOR(v, 0, N) {
        // set up augmented edges.
        V<int> w_here;
        for(auto& [vo, wo] : G[v]) {
            w_here.push_back(wo);
        }
        ...
        for(auto& [vo, wo] : G[v]) {
            for(auto& w : w_here) {
                ...
                ll cost;
                if (w == -1) {
                    // if w is -1, then this step costs nothing and ends with wo.
                    cost = 0;
                    wnew = wo;
                } else {
                    cost = wo + w;
                ...
                dbg(MP(v,w), MP(vo,wnew), cost);
                dij.ae(p2f(v,w), p2f(vo, wnew), cost);
            }
        }
    }
    dij.gen(p2f(0, -1));
