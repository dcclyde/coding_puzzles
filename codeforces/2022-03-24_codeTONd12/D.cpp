...
void solve() {
    ...
    // check for first odd prime divisor.
    for ( auto& [p, exp] : factors ) {
        if ( p == 2 ) {
            continue;
        }
        if ( p <= 1'000'000'000 && (p*(p+1))/2 <= N ) {
            return ps(p);
        }
        break;
    }

    // ok so I guess N is 2^e, maybe times some odd prime bigger than ~sqrt(N).
    if ( factors.size() == 1) {
        // powers of 2 don't work I think
        return ps(-1);
    }
