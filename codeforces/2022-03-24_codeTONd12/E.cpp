...
void learn() {
    ...
    // (value, sees)
    vector<pii> dp(N, MP(-1,-1));

    function<bool(int,int)> Dfs = [&](int curr, int parent) {
        auto& [cval, csees] = dp[curr];
        if ( parent != -1 ) {
            auto& [pval, psees] = dp[parent];
            int pn = G[parent].size();
            int cn = G[curr].size();

            csees = pval + (pn - 1) * psees;
            cval = psees - (cn - 1) * csees;
            if ( abs(cval) > N ) {
                return false;
            }

            // dbgP(MP(parent, curr), MT(pval, psees, pn), MT(cval, csees, cn));
        }

        bool ok = true;
        for ( auto& prop : G[curr] ) {
            if ( prop == parent ) {
                continue;
            }
            if ( !Dfs(prop, curr) ) {
    ...
    FOR(root_val, 1, MAX+1) {
        FOR(root_sees, -MAX*MAX, MAX*MAX) {
            dp.assign(N, MP(-1,-1));
            dp[root] = MP(root_val, root_sees);
            bool result = Dfs(root, -1);
            dbg(result, root, root_val, root_sees, dp);
            if ( result ) {
                V<int> labels(N);
                FOR(k, 0, N) {
                    labels[k] = dp[k].first;
                }
                dbgcY("good", root, root_val, root_sees, labels);
            } else {
                // dbgcP("bad", root, root_val, root_sees);
            }
            el;
        }
    }
...
void solve() {
    ...
    color[root] = 1;
    deque<pii> todo;
    todo.emplace_back(root, -1);
    while ( !todo.empty() ) {
        auto [curr, parent] = todo.back();
        todo.pop_back();
        dbgP(curr, parent);
        for ( auto& prop : G[curr] ) {
            if ( prop == parent) {
            ...
            color[prop] = color[curr] * -1;
            todo.emplace_back(prop, curr);
            dbg(MP(prop, curr), todo);
        }
        dbgcP("done", curr, parent);
    }
    dbg(color);

    V<int> out(N);
    FOR(k, 0, N) {
        out[k] = color[k] * G[k].size();
    }
    pv(out);
