...
ll ctr = 0;
ll potential_score = -1;
map<ll, set<pair<ll, string>>> cached;

void learn() {
    el;
    ll N = 4;
    ll PLAYERS = 2*N + 1;
    ll TURNS_IN_CYCLE = PLAYERS * (PLAYERS-1);
    potential_score = (PLAYERS-1) * (PLAYERS-2);

    V<ll> team(2*N+1);
    V<V<ll>> team_to_players(2);

    // which people do we care about?
    // don't allow starting as the sub for now.
    ll A = rng() % (2*N);
    ll B = rng() % (2*N);
    while (B == A) {B = rng() % (2*N);}
    if (A > B) {swap(A, B);}
    ...
    // put people on initial teams randomly.
    // V<ll> team_size(2, 0);
    FOR(k, 0, PLAYERS-1) {
        if (us.find(k) != us.end()) {continue;}
        // dbg(k, team_size);
        ll my_team = -1;
        while (my_team == -1 || team_to_players[my_team].size() == N-1) {my_team = rng() % 2;}
        team[k] = my_team;
        team_to_players[my_team].push_back(k);
    }
    ll extra = 2*N;
    team[extra] = -1;
    ...
    string team_print;
    FOR(k, 0, PLAYERS) {
        if (us.find(k) != us.end()) {team_print.push_back('.');}
        else {team_print += to_string(team[k]);}
        team_print.push_back(' ');
    }
    team_print.pop_back();
    dbgY(ctr, team_print, MP(A,B), team_to_players[0], team_to_players[1]);

    V<ll> score(2);
    FOR(attempt, 0, 2) {
        team[A] = attempt; team[B] = attempt^1;
        // dbgY(ctr, attempt, MP(A,B), team);
        FOR(t, 0, TURNS_IN_CYCLE) {
            ll next = (extra + 1) % (2*N+1);
            team[extra] = team[next];
            team[next] = -1;
            extra = next;
            score[attempt] += (team[A] == team[B]);
        }
        // dbgR(team);
    }
    dbgGeneral(((score[0] == score[1]) ? OUT_PURPLE : OUT_CYAN), score, potential_score);
    cached[score[0]].emplace(score[1], team_print);
...
int main() {
    ...
    ll NUM_TRIALS = 10'000;
    rep(NUM_TRIALS) {learn(); ++ctr;}
    el; el; el;
    for(auto& [score, data] : cached) {
        dbgBold(score, score*1./potential_score);
        dbg(pdh(data));
        el;
    }
